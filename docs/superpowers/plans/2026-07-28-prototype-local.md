...
**i2v 파라미터 — 이번에 새로 보내는 것**
- [ ] **`prompt` 가 실제로 먹히는가.** 컷마다 다른 움직임을 지시했는데 클립이 그대로
      움직이는가, 아니면 무시되는가. 검색으로는 LTX i2v 가 prompt 를 받는다고 나오지만
      **문서 원문을 못 봤다**(fal 이 429 를 돌려줬다)
- [ ] **`duration` 이 받아들여지는가.** 검색 결과의 코드 예시에는 `image_url` 과 `prompt` 만
      있었다. `duration` 이 무시되면 클립 길이가 낭독과 어긋나고, 거부되면 400 이 난다.
      `data/costs.json` 의 duration 과 실제 클립 길이(`cut.video.seconds`)를 대조한다
- [ ] 얼굴·손가락을 피하라는 지시가 지켜지는가 — 뭉개진 인물이 나오면 SHOWS_SYSTEM 의
      motion 규칙을 조인다
- [ ] motion 이 없는 컷의 폴백("거의 정지, 아주 느린 카메라 이동")이 실제로 조용한가
...
- [ ] **컷이 몇 개로 쪼개지는가.** 가짜 모드 관측(2026-07-28, rich 자료 30초): 원고가
      **컷 3개**로만 나뉘었고 길이가 **13초·9초·7초**였다. 첫 컷이 이미 i2v 상한(10초)을
      넘는다. 실측 낭독이 붙으면 더 벌어질 수 있다 — 잘린 컷이 몇 개인지, 잘린 자리에서
      소리와 그림이 얼마나 어긋나 보이는지 눈으로 확인한다
