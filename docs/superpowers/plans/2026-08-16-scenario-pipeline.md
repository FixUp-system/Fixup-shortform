...
const seed = (shots, target = 30) => ({
  settings: { i2v_model: "seedance-2.0", target_seconds: target },
});
...
describe("checkScenario", () => {
  it("규칙을 다 지키면 통과한다", () => {
    ...
    expect(checkScenario(s, seed())).toEqual({ ok: true, problems: [] });
  });

  it("★ 초의 합이 목표와 다르면 걸린다", () => {
    const s = { shots: [shot(10), shot(10), shot(8)] };
    const got = checkScenario(s, seed());
  ...
  it("★ 컷 하나가 8초를 넘으면 걸린다 — 정확히 8초는 통과", () => {
    expect(checkScenario({ shots: [shot(8), shot(8), shot(8), shot(6)] }, seed()).ok).toBe(true);
    const over = checkScenario({ shots: [shot(9), shot(8), shot(8), shot(5)] }, seed());
  ...
  it("★ 컷 하나가 모델 하한보다 짧으면 걸린다 — 정확히 4초는 통과", () => {
    expect(checkScenario({ shots: [shot(4), shot(8), shot(8), shot(6), shot(4)] }, seed()).ok).toBe(true);
    const under = checkScenario({ shots: [shot(3), shot(8), shot(8), shot(7), shot(4)] }, seed());
  ...
  it("★ 컷 개수가 길이÷하한을 넘으면 걸린다 — 15초·Seedance면 3개", () => {
    ...
    expect(checkScenario(three, seed(null, 15)).ok).toBe(true);
    const four = { shots: [shot(4), shot(4), shot(4), shot(3)] };
    expect(checkScenario(four, seed(null, 15)).ok).toBe(false);
  });
  ...
  it("대사가 빈 컷은 화자를 요구하지 않는다 — 무음 컷이다", () => {
    ...
    expect(checkScenario(s, seed()).ok).toBe(true);
  });

  it("★ 컷이 하나도 없으면 걸린다", () => {
    expect(checkScenario({ shots: [] }, seed()).ok).toBe(false);
    expect(checkScenario(null, seed()).ok).toBe(false);
  });
...
describe("buildScenarioMessages", () => {
  ...
  it("★ 길이·화면비·모델 하한을 사실로 알려 준다", () => {
    ...
    expect(all).toContain("4");       // Seedance 하한
  });
