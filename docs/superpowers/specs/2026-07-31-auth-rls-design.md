...
| ⑦ | 운영자 승인 화면(대기 목록·승인·차단) + 원장 화면 `/costs` 를 운영자 전용으로 잠그고 `actor` 를 표시 이름으로 그린다 |
...
- `scripts/measure/*` 는 요청 컨텍스트가 없다 → **스크립트 진입점에서
  `runWithActor("admin", …)` 로 감싼다.** 안 하면 측정 도구가 전부 죽는다.
  기존 하드코딩 값 `"local"` 을 대체하는 자리다 — 측정이 낸 비용은 운영자 지출이다

> `"admin"` 은 uuid 가 아니라 **문자열 리터럴**이라 사장님 계정의 uuid 와 다른 버킷이 된다.
> 의도한 것이다 — 측정 스크립트가 태운 돈이 사장님의 사용자별 상한을 잡아먹으면, 프롬프트를
> 재던 날 화면에서 영상이 안 만들어진다. 전역 상한에는 둘 다 함께 잡힌다

### ★ 저장하는 값과 보여주는 값을 나눈다
`cost_records.actor` 에는 **uuid 를 저장한다.** 집계와 사용자별 상한이 이 값으로 돌기
때문이다 — 이메일이나 표시 이름은 바뀌고 중복되지만 uuid 는 그렇지 않다.

그런데 `app/costs/page.js:84` 가 그 값을 **그대로 화면에 찍는다.** 지금은 `local` 이 뜨지만
인증이 붙으면 **uuid 가 노출된다.** 사람이 읽을 수 없고, 남의 계정 식별자를 화면에 뿌리는
것이기도 하다.

**`listRecords()` 가 표시용 이름을 함께 실어 보낸다**(`actor_label`). 화면은 그것만 그린다.

| 저장된 `actor` | 화면 표시 |
|---|---|
| 운영자(`profiles.role = 'admin'`)의 uuid | `admin` |
| 일반 사용자의 uuid | 이메일 |
| 문자열 `"admin"`(측정 스크립트) | `admin` |
| 옛 기록의 `"local"` | `local` (그대로 — 과거 사실이다) |

이름 조회는 원장 행마다 하지 않는다. **한 번에 모아 `profiles` 를 조회하고 uuid→라벨 맵을
만든다** — 행마다 조회하면 원장 화면 하나가 DB 왕복 수십 회가 된다.
...
| `/costs` | **`role = admin` 만.** 전사 원장이라 일반 사용자에게 보이면 안 된다. `actor` 는 uuid 가 아니라 표시 이름으로 그린다(§3) |
