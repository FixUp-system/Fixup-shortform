...
## 12-bis. 다음 단계 — HLS (사장님 지시, 2026-09-07)

**R2 다음에 한다. 순서를 뒤집으면 두 번 만든다** — 세그먼트를 어떻게 인가할지가 저장소에
달려 있어서, Supabase Storage 위에 지어 두면 R2 로 옮길 때 그 부분을 다시 짜야 한다.

### 왜 하는가

지금은 재생하면 **완성본 MP4 를 통째로** 내려받는다. 브라우저가 앞질러 버퍼링하므로,
카드에 마우스를 2초 올려도 수 MB 가 나간다. HLS 는 **본 만큼만** 받고, 작은 화면에는
작은 화질을 준다. 09-07 에 넣은 것들(주소 재사용·poster·crf 26)과 **곱해진다.**

### 굽기 — 파이프라인에 한 단계 추가

완성본이 나온 뒤 ffmpeg 을 한 번 더 돌린다. **원본 MP4 는 그대로 남긴다**(내려받기용이자
폴백이다).

- 화질 둘이면 충분하다: **360p**(카드 미리보기) · **720p**(상세). 원본은 내려받기.
- `-hls_time 4 -hls_playlist_type vod -hls_segment_type fmp4`
- 키 모양: `hls/<projectId>/master.m3u8` · `hls/<projectId>/<화질>/seg%03d.m4s`

### 🔑 인가 — 이 설계의 핵심

지금 규약은 "라우트가 판정하고 **짧은 수명의 서명 URL 로 302**"다. HLS 는 한 편이 파일
수백 개라 그대로는 안 맞는다. **재생목록을 우리가 그 자리에서 만든다:**

```
GET /api/renders/<id>/master.m3u8
  → 라우트가 프로젝트를 판정한다 (지금과 **같은 문**)
  → m3u8 텍스트를 만들면서 세그먼트 주소를 **그때 서명해 박아 넣는다**
  → 브라우저는 그 서명 주소로 R2 에서 직접 세그먼트를 받는다
```

- 재생목록은 **텍스트라 작다** — 함수로 흘려도 4.5MB 상한과 무관하다
- 세그먼트는 함수를 안 지난다 — 지금 302 갈래와 같은 성질이다
- **판정 자리가 하나 그대로다**(`withUser` + 프로젝트 존재). 인가 모델이 안 바뀐다
- 서명 수명은 **재생 시간을 덮어야 한다** — VOD 는 master 를 한 번만 읽으므로 그 안의
  주소가 끝까지 유효해야 한다. 2시간이면 넉넉하다(§13 의 수명 결정과 함께 정한다)

### 화면

- Safari 는 HLS 를 그냥 재생한다. Chrome·Firefox 는 **hls.js** 가 필요하다
- **카드**: 마우스를 올리면 그때 hls.js 를 붙여 **360p** 를 문다 ← 절감이 제일 큰 자리
- **상세**: master 를 물려 적응형으로 둔다

### 폴백 — R2 이관과 같은 규율

**HLS 자산이 없으면 지금의 MP4 갈래로 떨어진다.** 옛 프로젝트 46편은 다시 굽지 않는 한
HLS 가 없고, 그래도 화면은 멀쩡해야 한다.

### 비용

파일 수가 편당 수백으로 는다(Class A·B 연산). R2 무료 한도(쓰기 100만·읽기 1,000만)에
견주면 무시할 수준이다. 저장은 화질 둘이 붙어 **약 1.6배** — 10GB 안에서 여전히 넉넉하다.
