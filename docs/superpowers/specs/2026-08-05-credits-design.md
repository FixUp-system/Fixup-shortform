...
1. **시작 게이트** — 잔액이 모자라면 **402**로 막는다. 거는 자리는 **fal 이 나가는
   시작점**이다: `POST /api/projects/[id]/auto`(자동 관통) ·
   `/voice` · `/images` · `/clips` · 컷별 regen 3종(`/cuts/[idx]/regen` ·
   `/voice/[idx]/regen` · `/clips/[idx]/regen`).
   - 자동 관통: 하한 = `PER_VIDEO_USD` 한 편치. 잔액 $0.50 으로 시작해 중간에 멈추면
     **돈만 나가고 영상이 없다** — 그걸 막는 자리다
   - 단계별: 하한 = **잔액 > 0**. 그 단계가 얼마 쓸지는 컷 수에 달려 있어 미리 정확히
     알 수 없고, 넘어서는 부분은 호출 게이트가 컷 단위로 끊는다(단계별은 컷마다 끊겨도
     사장님이 화면에서 보고 있으므로 자동 관통과 달리 "돈만 나가고 결과 없음"이 아니다)
   - `POST /cuts`(컷 분할)에는 걸지 않는다 — OpenAI 만 쓰고 fal 비용이 0 이다
2. **호출 게이트** — `assertBudget` 의 `limitUser()` 가 고정 $5 대신 **그 사용자의 잔액**을
   돌려준다. 전역(`SHOTFORM_BUDGET_TOTAL_USD`)·프로젝트 상한은 **회사 안전핀으로 그대로** 둔다.
   호출부는 `lib/i2v.js` · `lib/imagegen.js` · `lib/tts.js` 셋이고 **건드리지 않는다**
   (limitUser 만 바뀐다).

> ⚠️ **OpenAI 지출은 지금도 가드가 없다** — `lib/llm.js` 는 비용을 기록만 하고
> `assertBudget` 을 부르지 않는다. 이번 스펙이 만든 구멍이 아니라 원래 그랬고,
> 편당 ~$0.04 라 크레딧 정확도에는 영향이 작다(원장에는 남으므로 **잔액에서는 깎인다**).
> 가드를 llm 까지 넓히는 것은 별도 과제로 둔다.
