...
**★ 합성(render)은 멈춤 판정에서 뺀다.** 계획을 쓰다 잡은 구멍이다 — 합성은 단일 ffmpeg
작업이라 중간 진척이 없고 정상적으로 최대 10분까지 걸린다. 120초 임계를 그대로 적용하면
잘 돌고 있는 합성이 **전부** "멈췄어요"가 된다. 제외 목록도 `lib/progress.js` 에 둔다
(`STALL_EXEMPT_PHASES`). ⑥완성 화면은 지금의 5분 폴링 상한이 그 역할을 계속 한다.
