...
//
//   ⚠️ 다만 **되살리는 것도 판단이다.** 처음에 넷을 되살렸는데 그중 셋(세로 구도·대사
//   길이·화자 하나)은 사장님이 요청한 적도 없고 레퍼런스 프롬프트(referenece/)에도 없는
//   것이었다 — 옛 지문에 있었다는 이유만으로 되살린 것이고, 사장님이 그 자리에서 잡았다.
//   이 저장소의 방향은 **규칙을 더하는 것이 아니라 모델을 믿는 것**이다. 여기 목록은
//   "잃으면 안 되는 것"이지 "옛 지문에 있던 전부"가 아니다.
//
...
// ★★★ 2026-08-27 사장님 결정: **사장님이 쓴 글이 고른 값을 이긴다.**
//   근거는 옵션에 **기본값이 있다**는 것이다 — 포맷은 안 고르면 "제품 히어로", 분위기는
//   "고급스러운"이다. 즉 고른 값은 "그냥 놔둔 것"일 수 있는데, 쓴 글은 직접 적은 것이라
//   뜻이 분명하다.
//   ★ 다만 **넷은 예외다**: 길이·비율·화질·모델. 그 넷은 글이 아니라 우리가 fal 에 따로
//     넘기는 값이라(duration·aspect_ratio·resolution·엔드포인트), 글만 바뀌면 사장님이
//     화면에서 본 것과 실제로 나오는 영상이 갈린다.
describe("★ 사장님이 쓴 글이 고른 값을 이긴다", () => {
  it("어긋나면 쓴 글이 이긴다고 말한다", () => {
    expect(AD).toContain("쓴 글이 이긴다");
  });

  it("기술 값 넷은 고른 값이 이긴다고 말한다", () => {
    expect(AD).toContain("길이(초)·화면 비율·화질·모델 넷은 고른 값이 이긴다");
  });
});

// ★★ 사장님이 실제로 만든 프롬프트가 3,785자인데 줄바꿈이 **0개**였다(2026-08-27).
//   시나리오 화면은 그 글을 읽고 [이대로 만들기]를 누를지 정하는 자리라, 못 읽으면 죽는다.
describe("★ 단마다 줄을 바꾸라고 말한다", () => {
  it("한 덩어리로 붙여 쓰지 말라고 한다", () => {
    expect(AD).toContain("단마다 줄을 바꾼다");
  });
});
