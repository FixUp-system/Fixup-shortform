...
describe("청구", () => {
  ...
  it("컷당 첫 재생성은 공짜, 둘째부터 값을 치른다", async () => {
    ...
    expect(await balanceFor(A)).toBe(5000 - REGEN_PRICE.clip["kling-v3"]["720p"]);
  });
  ...
  it("빚도 버림이다 — 음수 잔액은 더 큰 빚으로 내려간다", async () => {
    await grant(0.5);
    await chargeVideo({ userId: A, projectId: P, seconds: 30 });
    expect(await balanceFor(A)).toBe(-VIDEO_PRICE["kling-v3"]["720p"][30]);   // 0.5 - 450 = -449.5 → -450
  });
  ...
  it("환불한 프로젝트를 다시 돌리면 다시 받는다 — 되돌려줬으면 공짜가 아니다", async () => {
    ...
    expect(await balanceFor(A)).toBe(5000 - VIDEO_PRICE["kling-v3"]["720p"][30]);   // 두 번 줄고 한 번 돌아왔다
    expect(await alreadyChargedVideo(P)).toBe(true);
  });
  ...
  it("환불 행의 주인은 원 청구 행의 주인이다", async () => {
    ...
    expect(await balanceFor(A)).toBe(5000);            // 돈은 낸 사람에게 돌아간다
...
describe("청구가 모델을 탄다", () => {
  ...
  const 충전 = () =>
    getStore().insertGrant({ user_id: A, amount_credits: 5000, reason: "테스트", granted_by: ADMIN });
  ...
  it("Seedance 프로젝트는 정가가 세 배다", async () => {
    ...
    expect(await balanceFor(A)).toBe(5000 - videoPrice(30, "seedance-2.0"));
  });
  ...
  // ⚠️ 청구는 220 을 받고 환불이 60 을 돌려주면 사장님이 160 크레딧을 잃는다.
...
describe("장부에는 정수만 들어간다", () => {
  ...
  it("영상 청구가 정수다", async () => {
    await getStore().insertGrant({ user_id: A, amount_credits: 5000, reason: "충전", granted_by: ADMIN });
