...
describe("②시나리오 화면", () => {
  ...
  // ★★ 2026-08-18 — **뒷단 값을 사장님 화면에 늘어놓지 않는다**(사용자 지시).
  //    전달 방식(angle)·내레이터 목소리는 사장님이 판단할 것이 아니라 우리가 모델에 주는
  //    지시다. 값은 그대로 만들어지고 그대로 저장된다(화면이 `{ scenario }` 를 통째로
  //    보내므로 칸이 없어도 왕복한다 — tests/scenario-route.test.js 의 왕복 테스트가 짝이다).
  it("★ 전달 방식은 사장님 화면에 없다 — 값은 그대로 흐른다", () => {
    expect(page, "전달 방식 칸이 남아 있다").not.toMatch(/이 영상을 어떻게 전달하나/);
    expect(page, "angle 을 고치는 칸이 남아 있다").not.toMatch(/edit\(\{\s*angle/);
    // 그 칸에 딸린 안내도 함께 사라져야 한다 — 없는 칸을 설명하는 문장이 남으면 거짓말이다
    expect(page, "없는 칸의 안내가 남았다").not.toMatch(/다음에 컷을 나눌 때/);
  });

  // ★ 내레이터 목소리도 화면에서 걷었다. **다만 완전히 걷지는 않는다** — 값이 비면
  //   확정이 막히는데(lib/scenario-rules.js 의 checkScenario), 칸이 아예 없으면 사장님이
  //   그 상태에서 빠져나올 수단이 없다. 그래서 **비었을 때만** 나타난다:
  //   정상 흐름(모델이 채워 온 경우)에는 화면에서 사라지고, 막히는 자리만 열린다.
  it("★ 내레이터 목소리는 비어서 확정이 막힐 때만 나타난다", () => {
    expect(page, "narrator_voice 칸이 아예 없다 — 막히면 탈출구가 없다").toContain("narrator_voice");
    expect(page, "판정을 화면이 손으로 다시 적었다").toContain("hasNarration");
    // 조건이 "내레이션이 있다"만이면 채워져 있을 때도 보인다 — 빈 값 판정이 함께 있어야 한다
    expect(page, "비었을 때만 보이는 조건이 없다").toMatch(/!\s*scenario\.narrator_voice/);
  });

  // ★★ angle 의 안내(아래 "다음에 컷을 나눌 때")와 짝인데 방향이 반대다 — 이 값은 클립
  //    프롬프트에 실려 **곧바로 반영된다**. 임시저장은 규칙을 안 물으므로(confirmed 없는
  //    PATCH) 사장님이 이 칸을 비운 채로 저장할 수 있고, 그러면 Voice 절이 통째로 빠져
  //    다시 만든 영상은 컷마다 다른 사람이 읽는다 — 이 필드가 막으려던 바로 그 실패다.
  //    말 안 하면 무엇을 잃는지 모른 채 ⑤에서 유료 버튼을 누른다.
  it("★ 이미 만든 영상이 있으면 목소리를 고칠 때 무엇이 드는지 말한다", () => {
    expect(page, "영상이 있는지 안 보고 안내한다").toMatch(/video\?\.url/);
    expect(page).toMatch(/다시 만들어야 해요/);
    expect(page, "값이 든다는 말이 없다").toMatch(/크레딧이 들어요|유료/);
  });
  ...
  // ★★ 옛 계약(2026-08-16 Important 5)은 "컷이 있으면 전달 방식 칸이 언제 반영되는지 말한다"
  //    였다. 그 안내는 **칸이 화면에 있을 때만** 필요한 것이었고, 칸을 걷은 지금은 위
  //    "전달 방식은 사장님 화면에 없다" 가 그 자리를 대신 지킨다(안내 문구까지 함께 못 박는다).
  //    Important 5 의 교훈 자체는 살아 있다 — 고칠 수 없는 값을 고칠 수 있는 척 보여 주지 않는다.
