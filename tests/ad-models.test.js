...
// ★★ 2026-08-20 — 2.5 가 **숨김에서 등급으로 옮겨졌다.** 그 전에는 `hidden: true` 였는데,
//   그것은 화면에서만 거르는 것이라 서버는 그대로 받았다(isAdModel 이 true) — 잠금이 아니라
//   가림막이었다. 이제 lib/tiers.js 가 "누가 쓸 수 있나"를 정하고 서버가 판정한다.
//   등급 쪽 시험은 tests/tiers.test.js 에 있다.
//
// ★ 다만 이 describe 의 **절반은 그대로 지켜야 한다**: 표에서 **지우는 것**은 여전히 안 된다.
//   지우면 이미 2.5 로 만든 문서가 가격 계산에서 던져 화면째 죽는다(seedance-2.0-fast 가
//   정확히 그랬다). 바뀐 것은 hidden 뿐이고, "표에 남는다"는 바뀌지 않았다.
describe("2.5 는 표에서 지우지 않는다", () => {
  it("★ 표에 남아 있다 — 지우면 그것으로 만든 옛 문서가 죽는다", () => {
    const m = AD_MODELS.find((x) => x.id === "seedance-2.5");
    expect(m, "2.5 가 표에서 사라졌다 — 옛 문서가 죽는다").toBeTruthy();
    expect(m.endpoints?.r2v, "엔드포인트가 사라졌다").toBeTruthy();
    expect(m.perSecUsd, "원가가 사라졌다 — 값 계산이 던진다").toBeTruthy();
  });

  it("★ 더 이상 hidden 이 아니다 — 숨겨 두면 프로 등급도 못 고른다", () => {
    expect(AD_MODELS.find((x) => x.id === "seedance-2.5").hidden).toBeFalsy();
  });
