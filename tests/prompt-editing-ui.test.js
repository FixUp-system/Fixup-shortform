...
describe("⑤영상 — 프롬프트 편집", () => {
  ...
  it("★ 텍스트칸은 본문만 쥔다 — 꼬리는 떼어 내 보여 준다", () => {
    ...
    // ★ 2026-08-18 — 칸이 **둘**이 됐다: 프롬프트 본문 + 수정사항(사장님 지시로 이 화면에도
    //   생겼다). 옛 못은 "칸이 하나"였는데, 그것은 "꼬리는 못 고친다"를 재려던 **대리 지표**였다.
    //   지금은 그 뜻을 직접 잰다: **꼬리를 값으로 쥔 텍스트칸이 없어야** 한다.
    //   (수정사항 칸은 꼬리와 무관하다 — 그 글은 서버가 본문을 다시 쓰는 재료다.)
    const areas = video.match(/<textarea[\s\S]{0,240}?\/>/g) || [];
    expect(areas.length, "텍스트칸을 못 찾았다").toBeGreaterThan(0);
    for (const a of areas) {
      expect(a, "꼬리를 텍스트칸이 쥔다 — 저장할 때마다 꼬리가 두 벌이 된다").not.toMatch(/fixedTail|full/);
    }
  ...
  // ★ 자리가 옮겨졌다(2026-08-18) — 좌측 카드에 있던 [다시 만들기]를 걷어내고, 두 화면이
  //   같은 작업대(우측 ①결과 → ②고치기 → ③들여다보기)를 쓴다. 버튼은 사라진 것이 아니라
  //   ②고치기 단으로 갔다. 재는 것은 **자리가 아니라 살아 있는가**다.
  it("접혀 있고, 지시 없이 그냥 다시 만드는 문이 살아 있다", () => {
    expect(video).toMatch(/<details/);
    expect(video, "컷별 다시 만들기가 사라졌다").toMatch(/onClick=\{\(\) => onRegen\?\.\(cut\.idx\)\}/);
  });
});
...
describe("사장님이 쓴 글의 자리를 알려 준다", () => {
  ...
  for (const [step, fold] of [["④이미지", () => foldIn(images)], ["⑤영상", () => foldIn(video)]]) {
    ...
    // ★★★ 2026-08-18 (다섯째 판, 사용자 지시) — **한 덩어리로 읽힌다**:
    //    "고칠 수 없어요 뒤에 붙여요로 분리하지 말고 하나의 지시문으로 붙여서 보여줘."
    //    앞은 고칠 수 있고 뒤는 코드가 붙이는 글이지만, 사장님에게는 **모델이 받는 문장
    //    하나**다. 사이에 안내나 버튼이 끼면 서로 다른 두 개로 읽힌다.
    //    ★ 붙이는 것은 **보이는 방식**이고, 고치는 자리는 그대로 본문 하나다 — 꼬리를
    //      텍스트칸에 넣으면 저장할 때마다 꼬리가 두 벌이 된다(이 파일 머리말 ②).
    it(`${step} — 본문과 꼬리가 한 덩어리로 이어져 보인다`, () => {
      const box = fold();
      // 편집칸과 꼬리가 **같은 상자** 안에 있어야 한다
      expect(box, "본문과 꼬리를 잇는 상자가 없다").toMatch(/prompt-one/);
      // 사이를 가르던 안내가 없어야 한다 — 그 문장이 두 토막으로 읽히게 만들던 자리다
      expect(box, "본문과 꼬리 사이에 안내가 끼어 있다").not.toMatch(/뒤에 붙여요/);
      // 그래도 **고치는 것은 본문뿐**이다: 편집칸에 꼬리를 넣으면 안 된다
      expect(box, "꼬리를 편집칸에 넣었다 — 저장할 때마다 꼬리가 두 벌이 된다")
        .not.toMatch(/value=\{prompt \+ fixedTail\}|value=\{`\$\{prompt\}/);
    });
  }

  // 상자가 실제로 이어 보이려면 CSS 가 있어야 한다 — 클래스만 붙이고 규칙이 없으면
  // 화면에서는 여전히 두 상자로 보인다(소스 검사가 놓치기 딱 좋은 자리다).
  it("★ 이어 보이게 하는 규칙이 있다", () => {
    const css = readFileSync("app/globals.css", "utf8");
    expect(css, "prompt-one 규칙이 없다").toMatch(/\.prompt-one/);
  });
});
