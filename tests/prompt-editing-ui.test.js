...
describe("사장님이 쓴 글의 자리를 알려 준다", () => {
  ...
  for (const [step, fold] of [["④이미지", () => foldIn(images)], ["⑤영상", () => foldIn(video)]]) {
    it(`${step} — 공통 지시 안내는 그 값이 있을 때만 뜬다`, () => {
      const box = fold();
      expect(box, "공통 지시가 꼬리에 있다는 말이 없다").toMatch(/함께 보내는 지시/);
      // 조건이 붙어 있어야 한다 — 값이 없는 프로젝트에서 없는 것을 설명하면 안 된다
      expect(box, "값이 없어도 안내가 뜬다 — 없는 것을 설명한다")
        .toMatch(/(image_note|clip_note)[\s\S]{0,120}함께 보내는 지시/);
      // 없어진 두 자리를 가리키면 안 된다: ①자료에도, 첫 화면에도 그 칸이 없다
      expect(box, "없어진 자리로 사장님을 보낸다")
        .not.toMatch(/함께 보내는 지시[\s\S]{0,200}(자료 단계|만들 때)/);
    });
  }
});
