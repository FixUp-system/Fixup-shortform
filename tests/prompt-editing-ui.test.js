...
describe("사장님이 쓴 글의 자리를 알려 준다", () => {
  ...
  for (const [step, fold] of [["④이미지", () => foldIn(images)], ["⑤영상", () => foldIn(video)]]) {
    it(`${step} — 꼬리 안의 공통 지시가 어디서 정해지는지 적는다`, () => {
      const box = fold();
      expect(box, "공통 지시가 꼬리에 있다는 말이 없다").toMatch(/함께 보내는 지시/);
      expect(box, "언제 정하는 값인지 안 알려 준다").toMatch(/함께 보내는 지시[\s\S]{0,200}만들 때/);
      expect(box, "없어진 자리(①자료)로 사장님을 보낸다 — 거기엔 그 칸이 없다")
        .not.toMatch(/함께 보내는 지시[\s\S]{0,200}자료 단계/);
    });
  }
});
