...
describe("단계별(reel) — 같은 등급 이름을 쓴다", () => {
  ...
  // ⚠️ 2026-09-01 — **갈라졌다.** 단계별의 기본은 2.0 이고 원클릭의 기본은 H3 다.
  //   같은 이름이 두 경로에서 다른 모델을 가리킨다 — 표가 둘이라 그럴 수 있다.
  it("단계별의 기본은 2.0 이다 — 원클릭(H3)과 다르다", () => {
  ...
  it("★ 단계별에서 내려간 H3 는 등급 이름을 안 받는다", () => {
    expect(byId("minimax-h3").label).toBe("MiniMax H3");
  });
