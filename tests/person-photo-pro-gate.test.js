// **프로(2.5)에서는 인물 사진을 안 받는다** (2026-09-01 사장님 결정).
//
...
describe("참조 적재 — 화면을 지나쳐도 안 실린다", () => {
  it("★★★ 프로면 인물 참조가 빠진다", () => {
    expect(kinds("seedance-2.5")).toEqual(["thing"]);
  });
  ...
  it("★★ 인물 사진을 **사물처럼** 꽂아도 프로에서는 빠진다 — 라벨로 새는 길을 막는다", () => {
    const p = project("seedance-2.5");
    const out = describeCutRefs({ idx: 0, ref_ids: ["p9"] }, p);
    expect(out, "인물 사진이 thing 자리로 들어와 실렸다").toEqual([]);
  });
});
