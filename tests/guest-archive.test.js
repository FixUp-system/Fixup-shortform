...
describe("열린 자리 — 보관함을 보는 데 필요한 것뿐이다", () => {
  const open = [
    "/",
    // 2026-09-09 — 첫 화면이 홈이 됐다(사장님 확정). 읽기 전용 화면이고, 여기서 부르는
    // 목록(/api/projects)은 이미 손님에게 열려 있다 — 새로 새는 자리가 없다.
    "/home",
    "/archive",
    `/archive/${ID}`,
    "/api/projects",
...
describe("★ 닫힌 자리 — 값이 나가거나 남을 건드리는 문은 전부 막힌다", () => {
  const closed = [
    ...
    //
    // ★★ 2026-09-09 — `/home` 은 여기서 **위(열린 자리)로 옮겼다.** 홈은 만드는 화면이
    //   아니라 보여 주는 화면이고(링크만 그린다), 이제 손님의 첫 화면이다. 아래 둘은
    //   그대로 잠가 둔다 — 홈의 도구 격자가 이 주소들을 **가리키기만** 하고, 누르면
    //   middleware 가 로그인으로 보내는 것이 이 개편의 경계선이다.
    [`/reel/${ID}/video`, "GET"],
    ["/create", "GET"],
  ];
...
// ★★ 2026-09-09 정정 — 루트는 이제 **갈리지 않는다.** 그전에는 로그인했으면 홈, 아니면
//   보관함이었는데, 홈이 손님에게도 열리면서 목적지가 하나가 됐다(사장님 확정).
//   갈리는 것은 화면 **안의 버튼**이고, 그것은 tests/home-sections-ui.test.js 가 잰다.
//   ★ 08-27 지시("기본으로 보관함 바로 확인")는 죽지 않았다 — 홈의 결과물 띠가 보관함으로
//     가는 길을 들고 있고, 그 길도 같은 판이 잰다.
describe("루트는 첫 화면 하나로 보낸다", () => {
  const src = readFileSync("app/page.js", "utf8");

  it("신원을 묻지 않고 홈으로 보낸다", () => {
    expect(src, "홈으로 안 보낸다").toMatch(/redirect\(\s*["']\/home["']\s*\)/);
    // 목적지가 하나면 신원을 볼 일이 없다. 여기서 세션을 다시 판정하면 그 판정이 두 벌이 된다.
    expect(src, "화면이 세션을 다시 판정한다").not.toContain("createServerClient");
    expect(src, "목적지가 하나인데 신원을 읽는다").not.toContain("USER_HEADER");
  });

  it("★ 루트와 홈이 손님 목록에 있다 — 없으면 첫 화면에서 로그인 벽을 만난다", () => {
    expect(isGuestPath("/", "GET")).toBe(true);
    expect(isGuestPath("/home", "GET")).toBe(true);
  });
});
...
describe("로그인 화면의 문은 메인 하나다", () => {
  // ⚠️ 주석을 걷고 본다 — 이 절의 "없어야 한다" 판은 **코드**를 재는 것이지, 왜 걷었는지
  //   적어 둔 주석의 낱말을 재는 것이 아니다(실제로 그 주석에 걸려 빨개졌다).
  const raw = readFileSync("app/login/page.js", "utf8");
  const src = raw.replace(/\/\*[\s\S]*?\*\//g, "").replace(/(^|[^:])\/\/.*$/gm, "$1");

  // ★★★ 2026-09-11 뒤집혔다. 여기에는 "보관함으로 가는 길이 있다" 가 서 있었다
  //   (2026-08-27 사장님 지시: "보관함을 확인할 수 있어야 해"). 같은 분이 걷었다 —
  //   "그냥 메인화면 보기 버튼만 만들어줘, 로그인 없이 보관함 보기는 제거해줘."
  //   ★ 손님에게 보관함이 닫힌 것이 **아니다** — /archive 는 그대로 열려 있고(위 판들이
  //     그것을 잰다), 메인(/home)을 거쳐 간다. 걷은 것은 **이 화면의 지름길**뿐이다.
  it("★★★ 보관함 지름길이 없다 — 옛 지시를 근거로 되살리지 마라", () => {
    expect(src, "보관함 문이 아직 있다").not.toContain("href=\"/archive\"");
    expect(src, "옛 문구가 남아 있다").not.toContain("로그인 없이 보관함 보기");
  });
...
describe("로그인 화면의 문 — 링크가 아니라 버튼으로 보인다", () => {
  const src = readFileSync("app/login/page.js", "utf8");
  // 2026-09-11 — 보관함 문이 걷히면서 이 판이 보는 줄도 **메인 문**으로 옮겼다.
  //   밑줄 교훈 자체는 그대로 산다 — 문이 무엇이든 맨 <Link> 면 밑줄이 그어진다.
  const line = src.split("\n").find((l) => l.includes('href="/home"') && l.includes("mini")) || "";

  it("★★★ 그 자리가 아직 있다 — 로그인이 유일한 문처럼 보이면 안 된다", () => {
    expect(line, "나가는 문이 사라졌다").toMatch(/메인/);
  });
