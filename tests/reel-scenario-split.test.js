...
describe("②시나리오 — 글 ↔ 그림", () => {
  ...
  it("★★★ 세 영역이 **한 보드** 안이다 — 글 칸이 제 카드를 따로 갖지 않는다", () => {
    // ★ 겉 클래스는 템플릿 문자열이다(그림 유무로 rv-wide 가 붙는다) — 따옴표 모양을 재지 않는다.
  ...
  //   ★ 푸는 자리는 **표식 하나**(`.rv-wide`)다. 처음에는 ②만 `:has(.is-two)` 로 풀었는데,
  //     ③이미지도 같은 것이 필요해지자 이름이 둘이 될 참이었다 — 한 이름으로 합쳤다.
  //   ★ 다만 **언제 붙일지는 화면이 정한다**: ②는 그림이 있을 때만(두 칸일 때만) 넓어진다.
  //     한 칸에서 상한을 풀면 글이 1,100px 로 늘어져 한 줄에 100자가 넘는다.
  it("★★★ 폭을 푸는 표식은 하나다 — 그리고 ②는 **두 칸일 때만** 붙인다", () => {
    const m = /\.rv-wide\s*\{([^}]*)\}/.exec(css);
    expect(m, "폭을 푸는 규칙(.rv-wide)이 없다").toBeTruthy();
    expect(m[1], "상한을 안 푼다").toMatch(/max-width:\s*none/);
    // 화면이 그림 유무로 붙인다 — CSS 가 짐작하지 않는다.
    expect(page, "②가 한 칸에서도 넓어진다 — 글이 늘어진다").toMatch(/sheetUrl\s*\?[\s\S]{0,40}?rv-wide/);
    // 앱 공통 880 은 그대로 남아 있어야 한다.
    expect(css, "공통 카드 상한을 바꿨다").toMatch(/\.panel--wide\s*\{\s*max-width:\s*880px/);
  });

  // ★★★ ③이미지는 **나누지 않는다** — 통짜 갈래에는 컷별 프롬프트가 없어서(④ 주석)
  //   화면에 있는 것이 전부 그림이다. 나눌 축이 없으니 폭만 다 쓴다.
  it("★★★ ③이미지도 폭을 다 쓴다 — 그림이 본문인 단계다", () => {
    const images = strip(readFileSync("app/reel/[id]/images/page.js", "utf8"));
    expect(images, "③이 폭을 안 푼다").toMatch(/className="panel[^"]*rv-wide"/);
  });

  // ★★★ 2026-09-15 실측 — ③의 카드를 넓혔더니 컷별 그림이 **안 따라왔다.**
  //   그 자리는 `.panel-cards` 이고 칸이 **150px 고정**이었다(2026-08-27, 안 A) — 넓히면
  //   빈 칸만 는다(실측: 폭 1,132px 에 여섯 칸, 컷은 넷 → 두 칸이 통째로 빔).
  //   ⚠️ 처음에 `.cut-shots` 를 고쳤다가 되돌렸다 — **그 갈래는 이 화면에 안 뜬다**
  //     (컷별로 다시 그린 프로젝트에서만 뜬다). 화면에서 재기 전에는 고치지 마라.
  //   ★ 08-27 의 걱정("컷이 셋이면 그림이 화면을 먹는다")은 지금도 맞다 — 그래서 `1fr` 로
  //     열지 않고 **상한(240px)** 을 둔다. 막을 것은 상한이 막고 남는 자리는 나눠 갖는다.
  it("★★★ 컷별 그림이 빈 칸을 남기지 않는다 — 상한은 두되 고정은 푼다", () => {
    const m = /\.panel-cards\s*\{([^}]*)\}/.exec(css);
    expect(m, ".panel-cards 규칙이 없다").toBeTruthy();
    expect(m[1], "자리가 남으면 빈 칸을 만든다 — auto-fit 이어야 한다").toMatch(/auto-fit/);
    // 08-27 결정: 그림이 화면을 먹으면 안 된다 — 상한 없이 1fr 로 열지 마라.
    expect(m[1], "상한 없이 열었다 — 컷이 적으면 그림이 화면을 먹는다").not.toMatch(/,\s*1fr\)/);
    expect(m[1], "상한이 없다").toMatch(/minmax\([^)]*px\s*,\s*\d+px\)/);
    expect(m[1], "남는 자리가 한쪽으로 몰린다").toMatch(/justify-content:\s*center/);
  });
