...
describe("로그인 화면 시각", () => {
  ...
  it("상자가 420px 로 좁다", () => {
    ...
    // ★★★ 2026-09-11 — 상한만 두면 **아무 일도 안 한다.** 부모가 flex 세로 정렬인데 가로
    //   여백이 auto 라 stretch 가 꺼져, 카드가 내용 폭(실측 287px)으로 쪼그라들어 있었다.
    //   `width: 100%` 가 있어야 420 이 실제 폭이 된다.
    expect(css, "폭을 직접 채우지 않아 상한이 걸리지 않는다").toMatch(
      /\.login-card \{[^}]*width:\s*100%/
    );
    expect(css, "부제·절차만 내용 폭으로 서서 카드와 갈린다").toMatch(
      /\.login-head \{[^}]*width:\s*100%/
    );
  });
