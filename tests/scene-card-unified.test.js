...
describe("장면 카드 — 두 흐름이 같은 모양", () => {
  ...
  // ★★★ 2026-09-15 — 이 단정이 바뀌었다. 원래는 두 화면이 **같은 폭의 판**(panel--wide)에
  //   서야 한다는 것이었는데(2026-08-18 "규격이나 배치도 통일 시켜줘"), ②시나리오가
  //   **글 ↔ 그림 두 칸**이 되면서 그 전제가 깨졌다(사장님 지시). 광고는 한 칸 880px,
  //   ②는 두 칸이라 1,180px 다 — 폭을 억지로 맞추면 두 칸이 서지 못한다.
  //   ★ 그래도 **카드 자체는 같은 것**을 쓴다(.panel) — 면·모서리·안쪽 여백이 갈리면
  //     그때는 정말 다른 화면이 된다. 통일의 뜻을 폭에서 **카드**로 옮겼다.
  //   ★ 광고 화면은 손대지 않았다.
  it("★★ 같은 카드를 쓴다 — 폭은 칸 수가 정한다", () => {
    expect(ad, "광고가 넓은 판이 아니다").toContain("panel panel--wide");
    expect(sc, "②시나리오가 카드를 안 쓴다 — 면·모서리가 갈린다").toMatch(/className="panel[ "]/);
  ...
  // ★★ 네 번째 판(2026-08-18): "시나리오 확인해 주세요도 광고와 간격이 달라."
  //    재어 보니 6px 였다. 원인은 **목록 바로 위 줄의 아래 여백**이다 —
  //    광고는 그 자리가 `.script-src`(아래 여백 0)라 목록 머리의 12px 가 그대로 간격이 되고,
  //    ②시나리오는 `.pgsub`(아래 여백 18px)라 붙어 있는 여백끼리 겹쳐 18px 가 이긴다.
  //    앞 요소가 무엇이냐에 간격이 달리면 화면마다 리듬이 갈린다 → 그 자리에 이름을 준다.
  it("★★ 목록 바로 위 줄의 아래 여백을 화면이 정하지 않는다", () => {
    expect(css, "목록 위 줄의 이름(.plan-note)이 없다").toMatch(/\.plan-note/);
