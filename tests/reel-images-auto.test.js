...
describe("보여 주는 것은 컷별 [그림 · 지문] 이다", () => {
  ...
  // ★★★ 2026-09-15 — 이 단정의 뜻을 **고정에서 상한으로** 옮겼다.
  //   원래 요구는 "늘어나면 그림 하나가 화면을 먹는다"(2026-08-27, 안 A)였고 그 걱정은
  //   지금도 맞다. 그런데 ③의 카드가 작업 칸 폭을 다 쓰게 되면서 **고정이 빈 칸을 낳았다**
  //   (실측: 폭 1,132px 에 150px 칸이 여섯 · 컷은 넷 → 두 칸이 통째로 빔).
  //   ★ 그래서 `1fr` 로 열지 않고 **상한(240px)** 을 둔다 — 막으려던 것은 상한이 막고,
  //     남는 자리는 칸들이 조금씩 나눠 갖는다. 남는 자리는 가운데로 모은다.
  it("(2) 카드 폭에 **상한**이 있다 — 상한이 없으면 그림 하나가 화면을 먹는다", () => {
    const css = readFileSync("app/globals.css", "utf8");
    const m = /\.panel-cards\s*\{([^}]*)\}/.exec(css);
    expect(m, ".panel-cards 규칙이 없다").toBeTruthy();
    expect(m[1], "상한 없이 열었다 — 컷이 셋이면 그림이 화면을 먹는다").not.toMatch(/,\s*1fr\)/);
...
// ★★ 2026-09-16 — 위 09-14 리팩터가 "영어 원문 접힘 칸을 없앤다"는 뜻이었는데, 실은
//   PromptWithKo 자체를 걷어 버려 **한국어 번역까지 함께** 사라졌다. 그 결과 이 화면에는
//   지문이 아예 안 보였다(사장님 신고). PromptWithKo 는 이미 한국어만 보여 주도록 고쳐져
//   있으니(2026-09-14, components/PromptWithKo.jsx) 그 컴포넌트를 다시 쓰는 것과 "영어
//   원문 접힘 칸을 안 되살린다"는 서로 다른 약속이다 — 아래는 후자만 지킨다.
describe("지문 전체 칸이 없다 (2026-09-14), 한국어 번역은 있다 (2026-09-16)", () => {
  it("★ '이미지 생성 지문 전체' 접힘 칸이 없다 — 영어 원문 UI 는 안 되살린다", () => {
    expect(src, "지문 전체 칸이 돌아왔다").not.toContain("이미지 생성 지문 전체");
    expect(src).not.toContain("lib-fold");
  });

  it("★ 한국어 번역은 PromptWithKo 로 보인다 — 지문이 없으면 안 그린다", () => {
    expect(src, "지문의 한국어 번역이 안 보인다").toMatch(/<PromptWithKo/);
    expect(src, "지문이 없을 때도 그린다").toMatch(/savedPrompt\s*&&\s*<PromptWithKo/);
    expect(src, "image_prompt_ko 를 안 읽는다").toMatch(/image_prompt_ko/);
  });
