...
// 480p 를 고른 사장님에게 720p 값만 걷힌다(원가는 2.25배라 그 차액이 그대로 우리 손해다).
...
describe("화질이 청구액까지 관통한다", () => {
  ...
  it("클립 재생성도 480p 값이다", async () => {
    ...
    expect(await balanceFor(A)).toBe(base - REGEN_PRICE.clip["seedance-2.0"]["480p"]);
  });

  // 이미지·목소리 재생성은 모델도 해상도도 안 탄다(값이 하나다). 그러나 그 라우트도
  // **정가 게이트**를 지나므로, 환불된 프로젝트로 들어오면 거기서 480p 정가를 받아야 한다.
  const regens = [
