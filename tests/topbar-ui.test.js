...
describe("상단 계정 바", () => {
  ...
  it("내 정보를 못 읽어도 로그아웃은 그린다 — 조기 반환도 조건부 반환도 없다", () => {
    ...
    expect(
      ...
      // ★ 2026-09-14 다섯 → 여섯: `me?.internal`(내부 계정 표시)을 더했다. 잔액 자리의 삼항 안쪽이라
      //   로그아웃 드롭다운과 무관하다 — 눈으로 확인했다.
      // ★ 2026-09-14 저녁 여섯 → 다섯: 잔액이 늘 보이게 되면서 `me.gated` 조건을 **뺐다**(가리는 자리가 준 쪽).
      //   드롭다운 게이트 `{open && (` 는 그대로다 — 눈으로 확인했다.
      "UserMenu 본문의 `me` 언급이 다섯(선언·크레딧 조건·잔액·내부 계정·이름)에서 달라졌다 — " +
