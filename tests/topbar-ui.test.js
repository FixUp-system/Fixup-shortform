...
describe("상단 계정 바", () => {
  ...
  // ★ 2026-09-10 — 가릴 것이 하나 줄었다(크레딧 잔액을 걷었다). 이름은 기본 라벨로 채워
  // 빈 버튼을 안 만드는 규율은 그대로다. 그래서 `me` 언급이 다섯에서 **둘**이 됐다.
  //
  ...
  it("내 정보를 못 읽어도 로그아웃은 그린다 — 조기 반환도 조건부 반환도 없다", () => {
    ...
    expect(
      ...
      "UserMenu 본문의 `me` 언급이 둘(선언·이름)에서 달라졌다 — " +
    ...
    // ★ 2026-09-10 — 여기서 가리던 잔액(um-credit)을 걷었다. 이 판이 지키는 것은
    //   "로그아웃이 어떤 상태에서도 도달 가능하다"이고, 남은 조건부는 이름 하나다.
