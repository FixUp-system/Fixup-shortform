...
describe("isCutDone — 단계별 끝남 판정", () => {
  ...
  // TTS 루프는 무음 컷을 건너뛴다 — audio 도 voice_error 도 안 생긴다.
  // 그것을 "안 끝났다"로 세면 done 이 total 에 못 닿아 정상 낭독이 2분 뒤 "멈춤"이 된다.
  it("무음 컷은 낭독 단계에서 처음부터 끝난 것이다", () => {
    expect(isCutDone({ silent: true, sentence: "" }, "voice")).toBe(true);
    expect(isCutDone({ sentence: "   " }, "voice")).toBe(true);
    // 문장 키가 아예 없는 컷은 무음이 아니라 아직 모르는 컷이다
    expect(isCutDone({ sentence: "안녕하세요" }, "voice")).toBe(false);
  });

  it("무음 컷이 섞여도 진척이 total 까지 차오른다", () => {
    const cuts = [
      { idx: 0, sentence: "안녕하세요", audio: {} },
      { idx: 1, sentence: "", silent: true },
      { idx: 2, sentence: "안녕히 계세요", audio: {} },
    ];
    const done = cuts.filter((c) => isCutDone(c, "voice")).length;
    expect(done).toBe(cuts.length);
    expect(generationState({ done, total: cuts.length, phase: "voice", stepPhase: "voice", stalledForMs: 999_999, busy: true }).kind)
      .toBe("done");
  });
