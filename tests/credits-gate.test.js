...
describe("호출 게이트 — 사용자 축은 잔액이다", () => {
  ...
  it("정가를 다 쓴 잔액 0 은 통과한다 — 그 값은 시작 전에 이미 받았다", async () => {
    await grantTo(A, VIDEO_PRICE["kling-v3"]["720p"][30]);
    await chargeTo(A, VIDEO_PRICE["kling-v3"]["720p"][30], "paid-1");
    await runWithActor(A, async () => {
...
describe("시작 게이트 — 자동 관통", () => {
  ...
  it("정가에 한 크레딧이라도 모자라면 402 — 반 편만 만들게 두지 않는다", async () => {
    const p = await makeProject();
    await grantTo(A, VIDEO_PRICE["kling-v3"]["720p"][30] - 1);
...
describe("시작 게이트 — 단계별", () => {
  ...
  it("컷당 첫 재생성은 공짜라 잔액 0 이어도 200 이다", async () => {
    await grantTo(A, VIDEO_PRICE["kling-v3"]["720p"][30]);          // 정가만 내고 잔액 0
