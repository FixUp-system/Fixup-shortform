...
describe("보관함 화면 — 전체/내 영상", () => {
  ...
  // 2026-08-19: 첫 탭을 **주소**도 정할 수 있게 되면서(상세에서 보던 탭으로 돌아오기)
  // `useState("mine")` 리터럴이 사라졌다. 뜻은 그대로다 — 주소가 "all" 이라고 말하지
  // 않으면 내 영상이다. 그래서 리터럴이 아니라 **그 판정**을 잰다.
  it("기본값은 내 영상이다 — 켜자마자 남의 것이 쏟아지면 안 된다", () => {
    expect(page, "주소가 all 일 때만 전체가 되어야 한다")
      .toMatch(/===\s*["']all["']\s*\?\s*["']all["']\s*:\s*["']mine["']/);
  });
