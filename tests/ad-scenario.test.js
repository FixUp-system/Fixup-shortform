...
describe("environment · transition", () => {
  ...
  // ★★ 2026-08-20 에 **동작이 바뀌었다.** "첫 컷은 비운다"는 컷마다 이음을 적게 하던
  //   시절의 규칙인데, 이제 이음을 아예 안 요구한다 — 통짜로 굽는 경로엔 그 문제가 없고,
  //   오히려 화면에 없는 것을 가리키는 문장이 생겼다(실측: "as the dish is plated" 인데
  //   담는 장면이 없었다). 그래서 이 시험은 지우지 않고 **새 동작을 재는 것**으로 바꾼다.
  it("★ SYSTEM 이 이음을 아예 요구하지 않는다 — 통짜로 굽는 경로엔 그 문제가 없다", () => {
    expect(sys()).not.toMatch(/앞 컷에서 어떻게 넘어오는지/);
  });
...
// ★★ 이음(transition)을 지시문에서 걷는다(2026-08-20 실측).
//
// 분할 생성의 유산이다. 컷마다 따로 만들 때는 이음을 적어 줘야 했지만, 통짜로 굽는 지금은
// 모델이 알아서 잇는다 — 오히려 **화면에 없는 것을 가리키는 문장**을 만들어 낸다.
//
// 실측(떡볶이, tries=3): 장면 넷 중 셋에 이음이 붙었고 그중 하나가
//   "as the dish is plated, the camera settles close on the finished bowl"
// 였다. **담는 장면이 없는데 "담기면서"** 라고 적은 것이다. 그 문장이 굽기 지시문에 그대로
// 실렸고, 사장님이 7초 지점의 전환을 어색하다고 지적했다.
//
// ★ 칸은 남긴다 — music 을 걷을 때와 같은 규율이다(2026-08-19). 지우면 옛 문서가 깨진다.
//   걷는 것은 **모델에게 이음을 적으라고 요구하는 규칙**이다.
describe("이음은 더 요구하지 않는다 — 통짜로 굽는 경로엔 그 문제가 없다", () => {
  const sys = () => buildScenarioMessages({ settings, material: { text: "소재", photos: [] } }).system;

  it("★ 장면마다 이음을 적으라고 요구하지 않는다", () => {
    expect(sys()).not.toMatch(/앞 컷에서 어떻게 넘어오는지 적는다/);
  });

  it("★ 무대 규칙도 이음에 기대지 않는다 — 그 규칙만 남으면 없는 칸을 가리킨다", () => {
    const s = sys();
    const rule = s.slice(s.indexOf("무대를 정한다")).split(/^★ /m)[0];
    expect(rule).not.toMatch(/transition/);
  });

  it("칸 자체는 스키마에 남는다 — 지우면 옛 문서가 깨진다(music 과 같은 규율)", () => {
    expect(Object.keys(SCENARIO_SCHEMA.properties.shots.items.properties)).toContain("transition");
  });
});

// ★★ 장면 수 — **없던 정보를 준다**(2026-08-20).
//
// 지금까지 장면 수에 아무 지침이 없었다. 15초짜리에서 LLM 이 넷을 골랐고(장면당 3.75초),
// 짧은 장면이 많을수록 잇는 자리가 늘어난다. 사장님이 지적한 7초 전환이 그 자리다 —
// 같은 음식(팬 위 / 그릇 안)을 연속 두 컷으로 보여 주고 있었다.
//
// ★ 규칙을 조이는 것이 아니라 **왜 그런지를 알려 주는 것**이다. 이 저장소가 네 번 확인한
//   것: 프롬프트로 규칙을 조이면 다른 데로 새고, 없던 정보를 주면 통한다.
describe("장면 수 — 짧은 장면을 많이 만들지 않는다", () => {
  const sys = () => buildScenarioMessages({ settings, material: { text: "소재", photos: [] } }).system;

  it("★ 장면 수를 몇 개로 할지 말해 준다 — 지금까지 아무 지침이 없었다", () => {
    expect(sys()).toMatch(/장면 수/);
  });

  it("★ 왜 적은 편이 나은지 이유를 함께 준다 — 숫자만 주면 다른 데로 샌다", () => {
    const s = sys();
