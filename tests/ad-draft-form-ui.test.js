...
// **[시나리오 만들기] 옆의 [무료] 를 뗀다** (2026-09-01 사장님 지시).
//
// ★ 이 저장소는 같은 방향을 이미 한 번 갔다 — 단계별 화면에서 값·크레딧 문구를 전부
//   걷어내고 값은 ⑤영상 한 자리에서만 말하기로 했다. 원클릭의 이 버튼도 같은 자리다:
//   무료라는 사실은 누르기 전에 알아야 할 것이 아니라, 값이 나가는 자리에서만 말하면 된다.
// ★ **[다시 쓰기] 는 손대지 않는다** — 사장님이 말한 것은 시나리오 만들기 버튼이다.
//   함께 걷으면 요청 밖으로 넘어간다.
describe("원클릭 — [시나리오 만들기] 에 값 문구가 없다", () => {
  const FILES = ["app/ads/new/page.js", "app/ads/[id]/page.js"];
  const strip = (s) => s.replace(/\/\*[\s\S]*?\*\//g, "").replace(/(^|[^:])\/\/.*$/gm, "$1");

  // ⚠️ **주석을 걷고 잰다.** 이 파일들은 코드 옆 설명에서 "LLM 만 쓰고 무료다" 처럼
  //   같은 낱말을 쓴다 — 안 걷으면 화면을 다 고쳐도 영원히 빨간 판이 된다.
  it("★★★ 그 버튼 옆에 [무료] 배지가 없다", () => {
    for (const f of FILES) {
      const src = strip(readFileSync(f, "utf8"));
      for (const m of src.matchAll(/시나리오 만들기/g)) {
        const around = src.slice(m.index, m.index + 120);
        expect(around, `${f} 의 시나리오 만들기 옆에 값 문구가 남았다`).not.toMatch(/무료/);
      }
    }
  });

  it("★ [다시 쓰기] 는 그대로다 — 요청은 시나리오 만들기 버튼이었다", () => {
    const src = readFileSync("app/ads/[id]/page.js", "utf8");
    expect(src, "다시 쓰기의 값 문구까지 걷었다 — 요청 밖이다").toMatch(/다시 쓰기 · 무료/);
  });
});
