...
describe("마이페이지", () => {
  ...
  // 보관함은 사이드바에 늘 있다 — 마이페이지가 편수와 링크로 한 번 더 말할 이유가 없다
  // (2026-08-13 사용자 결정). 흡수하지 않는다는 원칙은 그대로다: 목록을 여기로 옮기지 않았다.
  it("보관함을 흡수하지 않는다 — 목록을 여기에 그리지 않는다", () => {
    expect(me).not.toMatch(/ProjectCards|loadProjects/);
  });
...
// 2026-08-13 사용자 결정: 마이페이지는 "내 정보"만 둔다.
describe("마이페이지 — 덜어낸 것과 접은 것", () => {
  it("내 영상 편수를 여기서 또 말하지 않는다 — 보관함이 그 자리다", () => {
    expect(me).not.toMatch(/projectCount/);
  });

  // 입력칸 셋이 늘 펼쳐져 있으면, 바꿀 생각이 없는 사람에게도 화면의 절반이 비밀번호다.
  it("비밀번호 칸은 접어 두고 눌러야 편다", () => {
    expect(me).toMatch(/showPw|pwOpen/);
  });

  it("펼친 뒤 되돌릴 길이 있다", () => {
    expect(me).toMatch(/취소/);
  });
});
