...
import { balanceFor, creditsEnabled, creditsEnabledFor } from "../../../lib/charges.js";
import { fakeFal } from "../../../lib/fake.js";

export const GET = withUser(async (_req, _ctx, user) => {
  const enabledForMe = await creditsEnabledFor(user.id);
  return Response.json({
    ...
    // ★ 2026-09-14 — 계정마다 가른다(내부 계정 면제). 서버 청구와 같은 판정이다.
    gated: !fakeFal() && enabledForMe,
    // ★ 내부 계정인가 — 화면이 잔액 대신 "내부 계정 · 차감 없음"을 적는 데 쓴다.
    //   전역 스위치로 꺼진 것과 가른다(그때는 내부 계정이 아니다).
    internal: creditsEnabled() && !enabledForMe,
  });
});
