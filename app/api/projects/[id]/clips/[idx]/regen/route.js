...
export const POST = withUser(async (req, { params }, user) => {
  ...
  if (!fakeFal()) {
    ...
    // 뒤에 두면 4회째에 값을 받고 나서 regenClip 이 같은 상한으로 던져 400 이 된다 —
    // 내고 아무것도 못 받는 응답이다. 정가 게이트를 이 앞에 두었을 때도 같은 결함이
    // 환불된 프로젝트에서만 되살아났다. 드문 것과 없는 것은 다르다.
    if (prior >= MAX_REGEN_PER_CUT) {
      return Response.json({ error: "영상 다시 만들기는 컷당 3회까지예요" }, { status: 400 });
    }

    // ★ 재생성도 유료 입구다 — /clips 와 **같은 문**을 쓴다.
    // 회차 가격만 보면 구멍이 난다: 실패 → refundVideo(잔액 복구) → 그림·컷은 남음 →
    // 컷별 재생성(컷당 첫 회 무료, 실원가 $0.42/컷) → POST /render(로컬 ffmpeg 0원)
    // = **순지불 0 완성본**. regenClip 은 기존 클립을 요구하지 않고 cut.image?.url 만
    // 보므로 여기가 가장 크게 새는 자리였다. `balance < 0` 그물은 잔액이 양수라 못 잡는다.
