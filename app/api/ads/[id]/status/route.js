...
//
// ★★★ 2026-09-08 — **여기는 이제 가볍다.** 수거는 fal 에 한 번 묻고 "끝났다"는 사실만
//   문서에 적는다. 내려받기·저장·자막 굽기는 POST …/finish(상한 300초)로 옮겼다.
//   그전에는 그 무거운 일을 이 60초 라우트가 했고, 화면이 응답을 안 기다리고 2초마다
//   두드리므로 수십 개가 겹쳐 인스턴스가 메모리 초과로 죽었다 — 아무도 마무리를 못 하니
//   문서는 영영 rendering 이고 화면은 25분째 "만드는 중"이었다(실측 로그로 확인).
//   ⚠️ 무거운 일을 이 자리로 되돌리지 마라. 그러면 그 사고가 그대로 돌아온다.
//
...
export const GET = withUser(async (_req, { params }, user) => {
  ...
  return Response.json({
    ...
    // ★★ 마무리가 남았다는 신호(2026-09-08). 화면은 이것을 보고 전용 라우트(POST …/finish,
    //   상한 300초)를 **한 번** 부른다 — 무거운 일을 이 60초 라우트로 되돌리지 마라.
    finish_needed: Boolean(project.ad_job?.ready),
  });
});
