...
function ClipPromptEdit({ cut, project, busyCut, onSavePrompt }) {
  ...
  return (
    ...
    <details className="prompt-edit">
      ...
      {/* ★ 2026-08-18 — 이 문장을 하루에 두 번 고쳤고, 두 번 다 **사실이 바뀐** 것이다.
          아침에는 칸이 ①자료에서 첫 화면으로 옮겼고("자료 단계에서 고칠 수 있어요"가 거짓이
          됐다), 저녁에는 **칸 자체가 사라졌다**(사용자 지시로 첫 화면에서 걷었다) — 그래서
          "만들 때 정하는 값"도 거짓이 됐다. 이제 아무 데서도 못 정한다.
          ★ 그 값을 **가진 프로젝트에만** 뜬다. 새 프로젝트에는 값이 아예 없으므로, 그때도
          이 줄을 그리면 있지도 않은 자리를 설명하는 셈이다. */}
      {project?.settings?.clip_note && (
        <p className="preview-note">
          <strong>모든 영상에 함께 보내는 지시</strong>도 위 꼬리에 들어 있어요 —
          지금은 고칠 수 없어요. 이 컷만 바꾸려면 위 칸을 고쳐 주세요.
        </p>
      )}
