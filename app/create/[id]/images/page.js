      ...
      {isPhoto ? (
        ...
        <div className="preview-edit">
          ...
          <details className="prompt-edit">
            ...
            {/* ★ **그 값을 가진 프로젝트에만** 뜬다(2026-08-18, 셋째 판). 공통 지시 칸을 첫
                화면에서 걷었으므로 새 프로젝트에는 이 값이 아예 없다 — 없는 것을 설명하면
                사장님은 있지도 않은 자리를 찾는다. 옛 프로젝트에는 값이 남아 있고, 그때는
                "지금은 못 고친다"가 사실이다(고칠 자리가 저장소 어디에도 없다). */}
            {project?.settings?.image_note && (
              <p className="preview-note">
                <strong>모든 이미지에 함께 보내는 지시</strong>도 위 꼬리에 들어 있어요 —
