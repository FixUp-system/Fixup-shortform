...
export default function CreatePage() {
  ...
  //   화질이 정가를 바꾸므로(Seedance 30초: 480p 700 · 720p 1,400 — 09-14 새 단위) 고르는 자리는 결제 앞이어야
  //   ★ 2026-09-07 — **1080p 를 닫았다**(사장님 지시). 30초 3,050 크레딧(새 단위)이라 고를 수 있게
  ...
  //
  // ★ 이것이 잠금 없는 자리의 유일한 방어선이다. Seedance(480p·720p) → Kling(화질 없음) 으로
  //   바꿔도 옛 값이 상태에 남아 있으면 그 값이 그대로 생성 요청에 실려 **400** 이고
                ...
                <div className="chips">
                  {I2V_MODELS.map((m) => (
                    <button key={m.id} className={`chip${model === m.id ? " on" : ""}`}
                      ...
                      {m.label}{showCredits && ` · ${formatCredits(videoPrice(seconds, m.id, defaultResolutionForModel(m.id)))} 크레딧`}
                    </button>
                  ))}
                </div>
                  ...
                  <div className="chips">
                    {resolutions.map((r) => (
                      <button key={r} className={`chip${resolution === r ? " on" : ""}`}
                        ...
                        {r}{showCredits && ` · ${formatCredits(videoPrice(seconds, model, r))} 크레딧`}
                      </button>
                    ))}
                  </div>
