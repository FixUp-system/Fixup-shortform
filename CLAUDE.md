...
> (2026-08-04 기준: 테스트 **922 그린 / 10 skip** · **`main` = `feature/video-compose` 동일
> 커밋**(fast-forward 병합 완료) · **origin/main 대비 306커밋 미푸시**)
...
> 결제**다. 크레딧까지 끝났고 다음은 결제다. 원가가 사용자 행동당 실제 현금(영상 한 편 $2.59)이라
> **크레딧은 "나중에 붙일 기능"이 아니라 현금 방어선**이다 — 무료 체험 한 편이 $2.59 손실이고,
...
0-2. ~~**크레딧**~~ → **완료(08-05 코드).** 아래 "크레딧" 절 참고.

> ### 크레딧 — 붙었다 (2026-08-05)
>
> **잔액을 컬럼으로 두지 않는다: 잔액 = 충전 합계(`sum_grants`) − 쓴 합계(`sum_costs`).**
> 충전은 `credit_grants` 에 행으로 쌓이고(누가·언제·왜가 남는 감사 로그), 차감은 기존
> 비용 원장이 그대로 맡는다 — 두 값이 갈라질 자리가 없다. 게이트는 **둘**이다: 자동 관통은
> 한 편치(`perVideoUsd()`)를 요구하고, 단계별 유료 라우트는 잔액이 사실상 0 이면 막는다
> (부족하면 **402**). 화면·`GET /api/credits` 는 USD 가 아니라 **남은 편수**로 보여준다
> (`videosLeft`, 내림 — "3편"을 보고 눌렀는데 두 편 반이면 안 되므로).
> 편수 환산 기준가는 `SHOTFORM_PER_VIDEO_USD`(기본 $2.59)다.
> 옛 고정 상한 env `SHOTFORM_BUDGET_USER_USD` 는 **죽은 env** 다 — 사용자 축을 잔액이
> 대신한다. 남은 안전핀은 전역·프로젝트 둘뿐이다.
>
> ⚠️ **`db/schema.sql` 을 라이브에 먼저 올려라 — 그 전에는 배포하지 마라.**
> 라이브 DB 에 `credit_grants` 테이블과 `sum_grants` 함수가 없으면 RPC 실패가 그대로
> 던져져(fail-closed) **모든 유료 라우트·`GET /api/credits`·admin 목록이 500** 이 된다.
> 돈이 새지는 않지만 **서비스가 전면 불능**이다. `db/schema.sql` 은 **통째로 다시 올려도
> 안전하다**(`if not exists`·`or replace`) — 크레딧 부분만 떼어낼 것 없이 파일 전체를 올린다.
>
> 원가는 영상 한 편 **$2.59**(클립 81%)다 — 무료 체험 한 편이 그대로 손실이다.
