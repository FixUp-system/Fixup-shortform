...
0. ~~**저장 계층 Supabase 이전**~~ → **완료(07-31).** 위 "저장 계층" 절 참고.
   라이브 관통·이관·멱등·낙관적 락 실측까지 확인. 다음은 **인증·RLS**다 —
   `owner_id` 컬럼과 RLS는 이미 자리를 잡아 뒀고, addog/Fixup-Insight 의 Supabase Auth +
   승인제 + 백오피스가 프로덕션 선례다(ES256/JWKS 로 검증해야 하는 함정 포함)
