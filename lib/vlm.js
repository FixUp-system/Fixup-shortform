...
export async function describePhoto({ photoBytes, photoKey, projectId, fetchImpl = fetch, apiKey = process.env.OPENAI_API_KEY }) {
  const none = { person: false, what: "", who: null, lettering: "", scale: "" };
  ...
  try {
    const content = [
      { type: "text", text: `이 사진에 무엇이 담겼는지 본다. JSON만 출력: {"person":true|false,"what":"무엇이 보이는지 한 마디 — **색**을 반드시 넣는다","who":"사람이면 나이대와 성별(예: 50대 남성), 사람이 아니면 빈 문자열","lettering":"제품에 인쇄된 글자를 보이는 그대로 (없거나 못 읽으면 빈 문자열)","scale":"이 물건이 얼마나 큰지 — 함께 찍힌 것에 견주어 한 마디 (모르면 빈 문자열)"}
person 은 사람의 얼굴·상반신이 알아볼 수 있게 담겼을 때만 true 다. 멀리 지나가는 행인이나 뒷모습만 있으면 false 다.
what 에는 **눈에 보이는 색**을 반드시 적는다(예: "가방에 달린 보라색 토끼 인형"). 색을 빼면 뒤에서 우리가 색을 지어내게 된다.
...
scale 은 **다른 사물에 견준 크기**다 — "작다"가 아니라 무엇에 견주어 얼마나인지 적는다
(예: "가방 앞면의 4분의 1도 안 되는 작은 참", "손바닥 절반 크기"). 함께 찍힌 것이 없어
견줄 데가 없으면 **비운다** — 짐작한 크기가 그대로 그림의 크기가 된다.` },
    ];
    if (photoBytes) {
    ...
    return {
      ...
      // ★★ 크기·비례(2026-08-19 사장님 지적: "키링인데 가방 전면보다 크게 그려진다").
      //   프롬프트 어디에도 크기가 없었다 — 모델이 크기를 정할 근거가 아예 없는데, 반대로
      //   "이것이 주인공이다·일관되게 유지하라"고 미는 문장은 여러 번 실린다. 그러면 모델은
      //   주인공을 화면에 크게 그린다. 근거를 여기서 만든다.
      //   ★ `what` 에 섞지 않고 칸을 따로 두는 이유: `what` 은 사물 사진을 컷에 붙일 때
      //     짝을 맞추는 데도 쓰인다(lib/pipeline.js things). 거기에 크기 서술이 섞이면
      //     짝 맞추기가 흐려진다. 크기는 크기 칸에 둔다.
      //   ★ 옛 프로젝트의 저장된 vision 에는 이 칸이 없다(`if (p.vision) continue` 라 다시
      //     안 읽는다) — 읽는 쪽은 전부 `|| ""` 로 없는 것을 견딘다.
      scale: typeof out.scale === "string" ? out.scale.trim().slice(0, 80) : "",
    };
  } catch {
