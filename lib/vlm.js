...
export async function selectCandidate({ cut, scene, candidates, refImage, projectId, fetchImpl = fetch, apiKey = process.env.OPENAI_API_KEY }) {
...
검수 기준: 장면 설명과 일치 / 신체·손가락 오류 / 이미지 안 글자 깨짐 / 거울·유리 반사가 실제와 어긋남(등지고 선 사람이 거울에 정면으로 비치는 등) / 그림자 방향이 빛과 어긋남${refImage ? " / 레퍼런스 피사체와 외형 일치" : ""}` },
