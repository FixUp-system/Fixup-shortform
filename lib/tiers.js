...
export const TIERS = Object.freeze([
  Object.freeze({
    // ★★ 2026-09-01 — 기본은 **H3 하나**다. 2.0 이 프로가 되면서 올라갔다.
    id: "basic", label: "기본", hint: "MiniMax H3",
    models: Object.freeze(["minimax-h3"]),
  }),
  Object.freeze({
    // ★★★ 2026-09-01 — **프로가 2.0 이다**(사장님 지시). 2.5 는 목록에서 내렸지만
    //   등급에는 남긴다 — 이미 2.5 로 만든 문서가 다시 구워져야 한다(hidden 을 안 쓰는 이유).
    id: "pro", label: "프로", hint: "Seedance 2.0 까지",
...
export function modelsForTier(tier, { admin = false } = {}) {
  // ★★ 2026-09-01 — `retired` 도 여기서 걸러진다. **고르는 목록**이기 때문이다.
  //   이미 그 모델로 만든 문서의 재굽기는 아래 tierAllowsModel 이 따로 열어 준다.
...
export function tierAllowsModel(tier, modelId, opts) {
  if (modelsForTier(tier, opts).some((m) => m.id === modelId)) return true;
  // ★★★ **은퇴 모델은 여기서만 통과한다**(2026-09-01). 고르는 자리에는 안 나오지만
  //   이미 그것으로 만든 문서는 다시 구워져야 한다 — 안 그러면 사장님이 값을 치르고
  //   만든 영상을 손볼 수 없다(08-31 에 2.0 을 숨겨 정확히 그 상태를 만들었다).
  //   ★ `hidden` 은 여전히 통과 못 한다 — 그것은 "고장 났거나 은퇴했다"가 아니라
  //     **"아무도 못 쓴다"** 다.
  const m = AD_MODELS.find((x) => x.id === modelId);
  if (!m || m.hidden || !m.retired) return false;
  if (opts?.admin === true) return true;
  const t = TIERS.find((x) => x.id === tier) || TIERS.find((x) => x.id === DEFAULT_TIER);
  return t.models.includes(modelId);
}
