...
// 테스트 모드 — SHOTFORM_FAKE_IMAGES=1이면 fal을 부르지 않고 플레이스홀더를 즉시 돌려준다.
// 비싼 이미지 생성 없이 파이프라인·화면을 반복 확인하기 위한 것. 비용도 기록하지 않는다.
function fakeEnabled() {
  return process.env.SHOTFORM_FAKE_IMAGES === "1";
}
...
export async function generateImage({ prompt, aspect_ratio, refImagePath, fetchImpl = fetch }) {
  if (fakeEnabled()) return { url: placeholderImage(prompt, aspect_ratio) };
