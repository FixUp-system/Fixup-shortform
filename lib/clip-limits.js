...
//
// 화면 밖 내레이션(사물·정보 영상)의 설계는 아직 없다. 그때도 여기서 false 로 떨어지는데,
// 이제는 **추론이 아니라 시나리오가 적어 준 값**으로 떨어진다(아래 narration 절).
export function projectSpeaks(project) {
  ...
  // ★ 시나리오가 **화면 밖 목소리**로 표시한 컷이 하나라도 있으면 이 프로젝트는 말하지 않는다.
  //
  // 예전에는 그 결정이 코드에 닿지 않아 캐스팅 결과로 추론됐다(lib/cuts.js shotsToCuts 머리말
  // 참고, 2026-08-16 최종 리뷰 Critical 1). 캐스팅이 지문대로 내레이션을 빼면 그 컷에 사람이
  // 없어 아래 every() 가 거짓 — 지금과 결론은 같지만, 캐스팅이 사람을 붙여 버리면 화면 밖
  // 대사를 인물이 립싱크했다. **같은 시나리오가 두 결과를 냈다.**
  //
  // 여기서 false 를 내면 한 편이 통째로 TTS(③목소리)로 간다 — 그 대사는 실제로 소리가 되고
  // 사장님 눈에 단계가 하나 다시 나타난다. 대안(클립에게 화면 밖 목소리를 시키는 것)은
  // **아무도 검증한 적 없는 능력**이라, 실패해도 mp4 가 나와 조용히 대사만 사라진다.
  // 눈에 보이게 지는 쪽을 고른다.
  if (cuts.some((c) => c?.narration === true)) return false;
  ...
  // TTS 로 끌어내린다.
  //
  // ⚠️ 뺀 것은 이 컷들뿐이다. **대사가 있는 컷에는 여전히 말할 사람이 있어야 한다** —
  //    화면 밖 내레이션의 설계가 아직 없어서, 그 조건까지 풀면 원고가 소리 없이 사라진다.
