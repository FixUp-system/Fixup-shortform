...
export const CLIP_PROFILES = [
  ...
  },
  {
    ...
    // ★★ 2026-09-07 — **1080p 를 뺐다**(사장님 지시). 이 모델이 단계별의 "기본"이고,
    //   clip 축에서 1080p 를 열던 유일한 자리였다. 값이 크게 갈리는 축이라(30초 기준
    //   720p 160 · 1080p 360 크레딧) 고를 수 있게 두는 것 자체가 비용이었다.
    //   ★ **광고는 그대로다** — 광고의 해상도 목록은 lib/ad/models.js 의 독립된 표이고
    //     거기 1080p 는 가격까지 붙어 관리자 전용으로 열려 있다. 두 축을 섞지 않는다.
    //   ★ 옛 문서는 안 깨진다 — resolutionForProject 가 목록 밖 저장값을 그 모델의
    //     기본(720p)으로 떨어뜨린다. 실측으로 1080p 로 저장된 프로젝트는 0건이었다.
  ...
  },
  {
    ...
    refAspect: { min: 0.4, max: 2.5 },
    // ★★★ 2026-09-03 — **`facesInRefs: false` 를 걷어냈다**(사장님 지시). 그전에는
    //   "참조에 얼굴이 있으면 무조건 막는다"였고 근거는 2026-08-31~09-01 실측이었다.
    //   **그 실측은 격자 방식이 틀렸던 것이다.**
    //     · 그때 쓴 격자: 판 전체 · 27×27 · 2px · **시안 반투명** → 8회 전부 거절
    //     · 09-03 실측: **얼굴에만 · 흰색 · 불투명 10×10** → **2.0 과 2.5 둘 다 통과**
    //       (2.5 는 4초 480p 로 확인 · billable 38.83)
    //   즉 2.5 가 얼굴을 못 받는 것이 아니라 **우리가 얼굴을 가리는 법을 몰랐다.**
    //   격자는 굽기 직전에 코드가 씌운다(lib/reel/face-grid.js · pipeline 의 sheetRef).
    //
    //   ⚠️ **이 값을 되돌리려면 격자 배선도 함께 봐라.** 이 한 줄이 넷을 동시에 푼다:
    //     ① 화면의 `＋인물` 버튼(visiblePhotoRoles) ② 참조 적재(describeCutRefs)
    //     ③ 판을 얼굴 없이 그리기(panels 의 faceSafeOn) ④ 원클릭 화면(blocksFacesForEndpoint)
    //   격자 없이 이 줄만 지우면 얼굴 든 판이 그대로 나가 다시 거절된다.
...
export const I2V_MODELS = [
  // ★★ 순서가 곧 칩 순서다 — **기본이 프로보다 앞**(2026-08-31, 원클릭과 같은 규칙).
  {
    ...
    // ★ 2026-09-01 — 단계별에서 내려가며 등급 이름을 반납했다(원클릭에서는 여전히 "기본"이다).
    label: "MiniMax H3",
  ...
  },
  {
    ...
    // ★★★ 2026-09-01 — **단계별의 기본**(사장님 지시). 얼굴 든 참조를 받는 유일한 모델이다.
    label: "기본",
...
// ⚠️ 2026-09-01 — **되돌렸다**(사장님 지시). 단계별의 기본이 다시 2.0 이다 —
//   인물이 주인공인 영상은 판에 얼굴이 있어야 하는데, 그 조건을 만족하는 모델이 2.0 뿐이다.
//   ★ 이 값은 **REEL_MODEL_IDS 의 첫 항목과 같아야 한다** — 안 그러면 화면이 못 고르는
//     모델이 기본값으로 저장된다.
...
// ★ 2026-09-16 — 보관함 카드·상세의 **모델 배지**용. modelIdForProject 와 달리
//   **추측하지 않는다** — 값이 없거나 모르면 null 이다. 옛 문서를 LEGACY_I2V_MODEL 로
//   떨어뜨려 "Kling v3 로 만들었다"고 말하면, 실제로는 이 기능이 붙기 전이라 무엇으로
//   만들었는지 모르는 것을 안다고 말하는 셈이다(사장님 지시: "그때도 아무것도 안 그린다").
export function i2vModelLabelOf(id) {
  return I2V_MODELS.find((m) => m.id === id)?.label || null;
}
...
// 같은 판정을 **엔드포인트로 바로** 묻는 판 — 프로젝트 모양이 다른 광고(원클릭)가 쓴다
// (광고는 settings.model 이 AD_MODELS 의 id 라 clipProfileForProject 를 못 탄다).
// 표는 위 PROFILES 하나다 — 광고 쪽에 얼굴 표를 따로 두면 두 표가 갈린다.
export function blocksFacesForEndpoint(endpoint) {
  return profileFor(endpoint)?.facesInRefs === false;
}

// 큐 주소의 앱 이름 — 엔드포인트 앞 두 마디다(`bytedance/seedance-2.0/text-to-video`
// → `bytedance/seedance-2.0`). fal 의 큐 API 가 그 단위로 답한다(2026-09-08 실측).
//
// ★ 2026-09-09 — 광고(lib/ad/pipeline.js)에 있던 것을 **여기로 옮겼다.** reel 구조선도
//   같은 계산이 필요해졌고, 두 벌로 두면 fal 이 큐 주소 규칙을 바꿀 때 한쪽만 고쳐진다.
//   이 파일은 엔드포인트 표가 사는 자리이고 import 가 순수하므로 여기가 맞다.
export function queueAppOf(endpoint) {
  return String(endpoint || "").split("/").slice(0, 2).join("/");
}
...
// ★★★ 2026-09-01 — **단계별의 기본이 H3 → 2.0 으로 바뀌었다**(사장님 지시).
//   근거는 그날 실측이다: 2.5 는 참조 이미지에 얼굴이 있으면 **아홉 번 전부 거절**했고
//   (`partner_validation_failed`), 2.0 은 **같은 판을 첫 시도에 통과**시켰다.
//   브이로그처럼 **인물이 주인공인 영상**은 판에 얼굴이 있어야 하는데, 그 조건을
//   만족하는 모델이 2.0 뿐이었다. 값도 싸고($4.55 대 $6.93) 빠르다(217초 대 396초).
//   ★ **원클릭은 안 바뀐다** — 그쪽은 기본이 H3 그대로다(표가 다르다: AD_MODELS).
//     원클릭은 사장님이 올린 사진을 넘기고 인물은 글로만 요구하므로 이 문제가 없다.
//   ⚠️ 대가: 2.0 의 통짜 상한이 15초라 단계별에서 **H3 의 5초 짧은 컷**이 사라진다.
export const REEL_MODEL_IDS = Object.freeze(["seedance-2.0", "seedance-2.5"]);
...
// ★★★ **관리자는 등급을 안 탄다**(2026-09-02) — 원클릭 세 라우트와 같은 축이다
//   (lib/tiers.js 의 modelsForTier 머리말). 등급은 "손님이 무엇을 살 수 있나"이고
//   관리자는 손님이 아니다.
// ★ 기본값이 **닫힌 쪽**이다 — 넘기는 것을 잊으면 덜 열어 주는 방향으로 틀린다.
export function reelModelsForTier(tier, { admin = false } = {}) {
  const open = I2V_MODELS.filter((m) => isReelModel(m.id));
...
// 서버가 보는 판정 — 이 사람이 단계별에서 이 모델을 써도 되는가.
//
// ★★★ 2026-09-02 — **단계별은 단계별 표로 판정한다.** 그전에는 원클릭의 tierAllowsModel
//   (AD_MODELS 를 거른다)을 썼는데, 08-31 에 2.0 을 원클릭에서 은퇴시키자(`hidden: true`,
//   480p 결함) 그 표시가 **단계별까지 막았다**. 은퇴 표시는 등급·역할보다 강해서 관리자도
//   못 뚫는다 — 09-01 에 단계별 기본이 2.0 이 되면서 **아무도 단계별 영상을 못 만들었다.**
//   표가 둘인데 문이 하나면 반드시 갈린다. 화면(reelModelsForTier)과 **같은 함수**를 본다.
// ★ 모르는 모델은 **관리자도** 못 쓴다 — 통과시키면 값이 나간 **뒤에** fal 이 404 를 낸다.
export function reelAllowsModel(tier, modelId, opts) {
  return reelModelsForTier(tier, opts).some((m) => m.id === modelId);
}
...
// 시작에 실을 길이 — ①입력의 [시작하기] 잠금과 보내는 값이 **같은 판정**을 본다(2026-09-15 회귀 처방).
// ★★★ 선택지가 하나면 그 값이 곧 고른 값이다. 화면은 고를 게 하나뿐인 줄을 숨기는데(누를 수 없는 칩),
//   그때도 "안 골랐다"로 잠그면 사장님이 누를 칸이 없는 채로 버튼이 영원히 잠긴다(실제로 라이브에서 그랬다).
// ★ 둘 이상이면 **고른 값만** — 기본값을 몰래 박으면 사장님이 못 보고 지나간다(길이는 안 고르면 막는 축이다).
//   모델이 안 받는 값을 쥐고 있으면 고르지 않은 것으로 본다(서버가 400 이다).
export function pickedSeconds(modelId, target) {
  const choices = secondsForModel(modelId);
  if (choices.length === 1) return choices[0];
  return choices.includes(target) ? target : null;
}
