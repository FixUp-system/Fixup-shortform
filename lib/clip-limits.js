...
export function projectSpeaks(project) {
  ...
  // ★ 대사가 없는 컷은 이 판정에서 뺀다 (2026-08-15).
  //
  // 컷 분할은 **대사 없는 컷을 정상적으로 만든다**(`silent: true`, 빈 문장) — 프로덕션
  // 실측 810d2361 은 컷 3개 중 둘이 그렇다. 예전에는 "모든 컷에 대사가 있어야 한다"였는데,
  // 그러면 말할 것이 없는 컷 하나 때문에 한 편 전체가 TTS 경로로 떨어졌다.
  // 위 규칙의 취지("원고 일부가 안 들리면 안 된다")는 **대사가 있는 컷**에만 걸린다 —
  // 빈 컷에는 안 들릴 원고가 없다.
  //
  // ⚠️ 걷어낸 것은 문장 조건뿐이다. **대사가 있는 컷에는 여전히 말할 사람이 있어야 한다** —
  //    화면 밖 내레이션의 설계가 아직 없어서, 그 조건까지 풀면 원고가 소리 없이 사라진다.
  const speaking = cuts.filter((cut) => typeof cut?.sentence === "string" && cut.sentence.trim() !== "");
  // ⚠️ 전 컷이 무음이면 읽을 원고가 없다 — 말하는 프로젝트가 아니다.
