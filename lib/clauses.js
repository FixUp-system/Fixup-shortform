...
//
// **여기 두는 이유는 import 그래프다.** 이 규칙을 쓰는 쪽이 둘인데 서로를 못 끈다:
// 컷별 덮어쓰기(lib/cuts.js promptOverride)와 프로젝트 공통 지시(lib/styles.js promptNoteOf).
// cuts.js → styles.js 방향의 import 가 이미 있어 반대는 순환이고, styles.js 는 "화풍 프리셋"
// 이라 문장부호 규칙의 집으로는 결이 다르다. 이 파일은 import 없는 leaf 이고 이미
// **문장·절 경계 판정의 유일한 집**이라(머리말 참고) 둘 다 여기서 끌 수 있다.
//
