...
// 카드 그림의 비율 — **영상 비율을 따르지 않는다**(2026-09-02 사장님 지시: "보드 안
// 이미지는 같은 비율일 필요 없다"). 대신 **격자가 캔버스를 꽉 채우도록 푸는 변수**다.
// 아무 비율이나 되는 것은 아니다 — 보기 좋은 범위(세로 4:5 ~ 가로 3:2)로 죈다.
// 그림은 fit:cover 로 잘라 앉히므로 어떤 비율이든 안 찌그러진다.
const IMG_AR_MIN = 0.72;   // 4:5 보다 조금 세로
const IMG_AR_MAX = 1.5;    // 3:2
const IMG_AR_IDEAL = 1.05; // 참조 시안의 카드 그림이 이 언저리다
...
export function boardLayout(cutCount, aspectId) {
  ...
  const areaH1 = (1 / target) * (1 - HEADER_RATIO) - 2 * PAD_RATIO;
  const areaAR = areaW1 / Math.max(0.05, areaH1);

  // 열 수마다 "그 자리를 정확히 채우는" 그림 비율을 **역산**하고, 범위로 죈 뒤
  // ① 남는 자투리 ② 이상 비율과의 거리 로 점수를 매긴다.
  let best = null;
  for (let cols = 1; cols <= n; cols++) {
    const rows = Math.ceil(n / cols);
    const gw = cols + GAP_RATIO * (cols - 1);
    const wantH1 = (gw / areaAR - GAP_RATIO * (rows - 1)) / rows; // 채우는 카드 높이
    const wantAR = (1 - 2 * PADC_RATIO) / (wantH1 - 2 * PADC_RATIO - CAP_RATIO);
    const imgAR = Math.min(IMG_AR_MAX, Math.max(IMG_AR_MIN, wantAR > 0 ? wantAR : IMG_AR_MAX));
    const cardH1 = 2 * PADC_RATIO + (1 - 2 * PADC_RATIO) / imgAR + CAP_RATIO;
    const gh = rows * cardH1 + GAP_RATIO * (rows - 1);
    const leftover = Math.abs(Math.log((gw / gh) / areaAR));      // 못 채우는 자투리
    const taste = Math.abs(Math.log(imgAR / IMG_AR_IDEAL));      // 이상 비율과의 거리
    const score = leftover * 2 + taste * 0.35;
    if (!best || score < best.score) best = { cols, rows, gw, gh, score, imgAR, cardH1 };
  }
  const imgAR = best.imgAR;
  const cardH1 = best.cardH1;
...
export function boardSvg({ project, cuts = [], layout, layer = "full", palette }) {
  ...
  if (kicker) over.push(glyph("serif", kicker, L.pad, headTop + headH * 0.2, fsKick, P.ink));
  ...
  over.push(glyph("serif", TITLE, L.pad, headTop + headH * 0.2 + fsTitle * 0.95, fsTitle, P.green));
  // 제목 아래 가는 초록 줄 — 참조의 "— STORYBOARD —" 자리(장식 축).
  const ruleY = headTop + headH * 0.2 + fsTitle * 1.18;
  over.push(`<path d="M ${L.pad} ${ruleY} H ${L.pad + leftW * 0.5}" stroke="${P.green}" stroke-width="${Math.max(1.5, headH * 0.008)}"/>`);
  ...
  boxes.forEach((b, i) => {
    ...
    over.push(`<rect x="${bx}" y="${headTop}" width="${bW}" height="${h1}" rx="${boxR}" fill="${P.card}" stroke="${P.line}" stroke-width="1.5"/>`);
    const is = h1 * 0.34;
    over.push(icon(b.icon, bx + bW * 0.09, headTop + h1 * 0.33, is, P.green));
    const tx = bx + bW * 0.09 + is + bW * 0.06;
    over.push(glyph("bold", b.label, tx, headTop + h1 * 0.42, h1 * 0.15, P.muted));
    over.push(glyph("bold", b.value, tx, headTop + h1 * 0.75, h1 * 0.21, P.ink));
  });
  const narration = String(project?.scenario?.narration?.text || "").trim();
  if (narration) {
    const ny = headTop + h1 + rowGap;
    over.push(`<rect x="${panX}" y="${ny}" width="${panW}" height="${h2}" rx="${boxR}" fill="${P.card}" stroke="${P.line}" stroke-width="1.5"/>`);
    const is = h2 * 0.2;
    over.push(icon("wave", panX + panW * 0.035, ny + h2 * 0.16, is, P.green));
    over.push(glyph("bold", "내레이션", panX + panW * 0.035 + is + panW * 0.02, ny + h2 * 0.3, h2 * 0.15, P.green));
    const fsN = h2 * 0.14;
    wrapPx("body", narration, fsN, panW * 0.92, 3).forEach((ln, i2) =>
      over.push(glyph("body", ln, panX + panW * 0.04, ny + h2 * 0.5 + i2 * fsN * 1.45, fsN, P.body))
    );
  }
