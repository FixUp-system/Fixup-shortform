...
// ★ named import 다 — 이 패키지는 default export 가 없어 default 로 받으면 웹팩이 경고한다.
...
const BASE_W = 1600;
const PADC_RATIO = 0.045;  // 카드 안 여백 / 카드 폭 (그림이 카드 가장자리에 붙지 않게)
const CAP_RATIO = 0.30;    // 캡션 높이 / 카드 폭
const GAP_RATIO = 0.05;    // 카드 사이 / 카드 폭
const PAD_RATIO = 0.04;    // 캔버스 가장자리 / 캔버스 폭
const HEADER_RATIO = 0.15; // 머리글 높이 / 캔버스 높이

// 참조 시안의 판 — 크림 바탕, 진초록 포인트, 흰 카드.
const C = {
  bg: "#F5F1E6",
  ink: "#22301F",
  green: "#2F5D3F",
...
export function boardLayout(cutCount, aspectId) {
  ...
  for (let cols = 1; cols <= n; cols++) {
    ...
    const score = Math.abs(Math.log((gw / gh) / target)); // 로그 거리 — 방향 무관 동일 무게
...
function icon(kind, x, y, s, color = C.green) {
...
export function boardSvg({ project, cuts = [], layout, layer = "full" }) {
  ...
  if (layer !== "overlay") base.push(`<rect width="100%" height="100%" fill="${C.bg}"/>`);

  // ── 머리글 — 왼쪽 제목(세리프 두 줄), 오른쪽 메타 상자들.
  //   가로 경계는 실측 폭으로 지킨다(제목 크기를 폭에서 뽑는다) — 상수 추정은 두 번 틀렸다.
  const headTop = L.pad;
  const headH = L.header.h;
  const leftW = W * 0.44 - L.pad;
  const moodLabel = AD_MOODS.find((m) => m.id === s.mood)?.label || s.mood || "";
  const styleLabel = styleFor(s.style)?.label || s.style || "";
  const kicker = [moodLabel, styleLabel].filter(Boolean).join(" · ");
  const fsKick = headH * 0.11;
  if (kicker) over.push(glyph("serif", kicker, L.pad, headTop + headH * 0.2, fsKick, C.ink));
  const TITLE = "STORYBOARD";
  let fsTitle = headH * 0.36;
  {
    const F = loadFonts();
    const w0 = adv(F === false ? null : F.serif, TITLE, fsTitle);
    if (w0 > leftW) fsTitle = (fsTitle * leftW) / w0;
  }
  over.push(glyph("serif", TITLE, L.pad, headTop + headH * 0.2 + fsTitle * 0.95, fsTitle, C.green));
  // 제목 아래 가는 초록 줄 — 참조의 "— STORYBOARD —" 자리(장식 축).
  const ruleY = headTop + headH * 0.2 + fsTitle * 1.18;
  over.push(`<path d="M ${L.pad} ${ruleY} H ${L.pad + leftW * 0.5}" stroke="${C.green}" stroke-width="${Math.max(1.5, headH * 0.008)}"/>`);

  // 오른쪽 메타 패널 — 상자 문법은 참조와 같다: 흰 상자 + 테두리 + 아이콘 + 라벨/값.
  const panX = W * 0.48;
  const panW = W - panX - L.pad;
  const rowGap = headH * 0.06;
  const h1 = headH * 0.38;
  const h2 = headH * 0.5;
  const boxR = Math.max(8, h1 * 0.18);
  const boxes = [
    { icon: "clock", label: "DURATION", value: `${s.target_seconds || s.seconds || "—"}초` },
    { icon: "frame", label: "RATIO", value: `${s.aspect_ratio || "9:16"} ${aspectFor(s.aspect_ratio).label}` },
    { icon: "grid", label: "CUTS", value: `${cuts.length}컷` },
  ];
  const bGap = panW * 0.025;
  const bW = (panW - bGap * 2) / 3;
  boxes.forEach((b, i) => {
    const bx = panX + i * (bW + bGap);
    over.push(`<rect x="${bx}" y="${headTop}" width="${bW}" height="${h1}" rx="${boxR}" fill="${C.card}" stroke="${C.line}" stroke-width="1.5"/>`);
    const is = h1 * 0.34;
    over.push(icon(b.icon, bx + bW * 0.09, headTop + h1 * 0.33, is));
    const tx = bx + bW * 0.09 + is + bW * 0.06;
    over.push(glyph("bold", b.label, tx, headTop + h1 * 0.42, h1 * 0.15, C.muted));
    over.push(glyph("bold", b.value, tx, headTop + h1 * 0.75, h1 * 0.21, C.ink));
  });
  const narration = String(project?.scenario?.narration?.text || "").trim();
  if (narration) {
    const ny = headTop + h1 + rowGap;
    over.push(`<rect x="${panX}" y="${ny}" width="${panW}" height="${h2}" rx="${boxR}" fill="${C.card}" stroke="${C.line}" stroke-width="1.5"/>`);
    const is = h2 * 0.2;
    over.push(icon("wave", panX + panW * 0.035, ny + h2 * 0.16, is));
    over.push(glyph("bold", "내레이션", panX + panW * 0.035 + is + panW * 0.02, ny + h2 * 0.3, h2 * 0.15, C.green));
    const fsN = h2 * 0.14;
    wrapPx("body", narration, fsN, panW * 0.92, 3).forEach((ln, i2) =>
      over.push(glyph("body", ln, panX + panW * 0.04, ny + h2 * 0.5 + i2 * fsN * 1.45, fsN, C.body))
    );
  }
  ...
  cuts.forEach((cut, i) => {
    ...
    base.push(
      `<rect x="${x}" y="${y + cw * 0.008}" width="${cw}" height="${L.card.h}" rx="${L.card.r}" fill="${C.shadow}"/>`,
      `<rect x="${x}" y="${y}" width="${cw}" height="${L.card.h}" rx="${L.card.r}" fill="${C.card}" stroke="${C.line}" stroke-width="1.5"/>`,
      `<rect x="${imgX}" y="${imgY}" width="${L.card.imgW}" height="${L.card.imgH}" rx="${L.card.imgR}" fill="${C.imgSlot}"/>`
    );
    ...
    const fsNo = bw * 0.58;
    over.push(
      `<rect x="${imgX + bw * 0.35}" y="${imgY + bw * 0.35}" width="${bw}" height="${bw}" rx="${bw * 0.22}" fill="${C.green}"/>`,
    ...
    const tcH = fsTc * 1.9;
    over.push(
      `<rect x="${imgX + L.card.imgW - tcW - bw * 0.35}" y="${imgY + bw * 0.35}" width="${tcW}" height="${tcH}" rx="${tcH / 2}" fill="${C.green}" opacity="0.95"/>`,
    ...
    over.push(`<path d="M ${imgX} ${capTop} H ${imgX + innerW}" stroke="${C.divider}" stroke-width="1.5"/>`);
    ...
    cells.forEach((c, k) => {
      ...
      if (k > 0) over.push(`<path d="M ${cx0} ${capTop + 6} V ${y + L.card.h - padc}" stroke="${C.divider}" stroke-width="1.2"/>`);
      const cx = cx0 + colW * 0.08;
      const iy = capTop + fsLb * 0.9;
      const is = fsLb * 1.25;
      over.push(icon(c.icon, cx, iy, is));
      over.push(glyph("bold", c.label, cx + is + fsLb * 0.5, iy + is * 0.82, fsLb, C.green));
      wrapPx("body", c.text, fsBd, colW * 0.84, 3).forEach((ln, li) =>
        over.push(glyph("body", ln, cx, iy + is + fsBd * (1.15 + li * 1.3), fsBd, C.body))
      );
    });
  });
...
export async function drawBoard({ project, cuts = [], readImage, sharpImpl }) {
  ...
  const bg = await sharp(Buffer.from(boardSvg({ project, cuts, layout, layer: "base" }))).png().toBuffer();
  const overlay = Buffer.from(boardSvg({ project, cuts, layout, layer: "overlay" }));
