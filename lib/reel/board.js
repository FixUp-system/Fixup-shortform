...
export function boardLayout(cutCount, aspectId) {
  ...
  return {
    ...
    height: geo.height,
    gap: cardW * GAP_RATIO,
    // 세로 간격 = 기본 간격 + 균등 분배분. 가로(gap)와 다른 값일 수 있다.
    gapY: cardW * GAP_RATIO + geo.extraPer,
    pad: width * PAD_RATIO,
...
function fit(width, target, best, cardH1) {
  ...
  // 세로 잉여를 (행 수 + 1) 자리 — 격자 위·행 사이·격자 아래 — 에 똑같이 나눈다.
  const extra = Math.max(0, areaH - cardW * best.gh);
  const extraPer = extra / (best.rows + 1);
  return {
    height, headerH, cardW, extraPer,
    originX: (width - cardW * best.gw) / 2, // 가로 자투리는 좌우 가운데
    originY: pad + headerH + extraPer,
  };
}
