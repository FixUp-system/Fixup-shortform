...
const HEADER_RATIO = 0.05; // 머리글 높이 / 캔버스 높이 — 제목 한 줄
...
function fit(width, target, best, cardH1) {
  ...
  // ★ 세로 자투리는 **전부 제목 자리로** 몬다(격자를 바닥에 붙인다) — 고정 비율 그림을
  //   고정 비율 캔버스에 채우면 자투리가 수학적으로 남는데, 위아래로 가르면 둥둥 떠 보인다
  //   (2026-09-02 실측 306px×2). 포스터처럼 머리판이 숨을 쉬고 아래는 그림으로 꽉 찬다.
  return {
    height, headerH, cardW,
    originX: (width - cardW * best.gw) / 2,
    originY: pad + headerH + (areaH - cardW * best.gh),
  };
}
...
export function boardSvg({ project, cuts = [], layout, layer = "full", palette }) {
  ...
  // 제목은 격자 위 공간(자투리 포함)의 **세로 가운데**에 선다.
  const TITLE = "STORYBOARD";
  const headSpace = Math.max(L.header.h, L.origin.y - L.pad);
  let fsTitle = Math.min(headSpace * 0.42, W * 0.085);
  {
    const F = loadFonts();
