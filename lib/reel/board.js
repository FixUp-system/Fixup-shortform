...
export function boardLayout(cutCount, aspectId) {
  ...
  // ★★★ 2026-09-02 사장님 최종 확정: **보드도 이미지도 정확히 영상 비율**. 그리고 같은 날
  //   후속 지시 — "비율은 유지하고 이미지 컷 자체 크기를 키우면 안 되는 거야?"
  //   그래서 열 수 규칙이 "세로 꽉 채움"에서 **카드 최대화**로 바뀌었다:
  //   각 열 수에서 가로·세로에 다 들어가는 최대 카드를 재고, 그중 큰 쪽을 고른다
  //   (9:16 5컷: 2열 445px → 3열 505px, 3+2 두 줄). 남는 공백은 수학이다 — 위·줄 사이·
  //   아래에 **균등 분배**해 일부러 띄운 간격으로 읽히게 한다(한 덩어리로 몰면 빈 자리로
  //   보인다 — 같은 날 실측).
  const imgAR = target;
  const cardH1 = 2 * PADC_RATIO + (1 - 2 * PADC_RATIO) / imgAR + CAP_RATIO;
  const areaH1v = 1 / target - HEADER_W_RATIO - 2 * PAD_RATIO; // 폭 1 기준 세로 자리
  const areaW1v = 1 - 2 * PAD_RATIO;
  let best = null;
  for (let cols = 1; cols <= n; cols++) {
    const rows = Math.ceil(n / cols);
    const gw = cols + GAP_RATIO * (cols - 1);
    const gh = rows * cardH1 + GAP_RATIO * (rows - 1);
    const card1 = Math.min(areaW1v / gw, areaH1v / gh); // 가로·세로에 다 들어가는 최대 카드
    if (!best || card1 > best.card1 + 1e-9) best = { cols, rows, gw, gh, card1 };
  }
  ...
  return {
    ...
    height: geo.height,
    gap: cardW * GAP_RATIO,
    // 세로 간격 = 가로와 같은 기본 간격 — 줄은 붙이고 잉여는 위아래로(fit 머리말).
    gapY: cardW * GAP_RATIO,
    pad: width * PAD_RATIO,
...
// 캔버스는 **정확히 영상 비율**이고, 카드는 가장 크게 — 남는 세로는 균등 분배(머리말).
function fit(width, target, best, cardH1) {
  ...
  const cardW = Math.min(areaH / best.gh, areaW / best.gw);
  // ★ 2026-09-02 최종 — 세로 잉여는 **위아래로만** 보낸다(사장님: "차라리 위아래 여백을
  //   남기더라도 줄 사이 간격을 줄여줘"). 줄 사이는 기본 간격 그대로 붙는다.
  const extra = Math.max(0, areaH - cardW * best.gh);
  return {
    height, headerH, cardW, extraPer: 0,
    originX: (width - cardW * best.gw) / 2, // 가로 자투리는 좌우 가운데
    originY: pad + headerH + extra / 2,
  };
}
...
export function cardAt(L, i, total) {
  ...
  return {
    ...
    y: L.origin.y + row * (L.card.h + (L.gapY ?? L.gap)),
  };
}
...
export async function drawBoard({ project, cuts = [], readImage, sharpImpl }) {
  ...
  // 로고 — **우측 하단**(2026-09-02 사장님 지시). 저장소 파일(public/board-logo.png,
  //   원본은 사장님이 준 dnc.png)이어야 배포에서도 산다 — 다운로드 폴더를 읽으면 로컬에서만 돈다.
  //   못 읽어도 보드는 나온다(곁들이는 표식이 본체를 죽이면 안 된다).
  const logo = await (async () => {
    try {
      const lw = Math.round(W * 0.07);
      const buf = await sharp(path.resolve(process.cwd(), "public/board-logo.png"))
        .resize({ width: lw }).png().toBuffer();
      const meta = await sharp(buf).metadata();
      return {
        input: buf,
        left: Math.round(W - lw - layout.pad),
        top: Math.round(H - (meta.height || 0) - layout.pad),
      };
    } catch { return null; }
  })();
  ...
  const bytes = await sharp(bg)
    .composite([...composites, { input: overlay }, ...(logo ? [logo] : [])])
    .png().toBuffer();
  return { bytes, layout };
}
