...
export function boardLayout(cutCount, aspectId) {
  ...
  // ★★★ 2026-09-02 사장님 최종 확정(강조 지시): **보드도 이미지도 정확히 영상 비율**이다.
  //   세로를 내용에 맞추던 직전 판(e2f2e4a)은 보드 비율을 깨뜨려서 되돌렸다.
  //   두 비율이 다 고정되면 자투리가 수학적으로 남는다 — 사장님이 지운 것이 **위아래**이므로,
  //   격자가 **세로를 꽉 채우게** 하고 자투리는 좌우 여백으로 보낸다(가운데 정렬).
  //   열 수 규칙: 세로를 채웠을 때 폭 안에 들어가는 배치 중 **카드가 가장 큰 것**.
  const imgAR = target;
  ...
  for (let cols = 1; cols <= n; cols++) {
    ...
    const cardHfit = areaH1v / gh;             // 세로를 꽉 채우는 카드 폭(폭 1 기준)
    const fits = cardHfit * gw <= areaW1v;     // 그 폭이 가로 안에 드는가
    if (fits && (!best || cardHfit > best.card1)) best = { cols, rows, gw, gh, card1: cardHfit };
    // 어느 열도 세로 채움이 불가능한 극단(그럴 일은 거의 없다)을 위한 폴백 — 가로 채움
    const cardWfit = areaW1v / gw;
    if (!fallback || cardWfit > fallback.card1) fallback = { cols, rows, gw, gh, card1: cardWfit };
  }
...
// 캔버스는 **정확히 영상 비율**이고, 격자는 세로를 꽉 채운다(위 boardLayout 머리말).
function fit(width, target, best, cardH1) {
  ...
  return {
    ...
    originX: (width - cardW * best.gw) / 2, // 자투리는 좌우로 — 위아래는 0 이다
    originY: pad + headerH,
  };
}
