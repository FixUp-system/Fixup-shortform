...
// 자막 글자의 상한. 화면에 그대로 박히는 값이라 끝없이 받지 않는다.
export const SAID_MAX = 500;

// **사장님이 자막 글자를 직접 고친다** (2026-09-04).
//
// ★★★ 왜 필요한가. `video.said` 는 굽는 순간 코드가 적는 값이라, **이미 구운 편에는 없다.**
//   2026-09-03 에 사장님이 신고한 화장품 편이 그랬고, 고칠 자리가 화면에 없어서 내가 손으로
//   문서를 고쳐야 했다(프로덕션 데이터 직접 수정). 그 일이 다시 필요해서는 안 된다 —
//   다시 굽는 것은 한 편에 값이 나가므로 **글자 하나 때문에 쓸 수 없다.**
//
// ★ **영상과 각인은 안 건드린다.** 글자를 고쳤다고 산 영상이 낡으면 그것이 곧 재굽기다.
// ★ **낡은 시각을 걷는다** — 문장이 바뀌면 저장된 시각은 딴 문장의 것이다. 걷어 두면
//   다음 완성에서 다시 잰다(1센트 미만). 반쯤 맞은 시각보다 다시 재는 편이 낫다.
// ★ 빈 값은 **되돌리기**다 — bakedNarration 이 빈 문자열을 '없는 것'으로 보므로 시나리오의
//   내레이션으로 떨어진다. 지우는 문을 따로 만들지 않는다.
// ★ **통짜로 구운 편에만** 뜻이 있다. 컷별 갈래는 컷마다 sentence 가 자막이라 축이 다르다 —
//   그 갈래를 여기서 함께 다루면 "어느 글자가 자막인가"가 두 벌이 된다. 아니면 null 이다.
// ★ 원본을 안 바꾼다 — 저장 큐가 같은 문서를 다시 읽는다(lib/store 의 직렬 큐).
export function putSaid(project, text) {
  const cuts = Array.isArray(project?.cuts) ? project.cuts : [];
  const v = cuts[0]?.video;
  if (!v?.url || v.whole !== true) return null;
  const said = one(text).slice(0, SAID_MAX);
  const next = {
    ...project,
    cuts: cuts.map((c, i) => (i === 0 ? { ...c, video: { ...c.video, said } } : c)),
  };
  // ★★ 2026-09-16 리뷰 지적 — 옛 자리(narration_timing)만 지우고 있었다. 새 자리
  //   (reel.speech, lib/speech-timing.js 의 speechUnits 가 채운다)를 안 지우면
  //   narrationUnits 가 그것을 계속 이겨서 **딴 문장의 시각**을 얹고, 완성 라우트도
  //   `!reelOf(project).speech` 가 거짓이라 재측정을 건너뛴다(설계 문서 §4.7).
  if (next.reel && ("narration_timing" in next.reel || "speech" in next.reel)) {
    const { narration_timing: _drop, speech: _drop2, ...rest } = next.reel;
    next.reel = rest;
  }
  return next;
}
...
export function narrationUnits(project, seconds) {
  ...
  // ★★ 2026-09-16 — 시각의 자리가 **둘**이 됐다. 새 자리 `reel.speech.units`
  //   (Scribe v2 낱말 단위로 잰 값, lib/speech-timing.js 의 speechUnits)가 있으면
  //   그것이 이긴다. 없으면 옛 자리 `reel.narration_timing`을 읽는다(옛 편 회귀 0).
  // ★ 새 자리는 문장마다 `{ start, seconds, ok }`를 준다 — `ok`가 아니면 그 문장은
  //   못 믿는 값이라 안 얹는다(반쯤 맞은 시각보다 글자 수 비례 바닥이 낫다).
  ...
  // ★★★ 2026-09-16 리뷰 I1 — 못 잰 문장의 자리를 **전체 측정 구간(첫 ok 시작~마지막 ok
  //   끝)에서 글자 비례로** 정했더니, 그 구간 안 어디에 진짜 쉼이 있었는지 모른 채 자리를
  //   잡아 **앞 ok 문장과 겹쳤다**(리뷰 픽스처: units [1.0,2.0,ok]/버림/[7.0,2.0,ok], 15초
  //   → 둘째 문장이 2.67~5.34 로 첫 문장(0.85~2.85)과 0.18초 겹쳤다).
  //   대신 **바로 앞 ok 문장의 끝 ~ 바로 뒤 ok 문장의 시작** 사이에서만 나눈다 — 그 구간만
  //   "실제로 말 사이 여백"이라고 볼 근거(양옆 다 측정값)가 있다. 연속된 못 믿는 문장이
  //   여럿이면 그 구간 안에서 다시 글자 수 비례로 나눈다. 양옆에 ok 가 없으면 각각
  //   0(영상 시작)·total(영상 길이)을 경계로 쓴다 — 측정이 있는 이상 이 값들은 여전히
  //   "말한 구간 언저리"이지 늘어진 침묵이 아니다.
  const fallback = speech ? fallbackSpansFor(speech, weights, total) : null;
  return list.map((sentence, i) => {
    ...
    if (Number.isFinite(start) && Number.isFinite(spoken) && spoken > 0) {
      return { sentence, seconds: spoken, spoken_start: start, spoken_seconds: spoken };
    }
    const fb = fallback && fallback[i];
    if (fb) {
      return { sentence, seconds: fb.seconds, spoken_start: fb.start, spoken_seconds: fb.seconds };
    }
    // ★ 측정이 아예 없다(reel.speech·narration_timing 둘 다 없음) — 예전 그대로 영상
    //   길이 비례다(회귀 0).
    return { sentence, seconds: (total * (weights[i] || 1)) / sum };
  });
}

// 못 잰(ok:false) 문장들을 **연속 구간(run)** 으로 묶어, 각 run 을 감싸는 앞뒤 ok 문장의
// 끝·시작 사이에서 글자 수 비례로 나눈다. 앞(뒤)에 ok 가 없으면 0(total)을 그 경계로 쓴다.
function fallbackSpansFor(speech, weights, total) {
  const n = speech.length;
  const result = new Array(n).fill(null);
  let i = 0;
  while (i < n) {
    if (speech[i]?.ok) { i += 1; continue; }
    let j = i;
    while (j < n && !speech[j]?.ok) j += 1;
    const prevOk = i > 0 ? speech[i - 1] : null;
    const nextOk = j < n ? speech[j] : null;
    const left = prevOk && Number.isFinite(prevOk.start) && Number.isFinite(prevOk.seconds)
      ? prevOk.start + prevOk.seconds
      : 0;
    const right = nextOk && Number.isFinite(nextOk.start) ? nextOk.start : total;
    const span = Math.max(right - left, 0);
    const runWeights = weights.slice(i, j).map((w) => Math.max(w, 1));
    const wsum = runWeights.reduce((a, b) => a + b, 0);
    let acc = 0;
    for (let k = i; k < j; k += 1) {
      const w = runWeights[k - i];
      result[k] = { start: left + (acc / wsum) * span, seconds: (w / wsum) * span };
      acc += w;
    }
    i = j;
  }
