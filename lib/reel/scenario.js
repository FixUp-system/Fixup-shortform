...
import { AD_FORMATS, AD_MOODS, AD_LANGS, AD_STYLE_LINES, reelStyleLine } from "../ad/options.js";
...
export function buildScenarioMessages({ settings, material, edits }, opts = {}) {
  ...
  // ★ 2026-09-03 — **단계별은 reelStyleLine 을 쓴다**(원클릭·film 은 표를 그대로 쓴다).
  //   판에 얼굴이 사진처럼 그려지면 굽기가 초상 정책으로 거절되는 자리라, 그 방향을
  //   미는 낱말을 뺀 값이다(lib/ad/options.js 의 REEL_STYLE_OMIT).
  //   ★ 있는지 없는지는 **표**로 판정한다 — 뺀 뒤의 값이 아니라 원래 표에 그 화풍이
  //     있는가가 기준이다(빼기가 문자열을 통째로 비우는 날 이 문이 조용히 닫히지 않게).
  const styleLine = need(
    Object.keys(AD_STYLE_LINES).includes(settings.style) ? reelStyleLine(settings.style) : null,
    "화풍", settings.style
  );
