...
// ★★★ **2026-09-03 — 얼굴 제한을 걷었다**(사장님 결정). 아래 지시문에는 두 줄이 더
//   있었다: "얼굴이 화면을 채우게 쓰지 마라" · "카메라를 정면으로 응시하지 마라"
//   (근거는 2026-08-31 실측 "a cheerful young woman facing camera, eyes bright,
//   mid-bite smile" -> 422). 걷은 이유는 **그 방어의 효과가 확인된 적이 없기 때문**이다:
//   · 09-03 오후, **그 제한을 켠 채로** 초상 거절이 3연속 났다. 제한은 얼굴을 작게 할 뿐
//     없애지 못하고, 그 판에는 칸마다 얼굴이 2~3개씩 있었다.
//   · 최근 프로젝트 40건 중 **초상 거절 상태가 0건**이고, 거절 흔적(face_safe)은 3건인데
//     전부 08-31~09-02 — 격자가 없거나 틀렸던 시절이다.
//   · 실제로 막는 일을 하는 것은 굽기 직전의 **얼굴 격자**다(lib/reel/face-grid.js).
//   · 반대로 제한이 치른 값은 확실했다 — 팔만 몸에서 떨어져 나온 컷, 그리고 다섯 장면이
//     전부 제품컷만 된 판(57db7ad6). **효과가 확인 안 된 방어에 확실한 손해를 냈다.**
//   · 되돌리기 값도 싸다 — 거절은 값이 청구되지 않아 0원이다.
//   ⚠️ **되돌리려면 그 두 줄을 지시문에 다시 넣는다.** 다만 그 전에 물을 것은 "제한을 다시
//     걸까"가 아니라 **"격자가 왜 못 막았나"**다 — 약한 고리는 얼굴 좌표다.
//   ★★ **사연을 지시문 안에 두지 마라.** 처음에는 이 글을 지시문에 적었는데, 그러면
//     걷어낸 금지문을 **따옴표째 모델에게 보내게 된다**(모델은 인용도 규칙으로 읽는다).
//     사람이 읽을 것은 주석에, 모델이 읽을 것은 지시문에 — 자리가 다르다.
...
${AVATARS.map((a) => `- ${a.id} : ${a.traits}`).join("\n")}
  ...
  · 맞는 사람이 목록에 없으면 억지로 고르지 말고 **빈 문자열로 비운다** — 엉뚱한 얼굴이
    실리는 것보다 낫다.
  · ★★★ **사람이 제품을 쓰는 장면을 써라. 얼굴이 보여도 된다** — 표정은 광고의 절반이다.
    ★ **먼저 읽을 것은 되는 쪽이다.** 아래는 전부 권장이고, 광고에서 가장 많이 쓰는 그림이다:
      - 사람이 제품을 쓰며 **웃거나 말하는 얼굴** — 클로즈업도, 카메라를 보는 것도 된다
      - **손**이 제품을 집고 뜯고 따르고 건넨다
      - **상반신(가슴 위)**으로 먹고 마시고 쓴다 — 턱 아래에서 잘려도 좋다
      - **옆모습 · 뒷모습 · 어깨너머**
      - 여럿이 함께 쓰는 장면
    ★ **막는 것은 하나뿐이다**:
      ★★ **몸은 뒤에 있는데 팔·손만 앞으로 들어오는 화면**(2026-09-03 사장님 지적 —
        화장품 영상 마지막 컷). 사람이 화면 안에 있으면 **그 사람으로 보이게** 담는다.
        손만 보여 줄 셈이면 **몸을 아예 프레임 밖에 둔다** — 몸통은 배경에 두고 팔 하나만
        앞으로 뻗는 구도는 팔이 몸에서 떨어져 나온 것처럼 보이고, 그 어색함이 광고 전체를
        싸구려로 만든다. **둘 중 하나로 정하고 섞지 않는다.**
        ✗ shows: "a woman stands at the counter behind, her arm reaching into frame to
          pick up the serum"
        ✓ shows: "close-up of two hands lifting the serum bottle off the counter"
          (사람은 프레임 밖 — 손만 담는다)
        ✓ shows: "a woman at the counter, seen from the chest down, lifting the serum
          bottle with both hands" (사람을 담는다 — 팔이 그 몸에 이어져 보인다)
        ★ 이것은 avatar_id 를 적은 장면에서 특히 자주 난다 — 사람을 골라 놓고 shows 에
          손동작만 적으면 모델이 **사람은 뒤에, 손은 앞에** 그린다(위 실측과 같은 뿌리다).
    ★★ **사람을 빼지 마라.** 참조 이미지에 알아볼 수 있는 얼굴이 들어가면 만드는 쪽이
      초상으로 보고 거절할 수 있는데(실측 2026-08-31: "a cheerful young woman facing
      camera, eyes bright, mid-bite smile" → 422), 그것이 무서워 **사람을 통째로 빼면
      광고가 죽는다** — 같은 날 다른 실측(57db7ad6)에서는 다섯 장면이 전부 제품컷만
      되어 사람이 하나도 없었다.
      ★ **그 거절을 막는 일은 지문이 아니라 코드가 한다** — 굽기 직전에 얼굴 격자를 씌운다
        (lib/reel/face-grid.js). 그러니 여기서는 **장면을 좋게 쓰는 데만 집중한다.**
...
- ★ **action 은 제품의 실제 물성과 상태를 따른다.** 닫힌 용기의 내용물은 흐르지 않고, 갓
  조리된 것이 아니면 음식에서 과한 김이 피어오르지 않으며, 실내의 신발 곁에서 흙먼지가
  일지 않는다. 과장된 연출은 제품을 가짜로 보이게 한다 — 자연스러운 움직임이 먼저다.
