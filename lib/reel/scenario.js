...
${AVATARS.map((a) => `- ${a.id} : ${a.traits}`).join("\n")}
  ...
  · 맞는 사람이 목록에 없으면 억지로 고르지 말고 **빈 문자열로 비운다** — 엉뚱한 얼굴이
    실리는 것보다 낫다.
  · ★★★ **얼굴이 화면을 채우게 쓰지 마라.** 참조 이미지에 알아볼 수 있는 얼굴이 크게
    들어가면 만드는 쪽이 **초상으로 보고 거절한다**(실측 2026-08-31: "a cheerful young
    woman facing camera, eyes bright, mid-bite smile" → 422). 기준은 **크기 · 정면성 ·
    응시** 셋이다. 그래서 이렇게 쓴다:
      - **허리 위(medium shot) 이상**으로 잡는다 — 얼굴만 꽉 찬 화면은 쓰지 않는다
      - **카메라를 정면으로 응시하지 않는다** — 시선은 제품이나 하고 있는 일로 내린다
      - "facing camera" · "looking at camera" · "close-up of her face" · "smiling at the
        camera" 같은 말을 쓰지 않는다
    ★ **사람을 빼라는 뜻이 아니다** — 얼굴이 주인공이 되지 않게 쓰라는 뜻이다.
      사람은 나오되 **제품이 주인공**이면 그대로 통과한다.
...
JSON 으로만 답한다:
{
  "text": "모델에 넘길 지시문 전체 (영어로 쓴다). **하나로 이어지는 흐름**으로 — 장면 번호도 초도 적지 않는다. ${narrationRule ? "**화면 밖 목소리의 대사를 여기 적지 마라** — 그것은 narration 한 벌이다" : "각 장면의 대사는 그 자리가 흐르는 대로 따옴표로 원문 그대로 넣는다"}",${narrationRule ? `
  "narration": { "text": "영상 전체에 흐르는 화면 밖 목소리 **한 벌** — 사장님 나라 말로, 한 사람이 이어서 말하는 한 문단", "say_as": "읽기 어려운 말의 발음 표기 (필요 없으면 빈 문자열)" },` : ""}
