...
export function lockedAxes(project) {
  ...
  // ★ 2026-09-16 사장님 지시 — "잠겼어요"는 "안 열려요"처럼 읽혀 왜 안 되는지가
  //   불분명했다. "…해서 수정이 불가능해요"로 바꿔 원인과 결과를 한 문장에 담는다.
  //   배지 "잠김"은 그대로다 — 그건 짧은 상태 이름이라 이 바뀐 문구와 다른 자리다.
  const byScenario = lock(scenarioDone, "시나리오를 확정해서 수정이 불가능해요");
  return {
    aspect_ratio: byScenario,
    target_seconds: byScenario,
    // 화풍만 「첫 그림」이다 — 이 값은 컷 수에 안 들어가고 그림에만 들어간다.
    style: lock(drawn, "첫 그림을 그려서 수정이 불가능해요"),
