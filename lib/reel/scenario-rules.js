...
// 스토리보드 **한 장의 가로세로비**. 격자와 화면 비율에서 낸다 — 픽셀을 구하는 함수
// (lib/reel/storyboard.js 의 storyboardImageSize)는 **서버 전용**이라 여기서 못 문다.
// 실제 판은 상한에서 비례 축소하고 8의 배수로 반올림할 뿐이라 비율이 사실상 같다:
//   1행×5열 · 9:16 → (5×9)/(1×16) = 2.8125   (실제 판 3600×1280 = 2.8125)
//
// ★ 집이 여기인 이유 — 이 파일이 격자를 만드는 자리이고 **import 0 건**이라 화면도
//   서버도 읽을 수 있다. lib/reel/oneshot.js 가 이것을 다시 내보낸다(계산은 한 곳이다).
// ★ 격자가 없으면 0 이다 — 판이 없다는 뜻이라 판정이 통과로 새면 안 된다.
export function sheetAspectFor(grid, aspect) {
  if (!grid?.rows || !grid?.cols) return 0;
  const [aw, ah] = ratio(aspect);
  return (grid.cols * aw) / (grid.rows * ah);
}

// 그 화질에서 쓸 수 있는 컷 수 — **격자가 정한다.**
// ★ 하한 3: 컷 둘짜리 영상은 이 흐름의 대상이 아니다(옛 표의 하한과 같다).
//
// ★★ 2026-08-31 — 셋째 인자로 **모델의 참조 이미지 비율 한계**를 받는다(사장님 결정 A).
//   안 주면 예전과 **글자 그대로** 같다(Seedance 에는 이 거절이 없다).
//   왜 필요했나 — 5 는 소수라 격자가 **1행×5열** 밖에 없고, 그 판은 비율 2.81 이라
//   H3 가 참조 이미지로 안 받는다(0.4~2.5). 그래서 5컷 프로젝트가 통짜로 못 가고
//   **컷별로 떨어졌다**(이음새를 잃고 fal 호출이 1번 → 5번이 된다).
//   굽기 직전에 떨어뜨리는 것보다 **애초에 그 수를 안 권하는 것**이 뿌리다.
// ★ 모델이 아니라 **한계 자체**를 받는다 — 이 파일은 import 0 건이라 clip-limits 를 못 문다.
export function reelCutChoicesFor(resolution = "720p", aspect = "9:16", refAspect = null) {
  const out = [];
  for (let n = REEL_MIN_CUTS; n <= 40; n++) {
    const grid = reelGridFor(n, { resolution, aspect });
    if (!grid) continue;
    if (refAspect) {
      const r = sheetAspectFor(grid, aspect);
      if (!(r >= refAspect.min && r <= refAspect.max)) continue;
    }
    out.push(n);
  }
  return out;
...
// ★★ 2026-08-31 — 넷째 인자로 **모델의 참조 이미지 비율 한계**를 받는다. 안 주면 예전
//   그대로다. 그 한계 밖의 격자가 나오는 컷 수는 목록에서 빠진다(위 reelCutChoicesFor).
export function reelSceneCountRule(targetSeconds, resolution = "720p", aspect = "9:16", refAspect = null) {
  const total = Number(targetSeconds) || 0;
  if (total <= 0) return null;
  const choices = reelCutChoicesFor(resolution, aspect, refAspect);
