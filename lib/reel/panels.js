...
export const FACE_SOFTEN = [
  ...
  [/\bchest-up\s+shot\b/gi, "waist-up shot from over the shoulder, face cropped above the mouth"],
  [/\bhead-and-shoulders\s+shot\b/gi, "waist-up shot from over the shoulder"],
];
...
export function buildStoryboardPrompt(project, cuts, grid, note, refs = []) {
  ...
  const faceSafeOn = project?.reel?.face_safe === true;
  const panels = cuts.map((c, i) => panelLine(c, i)).filter(Boolean)
    .map((line) => (faceSafeOn ? softenFace(line) : line));
  ...
  const faceSafe = project?.reel?.face_safe === true
    ? "Keep every person and every action exactly as described above, but frame them so no face "
      + "dominates a panel: show hands, torso from the chest down, profile or over-the-shoulder views, "
      + "and crop faces at the chin or turn them away. Nobody looks straight into the camera. "
