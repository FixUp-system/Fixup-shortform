...
import { toFalImageUrl, readRefBytes } from "../refs-io.js";
// 참조 비율 한계 — 넘는 사진을 실으면 굽기가 통째로 거절된다(아래 loadOneShotPhotoRefs).
import { clipProfileForProject, refAspectFor, refEndpointForProject, queueAppOf } from "../clip-limits.js";
...
import { planReelBake, reelWholePrompt, reelVoice, reelNarrates, buildOneShotPrompt, isReelOneShotStale, reelBakeCounts, reelCastLine, oneShotRefPhotos } from "./oneshot.js";
...
import { drawStoryboardSheet as drawStoryboardSheetImpl, mergeImages, fetchImageBytes } from "./storyboard.js";
// 판에 그려진 얼굴에 격자를 덧그린다 — 초상 정책 거절을 넘기는 유일한 길이다(실측).
import { gridFacesOnSheet, GRID_SUPPRESS_LINE } from "./face-grid.js";
...
import { getReelJob } from "../projects.js";
// 심장박동 간격 — 이 저장소의 상수 하나가 정한다(STALL_MS/4 = 30초).
import { HEARTBEAT_MS } from "../progress.js";
...
// **판과 함께 보낼 사진을 읽어 온다** — 바이트와 함께, 비율이 맞는 것만.
//
// ★★★ 왜 비율을 재나(2026-09-04). 모델마다 **참조 이미지 비율 한계**가 있고(clip-limits 의
//   refAspect), 넘는 사진이 하나라도 끼면 **굽기가 통째로 거절된다.** 게다가 그 거절 문구가
//   "초상"으로 나와 원인을 잘못 가리킨다(2026-08-31 실측: 비율 2.83 이 초상 문구로 거절).
//   로고는 넓적한 경우가 많아 정통으로 맞는 자리다 — 그래서 **보내기 전에 우리가 잰다.**
// ★ 치수를 알려면 바이트를 봐야 한다 — 사진에는 치수가 저장돼 있지 않다.
// ★ **못 읽거나 못 재면 건너뛴다.** 사진 한 장 때문에 이미 값을 다 치른 굽기를 잃을 수 없다
//   (lib/cut-refs.js 의 "실패를 null 로 뭉갠다"와 같은 규율이다).
// ★ 한계를 모르는 모델은 통과다 — 모르는 것으로 막지 않는다(sheetFitsModel 과 같은 결).
async function loadOneShotPhotoRefs(project, deps = {}) {
  const read = deps.readRefBytes || readRefBytes;
  const limit = refAspectFor(clipProfileForProject(project));
  const sharp = limit ? (await import("sharp")).default : null;
  const out = [];
  for (const photo of oneShotRefPhotos(project)) {
    const key = photo.url?.split("/").pop();
    if (!key) continue;
    const bytes = await read({ source: "upload", key }).catch(() => null);
    if (!bytes) continue;
    let send = bytes;
    if (limit) {
      const meta = await sharp(bytes).metadata().catch(() => null);
      const r = meta?.width && meta?.height ? meta.width / meta.height : 0;
      if (r > 0 && !(r >= limit.min && r <= limit.max)) {
        // ★★★ **버리지 않는다 — 여백을 넣어 맞춘다.** 버리면 사장님이 올린 로고가 조용히
        //   안 실리고, 그러면 이 작업이 없애려던 증상("로고가 깨진다")이 그대로 남는다.
        //   넓적한 로고가 정확히 그 경우다. 정사각형은 어느 모델의 한계에도 들어간다.
        //   ★ 흰 바탕이다 — 로고는 흰 바탕에 놓이는 것이 기본이고, JPEG 는 투명을 못 든다.
        const side = Math.max(meta.width, meta.height);
        send = await sharp(bytes)
        ...
        // 치수를 못 재면 보내지 않는다 — 모르는 것을 실어 굽기를 잃을 수 없다.
        continue;
      }
    }
    out.push({ photo, ref: { bytes: send, key } });
  }
  return out;
}
...
export async function runReelOneShot(projectId, ownerId, deps = {}) {
  ...
  // ★★ 2026-09-09 — 이 둘을 **주입 가능하게** 뺐다. 격자 기록을 재려면 이음매가 필요하고,
  //   덤으로 이 저장소의 **기준선 빨강**이 사라진다: 판이 `https://fal/sheet.png` 를 진짜로
  //   물어 DNS 가 죽는 데 걸리는 시간(실측 2.7초)을 통짜를 관통하는 판마다 먹고 있었고,
  //   회선이 느린 날에는 5초 상한을 넘겨 **같은 코드가 빨강·초록으로 갈렸다**.
  const fetchBytes = deps.fetchImageBytes || fetchImageBytes;
  const gridFaces = deps.gridFacesOnSheet || gridFacesOnSheet;
  ...
  // ★★★ 2026-09-03 — **이 편이 말할 문장을 여기서 한 번 읽는다.** 아래 각인(`of: body`)은
  //   본문만 물어서 이 값이 바뀌어도 옛 편이 안 낡는데, 그동안 ⑥완성은 **렌더 시점의**
  //   내레이션을 자막으로 태웠다 — 소리는 옛 문장, 자막은 새 문장(사장님 신고).
  //   그래서 **구운 편에 이 문장을 함께 적어 둔다**(`video.said`). 자막은 그것을 태운다
  //   (lib/reel/narration.js 의 bakedNarration). 다시 굽게 만들지 않으므로 값이 안 나간다.
  // ★★★ 2026-09-04 — **판과 함께 보낼 사진을 먼저 읽는다.** 지시문과 참조가 **같은 목록**을
  //   봐야 한다 — 지시문은 "로고가 붙어 있다"고 말했는데 비율 때문에 걸러져 안 붙으면
  //   모델은 없는 사진을 찾는다(이 저장소가 사진 누락에서 이미 겪은 종류의 어긋남이다).
  const photoRefs = await loadOneShotPhotoRefs(project, deps);
  const narration = reelNarration(project);
  const said = narration?.text || "";
  const prompt = buildOneShotPrompt(plan.grid, plan.count, body, {
    ...
    // ★ **실제로 붙는 사진만** 넘긴다(위 photoRefs) — 거른 것을 말하면 거짓이 된다.
    photos: photoRefs.map((x) => x.photo),
  });
  ...
  // ★★★ 2026-09-03 — **판에 그려진 얼굴에 격자를 덧그린다**(사장님 지시).
  //   fal 검사는 참조 이미지의 얼굴이 실존 인물 초상으로 보이면 굽기를 거절한다. 판에는
  //   사장님이 사진을 안 올려도 얼굴이 그려진다(시나리오에 사람이 나오면 모델이 그린다) —
  //   그래서 "인물 사진을 못 올리게" 막던 대응은 조준이 어긋나 있었다.
  //   실측(09-03): 얼굴에만·흰색·불투명 10×10 이면 **2.0 과 2.5 둘 다 통과**하고,
  //   프롬프트 꼬리(GRID_SUPPRESS_LINE)가 출력물에서 격자를 지운다.
  //   ★ 격자를 못 씌워도 **던지지 않는다** — 그때는 예전처럼 주소를 그대로 넘긴다.
  //     거절은 0원이라, 여기서 던져 얼굴 없는 편까지 못 굽는 쪽이 더 나쁘다.
  //   ★ 각인(imageOf)은 **원본 판 주소**를 그대로 쓴다(아래 접수증) — 각인은 "무엇에서
  //     나왔는가"이지 "무엇을 보냈는가"가 아니다. 격자 판은 그때그때 만드는 사본이다.
  //   ★★★ 2026-09-09 — **무슨 일이 있었는지 문서에 남긴다**(`reel.faceGrid`).
  //     프로덕션 편 `14fd0ce0` 이 또 초상으로 거절됐는데, 격자를 씌웠는지 알 방법이
  //     없었다. `gridded` 는 지역 변수라 프롬프트 꼬리를 정하는 데만 쓰이고 버려졌고,
  //     접수증은 접수가 422 로 죽어 아예 안 생겼다. 그래서 셋이 구분이 안 됐다:
  //       ① 얼굴을 못 찾아 안 씌웠다  ② 씌웠는데 좌표가 빗나갔다  ③ 터져서 원본을 보냈다
  //     ①②는 어디에도 안 남아, 09-03 회차는 매번 판을 내려받아 재현해야 했다.
  //     ★ 이름이 `faceGrid` 인 이유: `plan.grid` 는 **스토리보드의 행·열**이라 다른 것이다.
  let sheetRef = { url: sheetUrl };
  let gridded = false;
  let faceGrid = { faces: 0, gridded: false, reason: "no-sheet" };
  try {
    const raw = await fetchBytes(plan.sheet).catch(() => null);
    if (raw) {
      const out = await gridFaces({ bytes: raw, cells: plan.count, grid: plan.grid });
      const faces = Number(out?.faces) || 0;
      if (faces > 0) {
        sheetRef = { bytes: out.bytes, key: "sheet.jpg" };
        gridded = true;
        faceGrid = { faces, gridded: true, reason: "ok" };
      } else {
        // ★ 탐지가 **조용히 0 을 돌려주는 길이 셋**이다(lib/reel/face-grid.js 의
        //   askFaceBoxesOnce): 키가 없다 · 네트워크가 흔들렸다 · 429/크레딧 소진.
        //   셋 다 여기로 떨어지므로, 이 값이 "얼굴이 없는 판"과 "탐지가 죽었다"를
        //   가르지는 못한다 — 다만 **좌표가 빗나간 쪽은 확실히 배제**한다.
        faceGrid = { faces: 0, gridded: false, reason: "no-faces" };
      }
    }
  } catch (e) {
    console.error("판 격자 실패 — 원본으로 굽는다:", e?.message);
    faceGrid = {
      faces: 0, gridded: false, reason: "error",
  ...
  // ★★★ 기록은 **접수 앞**에 적는다. 거절이 나는 순간이 이 기록이 필요한 유일한 순간이고,
  //   접수 뒤에 적으면 바로 그때 안 남는다(접수가 422 로 죽으면 접수증도 안 생긴다).
  //   ★ at 은 밖에서 받는다 — updateProject 는 낙관적 락이라 CAS 에 지면 같은 patchFn 을
  //     다시 부른다. 안에서 Date.now() 를 부르면 시도마다 값이 달라져 순수 규약이 깨진다.
  const gridAt = Date.now();
  await updateProject(projectId, ownerId, (p) => putReel(p, { faceGrid: { ...faceGrid, at: gridAt } }));

  const out = await submit({
    // ★ 첫 프레임이 **없다.** 스토리보드는 움직임의 출발 그림이 아니라 순서를 담은 참조다 —
    //   그것을 첫 프레임으로 주면 격자가 그대로 움직이는 분할 화면이 된다.
    imageUrl: null,
    // ★★★ 판 **뒤에** 사진을 붙인다(2026-09-04). 판이 이 갈래의 주인공이라 앞자리를 지킨다.
    //   원본을 함께 보내는 이유는 로고·제품 글자가 판을 거치며 두 번 재생성돼 깨지기 때문이다.
    refs: [sheetRef, ...photoRefs.map((x) => x.ref)],
    seconds: plan.seconds,
    aspect_ratio: project.settings?.aspect_ratio || "9:16",
    // ★ 격자를 씌웠을 때만 억제 꼬리를 붙인다 — 안 씌웠는데 "격자를 그리지 마라"를 넣으면
    //   지문에 없는 것을 부정하는 군더더기다. **각인(of: body)은 안 바뀐다** — 꼬리는
    //   보내는 지문에만 붙으므로 이미 구운 옛 편이 이 변경으로 낡지 않는다.
    prompt: gridded ? `${prompt}\n\n${GRID_SUPPRESS_LINE}` : prompt,
    projectId,
    project,
  });
  ...
  if (!out.fake) {
    ...
    await updateProject(projectId, ownerId, (p) =>
      reelProgress(putReel(p, {
        job: { ...out, of: body, imageOf: plan.sheet, prompt, said, aspect_ratio: project.settings?.aspect_ratio || "9:16", startedAt },
        // ★ 소리가 바뀌면 잰 시각을 버린다 — 다음 완성에서 다시 잰다($0.002).
        // ★★ 2026-09-16 리뷰 I2 — 옛 자리(narration_timing)도 함께 지운다. 여기만
        //   지우면 09-03~09-15 편이 지닌 옛 시각이 새로 구운 영상에 그대로 얹힌다
        //   (측정이 실패했을 때 narrationUnits 가 speech 없이 옛 자리를 읽어서다).
        speech: null,
        narration_timing: null,
      }), "video", startedAt));
    return;
  }

  // ★ `said` 는 각인이 아니다 — 낡음 판정에 안 쓴다(그러면 글자를 고칠 때마다 이미 산
  //   편이 죽는다). **자막이 무엇을 태울지**를 정하는 값이다.
  const video = { url: out.url, seconds: out.seconds, of: body, imageOf: plan.sheet, whole: true, said };
  ...
  await updateProject(projectId, ownerId, (p) =>
    reelProgress(
      // ★★ 2026-09-16 리뷰 M2 — 가짜 모드(동기 갈래)도 새 영상을 꽂으므로 소리가 바뀐다.
      //   접수·수거·되붙이기 세 자리와 같은 이유로 speech·narration_timing 을 지운다 —
      //   안 지우면 옛 시각이 이 가짜 편에도 얹힌다.
      putReel(
        {
          ...p,
          cuts: (p.cuts || []).map((c, i) => {
        ...
        { speech: null, narration_timing: null }
      ),
      "video",
      at
    )
...
export async function collectReelOneShot(projectId, ownerId, deps = {}) {
  ...
  // ★★★ 2026-09-11 — **통짜를 안 읽는다.** 2초마다 불리는 자리인데 그전에는 매번 문서
  //   전체(실측 44.5KB)를 끌어왔다. 여기서 쓰는 것은 접수증 하나다 — collectClip 은
  //   `job` 안의 값만 본다(lib/i2v.js). 좁은 셀렉터로 바꾼 이유는 selectReelStatus 머리말에.
  // ★ 이음매 — 좁은 모양은 통짜 문서의 **부분집합**이다(status·reel·progress·cuts).
  //   판이 넣어 주는 통짜 읽기를 그대로 쓸 수 있고, 프로덕션에는 주입이 없어 좁게 돈다.
  const readHead = deps.getReelStatus || deps.getProject || getReelJob;
  const head = await readHead(projectId, ownerId).catch(() => null);
  const job = head?.reel?.job;
  ...
  try {
    ...
    if (!got.done) {
      ...
      //   쓴다(44.5KB + 44.5KB). 2초마다 하면 굽는 10분 동안 그것만 27MB 다.
      //   화면이 보는 것은 "마지막 박동이 STALL_MS(2분) 안인가" 하나뿐이므로,
      //   **HEARTBEAT_MS(30초)** 마다 한 번이면 충분하다 — 그 상수의 주석이 말하는 그대로
      //   "정상 실행은 임계에 닿기 전에 네 번 뛴다". 쓰기가 15분의 1로 준다.
      // ★ 값이 없으면(옛 문서·첫 박동) 무조건 한 번 뛴다 — 0 과 null 을 섞지 않는다.
      const beatAt = Date.now();
      const last = Number(head?.progress?.at);
      if (!Number.isFinite(last) || beatAt - last >= HEARTBEAT_MS) {
        await updateProject(projectId, ownerId, (p) => reelProgress(p, "video", beatAt)).catch(() => {});
      }
    ...
    // ★ 말한 문장도 **접수증에 담아 둔 것**을 쓴다 — 각인과 같은 이유다. 지금 다시 읽으면
    //   접수와 수거 사이에 고친 글이 실려, 소리와 다른 자막이 다시 생긴다.
    const video = { url: got.url, seconds: got.seconds, of: job.of, imageOf: job.imageOf, whole: true, said: job.said || "" };
    await updateProject(projectId, ownerId, (p) =>
      reelProgress(putReel({
        ...
        status: "clips",
        // ★ 소리가 바뀌면 잰 시각을 버린다 — 다음 완성에서 다시 잰다($0.002).
        // ★★ 2026-09-16 리뷰 I2 — 옛 자리(narration_timing)도 함께 지운다(위 접수 자리와 같은 이유).
        speech: null,
        narration_timing: null,
      }), "video", Date.now()));
...
//
// ★★★ 2026-09-09 — 광고에는 있는데(lib/ad/pipeline.js 의 attachAdVideo · 09-08 실물 사고)
//   **reel 에는 없었다.** fal 에서는 완성됐는데 우리 수거가 죽어 문서에 못 붙은 편이
//   생기면, 이미 값을 치른 영상을 앱에서 되찾을 길이 없다. 오늘 실물로 그런 편이 생겼다 —
//   초상 거절로 실패한 편(14fd0ce0)을 격자 여유를 키워 다시 구웠는데, 그 결과물이
//   우리 파이프라인 밖(직접 접수)에 있어 앱이 모른다.
//
// ★ **굽지 않는다.** 이미 만들어진 것을 가져오기만 하므로 이 문으로는 값이 새로 안 나간다.
//   판이 그것을 잰다(굽기를 부르면 빨개진다).
// ★ **운영자 전용이다**(라우트가 adminOnly). 열려 있으면 남의 편에 아무 영상이나 꽂는 문이 된다.
// ★ 굽는 중이면 거절한다 — 도는 굽기를 덮으면 그 값이 사라진다.
// ★★ 각인은 **지금 값**으로 찍는다. 수거(collectReelOneShot)는 접수증에 담아 둔 것을 쓰는데
//   여기에는 접수증이 없다(그것이 사라진 것이 이 함수가 필요한 이유다). 그래서 지금의
//   프롬프트·판 주소를 찍는다 — 안 그러면 붙이자마자 "낡았어요"가 뜬다.
//   ⚠️ 붙인 뒤에 시나리오를 고치면 그때는 정상적으로 낡음으로 잡힌다(그것이 맞다).
export async function attachReelVideo(projectId, ownerId, { requestId, url } = {}, deps = {}) {
  const getProject = deps.getProject || getProjectImpl;
  const updateProject = deps.updateProject || updateProjectImpl;
  ...
  if (reelOf(project).status === "rendering") {
    return { rendering: true, error: "지금 굽는 중이에요 — 끝난 뒤에 붙여 주세요" };
  }

  let videoUrl = url || null;
  let seconds = Number(project.settings?.target_seconds) || 0;

  if (!videoUrl) {
    if (!requestId) return { error: "fal 접수번호가 필요해요" };
    // 큐의 결과 주소를 **여기서 조립한다** — 접수 때 받아 두는 response_url 이 문서에서
    // 이미 사라진 편을 되살리는 것이 이 함수의 목적이라, 그 값을 못 쓴다.
    const app = queueAppOf(refEndpointForProject(project) || "");
    const res = await fetchImpl(`https://queue.fal.run/${app}/requests/${requestId}`, {
      headers: falHeaders(),
    ...
    seconds = Number(data?.video?.duration) || seconds;
    if (!videoUrl) return { error: "그 접수번호에는 영상이 없어요" };
  }

  const plan = planReelBake(project);
  const video = {
    url: videoUrl, seconds,
    of: reelWholePrompt(project), imageOf: plan.sheet, whole: true,
    said: reelNarration(project)?.text || "",
  };
  await updateProject(projectId, ownerId, (p) =>
    putReel({
      ...p,
      // 수거와 **같은 규율** — 첫 컷에 담고 나머지 컷의 옛 클립은 걷어낸다. 안 걷으면
      // lib/compose.js 가 video.url 이 있는 컷을 전부 이어 붙여 "한 편 + 옛 컷들"이 된다.
      cuts: (p.cuts || []).map((c, i) => {
        if (i === 0) return { ...c, video };
        if (!c?.video) return c;
        const { video: _drop, ...rest } = c;
      ...
      job: null, error: null, errorStep: null, status: "clips",
      // ★ 소리가 바뀌면 잰 시각을 버린다 — 다음 완성에서 다시 잰다($0.002).
      // ★★ 2026-09-16 리뷰 I2 — 옛 자리(narration_timing)도 함께 지운다(위 접수 자리와 같은 이유).
      speech: null,
      narration_timing: null,
    }));
  return { attached: true, url: videoUrl, seconds };
}
