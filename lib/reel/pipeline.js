...
import { planReelBake, reelWholePrompt, reelVoice, reelNarrates, buildOneShotPrompt, isReelOneShotStale, reelBakeCounts } from "./oneshot.js";
...
export async function collectReelOneShot(projectId, ownerId, deps = {}) {
  ...
  try {
    ...
    await updateProject(projectId, ownerId, (p) =>
      ...
      }, { job: null, error: null, errorStep: null }), "video", Date.now()));
    return { changed: true, url: got.url };
  } catch (e) {
    // ★★★ **초상 거절이면 한 번은 우리가 받아낸다**(2026-08-31 사장님 결정 A).
    //   굽는 쪽은 참조 이미지에 알아볼 수 있는 얼굴이 크게 들어가면 되돌린다. 그런데
    //   **거절은 0원이다** — 만들기 전에 되돌린 것이라 fal 도 안 물리고, 우리 원장에도
    //   기록이 없다(addRecord 가 성공 뒤에 있다. 실측: 거절난 두 프로젝트 모두 0건).
    //   그래서 **자연스럽게 먼저 써 보고 걸리면 낮추는** 순서가 값이 안 든다.
    //   ★ 여기서 멈추면 사장님 눈에는 그냥 실패였다 — 그것이 이 자리의 그전 모습이다.
    await updateProject(projectId, ownerId, (p) => putReel(p, { job: null })).catch(() => {});
    if (classifyFailure(e?.message || "").code === "rejected_likeness" && !reelOf(project)?.face_safe) {
      const again = await (deps.retryOneShotWithoutFaces || retryOneShotWithoutFaces)(projectId, ownerId, deps)
        .catch(() => null);
      if (again?.retried) return { changed: true, retried: true };
    }
