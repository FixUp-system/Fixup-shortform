...
export function storyboardImageSize(grid, aspect = "9:16", resolution) {
  ...
  // ★★★ 2026-09-16 사장님 지시 — **판은 720p 칸으로 그리고, 상한을 넘을 때만 줄인다.**
  //   그전에는 굽기 화질이 곧 칸 크기였다(480p → 칸 854). 그래서 480p 로 만든 편은
  //   완성본만 거친 것이 아니라 **모델이 보고 그리는 참조 그림까지** 함께 거칠어졌다
  //   (6컷 기준 판 2160×3840 → 1440×2562, 칸 넓이 약 45%). 값이 깎이는 자리가 둘이었다.
  //   ★ 이미지 값은 크기가 아니라 quality 축(high)이라 **판을 키워도 값이 안 는다.**
  const want = Math.max(bakeCellLong(resolution), STORYBOARD_CELL_LONG_SIDE);
  const size = (cellLong) => {
    const k = cellLong / Math.max(aw, ah);
    return { width: grid.cols * aw * k, height: grid.rows * ah * k };
  };
  let { width, height } = size(want);
  const long = Math.max(width, height);
  if (long > MAX_SIDE) {
    const s = MAX_SIDE / long;
    // ★ 줄인 뒤 칸이 **굽기 해상도보다 작아지면** 키운 것이 손해다 — 그때는 굽기 기준으로
    //   되돌린다(칸 수 목록이 그 크기로는 담기는 것만 열어 두므로 상한에 안 걸린다).
    if (want * s < bakeCellLong(resolution)) ({ width, height } = size(bakeCellLong(resolution)));
