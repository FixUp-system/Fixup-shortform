...
// ★ 2026-09-16 — 위 본문의 **번역**. 판정은 글자 그대로 같아야 한다 — 본문이
//   `reel.prompt`(고친 프롬프트)면 번역도 그 짝인 `reel.prompt_ko`, 본문이
//   `scenario.text`(기본 경로)면 번역도 그 짝인 `scenario.text_ko` 다. 둘을 따로
//   판정하면 본문과 번역이 서로 다른 글이 되는 경우가 생긴다(예: 고친 뒤에도 옛
//   scenario.text_ko 를 보여 주는 것) — app/reel/[id]/prompts/page.js 가 이것을 쓴다.
export function reelWholePromptKo(project) {
  const edited = typeof project?.reel?.prompt === "string" ? project.reel.prompt.trim() : "";
  if (edited) return project?.reel?.prompt_ko || "";
  return project?.scenario?.text_ko || "";
}
...
export function reelCastLine(project) {
  ...
  // ★★★ 2026-09-09 — **얼굴을 목록에 넣고, 참조 그림에 묶는다.**
  //   그전에는 `hair, build, clothing` 만 못 박았다. 그럴 이유가 있었다 — 08-31 에
  //   "2.5 가 얼굴 사진을 참조로 안 받는다"고 보고 **판에서 얼굴을 뺐고**, 그 뒤로 생김새는
  //   이 글만으로 날랐다(lib/reel/pipeline.js 의 cast 주석). 얼굴이 아예 없으니 말할 것도
  //   없었다.
  //   ★★ 09-03 에 얼굴 격자가 들어오며 사정이 뒤집혔다. 판에는 얼굴이 **있고** 격자가 그
  //     위를 덮는다 — "없다"가 아니라 "가려졌다"다. 가려진 만큼을 메울 채널이 이 글뿐인데
  //     목록에 얼굴이 빠져 있었다. 2026-09-09 에 사장님이 **스토리보드와 다른 인물이
  //     영상에 나왔다**고 신고했고, 모델이 그 빈자리를 스스로 채운 것이 원인이다.
  //   ★ 두 문장이 하는 일이 다르다: 얼굴을 목록에 넣으면 "그런 얼굴"까지고,
  //     참조 그림에 묶어야 "그 얼굴"이 된다. 둘 다 있어야 한다.
  //   ⚠️ 이것만으로 인물이 지켜진다고 믿지 마라 — 글은 그림보다 약하다. 진짜 지렛대는
  //     격자가 얼굴을 얼마나 덮느냐이고(lib/reel/face-grid.js 의 pad·opacity), 그 문턱은
  //     아직 실측되지 않았다.
  return `The people in this film: ${people.join(" / ")}. `
    + "Keep each person's look — face, hair, build, clothing — identical from the first shot "
    + "to the last, and identical to the person shown in the reference image.";
}
...
export function buildOneShotPrompt(grid, count, body, speech = {}) {
  ...
  //   판에는 그림 모델이 **다시 그린** 로고가 있고(작아서 글자가 뭉개진다) 첨부에는 원본이
  //   있다. 둘이 함께 들어가는데 "어느 쪽을 믿어라"가 없으면 모델이 판을 따를 수 있고,
  //   그러면 원본을 보내는 뜻이 사라진다. 그래서 축을 갈라 준다 —
  //   **판은 구도(순서·프레이밍), 첨부는 로고·포장·인쇄된 글자의 정본.**
  //   ★ 첨부가 없으면 이 문장도 없다 — 없는 사진을 가리키면 모델이 없는 것을 찾는다.
  const roles = attachedRoleLine(speech?.photos).trim();
  const attached = roles
    ? "The storyboard sheet defines the shot sequence and framing. Where they disagree, "
      + "the attached photographs are authoritative for any brand logo, product packaging, or "
      + "printed lettering — reproduce those exactly as attached, not as they appear in the "
      + "storyboard. " + roles
    : "";
