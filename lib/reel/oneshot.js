...
// ★ 화면 비율의 치수는 표 하나가 안다(lib/aspects.js). **import 0 건의 순수 모듈**이라
//   이 파일의 규율("사슬 끝에 fs·env 가 안 닿는다")을 깨지 않는다 — 여기서 "9:16" 을
//   손으로 파싱하면 그 표와 두 벌이 된다.
...
// 스토리보드 한 장의 **가로세로비**. 픽셀을 구하지 않고 격자와 화면 비율에서 낸다 —
// 실제 판(storyboardImageSize)은 상한에서 비례 축소하고 8의 배수로 반올림할 뿐이라
// 비율이 사실상 같고, 그 함수는 **서버 전용**이라 이 파일이 물 수 없다.
...
export function buildOneShotPrompt(grid, count, body, speech = {}) {
  ...
  return [head, text, spoken].filter(Boolean).join("\n\n");
}
