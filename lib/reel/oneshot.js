...
import { REEL_GRIDS } from "./scenario-rules.js";
import { canBakeReelClips } from "./doc.js";

// 한 번에 굽을 수 있는 최대 — 모델 상한 그대로다(lib/clip-limits.js 의 seedance-2.0 max).
//
// ⚠️ 여기서 15 를 다시 적는 이유: 이 파일은 순수해야 해서 clip-limits.js 를 못 문다
//   (그 파일도 순수하지만 이 흐름 밖이고, 위 순수 규율이 "./" 밖 import 를 막는다).
//   두 값이 갈리는 날은 모델 상한이 바뀌는 날이고, 그때는 **정가·컷 수 규칙도 함께**
//   봐야 하므로 조용히 따라가면 안 되는 자리다 — 그래서 표가 아니라 여기 명시한다.
...
export function storyboardGridFor(count) {
  return REEL_GRIDS[Number(count)] || null;
}
...
//   ① 길이가 15초 이하다 — 그 위는 모델이 한 번에 못 굽는다
...
export function planReelBake(project) {
  ...
  if (cuts.length > 0 && seconds > 0 && seconds <= ONESHOT_MAX_SECONDS && sheet && grid) {
