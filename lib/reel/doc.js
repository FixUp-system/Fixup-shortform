// reel 문서의 모양과 잠금.
//
// ★ **순수 함수다 — import 문을 두지 마라.** 화면("use client")도 이 파일을 읽는다.
//   사슬 끝에 fs 가 닿으면 빌드가 깨진다(이 저장소가 세 번 겪은 사고다).
// ★ 판정을 여기 하나에 둔다 — 라우트와 화면이 **같은 값**을 봐야 화면이 열어 준 버튼을
//   서버가 400 으로 막는 어긋남이 안 생긴다.

export function emptyReel() {
  return { status: "draft", video: null, error: null };
}

export function reelOf(project) {
  return project?.reel || emptyReel();
}

export function putReel(project, patch) {
  return { ...project, reel: { ...reelOf(project), ...patch } };
}

// ★★ **그 단계의 오류만 돌려준다**(2026-08-25 사장님 지적: "이미지 생성에서 영상 실패
//   로그가 같이 뜨고 있어").
//
// 뿌리 — `reel.error` 는 **단계별이 아니라 reel 하나에 공용**이다. 쓰는 곳은 둘
// (⑤영상의 clips 라우트 · ⑥완성의 render 라우트)인데 읽는 곳이 셋이라, ⑤가 넣은
// fal 422(초상 거절)가 ③이미지 화면에 자기 오류처럼 떴다.
//
// ⚠️ **더 나쁜 쪽은 반대 방향이다**: 그림이 진짜로 실패했을 때도 같은 한 칸을 쓰므로,
//   영상 오류가 남아 있으면 **그림의 실패가 그 문구에 가려진다.**
//
// ★ 단계별 흐름은 이 문제를 필드를 갈라서 풀었다(lib/step-errors.js 의 STEP_ERROR_FIELDS).
//   reel 은 필드가 하나뿐이라, **누가 쓴 오류인가**(errorStep)를 함께 적는 쪽으로 푼다 —
//   저장 모양을 안 바꾸므로 이미 만들어진 문서가 안 깨진다.
// ★★ 단계가 안 적힌 **옛 문서는 그대로 보여 준다.** 안 보여 주면 이미 난 실패가 조용히
//   사라진다 — 이 저장소의 규율(classifyFailure 가 모르는 것을 원문 그대로 내보내는 것)과
//   같은 방향이다: **정보를 줄이지 않는다.**
export function reelErrorFor(reel, step) {
  const message = typeof reel?.error === "string" ? reel.error : "";
  if (!message) return "";
  const at = reel?.errorStep;
  if (!at) return message;          // 옛 문서 — 전 단계가 읽는다(예전 그대로)
  return at === step ? message : "";
}

// 시나리오를 다시 쓸 수 있는가 — 못 막을 이유가 없으면 null 이다.
//
// ★ film 의 scenarioLock 과 같은 결이다. 다른 점은 방식이 하나뿐이라 films 를 안 돈다.
export function scenarioLock(project) {
  const reel = reelOf(project);

  // ① 이미 구운 편이 있다.
  //
  // ★★ 2026-09-03 사장님 지시 — 문구가 **"수정됨" 한 마디**다. 그전에는
  //   "이미 만든 영상이 있어요 — 시나리오를 바꾸려면 새로 시작해 주세요" 였는데
  //   사장님이 **헷갈린다**고 했다: "새로 시작해 주세요"가 무엇을 하라는 말인지
  //   (프로젝트를 새로 만들라는 건지, 이 화면에서 뭘 누르라는 건지) 안 드러났다.
  //   지금 이 자리가 할 일은 **"이건 이미 쓰인 지문이다"를 알리는 것** 하나다 —
  //   못 고친다는 사실은 버튼이 잠긴 것으로 이미 보인다.
  if (reel?.video?.url) {
    return { reason: "baked", message: "수정됨" };
  }

  // ② 굽는 **창** 동안도 같다. 접수는 됐는데 아직 수거 전인 구간은 video 가 비어 있어
  //    ①을 그냥 지난다 — 그 사이 고치면 굽고 있는 편만 옛 판이 되고, 값은 이미 나갔다.
  if (reel?.status === "rendering") {
    return { reason: "rendering", message: "지금 영상을 만드는 중이에요 — 끝난 뒤에는 새로 시작해 주세요" };
  }

  return null;
}

// "지금 굽는 중인가" — 그림 라우트의 N4 잠금이 보는 값(2026-08-21 재검토). scenarioLock
// 의 ②와 같은 값을 재지만, 그림 라우트는 시나리오를 다시 쓰는 게 아니라 **그림을 다시
// 그리는가**를 묻는 자리라 이름을 따로 둔다 — scenarioLock 을 그대로 불러 쓰면 "그림을
// 못 그리는 이유"가 "시나리오를 못 고치는 이유"의 문구를 빌려 쓰게 된다.
export function isReelRendering(reel) {
  return reel?.status === "rendering";
}

// 굽기 버튼을 열어도 되는가 — 컷마다 프롬프트가 있어야 한다.
//
// ★ 빈 배열은 **아니다.** 컷이 없는데 열면 0개를 굽고 "완성"이 된다.
export function isPromptsReady(cuts) {
  const list = Array.isArray(cuts) ? cuts : [];
  if (!list.length) return false;
  return list.every((c) => typeof c?.clip_prompt === "string" && c.clip_prompt.trim().length > 0);
}

// 굽기(POST /clips)를 시작해도 되는가 — **프롬프트뿐 아니라 그림까지** 본다.
//
// ★★ 2026-08-21 Task 12 리뷰 A2 — ⑤영상 화면과 `/clips` 라우트의 청구 앞 검사가
//   `isPromptsReady` 하나만 봤다. 그런데 `runReelClips`(lib/reel/pipeline.js)의 진짜 전제는
//   "프롬프트가 다 찼다 **그리고** 컷마다 image.url 이 있다"이고, 그 조합은 **청구 뒤
//   백그라운드**에서만 확인됐다 — 그 둘이 어긋나면 크레딧이 나간 뒤 `status:"error"`다.
//   화면 게이트와 서버의 청구 앞 검사가 이 함수 하나를 함께 본다(이 저장소 규율 — 화면과
//   서버는 같은 판정을 봐야 한다, 손으로 다시 적으면 갈린다).
export function canBakeReelClips(cuts) {
  const list = Array.isArray(cuts) ? cuts : [];
  return isPromptsReady(list) && list.every((c) => !!c?.image?.url);
}

// ── 그림 다시 그리기 그물 (2026-08-21, Task 11 리뷰 I7) ────────────────────
//
// ★ 그림에는 **청구가 없다**(정가는 클립 굽기 한 번에 붙는다, /clips 와 같은 문). 그런데
//   fal 이미지는 장당 ≈$0.08 이 실제로 나간다 — film 의 MAX_FILM_IMAGE_TRIES 와 같은
//   이유로, 청구가 없는 자리에는 횟수와 잠금이라도 있어야 한다(lib/film/doc.js 참고).
// ★★ 2026-09-11 — 6 → **1 + 3**(사장님: "재생성 상한을 3회로 전부 통일"). imageTries 는
//   **첫 그리기부터** 센다(images/route.js 의 tries+1)라 "다시 그리기 3회" = 4 다.
//   그리고 이제 다시 그리기는 공짜가 아니다 — 첫 1회만 무료, 그 뒤는 REGEN_PRICE.sheet
//   (같은 라우트가 chargeRegen 을 부른다).
// ★ 숫자를 여기 다시 적는다 — 이 파일은 **import 를 두지 않는다**(위 머리말). pricing.js 의
//   MAX_REGEN_PER_CUT(3) 과 갈리면 tests/regen-cap-unified.test.js 가 먼저 빨개진다.
export const MAX_REEL_IMAGE_TRIES = 1 + 3;

// 그리는 중 잠금이 **영원히 걸리지 않게** 하는 상한 — film 의 FILM_IMAGE_LOCK_MS 와 같다.
// 서버리스 인스턴스가 도중에 죽으면 이 창이 지난 뒤 다시 열어 준다(막다른 길을 안 만든다).
export const REEL_IMAGE_LOCK_MS = 10 * 60 * 1000;

// "지금 그림을 다시 그릴 수 있는가" — 재진입 잠금 판정. reel.imagesDrawing·imagesAt 을 본다.
export function isImagesLocked(reel, at = Date.now()) {
  return reel?.imagesDrawing === true && at - Number(reel?.imagesAt || 0) < REEL_IMAGE_LOCK_MS;
}

export function imageTriesLeft(reel) {
  return Math.max(0, MAX_REEL_IMAGE_TRIES - (Number(reel?.imageTries) || 0));
}

// ── 그림 다시 그리기 총량 방어선 (2026-08-21 재검토 B2) ─────────────────────
//
// ★★ 시나리오 재작성이 imageTries(시나리오판마다) 를 리셋한다(N3 처방) — 재작성이 곧
//   컷을 통째로 갈아 끼우는 것이라, 옛 그림은 어차피 다음 컷에 안 맞고 지킬 이유가 없어서다.
//   그런데 리셋만 있고 상한이 없으면 재작성(MAX_SCENARIO_TRIES=20회)마다 그림을 다시 6회
//   쓸 수 있어, 프로젝트 하나가 **시나리오 20판 × 그림 6회 × 컷 12개 × $0.08 ≈ $115** 를
//   크레딧 0으로 쓸 수 있다(재검토가 낸 숫자) — 시나리오 재작성 자체는 청구가 없어서다.
//   그래서 **판마다 리셋되는 상한과 별개로, 프로젝트 수명 동안 절대 안 리셋되는 상한**을
//   하나 더 둔다. imageTries 는 판이 바뀔 때마다 0 으로 돌아가지만 imageTriesTotal 은
//   scenario 라우트가 절대 안 건드린다(images 라우트만 올린다) — 그래서 재작성을
//   반복해도 이 상한만은 못 피한다.
// ★ 값은 "시나리오 4판 몫"(MAX_REEL_IMAGE_TRIES × 4)으로 잡았다 — 정상적인 반복 창작
//   (시나리오를 몇 번 다듬는 것)에는 넉넉하되, 최악(24 × 12컷 × $0.08 ≈ $23)은 옛 상한
//   (판당 6회, 시나리오 재작성이 없던 시절의 사실상 상한 ≈$5.76)의 몇 배 안쪽으로 묶는다
//   — 재검토가 낸 $115 보다 다섯 배 낮다.
export const MAX_REEL_IMAGE_TRIES_LIFETIME = MAX_REEL_IMAGE_TRIES * 4;

export function imageTriesLeftLifetime(reel) {
  return Math.max(0, MAX_REEL_IMAGE_TRIES_LIFETIME - (Number(reel?.imageTriesTotal) || 0));
}

// 잠금(재진입)·상한(횟수, 판별)·상한(횟수, 수명) 셋 다 지나야 그릴 수 있다 —
// 그림 라우트가 보는 문 그대로다.
export function canDrawReelImages(reel, at = Date.now()) {
  return !isReelRendering(reel) && !isImagesLocked(reel, at)
    && imageTriesLeft(reel) > 0 && imageTriesLeftLifetime(reel) > 0;
}

// **통짜로 구운 한 편은 그 자체가 완성본이다** — 보관함이 그것을 보게 한다(2026-09-01).
//
// ★★★ 왜 필요한가: 합성("이대로 완성하기")은 **수동**이라, 굽기만 하고 완성 화면에 안
//   들어가면 `doc.render` 가 영영 안 채워진다. 보관함은 그 자리만 보고 있었기 때문에
//   단계별로 구운 편이 **한 편도** 안 떴다(2026-09-01 실측, 프로젝트 8편 전부).
//
// ★★ **`whole` 이 가른다.** 통짜 결과에만 이 표시가 붙는다(lib/reel/pipeline.js).
//   컷별의 `cuts[0]` 은 여러 조각 중 첫 조각일 뿐이라, 그것을 완성본으로 보여 주면
//   아직 안 이어 붙인 편을 다 된 것처럼 말하는 셈이 된다.
//
// ★ 순서상 **합성본이 이긴다** — 자막까지 태운 편이 진짜 완성본이다. 이 값은 그것이
//   없을 때만 쓰인다(lib/store/memory.js · lib/store/supabase.js 의 목록 두 곳).
// ★ film 의 `filmVideoUrlOf`(lib/film/doc.js)와 **같은 규율**이다 — 판정을 한 곳에 둔다.
export function reelWholeVideoUrlOf(doc) {
  const video = doc?.cuts?.[0]?.video;
  return video?.whole === true && video?.url ? video.url : null;
}

// 모델이 원고와 크게 다르게 말했는가. 글자 몇 개 차이는 보지 않는다
// (발음 표기 차이는 실측 +2~3%: "V라인"→"브이라인", "3분"→"삼 분").
// ★ LLM 에게 묻지 않는다 — 이 저장소는 LLM 검수가 조용히 통과만 시킨 전례가 있다(VLM 9회).
export function speechMismatch(project) {
  const speech = project?.reel?.speech;
  if (!speech || !Array.isArray(speech.units) || !speech.units.length) return null;
  const said = (project?.cuts?.[0]?.video?.said || "").replace(/[\s\p{P}\p{S}]/gu, "");
  const heard = speech.heard || { chars: 0, text: "" };
  // ★★ 2026-09-16 리뷰(I4) — `ok:false` 는 네 판정 중 어디에 걸려도 붙는다. 이상치·순서·
  //   범위는 "재기가 흔들렸다"이지 "말하지 않았다"가 아니다 — 근거 없는 경고는 무시하게
  //   된다(설계 §4.8). "말하지 않은 문장이 있어요"는 **낱말이 하나도 안 묶인**(reason
  //   "empty") 문장이 있을 때만 뜬다. 옛 문서(reason 없음)는 안전 쪽으로 켠다 — 판정이
  //   생기기 전 기록은 구분할 수 없으니 예전처럼 알린다.
  if (speech.units.some((u) => u?.ok === false && (u.reason === "empty" || u.reason === undefined))) {
    return { reason: "missing-sentence", heard: heard.text || "" };
  }
  if (said.length && heard.chars / said.length < 0.75) return { reason: "short", heard: heard.text || "" };
  return null;
}
