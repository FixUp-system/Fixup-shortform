...
export const supabaseStore = {
  ...
  // ★ 두 번 묻는 이유: reel 과 광고는 "굽는 중"의 뜻이 다르다(reel 은 접수증,
  //   광고는 접수증 + status="rendering"). or() 한 줄로 붙이면 조건이 섞여 읽기 어렵고,
  //   광고 쪽 status 조건이 reel 에도 걸리는 사고가 나기 쉽다.
  async selectBakingProjects() {
    const [reels, ads] = await Promise.all([
      db().from("projects").select("id, owner_id")
        .eq("doc->>kind", "reel")
      ...
      db().from("projects").select("id, owner_id, ad_ready:doc->ad_job->ready")
        .eq("doc->>kind", "ad")
        .eq("status", "rendering")
