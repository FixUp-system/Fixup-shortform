...
export const supabaseStore = {
  ...
  async listProfiles() {
    ...
    //
    // ★★★ 2026-09-01 — `admin_name` 은 **컬럼이 없어도 화면이 살아야 한다.** 이 저장소는
    //   "스키마를 먼저 올리고 배포하라"를 규칙으로 두는데, 그 순서가 어긋나면 이 조회가
    //   통째로 실패해 **사용자 관리 화면이 500 으로 죽는다**(실측: 컬럼 없이 HTTP 500).
    //   멀쩡하던 화면을 배포 순서 하나로 잃는 것이 그 값을 못 보는 것보다 나쁘다.
    //   그래서 없으면 그 칸만 빼고 다시 읽고, **왜 비었는지 로그로 크게 남긴다** —
    //   조용히 비면 "저장이 안 된다"로만 보이고 원인을 못 찾는다.
    // ★ 열을 **글자로 두 번 적는다.** 변수로 묶으면 짧아지지만, tests/tier-gate.js 가
    //   select 안의 글자를 읽어 "두 조회가 tier 를 함께 판다"를 재는 그물이 풀린다 —
    //   되살림 조회도 그 검사를 받아야 한다(거기서 tier 를 빠뜨리면 그 화면만 basic 이 된다).
    let { data, error } = await db()
      .from("profiles")
      .select("id, email, status, role, tier, display_name, admin_name, created_at")
      .order("created_at", { ascending: false })
      .limit(500);
    if (error && /admin_name/.test(error.message || "")) {
      console.error(
        "⚠️ profiles.admin_name 컬럼이 없다 — db/schema.sql 을 올려야 운영자 이름이 저장된다. " +
          "그때까지 그 칸은 빈 채로 둔다."
      );
      ({ data, error } = await db()
        .from("profiles")
