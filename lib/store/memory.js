...
const projects = new Map(); // id → { version, owner_id, touched_at, doc }

// 크론이 한 회차에 훑을 편 수 — **supabase.js 의 같은 이름과 같은 값이어야 한다.**
//
// ⚠️⚠️ 값을 그쪽에서 import 하는 쪽이 옳아 보이지만 **하면 안 된다**(2026-09-10 실측).
//   vitest.setup.js 가 이 파일을 import 하므로, 여기서 supabase.js 를 끌면
//   `@supabase/supabase-js` 가 **테스트 파일의 vi.mock 보다 먼저** 로드된다 — 그 순간
//   가짜 클라이언트를 세우는 판들이 진짜 모듈을 쥐고 "Expected 3 parts in JWT" 로 죽는다
//   (그렇게 해 봤더니 판 36개가 빨개졌다). 그래서 이 저장소는 **저장소끼리 import 하지 않는다.**
// ★ 그래도 두 벌로 갈리지는 않는다 — 판이 **프로덕션 쪽 값을 import 해서** 인메모리 결과를
//   잰다(tests/cron-collect.test.js). 숫자가 어긋나면 그 자리에서 빨개진다.
const BAKING_LIMIT = 20;

// 쓰기 순번 — Supabase 의 `updated_at` 자리다("가장 오래 안 건드린 편 먼저" 정렬의 근거).
//
// ★ **시계가 아니라 순번**이다. Date.now() 로 적으면 같은 밀리초에 넣은 둘의 순서가
//   안 정해져 판이 흔들린다. 뜻은 같다 — 큰 값이 더 최근에 쓰인 것이다.
let writeSeq = 0;
...
const creditCodes = []; // 크레딧 코드 — { code, amount_credits, batch, meta, created_by, created_at, redeemed_by, redeemed_at }

// 보관함 종류 필터 — supabase 쪽 byArchiveKind 와 같은 뜻이다(step = ad·film 이 아닌 전부, 옛 null 포함).
function memoryKindMatch(rowKind, kind) {
  if (kind === "ad") return rowKind === "ad";
  if (kind === "step") return rowKind !== "ad" && rowKind !== "film";
  return true;
}
...
export const memoryStore = {
  async insertProject(project, ownerId) {
    projects.set(project.id, {
      version: 0, owner_id: ownerId, touched_at: (writeSeq += 1), doc: clone(project),
    });
    return project;
  },
  ...
  // ★ 소유자 필터가 없다. 크론에는 사람이 없고, 걷어야 할 편은 **모든 사람의 것**이다.
  //   그래서 이 함수를 부르는 자리는 크론 라우트 하나여야 한다(사람이 지나가는 문에
  //   붙이면 남의 편 목록이 그대로 새는 문이 된다).
  // ★★ supabase.js 와 **같은 계약**이라야 한다 — 상한도, 순서도, 행 모양도.
  //   인메모리는 어차피 읽기량이 안 줄지만, 모양이 다르면 여기서 통과한 코드가
  //   프로덕션에서 깨진다(이 파일의 다른 셀렉터들과 같은 이유).
  // ★ projectId 를 주면 그 편만 — 웹훅이 쓴다. 상한이 생긴 뒤로는 다 받아 와서 코드로
  //   거를 수가 없다(찾는 편이 상한 밖이면 조용히 안 걷힌다).
  async selectBakingProjects({ projectId } = {}) {
    // 갈래(=프로덕션의 질의 하나)마다 따로 담는다. supabase.js 가 갈래마다 상한을 걸기
    // 때문이다 — 여기서 통째로 세면 상한의 뜻이 갈린다.
    const buckets = new Map();
    const push = (branch, at, row) => {
      if (!buckets.has(branch)) buckets.set(branch, []);
      buckets.get(branch).push({ at, row });
    };
    for (const [id, row] of projects) {
      if (projectId && id !== projectId) continue;
      const d = row.doc || {};
      const at = Number(row.touched_at) || 0;
      const owner_id = row.owner_id;
      if (d.kind === "reel" && d.reel?.job?.requestId) {
        push("reel", at, { id, owner_id, kind: "reel" });
      } else if (d.kind === "ad" && d.ad_job && d.status === "rendering") {
        // 마무리가 남았는지까지 실어 준다 — 쓸기가 수거와 마무리를 가르는 근거다.
        push("ad", at, { id, owner_id, kind: "ad", ready: Boolean(d.ad_job.ready) });
      } else if (d.kind === "film") {
        // ★★ 필름은 **방식마다 한 행**이다(order · refs 를 나란히 굽는 것이 정상 흐름).
        // ⚠️ 굽는 중 표시가 문서 맨 위가 아니라 `films[mode]` 안쪽에 있다 — film 문서의
        //   d.status 는 "scenario" 에 머물러 굽기와 무관하다. 광고를 흉내 내어 그쪽을
        //   보면 한 편도 안 걸린다.
        // ★ 조건은 collectFilmRender 가 보는 것 그대로다(`!job || status !== "rendering"`).
        //   셀렉터가 더 넓으면 1분마다 헛도는 행이 생긴다.
        for (const m of FILM_MODES) {
          const f = d.films?.[m.id];
          if (!f?.job?.requestId || f.status !== "rendering") continue;
          push(`film:${m.id}`, at, {
            id, owner_id, kind: "film", mode: m.id, ready: Boolean(f.job.ready),
          });
        }
      }
    }
    // 오래 안 건드린 편 먼저 · 갈래마다 상한.
    const take = (branch) => (buckets.get(branch) || [])
      .sort((a, b) => a.at - b.at)
      .slice(0, BAKING_LIMIT)
      .map((x) => x.row);
    // 마무리가 붙을 수 있는 갈래를 앞에 둔다 — supabase.js 와 같은 순서다.
    return [
      ...take("ad"),
      ...FILM_MODES.flatMap((m) => take(`film:${m.id}`)),
      ...take("reel"),
    ];
  },
  // ── 폴링용 부분 읽기 ────────────────────────────────────────────────────
  ...
  // 폴링 전용 좁은 셀렉터 둘 — supabase.js 의 같은 이름과 **같은 모양**을 돌려준다.
  // (인메모리는 어차피 통째로 들고 있어 절약이 없다. 계약을 맞추는 것이 목적이다.)
  async selectAdStatus(id, ownerId) {
    ...
    return {
      status: d.status,
      kind: d.kind ?? null,
      videos: Array.isArray(d.videos) ? clone(d.videos) : [],
      video_error: d.video_error || null,
      ad_job: d.ad_job ? clone(d.ad_job) : null,
      settings: d.settings ? clone(d.settings) : {},
      scenario: { text: d.scenario?.text || "" },
    };
  },
  ...
  async selectReelJob(id, ownerId) {
    ...
    return {
      status: d.status,
      kind: d.kind ?? null,
      reel: { job: d.reel?.job ? clone(d.reel.job) : null },
      progress: d.progress ? clone(d.progress) : null,
    };
  },
  ...
  async selectReelStatus(id, ownerId) {
    ...
    return {
      status: d.status,
      kind: d.kind ?? null,
      reel: d.reel ? clone(d.reel) : null,
      progress: d.progress ? clone(d.progress) : null,
      cuts: Array.isArray(d.cuts) ? clone(d.cuts) : [],
    };
  },
  ...
  // before·limit 는 supabase 쪽과 같은 뜻이다(「더 보기」).
  async listAllProjects({ before, limit = Infinity, kind, from, to } = {}) {
    return [...projects.values()]
      ...
      .filter((r) => (before ? r.created_ts < before : true))
      .filter((r) => memoryKindMatch(r.kind, kind))
      .filter((r) => (Number.isFinite(from) ? r.created_ts >= from : true) && (Number.isFinite(to) ? r.created_ts <= to : true))
      .slice(0, limit);
  },

  async listProjects(ownerId, { before, limit = Infinity, kind, from, to } = {}) {
    return [...projects.values()]
      ...
      .map((r) => ({
        ...
        image_url: r.doc.cuts?.[0]?.image?.url || null,
        film_modes: filmModesOf(r.doc),
        // ★ 2026-09-16 — supabase 구현과 같은 모양이어야 한다(카드의 모델 배지가 이 값을 읽는다).
        //   값이 없는 옛 문서는 그대로 null 이다 — 추측하지 않는다.
        i2v_model: r.doc.settings?.i2v_model ?? null,
        ad_model: r.doc.settings?.model ?? null,
      }))
      .sort((a, b) => b.created_ts - a.created_ts)
      .filter((r) => (before ? r.created_ts < before : true))
      .filter((r) => memoryKindMatch(r.kind, kind))
  ...
  async updateProjectRow(id, ownerId, expectedVersion, doc) {
    ...
    // touched_at 을 올린다 — Supabase 의 updated_at 과 같은 자리다(updateProjectRow 가
    // 그 컬럼을 매번 새로 적는다). 이것이 안 오르면 방금 걷은 편이 계속 맨 앞에 남는다.
    projects.set(id, {
      version: row.version + 1, owner_id: row.owner_id, touched_at: (writeSeq += 1), doc: clone(doc),
    });
    return true;
  },
  ...
  // actor·before 는 운영자의 사용자별 비용 화면이 쓴다(lib/cost-read.js).
  // 안 주면 예전 그대로다 — /costs 화면이 그렇게 부른다.
  async listCosts({ from, to, actor, before, limit = 2000 } = {}) {
    return [...costs.values()]
      .map(clone)
      .filter((r) => (!actor || r.actor === actor) && (typeof before !== "number" || r.ts < before))
  ...
  // ── 크레딧 코드(2026-09-14) ─────────────────────────────────
  // Supabase 쪽(primary key + redeem_credit_code 함수)과 같은 결과를 낸다.
  async insertCreditCodes(rows) {
    const taken = new Set(creditCodes.map((c) => c.code));
    for (const r of rows) {
      if (taken.has(r.code)) return false;  // 한 줄이라도 겹치면 통째로 안 넣는다(트랜잭션과 같다)
      taken.add(r.code);
    }
    const at = new Date().toISOString();
    for (const r of rows) {
      creditCodes.push({ created_at: at, redeemed_by: null, redeemed_at: null, meta: {}, ...clone(r) });
    }
    return true;
  },
  async listCreditCodes() {
    return creditCodes.map(clone).reverse();
  },
  async deleteCreditCode(code) {
    const i = creditCodes.findIndex((c) => c.code === code);
    if (i < 0) return "not_found";
    if (creditCodes[i].redeemed_by) return "used";
    creditCodes.splice(i, 1);
    return "deleted";
  },
  // ★ await 없이 한 번에 판정·기록한다 — 그래야 동시 호출이 끼어들 틈이 없다(SQL 함수의 조건부 update 자리).
  async redeemCreditCode(code, userId, reason) {
    const c = creditCodes.find((x) => x.code === code);
    if (!c) return { result: "not_found", credits: null };
    if (c.redeemed_by) return { result: "used", credits: null };
    c.redeemed_by = userId;
    c.redeemed_at = new Date().toISOString();
    grants.push({
      created_at: c.redeemed_at, user_id: userId, amount_credits: c.amount_credits, reason, granted_by: userId,
    });
    return { result: "ok", credits: c.amount_credits };
  },
  ...
  async listProfiles() {
    return [...profiles.values()].map((p) => ({ ...clone(p), internal: p.internal === true }));
  },
  ...
  async findProfiles(ids) {
    ...
    for (const id of ids) {
      ...
      if (p) out.set(id, {
        ...
        // 내부 계정(크레딧 면제). 참일 때만 참이다 — 없거나 이상한 값은 손님이다.
