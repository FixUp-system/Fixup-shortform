...
// **첨부한 사진이 무엇인지 모델에게 말해 주는 한 줄.**
//
// ★★ 안 말하면 모델이 사진을 "분위기 참고"로 읽고 제품을 자기 식으로 다시 그린다
//   (2026-08-25 실측 — lib/reel/panels.js 의 refLine 주석과 같은 자리다).
// ★ **번호를 안 붙인다** — 첨부 순서가 갈래마다 다르다(컷별은 첫 프레임이 앞자리,
//   통짜는 스토리보드가 앞자리다). 틀린 번호를 말하느니 종류만 말한다.
// ★ 같은 종류가 여럿이면 **한 번만** 말한다 — 같은 문장을 두 번 실으면 무게만 흐려진다.
// ★ 종류를 든 사진이 없으면 **빈 문자열**이다(옛 문서는 첨부 문장이 예전 그대로다).
// ★★★ 이 함수가 생긴 이유는 **두 벌을 막으려는 것**이다 — 컷별(lib/cuts.js 의 clipRefRoles)과
//   통짜(lib/reel/oneshot.js)가 같은 말을 해야 갈래에 따라 다른 지시가 안 나간다.
export function attachedRoleLine(photos) {
  const seen = new Set();
  let out = "";
  for (const ph of photos || []) {
    const role = photoRole(ph?.role);
