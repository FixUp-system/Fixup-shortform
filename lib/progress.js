...
// 파이프라인이 도는 동안 이 간격으로 한 번씩 박동한다(lib/pipeline.js 의 startHeartbeat).
// STALL_MS 의 1/4 이라 정상 실행은 임계에 닿기 전에 네 번 뛴다 — 한두 번 실패해도 살아 있는
// 실행이 "멈춤"으로 읽히지 않는다.
export const HEARTBEAT_MS = STALL_MS / 4;

// **이 단계의 생성이 지금 돌고 있는가.** 돈 쓰는 라우트가 두 번째 실행을 막는 데 쓴다.
//
// 판정을 심장박동 하나에 얹는 이유: 잠금 표식을 따로 세우고 실행이 끝날 때 지우는 방식은
// 얼어붙은 함수에서 영영 안 지워져 사장님이 재시도에 못 닿는다(그 자리에 탈출구가 없었던
// 사고가 이 저장소에 있다). 박동은 아무도 안 지워도 저절로 낡는다.
//
// ★ 그래서 이것은 **박동이 시간으로도 뛴다는 전제 위에서만** 참을 말한다. 완료 때만 뛰면
//   느린 컷을 도는 살아 있는 실행이 여기서 "안 돌고 있다"가 된다 — 그것이 원래의 구멍이다.
// ★ 임계 면제 단계(render)는 이 자로 재지 않는다 — 정상적으로 10분까지 박동이 없다.
//   면제 단계에서 겹침을 막아야 하면 그 단계의 자를 따로 만들어라(여기를 늘리지 말 것).
export function isGenerationLive(progress, phase, now) {
  if (!progress || progress.phase !== phase) return false;
  if (STALL_EXEMPT_PHASES.includes(phase)) return false;
  const at = progress.at;
  if (typeof at !== "number") return false;
  return now - at < STALL_MS;
}
...
// 덮개에 뜨는 말. **처음 만드는 것과 다시 만드는 것은 다른 말이다.**
//
// 문구가 "다시 만드는 중이에요" 하나로 못 박혀 있어서, 한 번도 안 만든 컷을 처음 굽는
// 동안에도 "다시"가 떴다(④이미지·⑤영상 둘 다). 사장님이 "내가 뭘 잘못 눌렀나"를
// 의심하게 만드는 종류의 거짓말이다.
//
// 판정은 하나뿐이다 — **사장님이 [재생성]을 눌렀는가**. 화면이 그것을 아는 자리는
// `regening`(로컬 상태) 하나다. 나머지 굽기는 전부 "처음"으로 말한다: 전체 생성이든
// 낡아서 다시 부르는 것이든, 새로고침 뒤라 누가 눌렀는지 화면이 잊은 경우든 —
// 모르면 덜 말하는 쪽이 맞다("다시"를 잘못 붙이면 없는 이력을 지어내는 것이다).
export function busyLabel(isRegen) {
  return isRegen ? "다시 만드는 중이에요" : "만드는 중이에요";
}
