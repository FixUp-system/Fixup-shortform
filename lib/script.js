...
// 척도는 최장 공통 부분수열 / 짧은 쪽 길이 — 1이면 짧은 쪽이 긴 쪽 안에 순서까지 그대로 들어 있다는 뜻이다.
// 긴 쪽으로 나누면 '보여줌' 전사를 놓친다: 보여줌에는 샷·조명 용어가 붙어 문장보다 길어서
// 겹친 만큼이 그 길이에 희석된다(라이브에서 0.57로 임계를 빠져나갔다).
// 다만 짧은 원본은 우연히 겹치기 쉬우므로(예: 할 말이 "가격") 8자 미만이면 판정하지 않는다.
...
// 임계 셋은 전부 라이브 출력을 재서 잡았다. 세 관계가 서로 겹침의 폭이 달라 한 값으로는 못 가른다.
//
// '할 말' 전사: 복사 1.00 / 제대로 쓴 문장 0.64 → 0.8
export const COPY_LIMIT = 0.8;
// '보여줌' 전사: 화면을 읊은 것 0.71 / 같은 장면을 제 말로 쓴 것 0.13~0.43 → 0.65.
// 낮게 잡을수록 억울한 문단을 잡는다. 0.60짜리(화면을 반쯤 읊되 사실도 말한 문장)는 일부러 살려 둔다.
export const SHOWS_LIMIT = 0.65;
...
export function paragraphsToRewrite(synopsis, script) {
  ...
  paragraphs.forEach((p, i) => {
    ...
    if (!scene) return;
    if (copyRatio(scene.says, p.text) > COPY_LIMIT) out.push({ idx: i, reason: "할 말 전사" });
    else if (copyRatio(scene.shows, p.text) > SHOWS_LIMIT) out.push({ idx: i, reason: "화면 설명 전사" });
    else if (repeatsWithin(p.text)) out.push({ idx: i, reason: "같은 말 되풀이" });
  });
  return out;
}
