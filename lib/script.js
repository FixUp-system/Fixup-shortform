...
export function paragraphFault(scene, text) {
  ...
  if (overLength(scene, text)) return "분량 초과";
  return null;
}

// 배분된 초를 크게 넘겼는가. 초는 눈금이지 자가 아니라 1.5배까지는 둔다 —
// 짧은 장면에서 한 문장만 길어져도 걸리면 멀쩡한 대본을 계속 다시 쓰게 된다.
// 3초 미만의 차이도 두고 본다(2초 장면에서 1초 넘긴 것을 문제라 하지 않는다).
// 구성 이전 프로젝트의 장면에는 초가 없다 — 그때는 따지지 않는다.
export const LENGTH_SLACK = 1.5;

export function overLength(scene, text) {
  const seconds = Number(scene?.seconds);
  if (!Number.isFinite(seconds) || seconds <= 0) return false;
  const spoken = (text || "").replace(/\s/g, "").length / CHARS_PER_SEC;
  return spoken > seconds * LENGTH_SLACK && spoken - seconds >= 3;
}
...
- '분량 초과'로 지목됐으면: 그 장면에 배분된 초 안에 들어오게 줄인다(초당 5.5자). 담을 것이 많으면
  앵글을 떠받치지 않는 것부터 버린다. 다 담으려다 늘어지면 숏폼이 아니다.
