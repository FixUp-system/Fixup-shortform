...
- 문장끼리 이어지게 쓴다. 앞 문장이 뒤 문장을 부르게 — '근데·그래서'로 이어지는 흐름을 만든다.
  사실을 나열하지 않는다. 각 사실을 그 결과·상황·의미로 전개한다("직접 삶습니다"에서 그치지 말고 "그래서 단맛이 다릅니다"까지).
...
export function buildScriptMessages(project, instruction) {
  ...
  user += `\n\n[분량] 공백 빼고 ${target}자 안팎(한국어 낭독 초당 5.5자 기준 약 ${Math.round(target / 5.5)}초).
넘기지 않는다. 담을 사실이 적으면 더 짧아도 된다.`;
...
// 목표 분량을 크게 넘겼는가. 목표는 눈금이지 자가 아니라 1.3배까지 둔다 —
// 장면별로 나눠 재던 때보다 판정이 단순해졌다(전체 글자수 하나만 본다).
