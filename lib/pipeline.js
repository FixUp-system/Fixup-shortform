...
import { shotsToCuts, buildShowsMessages, buildImagePrompt, buildClipPrompt, sceneBasisOf, promptBodyOf, CONTENT_MAX_SECONDS } from "./cuts";
// 수정 지시로 본문을 다시 쓸 때 무엇을 시킬지 — 조립은 순수 모듈이 쥔다(호출은 여기가 한다).
import { buildPromptReviseMessages } from "./prompt-revise.js";
// resolveCutRefs·AVATARS 는 여기서 안 쓴다 — 컷 레퍼런스 판정이 lib/cut-refs.js 로 옮겨 갔다.
import { buildCastMessages, mergePropsIntoCuts, resolveCastRefs, mergeCastIntoCuts, availableAvatars } from "./cast";
...
import { speechLangOf } from "./subtitle-langs.js";
...
import { isCutDone, HEARTBEAT_MS } from "./progress.js";
...
// shows 없는 컷은 buildImagePrompt가 **주제 앵커**로 폴백하므로 그림은 나온다(품질만 떨어진다).
//
// ★ 2026-08-17 정정: 예전에는 **낭독 문장**으로 폴백했다. 그것을 걷은 이유는 두 가지이고
//   첫째가 원래부터 있던 것이다 — 낭독 문장은 귀로 듣는 것이지 그릴 대상이 아니고(그러면
//   화면이 말을 되풀이해 그 컷의 정보량이 절반이 된다), 둘째로 프롬프트가 영어가 된 뒤로는
//   한국어 낭독이 영어 프롬프트 한복판에 들어가기까지 했다(실측: `Scene: 아침마다 커피가…`).
//   앵커도 비면 `Scene:` 절 자체를 넣지 않는다 — 빈 `Scene: .` 로 컷당 $0.08 을 쓰지 않는다.
export const defaultDeps = {
  splitCuts: async (project, ownerId) => {
    ...
    for (const p of project.material?.photos || []) {
      ...
      // 글자만 읽어 낸 사진도 성공이다 — 그 철자가 프롬프트에 실린다
  ...
  // 수정 지시로 프롬프트 본문을 다시 쓴다(lib/prompt-revise.js 가 무엇을 시킬지 정한다).
  //
  // ★ 가짜 모드에서도 **일이 일어난 것처럼** 보여야 한다 — 그래서 fake 를 준다.
  //   지시를 뒤에 붙인 문자열을 내면, 가짜로 관통할 때도 "고쳐졌다"는 것이 화면에 보인다.
  // ★ JSON 으로 받는다 — 맨 글로 받으면 모델이 머리말("좋아요, 여기…")을 붙이고 그 글자가
  //   그대로 그림 프롬프트가 된다.
  revisePrompt: async (msgs, projectId) =>
    callJson({
      system: msgs.system,
      messages: [{ role: "user", content: msgs.user }],
      stage: "프롬프트 수정",
      projectId,
      fake: { prompt: `${msgs.user.split("\n\n사장님 요구:\n")[0].replace("지금 지시문:\n", "")} [수정됨]` },
    }),
};
...
// 도는 동안 시계로 박동한다 — 돌려주는 함수를 부르면 멈춘다.
//
// 왜 필요한가: withProgress 는 **컷이 끝날 때** 불린다. 그런데 컷들은 Promise.all 로 한꺼번에
// 돌아서, 컷이 모두 임계보다 오래 걸리면 살아 있는 실행이 박동 없이 임계를 넘긴다. 그러면
// 화면은 거짓으로 "멈췄다"고 하고, 그 위에서 POST /clips 의 진행 중 잠금도 함께 무력해진다
// (박동이 낡았으니 "안 돌고 있다"로 읽힌다). 즉 이 함수가 그 잠금의 **전제**다.
//
// ★ 컷은 건드리지 않는다 — progress 만 새로 얹는다. 컷을 함께 쓰면 파이프라인의 setCut 과
//   같은 문서를 두 규칙으로 고치는 셈이고, 낙관적 락이 지는 횟수만 늘어난다.
// ★ done 은 여기서도 문서에서 판다(withProgress 가 한다) — 밖에서 세면 뒷걸음친다.
// ★ 실패는 삼킨다. 박동은 **관측 장치**라, 그것이 못 써졌다고 만들던 영상을 죽이면 안 된다.
//   못 뛰면 임계에 닿아 "멈춤"으로 보일 뿐이고 그것이 안전한 쪽 오답이다.
// ★ unref 로 이벤트 루프를 붙잡지 않는다 — 안 하면 테스트·스크립트가 안 끝난다.
// ★ 세는 규칙만 갈아 끼울 수 있다(patch). 시계·CAS·unref·실패 삼키기의 규율은 여기 한
//   벌로 두고, **무엇을 세느냐**만 부르는 쪽이 정한다 — reel 의 통짜 갈래는 컷이 여럿이어도
//   굽는 것이 한 편이라 total 이 1 이다(lib/reel/pipeline.js). 두 번째 타이머를 따로 만들면
//   그 규율이 두 벌이 된다.
// ★ update 도 갈아 끼운다 — 파이프라인이 주입받은 저장 함수를 쓰는 자리(테스트·reel)에서
//   여기만 진짜 저장소를 부르면 그 문서에는 박동이 안 남는다.
export function startHeartbeat(
  projectId,
  ownerId,
  phase,
  { intervalMs = HEARTBEAT_MS, patch = withProgress, update = updateProject } = {}
) {
  const timer = setInterval(() => {
    // 시각은 락 밖에서 잰다 — withProgress 주석 참고(CAS 재시도로 patchFn 이 다시 불린다)
    const at = Date.now();
    update(projectId, ownerId, (proj) => patch(proj, phase, at)).catch(() => {});
  }, intervalMs);
  timer.unref?.();
...
async function processCut(projectId, ownerId, cut, project, deps) {
  ...
  // 컷이 쓸 레퍼런스를 고르고 바이트까지 읽어 온다 — **판정은 lib/cut-refs.js 한 벌**이다
  // (④이미지의 [전문 복사]가 부르는 라우트도 같은 함수를 쓴다. 두 벌이면 사장님이 복사해 간
  //  프롬프트와 실제로 나가는 프롬프트가 갈린다).
  // 바이트를 미리 읽는 이유: 그림(프롬프트)과 심사(VLM)가 같은 것을 봐야 하고,
  // 두 번 읽으면 그 사이에 달라질 수 있다.
  const { refs } = await loadCutRefs(cut, project);
  // 화면 기준은 **조립하는 쪽이 정한다** — `sceneBasisOf`(lib/cuts.js)가 그림 프롬프트의
  // `Scene:` 절과 같은 판정을 내주고, 검수는 그 값을 받아 쓴다.
  //
  // ★ 예전에는 이 자리와 lib/vlm.js 가 조회식 **두 벌**을 들고 있었다. 프롬프트 쪽 폴백이
  //   낭독 문장을 걷은 날 두 기준이 갈렸고(실측: 저장된 컷 46개 중 13컷), 검수 기준이
  //   `장면 설명과 일치` 라 갈리면 물릴 근거가 생긴다 — 물리면 아래 `round 2` 가 그림을
  //   한 장 더 산다(컷당 +$0.08). 그러니 판정을 여기서 다시 쓰지 말고 그 함수를 불러라.
  const sceneBasis = sceneBasisOf(cut, project);
...
export async function regenCut(projectId, ownerId, idx, deps = defaultDeps, instruction = null) {
  ...
  // ★★ 수정 지시는 **본문을 다시 쓴다** — 꼬리에 덧붙이지 않는다(2026-08-18 사장님 지시).
  //
  //   덧붙이던 시절에는 원래 서술이 그대로 남은 채 새 요구가 뒤에 붙어, 모델이 서로 다투는
  //   두 지시를 받았다("배경은 밤거리" + "배경을 노을로"). 사장님이 실제로 쓰는 말은 한
  //   문장에 **고칠 것과 더할 것이 섞여** 온다 — 그것을 그대로 처리하려면 다시 쓰는 수밖에 없다.
  //
  // ★ **지금 본문에서 출발한다**(promptBodyOf 는 덮어쓰기가 있으면 그것을 준다) — 그래서
  //   두 번째 지시가 첫 번째 결과 **위에** 얹힌다. 매번 원점에서 고치면 앞서 고친 것이 사라진다.
  // ★ 상한을 **먼저 싸게 본다.** 아래 patchFn 의 판정이 권위이고(TOCTOU 를 그 자리가 막는다),
  //   여기서는 3회를 넘긴 것이 뻔한데 LLM 값을 치르는 일만 없앤다.
  // ★ 실패하면 **조용히 넘어간다** — 고쳐 쓰기가 안 됐다고 재생성 자체를 막으면, 사장님은
  //   값을 낼 준비를 하고 눌렀는데 아무것도 못 받는다. 그때는 옛 방식대로 지시가 꼬리에 실린다.
  let revised = null;
  if (note && (cut.regen_count || 0) < MAX_REGEN_PER_CUT) {
    const msgs = buildPromptReviseMessages({
      current: promptBodyOf("image", cut, project),
      instruction: note,
    });
    if (msgs) {
      try {
        const out = await (deps.revisePrompt || defaultDeps.revisePrompt)(msgs, projectId);
        const text = typeof out?.prompt === "string" ? out.prompt.trim() : "";
        if (text) revised = text;
      } catch (e) {
        console.error("프롬프트 고쳐 쓰기 실패 — 지시를 꼬리에 실어 그대로 만든다:", e?.message);
      }
    }
  }
    ...
    return {
      ...
      cuts: proj.cuts.map((c) =>
        c.idx === idx
          ? {
              ...c,
              regen_count: c.regen_count + 1,
              // 지시 원문은 그대로 남긴다 — 화면이 "지난 수정 지시"로 보여 준다.
              ...(note ? { edit_instruction: note } : {}),
              // ★ 그 지시를 **본문에 실었는가**. 꼬리(lib/cuts.js imagePromptTail)가 이 값을
              //   보고 같은 요구를 두 번 보내지 않는다 — 실렸으면 꼬리는 침묵하고, 못 실었으면
              //   꼬리가 유일한 전달자가 되어 한국어라는 것과 우선순위를 함께 말한다.
              //   위 주석("실패하면 옛 방식대로 지시가 꼬리에 실린다")이 원래 뜻이었고,
              //   여기가 그 뜻을 코드로 만든 자리다.
              ...(note ? { edit_applied: Boolean(revised) } : {}),
              // 고쳐 쓴 본문은 **덮어쓰기 자리**에 담는다 — 사장님이 접힌 칸에서 눈으로
              // 확인하고 손으로 더 고칠 수 있는 그 자리다(각인도 이 값을 본다).
              ...(revised ? { image_prompt: revised } : {}),
            }
          : c
      ),
    };
            ...
            cuts: proj.cuts.map((c) =>
              c.idx === cut.idx
                ? {
                    ...
                    //   화면에 있는 시간(seconds)은 **시나리오가 장면마다 정하고 사장님이 화면에서
                    //   확정한 값**이라 지킨다(2026-08-16. 예전에는 allocateCutSeconds 가 배분했다) —
                    //   여기서 덮으면 그 여백이 통째로 사라지고 영상이 다시 낭독 길이가 된다.
...
export async function runVideoPipeline(projectId, ownerId, deps = {}) {
  ...
  // 도는 동안 시계로도 박동한다 — 클립 하나가 임계보다 오래 걸려도 살아 있음이 보인다.
  // ★ finally 로 반드시 멈춘다. 안 멈추면 끝난 실행이 계속 뛰어 "영원히 만드는 중"이 되고,
  //   그러면 POST /clips 의 잠금이 사장님을 영영 가둔다.
  const stopHeartbeat = startHeartbeat(projectId, ownerId, "video", { intervalMs: deps.heartbeatMs });
  try {
  await Promise.all(
    cuts.map(async (cut) => {
      const setCut = (patch) => {
...
export async function regenClip(projectId, ownerId, idx, deps = {}, instruction = null) {
  const clip = deps.clip || generateClip;

  // ★★ 수정 지시는 **본문을 다시 쓴다** — regenCut 과 같은 규칙이고 같은 배관이다
  //    (2026-08-18 사장님 지시로 ⑤영상에도 수정사항 칸이 생겼다).
  //    영상 갈래에는 규칙이 하나 더 붙는다: **대사를 건드리지 않는다** — 그 글자는 자막으로도
  //    나가므로(ffmpeg 가 원고를 태운다) 여기서 고치면 들리는 말과 화면이 갈린다.
  //    그 규칙은 lib/prompt-revise.js 가 kind:"clip" 으로 쥔다.
  const note = typeof instruction === "string" && instruction.trim() ? instruction.trim() : null;
  let revised = null;
  if (note) {
    const before = await getProject(projectId, ownerId);
    const target = before?.cuts?.find((c) => c.idx === idx);
    // 상한을 넘긴 것이 뻔하면 LLM 값을 치르지 않는다(권위는 아래 락 안의 판정이다)
    if (target && (target.clip_regen_count || 0) < MAX_REGEN_PER_CUT) {
      const msgs = buildPromptReviseMessages({
        current: promptBodyOf("clip", target, before),
        instruction: note,
        kind: "clip",
      });
      if (msgs) {
        try {
          ...
          // 고쳐 쓰기가 안 됐다고 재생성을 막지 않는다 — 사장님은 값을 낼 준비를 하고 눌렀다.
          console.error("클립 프롬프트 고쳐 쓰기 실패 — 옛 본문 그대로 만든다:", e?.message);
        }
      }
    }
  }
    ...
    return {
      ...
      cuts: proj.cuts.map((c) =>
        c.idx === idx
          ? {
              ...c,
              clip_regen_count: (c.clip_regen_count || 0) + 1,
              // 지시 원문은 남긴다 — 화면이 "지난 수정 지시"로 보여 준다
              ...(note ? { edit_instruction: note } : {}),
              // 고쳐 쓴 본문은 덮어쓰기 자리에 — 사장님이 접힌 칸에서 눈으로 확인한다
              ...(revised ? { clip_prompt: revised } : {}),
            }
          : c
      ),
    };
...
export async function runRenderPipeline(projectId, ownerId, deps = {}) {
  ...
  const result = await compose({
    ...
    // ★★ 안 골랐으면 **말한 언어**로 떨어진다(2026-08-18). 예전에는 undefined 였고,
    //   그러면 굽는 쪽이 한국어 폰트(Pretendard)로 일본어를 그려 **두부(□□□)**가 됐다 —
    //   사장님이 "최초에 일본어 자막이 깨진다"고 한 자리가 여기다.
    lang: project.settings?.subtitle_lang || speechLangOf(project),
    // ★★ 원문 언어. 이것이 없으면 자막 글자를 고르는 자가 "한국어는 원문"이라 믿고
    //   일본어 영상의 한국어 자막에 **번역 대신 원문**을 굽는다(lib/subtitles.js).
...
export async function runSubtitlePipeline(projectId, ownerId, deps = {}) {
  ...
  const result = await burn({
    ...
    // 안 골랐으면 말한 언어다 — 위 compose 와 같은 자여야 한다(다르면 전체 합성과
    // 자막만 다시 굽기가 서로 다른 글자를 굽는다).
    lang: project.settings?.subtitle_lang || speechLangOf(project),
    sourceLang: speechLangOf(project),
    aspect_ratio: project.settings?.aspect_ratio || "9:16",
  });
