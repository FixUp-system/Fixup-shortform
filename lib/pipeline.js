...
async function processCut(projectId, cut, project, deps) {
  // ★ 여기에 직렬화 보장은 없다. 예전에는 updateProject 안에 프로젝트별 락이 있었지만
  // 저장소를 옮기면서 낙관적 재시도로 바뀌었다 — 읽은 버전이 그대로면 쓰고, 남이 먼저
  // 썼으면 다시 읽어 최대 5회까지 시도한다. 컷이 동시에 여러 개 도는 자리라 충돌은
  // 실제로 난다. 5회 안에 못 이기면 이 갱신은 던진다(조용히 사라지지는 않는다).
