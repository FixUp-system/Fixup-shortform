...
import { splitUnits, explodeLongRanges, allocateCutSeconds, fillSilentCuts, buildSplitMessages, buildShowsMessages, buildImagePrompt, buildClipPrompt, CONTENT_MAX_SECONDS } from "./cuts";
...
import { clipKey, renderKey, renderKeyBody, subtitleHead, isClipStale, toneKey } from "./steps.js";
...
import { projectSpeaks, clipProfileForProject } from "./clip-limits.js";
// 사장님이 고를 수 있는 길이 — 없으면(자동) 배분 목표를 spoken_seconds 합으로 대신한다.
import { TARGET_CHOICES } from "./script.js";
...
export const defaultDeps = {
  splitCuts: async (project, ownerId) => {
    ...
    // 모델의 길이 눈금은 프로젝트가 정한다 — 분할 프롬프트가 그것을 사실로 알려 준다
    const split = buildSplitMessages(units, project);
    ...
    // ★ 이 경고는 낭독(spoken_seconds) 기준이다 — 아래 배분이 seconds 를 늘리기 전에 잰다.
    const over = cuts.filter((c) => c.spoken_seconds > CONTENT_MAX_SECONDS).length;
    ...
    //   무조건 돈다: 고른 초가 없어도(자동 길이) 목표를 spoken_seconds 합으로 준다.
    //   목표=바닥 합이면 여백이 0이라 결과는 "바닥뿐"이 된다(하한 미만이던 컷만 하한으로 오른다) —
    //   그런데도 무조건 돌리는 이유는 무음 컷(spoken_seconds=0, Task 8) 때문이다.
    //   조건부로 두면 자동 길이 프로젝트의 무음 컷이 0초 그대로 주문돼 버린다.
    const chosen = project?.settings?.target_seconds;
    const targetSeconds = TARGET_CHOICES.includes(chosen)
      ? chosen
      : cuts.reduce((a, c) => a + (Number(c.spoken_seconds) || 0), 0);
    // ★ 배분 **앞**에 무음 컷을 채운다 — 뒤로 가면 채운 컷이 배분을 못 받아 0초로 남는다.
    //   그리고 캐스팅(아래 buildCastMessages)보다도 앞이다: 캐스팅이 컷 번호로 인물을 꽂으므로
    //   (cast[].cuts) 컷을 나중에 끼우면 사람이 한 칸씩 밀린 컷에 붙는다.
    cuts = fillSilentCuts(cuts, targetSeconds, clipProfileForProject(project));
    ...
    // 코드가 둘을 판정하고, 어긋나면 사유를 주고 한 번 더 부른다.
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (shot.ok && speed.ok) { redoReason = null; break; }
      redoReason = [shot.reason, speed.reason].filter(Boolean).join(" 그리고 ");
    }
...
async function processCut(projectId, ownerId, cut, project, deps) {
  ...
  const setCut = (patch) =>
    updateProject(projectId, ownerId, (proj) => ({
      ...proj,
