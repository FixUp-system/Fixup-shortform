...
import { clipKey, renderKey, isClipStale } from "./steps.js";
...
export const defaultDeps = {
  splitCuts: async (project) => {
    ...
    // 대비를 코드가 판정하고, 없으면 사유를 주고 한 번 더 부른다.
    // 판정만 하고 강제하지 않으면 안 된다 — 이 저장소가 컷 길이에서 이미 겪었다.
    //
    // 다만 대비가 없다고 화면 설계를 버리지는 않는다: 늘어진 화면이 화면 없는 것보다 낫고,
    // 사장님이 ②대본에서 손으로 고칠 수 있다. 그래서 마지막 응답은 대비와 무관하게 받는다.
    let designed = null;
    let contrastReason = null;
    for (let i = 0; i < 2; i++) {
      const msgs = contrastReason
        ? [...shots.messages, { role: "user", content: `[다시] 속도 대비가 없다 — ${contrastReason}. shows 는 그대로 두고 speed 만 고쳐 같은 형식으로 다시 낸다.` }]
        : shots.messages;
      ...
      const verdict = speedContrast(got || []);
      if (got && verdict.ok) { contrastReason = null; break; }
      contrastReason = got ? verdict.reason : "형식이 맞지 않았다";
    }
    if (designed && contrastReason) {
      console.warn(`[화면 ${project.id.slice(0, 8)}] 속도 대비 미달로 남았다 — ${contrastReason}`);
    }
    ...
    for (let round = 0; round < 2; round++) {
      ...
      if (verdict.passed) {
        await setCut({
          ...
          image: {
            url: candidates[verdict.selectedIndex].url,
            of: cut.shows || "",
            style_of: styleKey(project),
          },
...
export async function runVideoPipeline(projectId, deps = {}) {
  ...
  await Promise.all(
    cuts.map(async (cut) => {
      ...
      if (cut.video?.url && !isClipStale(cut)) return;
      try {
