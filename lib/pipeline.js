...
async function processCut(projectId, ownerId, cut, project, deps) {
  ...
  let bought = null;
  const engraved = (url) => ({ url, of: cut.shows || "", style_of: styleKey(project) });
  try {
    let note = "";
  ...
  await Promise.all(
    cuts.map(async (cut) => {
      ...
      try {
        ...
        await setCut({ video: { url, seconds, truncated, of: clipKey(cutForKey) }, video_error: null });
      } catch (e) {
...
export async function regenClip(projectId, ownerId, idx, deps = {}) {
  ...
  try {
    ...
    await updateProject(projectId, ownerId, (proj) => ({
      ...
      cuts: proj.cuts.map((c) =>
        c.idx === idx
          ? { ...c, video: { url, seconds, truncated, of: clipKey(cutForKey) }, video_error: null }
          : c
      ),
    }));
  } catch (e) {
...
export async function runRenderPipeline(projectId, ownerId, deps = {}) {
  ...
  const result = await compose({
    projectId,
    cuts: project.cuts || [],
    // ★ 프로젝트를 통째로 넘긴다 — 합성이 "클립이 스스로 말하는가"를 판정해야 한다
    // (말하면 소리 파일이 없고, 컷 길이가 받은 클립 길이다).
    project,
    aspect_ratio: project.settings?.aspect_ratio || "9:16",
