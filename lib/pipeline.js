...
// export 하는 이유는 하나다 — splitCuts가 buildCutsMessages와 validateCuts(obj, scenes)를
// 실제로 맞물리는 유일한 자리인데, 파이프라인 테스트는 전부 splitCuts를 주입해 우회한다.
...
async function processCut(projectId, cut, project, deps) {
  ...
  // 장면은 여기서 한 번만 뽑는다 — 그림(프롬프트)과 심사(VLM)가 갈라져 서로 다른 기준을
  ...
  try {
    ...
    for (let round = 0; round < 2; round++) {
      let prompt = buildImagePrompt(cut, project, scene);
