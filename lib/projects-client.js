...
// before — 「더 보기」의 커서(지금 목록 마지막 카드의 created_ts). 없으면 첫 쪽이다.
// filter — 보관함 좁히기 `{ kind, start, end }`.
//   · kind: 종류(ad·step). "all"·없음이면 안 싣는다(주소가 예전 그대로다)
//   · start·end: 만든 날짜 경계(ms, dayBounds 가 만든다). null 이면 안 싣는다
export async function loadProjects(fetchImpl = fetch, scope = "mine", before, filter = {}) {
  const { kind, start, end } = filter || {};
  const params = new URLSearchParams();
  if (scope === "all") params.set("scope", "all");
  if (kind && kind !== "all") params.set("kind", kind);
  if (Number.isFinite(start)) params.set("from_ts", String(start));
  if (Number.isFinite(end)) params.set("to_ts", String(end));
  if (before) params.set("before", String(before));
  const q = params.toString();
  let res;
  try {
    res = await fetchImpl(q ? `/api/projects?${q}` : "/api/projects");
  } catch {
  ...
  // hasMore — 뒤에 더 있는가(「더 보기」를 그릴지). 옛 응답에는 없으므로 없으면 false 다.
  return { projects: d.projects || [], err: "", guest: !!d.guest, hasMore: !!d.has_more };
}
