...
반드시 JSON 하나만 출력: {"shots":[{"shows":"화면에 보이는 것(정지 화면)","motion":"그 화면이 어떻게 움직이는지 한 줄","speed":"static|slow|realtime|fast|extreme_slowmo 중 하나"}]}
...
- **초점이 물건이어도 제품만 놓인 정물 컷을 되풀이하지 않는다.** 제품은 쓰이는 모습으로 팔린다 —
  신발이면 신고 뛰는 사람, 앰플이면 바르는 손. 정물 컷은 여는 한 방과 닫는 한 방 정도로 아껴 쓰고,
  가운데 컷들은 **그 제품이 쓰이는 순간**을 잡는다.
  실측에서 초점이 "신발"이었더니 6컷 중 사람이 나오는 컷이 하나뿐이고 나머지가 신발·발
  클로즈업이었다 — 기능은 설명됐지만 광고가 되지 않았다.
