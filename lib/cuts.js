...
반드시 JSON 하나만 출력: {"tone":"이 영상의 색과 질감 한 줄","environment":"이 영상의 무대 — 장소·시간대·조명 한 줄","shots":[{"shows":"화면에 보이는 것(정지 화면)",${MOTION_AXES.map((a) => `"${a.id}":"${a.label} 움직임 한 줄 — 없으면 빈 문자열"`).join(",")},"speed":"static|slow|realtime|fast|extreme_slowmo 중 하나","transition":"이 컷이 시작하는 구도 — 첫 컷에는 넣지 않는다"}]}
