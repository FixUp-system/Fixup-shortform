...
const CAMERA_WORDS = /(트래킹|오빗|팬(?![가-힣])|줌\s?(인|아웃)?|달리\s?(인|아웃)?|트럭|크레인|핸드헬드|틸트\s?(업|다운)?|카메라가?|\b(?:tracking|orbit(?:s|ing)?|dolly|dollying|handheld|steadicam|camera|panning|zooming|tilting|craning)\b|\b(?:zoom|pan|tilt|push|pull|truck|dolly|crane)(?:s|ed)?\s+(?:in|out|up|down|back|away|closer|around|across|left|right|over)\b|\bcrane\s+shot\b)/gi;
...
const EN_MOTION = /\b(?:push|pull|move|track|follow|orbit|circle|rotate|spin|turn|walk|run|ride|pass|lift|rise|fall|drift|float|pour|fill|flicker|blink|sway|shake|swing|approach|retreat|enter|exit|cross|come|go)(?:es|s)?\b/i;
...
const STILL_FORMS_EN = [
  /\b(?:sun)?(?:light|lights|sunlight|sunbeam|glow|shadow|shadows|dust|steam|haze)\b[^.]*\b(?:comes?|falls?|drifts?|floats?|streams?|pours?|enters?|spills?|hits?|catches?|lands?)\b/i,
...
export function stillOnly(shows) {
  ...
  for (const c of clauses) {
    ...
    if (!cleaned) continue;
    // 한국어가 섞인 절은 지금까지처럼 한국어 규칙으로 본다 — 형태론이 다르고, 저장된 옛
    // shows("클로즈업 of hands …")가 그 모양이다. 영어 규칙은 **영어 절에만** 적용된다
    if (/[가-힣]/.test(cleaned)) {
    ...
    if (EN_LEFTOVER_ONLY.test(cleaned)) continue;
    // 움직임 낱말이 없으면 구도 서술이다(영어 shows 의 대부분이 명사구다)
    if (!EN_MOTION.test(cleaned) && !EN_SPEED_ADVERBS.test(cleaned)) { kept.push(cleaned); continue; }
    if (EN_SPEED_ADVERBS.test(cleaned)) continue;
    if (STILL_FORMS_EN.some((re) => re.test(cleaned))) kept.push(cleaned);
  }
  return kept.join(", ");
}
...
const CAMERA_MOTION = /(트래킹|오빗|크레인|줌\s?(인|아웃)|달리\s(인|아웃)(?![가-힣])|틸트\s?(업|다운)|팬\s?(업|다운|인|아웃)|카메라가\s?(다가|물러|도는|돌|따라|움직|이동|올라|내려|밀|당기)|\b(?:tracking|orbits?|orbiting|dollying|craning|panning|zooming|tilting)\b|\b(?:zoom|pan|tilt|push|pull|dolly|crane|truck|track)(?:s|ed|ing)?\s+(?:in|out|up|down|back|away|closer|around|across|left|right|over)\b|\bcrane\s+shot\b|\bcamera\s+(?:\w+\s+){0,2}?(?:mov(?:es?|ed|ing)|push(?:es|ed|ing)?|pull(?:s|ed|ing)?|track(?:s|ed|ing)?|follow(?:s|ed|ing)?|orbit(?:s|ed|ing)?|circl(?:es|ed|ing)|ris(?:es|ing)|rose|dolli(?:es|ed)|pans?|tilts?|zooms?|drift(?:s|ing)?|clos(?:es|ing)\s+in))/i;
