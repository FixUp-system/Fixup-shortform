...
function imagePromptTail(cut, project, refs = []) {
  ...
  const label = (r) =>
    r.kind === "person" ? r.who || "the person" : anchor || "the product/subject";
  ...
  const thing = refs.find((r) => r.kind === "thing");
  if (thing) {
    p += ` ${refHintFor(style, "thing", numbered ? refs.indexOf(thing) + 1 : null)}`;
  }
