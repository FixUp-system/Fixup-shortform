...
//   · '카메라가' 는 조사까지 붙은 형태만 — "필름 카메라 특유의 입자감"은 명사 언급이다
//   · '핸드헬드'·'트럭' 제외 — "핸드헬드 다큐 질감"은 카메라 이동이 아니라 질감 서술이다
// g 플래그를 붙이지 않는다 — lastIndex 가 남지 않아 함정 자체가 없다.
const CAMERA_MOTION = /(트래킹|오빗|크레인|줌\s?(인|아웃)|달리\s?(인|아웃)|틸트\s?(업|다운)|팬(?![가-힣])|카메라가)/;
...
export function usableTransition(transition) {
  ...
  if (!t) return "";
  return CUT_REFERENCE.test(t) || CAMERA_MOTION.test(t) ? "" : t;
}
