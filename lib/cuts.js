...
import { photoRole, attachedRoleLine } from "./photos.js";
...
function imagePromptTail(cut, project, refs = []) {
  ...
  //   위 문구는 "사진에 인쇄된 글자를 그대로 재현하라"고만 말할 뿐, **그 글자가 무엇인지**는
  //   안 알려 준다 — 모델이 작게 찍힌 글자를 스스로 읽어야 한다. 읽는 것보다 받아쓰는 것이
  //   훨씬 정확하므로, 사진 판정(lib/vlm.js describePhoto)이 한 번 읽어 둔 값을 여기서 싣는다.
  //   ★ 있을 때만 덧붙인다 — 못 읽은 사진의 프롬프트는 글자 그대로 그대로다.
  //   ⚠️ 그래도 보장은 아니다: 영상 모델은 프레임마다 다시 그려서 이미지에서 맞은 철자가
  //     영상에서 흔들린다. 확률을 올리는 장치다.
  const photosById = new Map((project?.material?.photos || []).map((ph) => [ph.id, ph]));
  const lettered = refs
    .map((r, i) => ({ n: i + 1, text: photosById.get(r.photo_id)?.vision?.lettering || "" }))
    .filter((x) => x.text);
  for (const { n, text } of lettered) {
    p += ` Attached image [${n}] reads exactly "${text}" — spell it exactly that way, letter for letter.`;
  }
  // ★★ 2026-08-31 사장님 지시 — 사진마다 **종류**(로고·제품·인물)를 말하고 "변하면 안
  //   된다"를 못 박는다. 위 lettering 과 같은 결이다: 모델이 스스로 알아내야 했던 것을
  //   우리가 **알려 준다.**
  // ★ 번호는 lettering 과 **같은 자**를 쓴다(refs 순서) — 두 문장이 같은 첨부를 가리켜야
  //   모델이 이어 읽는다.
  // ★ 종류를 든 사진만 적는다 — 옛 문서는 한 글자도 안 바뀐다(각인 보호).
  const roled = refs
    .map((r, i) => ({ n: i + 1, role: photoRole(photosById.get(r.photo_id)?.role) }))
    .filter((x) => x.role);
  for (const { n, role } of roled) {
    p += ` Attached image [${n}] ${role.en}.`;
  }
  ...
  //
  // ★★ **본문에 이미 실린 지시는 여기서 다시 안 붙인다**(2026-08-19).
  //   원래 설계가 그랬다 — regenCut 은 지시로 **본문을 다시 쓰고**(lib/prompt-revise.js),
  //   그 주석은 "실패하면 옛 방식대로 지시가 꼬리에 실린다"고 적혀 있다. 그런데 코드는
  //   성공했을 때도 꼬리에 또 실었다. 그래서 같은 요구가 **두 번** 갔고, 그중 하나는
  //   프롬프트 전체가 영어인데 혼자 한국어였다.
  //   실측(2026-08-19): "인형 사이즈를 줄여줘"가 안 먹었다. 다시 쓴 본문은 영어로 작아진
  //   제품을 말하는데, 꼬리의 한국어 원문은 모델에게 잡음에 가깝고, 그 사이의 계약부는
  //   "이 제품을 일관되게 유지하라 · 첨부와 똑같은 비례로"라고 반대로 민다.
  //   **금지 문구를 하나 더 붙이는 대신 반대로 미는 사본을 걷어낸다** — 이 저장소가 오늘
  //   반복해서 얻은 교훈이다. 값도 안 든다(번역 호출을 새로 만들지 않는다).
  //
  // ★ 세 갈래인 이유는 **각인** 때문이다. `edit_applied` 가 아예 없는 컷은 이 기능 이전에
  //   저장된 옛 컷이다 — 그 컷의 프롬프트가 한 글자라도 달라지면 살아 있는 그림이 낡아
  //   사장님에게 유료 [다시 만들기]가 뜬다(컷당 $0.08). 그래서 옛 컷의 문장은 **글자 그대로**
  //   예전 그대로 두고, 새 문장은 "고쳐 쓰기가 실패한 것이 확인된" 컷에만 붙인다.
  if (cut.edit_instruction && cut.edit_applied !== true) {
    p += ` Important correction requested by the user, apply it strictly: ${cut.edit_instruction}.`;
    // 고쳐 쓰기가 실패해 **이 한 줄이 유일한 전달자**인 컷 — 그때만 두 가지를 알려 준다:
    // ①이 문장은 한국어다(나머지가 전부 영어라, 안 알려 주면 모델이 잡음으로 흘린다)
    // ②앞 문장과 부딪히면 이쪽이 이긴다(제품 일관성 절이 반대로 밀고 있다).
    if (cut.edit_applied === false) {
      p += ` That correction is written in Korean; follow it exactly, and where it contradicts any earlier sentence in this prompt (including size, proportion and consistency instructions) the correction takes precedence.`;
    }
  }
  return p;
}
...
//
// 클립 프롬프트에 실을 맥락 — 무대·인물·톤(·음악).
// ★ **제품은 여기 없다**(2026-08-18 에 걷어냈다 — 아래 제품 절 자리의 주석이 근거다).
//
...
// (stageOf·castLooksOf·usableTone 이 이미 그렇게 되어 있다) — 옛 컷이 값을
// 안 채웠으면 그 컷의 프롬프트는 늘지 않는다. 화면비는 여기서 다루지 않는다(아래 참고) —
// 그래서 이 함수의 절은 전부 "값이 없으면 안 붙는다"를 지킨다.
//
// 순서: 무대 → 인물 → 톤. buildImagePrompt 의 절 순서를 그대로 따른다(제품 절만 빠졌다) —
...
function clipContextClause(cut, project) {
  ...
  // ★ 제품 절(앵커+외형)은 **여기 없다** — 2026-08-18 에 걷어냈다.
  //
  // 두 단계가 받는 것이 다르다. 이미지 생성은 사장님 참조 **사진 바이트**를 함께 받아
  // 묘사가 틀려도 사진이 이긴다. 클립(i2v)은 사진이 안 간다 — **이미지 한 장 + 글로 쓴
  // 묘사**뿐이라 **묘사가 이미지를 이긴다.** 실측(프로젝트 1f9f66f8…): 첫 프레임의 라벤더
  // 토끼가 5초 뒤 프롬프트가 말한 크림색 토끼로 다시 그려졌고, 사장님이 인형을 지운 컷
  // (edit_instruction "가방에 있는 토끼인형 없애줘")에는 없던 인형이 만들어져 들어갔다.
  // 게다가 바로 뒤 꼬리의 `Keep the subject and style unchanged.` 와 정면으로 모순이다.
  //
  // ★ **외형(look)만이 아니라 앵커(anchor)까지** 뺀다. 넷 다 근거가 있다:
  //   ① 사고 문장의 절반이 앵커였다 — 앵커가 구도를 주장했다("an Esther Bunny plush
  //      keyring **clipped onto a woman's handbag strap**"). look 만 빼면 그 문장이 남는다.
  //   ② 앵커는 **영상 한 편에 하나**라 컷마다 갈리지 않는다. i2v 는 그 컷의 실제 프레임을
  //      이미 받으므로, 프레임에 없는 물건을 "이 영상의 피사체는 X 다"라고 말하는 것은
  //      곧 X 를 그려 넣으라는 지시가 된다.
  //   ③ 초점이 물건이 아니면 앵커는 topic 으로 떨어진다 — 자료가 기획서면
  //      "신발을 주인공으로 한 감각적인 광고 영상"이 피사체 이름으로 실린다.
  //   ④ 앵커가 이미지 프롬프트에서 하던 일(전 컷 일관성)은 클립에서는 꼬리 한 줄
  //      (`Keep the subject and style unchanged.`)과 **첫 프레임 그 자체**가 이미 한다.
  //
  // ⚠️ **이미지 프롬프트에서는 빼지 않는다**(imagePromptBody 는 그대로다) — 그림을 그릴
  //    때는 묘사가 있어야 하고, 참조 사진과 함께 가므로 안전하다.
  // ⚠️ 각인도 **같은 기준**으로 걷었다(lib/steps.js clipKey). 이 저장소의 불변이
  //    "프롬프트에 실리는 것만 각인에 담는다"라, 한쪽만 고치면 프롬프트는 같은데 낡았다고
  //    하는 자리가 생기고 그 버튼은 유료다.
  // ⚠️ 이 변경으로 **저장된 클립이 전부 낡는다**(30초 한 편 재생성 ~$9). 그대로 둔다 —
  //    지금 저장된 클립에는 틀린 제품이 들어 있으므로 낡음은 거짓 신호가 아니라 정확한
  //    신호다(설계 문서 "★ 각인 영향" 절, 사용자 승인됨).
...
// 이 컷에 꽂힌 사진의 **종류**를 말한다(2026-08-31 사장님 지시).
//
// ★ **번호를 안 붙인다** — 그림 프롬프트와 달리 클립 요청의 첨부 순서는 갈래마다 다르다
//   (컷별은 첫 프레임이 앞자리를 차지하고, 통짜는 스토리보드 한 장뿐이다).
//   틀린 번호를 말하느니 종류만 말한다 — 모델에게 반쪽 사실을 주는 것은 안 주느니만 못하다.
// ★ 같은 종류가 여럿이면 한 번만 말한다(같은 문장을 두 번 실으면 무게만 흐려진다).
// ★ 종류를 든 사진이 없으면 **빈 문자열** — 옛 문서는 첨부 문장이 예전 한 줄 그대로다.
// ★ 2026-09-04 — 문장은 **lib/photos.js 의 attachedRoleLine 하나**가 만든다. 통짜 갈래도
//   같은 말을 해야 한다(그전에는 이 함수만 있었고, 통짜에 붙이려다 두 벌이 될 뻔했다).
//   여기 남는 것은 "이 컷이 어느 사진을 물었나"를 푸는 일뿐이다.
function clipRefRoles(cut, project) {
  const ids = Array.isArray(cut?.ref_ids) ? cut.ref_ids : [];
  const byId = new Map((project?.material?.photos || []).map((ph) => [ph.id, ph]));
  return attachedRoleLine(ids.map((id) => byId.get(id)));
}
...
export function buildClipPrompt(cut, project, opts = {}) {
  ...
  const attach =
    opts.attach === "refs"
      ? ` The attached images show what this scene, the person and the product look like.${clipRefRoles(cut, project)}`
  ...
  // 무대·인물·톤 — 말하는 경로와 안 말하는 경로 둘 다 같은 절을 받는다.
  // (제품 절은 2026-08-18 에 빠졌다 — clipContextClause 안의 주석이 근거다.)
  ...
  if (spoken) {
    ...
    // 대사는 **원문 그대로**다 — 번역하거나 다듬으면 자막(ffmpeg 가 태우는 원고)과 갈린다.
    // ★ 무슨 말로 말할지는 첫 화면에서 고른 값이 정한다(settings.speech_lang).
    //   안 고른 프로젝트는 한국어라, 그 경우 이 문장은 예전과 **글자 그대로** 같다 —
    //   한 글자라도 달라지면 각인이 흔들려 이미 산 영상이 낡는다(컷당 8크레딧).
    ...
    if (spoken.narration) {
      return `${body} A narrator speaks in voiceover, off-screen — no one in frame speaks or moves their lips.${voice} Says exactly, in ${langLine}: "${line}". The narration is delivered by a native ${langLine} speaker with natural, fluent ${langLine} pronunciation and everyday intonation — never a foreign accent, never spelled out letter by letter.${context}${note}${scene}${attach} Keep the subject and style unchanged. No text or letters.`;
    }
    const who = spoken.who || "인물";
    return `${body} ${who} speaks to the camera with natural lip sync.${voice} Says exactly, in ${langLine}: "${line}". The narration is delivered by a native ${langLine} speaker with natural, fluent ${langLine} pronunciation and everyday intonation — never a foreign accent, never spelled out letter by letter.${context}${note}${scene}${attach} Keep the subject and style unchanged. No text or letters.`;
  }
