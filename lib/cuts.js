...
export function buildClipPrompt(cut, project) {
  ...
  if (spoken) {
    ...
    if (spoken.narration) {
      return `${base}.${pace} A narrator speaks in voiceover, off-screen — no one in frame speaks or moves their lips.${voice} Says exactly, in Korean: "${line}".${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
    }
