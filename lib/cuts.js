...
function imagePromptBody(cut, project) {
  ...
  //   차례가 있다.
  //   ① 첫째 이유는 처음부터 있었다 — 바로 위 주석이 그 말을 한다: "나레이션 문장은 귀로
  ...
  //   없는 것이라 ④이미지에 닿지 않는다.
  //
  //   ★ 이 고침은 **폴백 경로만** 건드린다. `stillOnly(cut.shows)` 가 값을 내는 컷의
  ...
  const shows = stillOnly(cut.shows) || stillOnly(legacyScene?.shows) || anchorFallback;
