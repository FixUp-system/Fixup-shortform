...
import { I2V_STEPS, I2V_MAX_SECONDS } from "./clip-limits.js";
...
function splitClauses(sentence) {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (isLast) break;
    const tok = tokens[i];
    const tokenText = tok[0];
    const tokenEnd = tok.index + tokenText.length;
    const next = tokens[i + 1];
    const sep = sentence.slice(tokenEnd, next.index);
    const buf = sentence.slice(start, tokenEnd);
    // 조각이 너무 짧으면 자르지 않고 계속 모은다. 공백이 한 칸이 아니면 자르지 않는다 —
    // 이었을 때 원래 모양을 복원할 수 없는 자리이기 때문이다.
    if (isClauseEnd(tokenText) && noSpace(buf) >= MIN_UNIT_CHARS && sep === " ") {
      parts.push(buf);
      start = next.index;
    }
  }
