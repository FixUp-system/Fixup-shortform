...
export function fillSilentCuts(cuts, targetSeconds, profile) {
  ...
  const silent = () => ({ idx: out.length, sentence: "", silent: true, spoken_seconds: 0, seconds: 0, source: "code", regen_count: 0 });
  const longest = (secs) => secs.reduce((a, b) => Math.max(a, b), 0);
  // 배분해 보고, 8초를 넘는 컷이 있으면 하나 더 넣고 다시 배분한다.
  let guard = 0;
  while (out.length < maxCuts && guard++ < 32) {
    const secs = allocateCutSeconds(out, target, profile);
    if (longest(secs) <= CONTENT_MAX_SECONDS) break;
    // ★ 넣어 보고 **가장 긴 컷이 실제로 짧아질 때만** 넣는다.
    //   낭독 시간이 바닥이라 무음 컷으로는 못 내리는 경우가 있다(쪼갤 자리가 없는 긴 문장 —
    //   한 컷이 10초를 말하면 그 컷은 어떻게 나눠도 10초다). 그때 계속 넣으면 화면만
    //   늘어나고 8초는 그대로다 — 총 초가 고른 초를 넘어 클립 값이 늘고 이미지도 컷당 더 산다.
    if (longest(allocateCutSeconds([...out, silent()], target, profile)) >= longest(secs)) break;
    out.push(silent());
  }
