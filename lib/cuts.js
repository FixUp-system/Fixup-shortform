...
//
// ⚠️ 한국어 섬이 하나 남아 있다 — **샷 크기 낱말**(극단적 클로즈업·클로즈업·미디엄 샷·풀 샷·광각).
//    lib/shots.js 의 shotSizeOf 가 shows 에서 그 낱말을 **한국어로** 찾아 클로즈업 쏠림을
//    판정한다(shotBalance → 화면 설계 재시도). 예시까지 영어로 바꾸면 그 판정이 조용히 죽어
//    카탈로그 같은 컷 구성을 아무도 막지 않는다. 그래서 낱말 하나만 한국어로 남기고,
//    지문도 "그 한국어 낱말 그대로 적는다"고 말한다. 목록이 영어로 옮겨지면 이 섬도 없어진다.
//
// ⚠️ 이 전환이 못 덮는 자리가 더 있다(같은 원인, 다른 파일) — 아래 문지기들이 한국어 전용
//    정규식이라 영어 값에는 아예 안 걸린다. 지문이 말로만 막는 상태다:
//      · stillOnly(CAMERA_WORDS·STILL_FORMS) — shows 의 움직임 절을 그림 지시에서 빼는 자리
//      · usableTone·usableTransition(CAMERA_MOTION·CUT_REFERENCE) — 카메라 어휘·앞 컷 참조
//    둘 다 이 태스크의 파일 범위 밖이라 손대지 않았다(각인에 닿는 자리라 함부로 넓히면
//    굳은 그림이 낡아 재구매가 열린다). 넓히는 일은 별개 태스크로 남긴다.
//
...
- **shows·tone·environment·transition·움직임 축은 영어로 적는다.** 이 값들은 그림·영상 모델에
  ...
  · 예외가 하나 있다 — **샷 크기 낱말은 아래 목록에서 골라 한국어 그대로 적는다.**
    코드가 그 낱말로 클로즈업 쏠림을 판정한다. 낱말 하나만 한국어이고 나머지 서술은 영어다.
...
- shows는 카메라가 잡는 것을 눈에 보이게 적는다 — 피사체·행동·샷 크기·앵글. 추상어로 쓰지 않는다.
  샷 크기는 극단적 클로즈업·클로즈업·미디엄 샷·풀 샷·광각(설정 샷) 중에서 골라 **그 한국어 낱말
  그대로** 적고(코드가 이 낱말을 읽는다), 앵글은 eye level·low angle·high angle·bird's-eye·
  over-the-shoulder·POV 중에서 골라 그 말 그대로 적는다.
  **시간대·날씨는 shows 에 적지 않는다** — 그것은 environment 가 정한다. 컷마다 적으면 한낮에서
  ...
  ✓ "a 7am kitchen, 클로즈업 of hands dropping whole strawberries into a blender, first light through the window"
  ✓ "an empty pre-dawn shop, 풀 샷, chairs stacked on the tables"
...
- 화면 안에서 읽히는 글자·숫자가 보이는 장면은 적지 않는다 — 가격표·간판·메뉴판·문서·화면 속 글자.
  ...
  ✗ "클로즈업 of a table with the ampoule bottle next to a price tag"
  ✓ "클로즈업 of a single ampoule bottle on a table, morning sunlight"
...
- **transition 은 이 컷이 시작하는 구도다.** 앞 컷 끝과 이어 보이게 하는 값이라 **첫 컷에는
  ...
  **앞 컷을 가리키는 말을 쓰지 않는다.** 이 값으로 그림을 그리는 쪽은 앞 컷을 볼 수 없다 —
  ...
  ✓ "클로즈업 of the feet on asphalt, at the same eye level"
  ✓ "미디엄 샷 with the bottle in hand cropped at the left edge, background blurred"
...
- 초점이 물건인데 사람이 보이는 컷이면, 사람과 물건을 한 화면에 담는다. 시선이나 손이 그 물건을 향하게 쓴다.
  ✗ "클로즈업 of a smiling woman in her late 20s" — 초점이 물건인데 물건이 화면에 없다
  ✓ "미디엄 샷 of a woman in her late 20s holding the ampoule bottle and looking at it, smiling"
