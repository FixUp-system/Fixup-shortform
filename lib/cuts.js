...
import { activeStyle, refHintFor } from "./styles.js";
...
- **★ tone 은 영상 하나의 색과 질감이다 — 하나만 정한다.** 컬러 그레이딩·명암·필름 질감을
  ...
  ✓ "빛바랜 필름 입자에 초록기가 도는 저대비 다큐 질감"
  ✗ "체육관, 스포트라이트"(그건 environment 다)
  ✗ "카메라가 다가가며 차가워지는 색"(카메라 움직임이다)
- **★ environment 는 이 영상 전체의 무대다 — 하나만 정한다.** 장소·시간대·조명을 한 줄로 적는다.
  시간대는 골든아워·한낮·황혼·새벽·야간 중에서 고르고, 날씨·공기(안개·이슬비·햇빛에 떠다니는 먼지)도
  여기서 정한다.
  예: "실내 농구 코트, 야간, 어두운 체육관에 강한 스포트라이트와 깊은 그림자"
  예: "성수동 골목 옷 수선집 안, 골든아워, 창으로 든 빛에 먼지가 떠다닌다"
...
- **transition 은 이 컷이 시작하는 구도다.** 앞 컷 끝과 이어 보이게 하는 값이라 **첫 컷에는
  ...
  · 카메라가 어떻게 움직이는지도 적지 않는다. 여기 적는 것은 **시작 순간의 구도** 하나이고,
    움직임은 움직임 축이 맡는다.
  ✗ "앞 컷에서 이어진다" / "직전 컷과 같은 각도" / "이어받아 카메라가 다가간 상태로 시작"
  ✓ "발 클로즈업, 아스팔트 위, 같은 눈높이"
  ✓ "손에 쥔 병이 화면 왼쪽에 걸린 미디엄 샷, 배경은 흐리다"
...
- **★ 광고의 샷 리듬을 지킨다. 대부분은 넓은 샷이고, 제품 클로즈업은 아껴 쓴다.**
  ...
  · 클로즈업 계열(극단적 클로즈업·클로즈업)이 **절반을 넘지 않게 한다.** 넘으면 카탈로그가 된다.
  · 가운데 컷들은 미디엄 샷·풀 샷·광각으로 **사람이 무엇을 하는지** 보이게 잡는다.
...
const CAMERA_WORDS = /(트래킹|오빗|팬(?![가-힣])|줌\s?(인|아웃)?|달리\s?(인|아웃)?|트럭|크레인|핸드헬드|틸트\s?(업|다운)?|카메라가?)/g;

export function stillOnly(shows) {
  const clauses = String(shows || "").split(/,\s*/).map((c) => c.trim()).filter(Boolean);
  const kept = [];
  for (const c of clauses) {
    // 카메라 낱말을 먼저 지운다 — 지우고 남는 것이 없으면 그 절은 카메라 서술뿐이었다
    ...
    if (!/(다|요)$/.test(cleaned)) { kept.push(cleaned); continue; }
    if (SPEED_ADVERBS.test(cleaned)) continue;
    if (STILL_FORMS.some((re) => re.test(cleaned))) kept.push(cleaned);
  }
  return kept.join(", ");
}
...
const CUT_REFERENCE = /((앞|이전)의?\s?(컷|장면)|직전|같은\s?컷|바로\s?전|이어받아|위와\s?같은)/;
...
const CAMERA_MOTION = /(트래킹|오빗|크레인|줌\s?(인|아웃)|달리\s(인|아웃)(?![가-힣])|틸트\s?(업|다운)|팬\s?(업|다운|인|아웃)|카메라가\s?(다가|물러|도는|돌|따라|움직|이동|올라|내려|밀|당기))/;
...
//    바깥은 언제나 buildImagePrompt 하나만 부른다.
function imagePromptBody(cut, project) {
  ...
  // ★★ 마지막 칸이 문장인데 **무음 컷에는 문장이 없다**(2026-08-14 최종 리뷰).
  //   화면 설계는 통째로 실패할 수 있고(validateShows 는 shows 가 한 칸이라도 비면 null 이다)
  //   파이프라인은 그것을 견딘다. 그때 말하는 컷은 문장으로 내려앉지만 무음 컷은 빈손이라
  //   `Scene: .` — 주어 없는 그림을 $0.08 에 사고 그 위에 클립까지 산다.
  //   그래서 무음 컷에는 제 버팀목을 준다: 이 영상의 주제(브리핑)다. 어느 컷에도 맞고,
  //   앵커와 같은 대상을 가리켜 다른 컷들과 한 편으로 보인다.
  const anchorFallback = cut.silent === true
    ? (project.briefing?.focus?.subject || project.briefing?.topic || "")
    : "";
  const shows = stillOnly(cut.shows) || stillOnly(legacyScene?.shows) || cut.sentence || anchorFallback;
  ...
  return `${style.medium} for a short-form video, ${orient} composition. Scene: ${shows}.${stage}${castClause}${subject}${noteClause}`;
}
...
function imagePromptTail(cut, project, refs = []) {
  const style = activeStyle(project);
  let p = ` ${style.finish}, no text or letters in the image.`;
  // 레퍼런스 문구는 종류에 따라 갈린다.
...
function promptOverride(value) {
  return typeof value === "string" ? value.trim() : "";
}

export function buildImagePrompt(cut, project, refs = []) {
  // 본문을 통째로 갈아 끼운 경우. 꼬리는 코드가 그대로 붙인다 — 사장님이 "글자를 크게
  // 넣어라"라고 써도 `no text or letters` 는 그 뒤에 남는다.
  //
  ...
  return body + imagePromptTail(cut, project, refs);
}
...
//
// ⚠️ export 하지 않는다 — 바깥은 언제나 buildClipPrompt 하나만 부른다. 호출부가 늘면
...
export function buildClipPrompt(cut, project) {
  // 이미지와 같은 규칙이다 — 본문만 갈리고 꼬리는 코드가 붙인다(promptOverride 참고).
  //
  // ★ 판형은 **붙이지 않는다.** 이미지와 다른 이유: 클립 프롬프트에는 애초에 판형 절이
  //   없다(lib/i2v.js 가 aspect_ratio 를 prompt 와 나란히 별도 요청 필드로 보낸다 —
  //   clipContextClause 주석에 그 근거가 있다). 없던 절을 덮어쓴 컷에만 만들면 그 컷의
  //   프롬프트 모양이 다른 컷과 갈린다.
  //
  // ★ 대사·목소리·립싱크 지시는 아래 꼬리에 그대로 남는다 — 사장님이 여기서 대사를
  //   지울 수 있으면 들리는 말과 ffmpeg 가 태우는 자막이 갈린다(lib/subtitles.js).
  const override = promptOverride(cut?.clip_prompt);
  const body = override || clipPromptBody(cut);
  ...
  if (spoken) {
    ...
    if (spoken.narration) {
      return `${body} A narrator speaks in voiceover, off-screen — no one in frame speaks or moves their lips.${voice} Says exactly, in Korean: "${line}".${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
    }
    const who = spoken.who || "인물";
    return `${body} ${who} speaks to the camera with natural lip sync.${voice} Says exactly, in Korean: "${line}".${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
  }

  return `${body}${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters. No talking faces or lip sync.`;
}
