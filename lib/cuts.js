...
export function buildImagePrompt(cut, project, refs = []) {
  ...
  const thingLook = focusThing && typeof project.briefing?.focus?.look === "string"
  ...
  const stage = typeof cut.environment === "string" && cut.environment.trim()
    ? ` Setting (same in every scene of this video): ${cut.environment.trim()}.`
    : "";
