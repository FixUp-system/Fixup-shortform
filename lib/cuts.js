...
반드시 JSON 하나만 출력: {"tone":"이 영상의 색과 질감 한 줄","environment":"이 영상의 무대 — 장소·시간대·조명 한 줄","shots":[{"shows":"화면에 보이는 것(정지 화면)","motion":"그 화면이 어떻게 움직이는지 한 줄","speed":"static|slow|realtime|fast|extreme_slowmo 중 하나","transition":"이 컷이 시작하는 구도 — 첫 컷에는 넣지 않는다"}]}
...
export function subjectOf(project) {
  const focusThing = project.briefing?.focus?.mode === "물건" ? project.briefing.focus.subject : "";
  const anchor = focusThing || project.briefing?.topic || "";
  const look = focusThing && typeof project.briefing?.focus?.look === "string"
    ? project.briefing.focus.look.trim()
    : "";
  return { anchor, look };
}
...
export function buildImagePrompt(cut, project, refs = []) {
  ...
  const castClause = here.length
    ...
    : "";
  let p = `${style.medium} for a short-form video, ${orient} composition. Scene: ${shows}.${stage}${castClause}${subject}${noteClause} ${style.finish}, no text or letters in the image.`;
  // 레퍼런스 문구는 종류에 따라 갈린다.
...
export function spokenOf(cut, project) {
  ...
  return s ? `${s.line}|${s.voice}|${s.who}` : null;
}
...
// 안 채웠으면 그 컷의 프롬프트는 늘지 않는다. 화면비(orientOf)만은 기본값이 있어 project
// 가 있으면 늘 실린다 — buildImagePrompt 가 orient 를 조건 없이 싣는 것과 같은 결이다.
//
// 순서: 무대 → 인물 → 제품 → 톤 → 화면비. buildImagePrompt 의 절 순서를 그대로 따른다 —
...
function clipContextClause(cut, project) {
  ...
  parts.push(`Frame: ${orientOf(project)}.`);
...
export function buildClipPrompt(cut, project) {
  ...
  // 무대·인물·제품·톤·화면비 — 말하는 경로와 안 말하는 경로 둘 다 같은 절을 받는다.
  ...
  if (spoken) {
    ...
    return `${base}.${pace} ${who} speaks to the camera with natural lip sync.${voice} Says exactly, in Korean: "${line}".${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
  }

  return `${base}.${pace}${context} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters. No talking faces or lip sync.`;
}
