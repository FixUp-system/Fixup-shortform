...
// 섞이면 갈리므로).
//
// ★ 화면 밖 목소리(narration)를 여기서 다시 재지 않는다 — projectSpeaks 가 그 컷 하나로
//   **프로젝트 전체**를 말하지 않는 쪽으로 내리므로, 여기 오는 컷에는 narration 이 없다.
//   같은 판정을 한 벌 더 두면 언젠가 두 벌이 갈린다(이 파일이 지키는 규칙 그대로).
