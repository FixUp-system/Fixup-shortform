...
// scene은 호출측(processCut)이 이미 뽑아 둔 장면이다. 그림과 심사가 같은 장면을 쥐도록
// 조회식을 한 곳에만 두기 위해 넘겨받는다. 넘어오지 않으면 스스로 뽑는다(다른 호출처 대비).
export function buildImagePrompt(cut, project, scene = project.synopsis?.scenes?.[cut.scene_idx]) {
