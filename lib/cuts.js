...
export function buildClipPrompt(cut, project) {
  ...
  //   축은 아래 템플릿이 붙이는 마침표와 겹쳐 ".." 가 된다. 그래서 끝 마침표를 걷어내고
  //   ". " 로 잇는다 — 마지막 하나는 템플릿의 `.` 가 닫는다.
  //   (마침표만 적힌 축처럼 걷어내면 아무것도 안 남는 값은 없는 것으로 본다 — 그래야
  //   빈 문자열이 base 가 되어 프롬프트가 "." 하나로 시작하는 일이 없다.)
  const axisText = axesOf(cut)
