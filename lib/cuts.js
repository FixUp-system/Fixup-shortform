...
export function fillSilentCuts(cuts, targetSeconds, profile) {
  ...
  const worstReducible = (arr) => {
    const secs = allocateCutSeconds(arr, target, profile);
    return secs.reduce((m, s, i) => (floorOf(arr[i]) <= CONTENT_MAX_SECONDS ? Math.max(m, s) : m), 0);
  };

  let n = 0;
  let cur = build(0);
  let guard = 0;
  while (cur.length < maxCuts && guard++ < 32) {
    const worst = worstReducible(cur);
    if (worst <= CONTENT_MAX_SECONDS) break;
    const next = build(n + 1);
    if (worstReducible(next) >= worst) break; // 더 넣어도 안 내려간다
    n += 1;
    cur = next;
  }
