...
export function buildClipPrompt(cut, project) {
  ...
  if (spoken) {
    ...
    if (spoken.narration) {
      return `${body} A narrator speaks in voiceover, off-screen — no one in frame speaks or moves their lips.${voice} Says exactly, in ${langLine}: "${line}".${context}${note} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
    }
    const who = spoken.who || "인물";
    return `${body} ${who} speaks to the camera with natural lip sync.${voice} Says exactly, in ${langLine}: "${line}".${context}${note} The attached image is the first frame — continue naturally from it. Keep the subject and style unchanged. No text or letters.`;
  }
