...
export async function sweepBakingProjects(deps = {}) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await runWithActor(row.owner_id || CRON_ACTOR, async () => {
        ...
        if (row.kind === "ad" && row.ready) {
          finishBudget -= 1;
          await finishAd(row.id, row.owner_id);
          finished += 1;
          return;
        }
        // 가벼운 수거 — fal 상태 한 번과 문서 쓰기 한 번뿐이라 회차 상한을 두지 않는다.
        if (row.kind === "ad") await collectAd(row.id, row.owner_id);
        else await collectReel(row.id, row.owner_id);
        collected += 1;
      });
    } catch {
