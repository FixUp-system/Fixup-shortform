...
//
// source 를 주면 **한 장부만** 준다 — "grant"(충전) · "charge"(사용 · 환불 포함). 그 밖의 값은 조건 없음.
// 마이페이지 [크레딧] 탭의 [전체 | 충전 | 사용] 이 쓴다(2026-09-14). 화면이 받은 쪽만 거르면
// 20줄 쪽 안에서만 걸러져 "충전 내역이 없다"로 잘못 보인다 — 자르기 **전에** 거른다.
export const LEDGER_SOURCES = ["grant", "charge"];

// fromTs·toTs 를 주면 그 시각 범위(양끝 포함)만 준다 — 날짜 칸의 경계는 **브라우저가** ms 로 만들어 보낸다
// (lib/costs-filter.js 의 dayBounds). 서버는 UTC 라 여기서 날짜를 자르면 한국 오전 기록이 하루 밀린다.
export async function readLedger(userId, { limit = LEDGER_PAGE, before, source, fromTs, toTs } = {}) {
  ...
  const all = [
    ...
    .filter((r) => (LEDGER_SOURCES.includes(source) ? r.source === source : true))
    .filter((r) => (Number.isFinite(fromTs) ? r.ts >= fromTs : true))
    .filter((r) => (Number.isFinite(toTs) ? r.ts <= toTs : true))
    .sort((a, b) => b.ts - a.ts);
  ...
  // 요약("450/1,000")용 합계 — 좁히기와 무관하게 **전체** 기준이다.
  // ★ used 를 청구 합계에서 따로 버림하지 않고 "총 충전 − 보유"로 낸다: 옛 장부의 소수 행 때문에
  //   셋을 각자 버리면 보유 + 사용 ≠ 총 충전 이 되어 화면의 수가 서로 안 맞는다.
  const granted = Math.floor(state.granted);
  return {
    balance: state.balance,
    granted,
    used: granted - state.balance,
