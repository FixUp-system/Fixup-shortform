...
export function speechUnits(sentences, words, { seconds } = {}) {
  ...
  // ★★ 2026-09-16 리뷰(I4) — 버린 이유를 문장마다 남긴다. "낱말이 하나도 안 묶였다"(empty)와
  //   "재기가 흔들렸다"(order·range·outlier)는 다르다 — 전자만 원고와 다르게 말했다는 증거이고,
  //   나머지는 정렬 도구가 흔들린 것뿐이다. speechMismatch(lib/reel/doc.js)가 이 구분으로
  //   경고 범위를 좁힌다.
  const bad = (reason) => ({ units: list.map(() => ({ start: null, seconds: null, ok: false, reason })), heard });
  if (!list.length || !parts.length) return bad("empty");

  const wanted = list.reduce((a, s) => a + charCount(s), 0);
  if (!wanted) return bad("empty");
  const ratio = heard.chars / wanted;
  // ★ 이 판정은 "문장 하나"가 아니라 "전체"를 못 믿는 것이다 — 개별 낱말 이상치가 아니라
  //   딴 말(빠진 문장·노랫말)을 들었을 때다. speechMismatch 는 이 값을 `heard`로 따로 다시
  //   재서 "short"로 알리므로, 여기 reason 은 missing-sentence 를 켜지 않게 "empty"가 아닌
  //   값이면 된다.
  if (ratio < MIN_HEARD_RATIO || ratio > MAX_HEARD_RATIO) return bad("heard-ratio");
  ...
  const units = aligned.map((u) => {
    ...
    // 이 문장에 낱말이 하나도 안 묶였다 — speechMismatch 가 "말하지 않은 문장이 있어요"로
    // 알릴 유일한 경우다.
    if (!Number.isFinite(start) || !Number.isFinite(dur) || dur <= 0) return { start: null, seconds: null, ok: false, reason: "empty" };
    if (start < prevEnd - 0.01) return { start: null, seconds: null, ok: false, reason: "order" };
    if (start + dur > limit + 0.01) return { start: null, seconds: null, ok: false, reason: "range" };
    const first = parts.find((w) => w.timestamp[0] >= start - 0.01);
    if (first && stretched(first)) return { start: null, seconds: null, ok: false, reason: "outlier" };
    prevEnd = start + dur;
