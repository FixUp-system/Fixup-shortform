...
export function describeCutRefs(cut, project) {
  ...
  //
  // ⚠️ **2026-09-03 — 지금 이 갈래를 타는 모델이 하나도 없다.** 2.5 의 `facesInRefs: false`
  //   를 걷어냈기 때문이다(사장님 지시). 그전 근거였던 "실측 9건 전부 거절"은 **격자
  //   방식이 틀렸던 것**이다 — 그때는 판 전체·27×27·2px·시안 **반투명**이었고,
  //   09-03 에 **얼굴에만·흰색·불투명 10×10** 으로 바꾸니 2.0 과 2.5 둘 다 통과했다.
  //   얼굴을 가리는 일은 이제 굽기 직전에 격자가 한다(lib/reel/face-grid.js).
  //   ★ **코드는 지우지 않는다** — 격자로도 안 되는 모델이 나오면 그 모델에
  //     `facesInRefs: false` 한 줄을 다시 두는 것으로 이 길이 살아난다.
  //
  //   (옛 근거) 판 지문은 "얼굴은 프레임 밖으로"라고 써 놓고 사진은 그대로 실어서
  //   **글과 사진이 서로 싸웠다** — 사진이 이기면 판에 얼굴이 그려지고 영상 단계에서
  //   막힌다(판값 $0.401 을 두 번 태운다).
  //
