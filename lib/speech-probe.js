...
export async function probeSpeech(audioDataUri, { fetchImpl = fetch, projectId, seconds, lang = "ko" } = {}) {
  ...
  try {
    ...
    // ★★ 2026-09-16 리뷰 M3 — **200 응답이면** 낱말 수와 무관하게 적는다. 예전에는
    //   `words.length` 도 함께 봐서, 성공했는데 낱말이 0개인 응답(무음 구간 등)이 원장에
    //   안 남았다 — 유료 호출인데 장부에 없는 값이 생긴다. 실패(!res.ok)·호출 자체를
    //   안 한 경우(가짜 모드·오디오 없음, 위에서 이미 return)는 여전히 안 적는다.
    if (Number(seconds) > 0) {
      await addRecord({
