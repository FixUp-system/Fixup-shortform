...
export function requireUser(req, { pending = false } = {}) {
  ...
  // ★ pending: true 는 **승인 대기만** 더 들인다(차단은 여전히 막힌다). 크레딧 코드 등록 문 하나가 쓴다 —
  //   와디즈 서포터가 승인 전에 코드를 넣어 두면 운영자가 알아보고 승인한다(2026-09-14 A안).
  if (status !== "approved" && !(pending && status === "pending")) throw new NotApproved(status);
...
// pending: true — 승인 대기 계정도 지난다(차단은 아니다). ★ adminOnly 와 함께 쓰지 마라.
export function withUser(handler, { adminOnly = false, guest = false, pending = false } = {}) {
  return async (req, ctx) => {
    let user;
    try {
      user = requireUser(req, { pending });
    } catch (e) {
