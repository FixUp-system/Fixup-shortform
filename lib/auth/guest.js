...
const GUEST_GET = [
  // 화면 — 들어오는 문(루트)과 첫 화면, 그리고 보관함 목록·상세
  // ★ 루트는 **보내기만 한다**(app/page.js) — 그 자리가 막혀 있으면 손님은 첫 화면에서
  //   로그인 벽을 만난다.
  new RegExp(`^/$`),
  // ★★ 2026-09-09 — 첫 화면(홈)을 손님에게 연다(사장님 확정). 그전에는 손님이 보관함으로
  //   갔다. **새로 새는 자리가 없다**: 홈은 읽기 전용 화면이고, 거기서 부르는 것은 이미
  //   이 목록에 있는 `/api/projects` 하나뿐이다(표지 그림은 아래 uploads 로 이미 열려 있다).
  //   만드는 문(`/ads/new`·`/create`·`/reel/new`)은 이 목록에 없으므로 그대로 로그인이 지킨다 —
  //   홈의 도구 격자는 **링크만** 그리고, 누르면 middleware 가 로그인으로 보낸다.
  new RegExp(`^/home$`),
  new RegExp(`^/archive$`),
