...
//
// ★ 402(서비스 제한)도 같은 계급이다 — 2026-09-07 라이브에서 밟았다. Supabase 는 egress
// 할당량을 넘기면 프로젝트를 막고 **auth·rest·storage 가 전부 402** 를 준다
// (`exceed_egress_quota`). 402 는 5xx 가 아니라 이 그물에 안 걸렸고, 그래서 로그인 화면에
// "이메일 또는 비밀번호가 맞지 않아요"가 떴다 — 비밀번호는 멀쩡한데 비밀번호를 의심하게
// 만드는, 이 파일이 막으려던 바로 그 실패다. 사장님이 고칠 것은 결제이지 비밀번호가 아니다.
//
...
export function isInfra(error) {
  ...
  return typeof s === "number" && (s === 0 || s === 402 || s === 429 || s >= 500);
}
