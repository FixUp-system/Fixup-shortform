...
export const PUBLIC_PATHS = [
  "/login",
  // ★★ 2026-09-11 — 이용약관·개인정보처리방침. **로그인 전에 읽고 동의하는 문서**라
  //   공개여야 한다. 여기 들어가면 BARE_PATHS 도 따라와 사이드바 없이 그려진다.
  //   ⚠️ 접두사로 여는 것이 맞는 드문 자리다 — `/legal` 아래에는 **정적 문서 화면만** 산다.
  //     값이 나가는 문도, 남의 것을 읽는 문도 없다. 여기에 다른 것을 만들지 마라.
  "/legal",
  "/api/auth/signup",
  "/api/auth/login",
  "/api/cron/collect",
...
// ★ 2026-09-14 — `/cost-table`(원가 표)을 더했다. 그전에는 모든 사용자에게 열린 화면이었다
//   (08-25 내부 테스트용). 손님을 받기 시작하면 우리 원가 구조가 드러난다(사장님 지시).
export const ADMIN_PATHS = ["/costs", "/admin", "/cost-table"];
...
// 로그인 뒤 **돌아갈 자리** — 손님이 눌렀던 그 자리로 되돌리기 위한 값이다 (2026-09-14).
//
// ★★★ 이 값은 **주소창에서 온다**(`/login?next=…`). 그래서 검사 없이 쓰면 우리 로그인
//   화면이 남의 사이트로 보내는 문이 된다(open redirect) — 메일에 `/login?next=https://…`
//   를 실어 보내면 우리 도메인에서 시작한 링크가 남의 로그인 화면에서 끝난다.
//   그래서 **우리 안의 경로만** 통과시킨다:
//     · `/` 로 시작해야 한다 (스킴도, 도메인도 못 적는다)
//     · `//evil.com` · `/\evil.com` 은 막는다 — 브라우저가 **다른 호스트**로 읽는 모양이다
//     · 제어문자가 섞이면 막는다 (헤더를 쪼개는 옛 수법)
//     · `/login` 으로 되돌아가는 것도 막는다 — 로그인하자마자 로그인 화면인 고리가 된다
//   ⚠️ 통과 못 하면 **빈 문자열**이다. 부르는 쪽은 그때 자기 기본값으로 간다(대개 `/`).
//     여기서 기본값을 정하지 않는 이유는 자리마다 기본이 다르기 때문이다.
export function safeNext(raw) {
  ...
  if (/[\u0000-\u001f\u007f]/.test(raw)) return "";
  if (matchesSegment(raw.split(/[?#]/)[0], "/login")) return "";
  return raw;
}
