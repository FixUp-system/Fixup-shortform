...
import { videoPrice, regenPrice, adVideoPrice, formatCredits } from "./pricing.js";

export class NoCredits extends Error {
  constructor(balance, price) {
    super(`크레딧이 모자라요 — 이 작업은 ${formatCredits(price)} 크레딧인데 ${formatCredits(balance)} 남았어요`);
    this.name = "NoCredits";
...
// 크레딧을 지금 쓰는가 — **스위치 하나가 이 판정의 유일한 자리다.**
//
// 내부 QA 동안 끈다(2026-08-14 사용자 결정 — 차감·지급을 실물로 확인했다). 코드를 지우지
// 않는 이유: 결제를 붙일 때 그대로 돌아와야 한다. Vercel 에서 이 값만 지우면 원래대로다.
//
// ★ 기본값은 **켜짐**이다. `=== "1"` 로 좁게 본다 — 오타나 빈 값이 무료로 새는 쪽으로
//   떨어지면 안 된다(모르는 값은 안전한 쪽으로, 이 저장소가 가짜 모드에서 쓰는 규율과 같다).
// ★ 화면도 이 판정을 본다 — /api/credits·/api/me 가 gated 로 내려준다. 두 벌로 두면
//   화면이 먼저 막는데 서버는 통과시키는(또는 그 반대) 어긋남이 생긴다(실제로 있었다).
export function creditsEnabled() {
  return process.env.SHOTFORM_NO_CREDITS !== "1";
}

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

// 내부 계정 판정 — **프로필 하나만 보는 순수 함수다.** creditsEnabledFor 가 이것을
// 전역 스위치와 합친다. "면제 규칙(전역 스위치 + profile.internal)"이 두 곳(여기·호출부)에
// 따로 적히면 언젠가 한쪽만 고쳐진다 — 이 함수가 그 규칙의 절반(계정 판정)을 쥐는 유일한 자리다.
export function isInternalAccount(profile) {
  return profile?.internal === true;
}

// **이 계정에게** 크레딧을 걷는가 — 청구·잔액 검사·체험 한도가 전부 이것을 본다.
//
// ★★ 2026-09-14 사장님 결정 — 내부 테스트 계정은 차감을 면제한다(profiles.internal).
//   전역 스위치(위)는 **모두**를 끄므로, 와디즈 손님에게도 크레딧이 안 걷힌다. 스위치는
//   끄고 계정마다 가른다. 스위치는 그대로 최상위다(켜면 누구도 안 걷는다).
// ★ 면제는 **걷지 않는 것**이지 원가를 안 적는 것이 아니다 — cost_records 는 그대로 쌓인다.
// ★ 모르는 계정은 **걷는 쪽**이다(프로필이 없음 · uuid 가 아닌 actor "cron" 등). 오타나 빠진
//   행이 무료로 새면 안 된다 — creditsEnabled 의 `=== "1"` 과 같은 방향이다.
//   uuid 가 아니면 조회하지 않는다: profiles.id 가 uuid 컬럼이라 그대로 넘기면 DB 가 던진다.
//
// ★★ 2026-09-16 — 둘째 인자 `knownProfile` 은 **선택**이다. GET /api/me 처럼 호출부가 이미
//   `store.findProfiles`로 같은 프로필을 읽어 둔 자리는 그 값을 넘긴다 — 여기서 또 읽으면
//   같은 프로필을 요청 하나에서 두 번 조회하게 된다(실측 23~68ms 짜리 왕복이 그대로 중복).
//   안 넘기면(undefined) 예전 그대로 스토어에서 읽는다 — 다른 호출자(청구·재생성 등)는
//   프로필을 들고 있지 않으므로 시그니처를 그대로 둔다.
export async function creditsEnabledFor(userId, knownProfile) {
  if (!creditsEnabled()) return false;
  if (typeof userId !== "string" || !UUID.test(userId)) return true;
  const profile = knownProfile !== undefined
    ? knownProfile
    : (await getStore().findProfiles([userId])).get(userId);
  return !isInternalAccount(profile);
}

export async function assertCanAfford(userId, price) {
  if (!(await creditsEnabledFor(userId))) return;
...
//
// ★ resolution 도 같은 규율이다 — 호출처가 resolutionForProject(project) 로 뽑아 넘긴다.
//   안 넘기면 videoPrice 가 720p 열로 떨어져, 1080p 를 고른 사장님에게 720p 값만 걷힌다
//   (원가는 2.25배다 — 그 차액이 그대로 우리 손해다). 해상도를 안 받는 모델(Kling·LTX)은
//   resolutionForProject 가 "" 를 주고, 그 값도 720p 열로 떨어진다 — 그 표에 열이 하나뿐이라
//   맞는 값이다.
export async function chargeVideo({ userId, projectId, seconds, model, resolution }) {
  if (!(await creditsEnabledFor(userId))) return 0;   // 내부 계정 — 장부에 줄을 안 쌓는다
  const { attempts, active } = await readVideoLedger(projectId);
  if (active) return 0;                      // 이미 산 회차가 살아 있으면 또 받지 않는다
  const credits = videoPrice(seconds, model, resolution);
...
export async function requireVideoCharge({ userId, projectId, seconds, model, resolution }) {
  // 크레딧을 끈 동안은 걷지 않는다 — 장부에 줄이 안 쌓인다.
  // (원가 장부 cost_records 는 그대로 쌓인다 — 우리가 실제로 낸 돈은 계속 봐야 한다.)
  // ★ 2026-09-14 — 계정마다 가른다(내부 계정 면제). creditsEnabledFor 가 전역 스위치도 본다.
  if (!(await creditsEnabledFor(userId))) return 0;
  if (await alreadyChargedVideo(projectId)) return 0;
  // 잔액 검사와 실제 청구가 **같은 값**을 봐야 한다 — 한쪽만 해상도를 넘기면 402 판정과
  // 걷는 값이 갈린다.
  await assertCanAfford(userId, videoPrice(seconds, model, resolution));
  return chargeVideo({ userId, projectId, seconds, model, resolution });
}
...
//
// ★ 클립 재생성만 모델·해상도를 탄다(Seedance 720p 25 · 1080p 57 · Kling 8).
//   안 넘기면 조용히 싼 쪽이 청구된다. 이미지·목소리는 값이 하나라 두 인자를 안 본다.
export async function chargeRegen({ userId, projectId, kind, idx, priorCount, model, resolution }) {
  if (!Number.isInteger(priorCount) || priorCount < 0) {
    throw new Error(`재생성 회차(priorCount)가 0 이상의 정수여야 해요: ${priorCount}`);
  }
  const credits = regenPrice(kind, priorCount, model, resolution);
...
// ★★ 돌려주는 것이 `{ credits, attempt }` 다(2026-08-19). 예전에는 credits 숫자만 줬다.
//   왜 넓혔나: **자기가 쓴 회차 번호를 아는 함수가 여기뿐이다.** 쓰고 나서 장부에 다시
//   물어보는 방식("살아 있는 마지막 회차가 몇 번이냐")은 원리적으로 경합에 진다 —
//   A 가 ad:1 을 쓰고 B 가 ad:2 를 쓴 **뒤에** A 가 물어보면 2 가 나온다. film 경로는
//   한 프로젝트에서 두 편을 나란히 굽는 것이 정상 흐름이라([둘 다 굽기]) 그 순서가 예외가
//   아니라 기본이고, 그러면 order 수거 실패가 refs 의 값을 환불한다. 쓴 자리에서 그 번호를
//   함께 돌려주면 물어볼 일이 아예 없다.
// ★ 안 걷혔으면 `{ credits: 0, attempt: null }` 이다 — attempt 는 "**내가 방금 쓴** 행"만
//   가리킨다. 살아 있는 남의 회차 번호를 여기서 주면 그것을 자기 것으로 착각해 환불한다.
// ★ 광고 호출부(lib/ad/pipeline.js)는 반환값을 안 쓴다 — 동작이 그대로다.
export async function chargeAd({ userId, projectId, seconds, model, resolution, openNewAttempt = false }) {
  // ★ 2026-09-14 — 이 입구만 스위치를 안 보고 있었다(끈 동안에도 광고 청구 행이 쌓였다).
  //   내부 계정 면제를 달면서 다른 청구와 같은 판정으로 맞췄다.
  if (!(await creditsEnabledFor(userId))) return { credits: 0, attempt: null };
  const { attempts, active } = await readAdLedger(projectId);
  if (active && !openNewAttempt) return { credits: 0, attempt: null };
  const credits = adVideoPrice(seconds, model, resolution);
  const attempt = attempts + 1;                          // 회차가 는다 — 새 청구 행이다
  // insertCharge 의 unique(idem_key) 가 마지막 방어선이다: 동시 클릭 둘이 같은 회차를
  // 계산해도 한쪽만 쓰이고 나머지는 false → 0 을 받는다.
  const wrote = await getStore().insertCharge({
    user_id: userId, project_id: projectId, kind: "ad_video",
    credits, idem_key: adKey(projectId, attempt),
  });
  return wrote ? { credits, attempt } : { credits: 0, attempt: null };
}

// 살아 있는 회차만 되돌린다. 환불 행의 주인은 **원 청구 행의 주인이다** —
// 소유가 어긋난 환불을 원천에서 막는다(refundVideo 와 같은 규칙).
// ★ attempt 를 주면 **그 회차**를 되돌린다. 안 주면 예전 그대로 살아 있는 마지막 회차다
//   (광고 경로는 안 준다 — 동작이 글자 그대로 유지된다).
//   지정한 회차가 없거나 이미 되돌아갔으면 아무 일도 안 한다 — 두 번 불러도 안전하다.
export async function refundAd({ projectId, attempt = null }) {
  const store = getStore();
  let target = null;
  if (attempt) {
    const charge = await store.findCharge(adKey(projectId, attempt));
    // 이미 음수 행이 있으면 건너뛴다 — idem_key 가 막아 주지만, 여기서 먼저 보는 편이
    // 무슨 일이 일어나는지 읽기 쉽다.
    const refunded = charge ? await store.findCharge(adRefundKey(projectId, attempt)) : null;
    if (charge && !refunded) target = { charge, attempt };
  } else {
    target = (await readAdLedger(projectId)).active;
  }
  if (!target) return;
  await store.insertCharge({
    user_id: target.charge.user_id, project_id: projectId, kind: "refund_ad",
    credits: -Number(target.charge.credits), idem_key: adRefundKey(projectId, target.attempt),
  });
}
