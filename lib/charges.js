...
// ★ Task 21 — model 을 받는다. adVideoPrice(seconds, model) 로 그대로 넘긴다: model 이
//   생략되면(옛 문서) pricing.js 가 2.0 값으로, 값이 있는데 모르는 모델이면 던진다 —
//   그 판정을 여기서 다시 하지 않는다(한 곳에 두는 lib/pricing.js 규율).
//
...
export async function chargeAd({ userId, projectId, seconds, model, openNewAttempt = false }) {
  const { attempts, active } = await readAdLedger(projectId);
  if (active && !openNewAttempt) return 0;
  const credits = adVideoPrice(seconds, model);
  const wrote = await getStore().insertCharge({
