...
// 지금 살아 있는 회차 번호. 없으면 null.
//
// ★★ 왜 필요한가(2026-08-19): refundAd 는 "살아 있는 **마지막** 회차"를 되돌린다. 광고는 한
//   프로젝트에 한 편이라 그것이 늘 맞지만, film 경로는 **한 프로젝트에서 두 편**(order·refs)을
//   굽는다. 그러면 두 회차가 동시에 살아 있을 수 있고, order 를 수거하다 실패했는데 마지막
//   회차인 refs 가 되돌아간다 — 사장님은 멀쩡한 영상값을 돌려받고 실패한 영상값은 그대로 낸다.
//   그래서 **접수할 때 회차 번호를 문서에 적어 두고**(films[mode].job.attempt) 수거 실패 때
//   그 번호로 되돌린다. 그 번호를 얻는 자리가 여기다 — 청구 직후에 부르면 방금 연 회차다.
export async function activeAdAttempt(projectId) {
  const { active } = await readAdLedger(projectId);
  return active ? active.attempt : null;
}
