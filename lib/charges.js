...
//   lib/pricing.js 규율).
//
// ★ Task 24 — resolution 을 받는다(선택 인자). 호출부가 안 넘기면(지금 lib/ad/pipeline.js
//   가 그렇다 — Task 23 이 쓰는 중이라 이번 태스크에서 못 고쳤다) adVideoPrice 가 720p 로
//   본다. lib/ad/generate.js 도 여전히 fal 에 "720p" 를 고정으로 보내므로(같은 이유로 못
//   고침) 실제로 만들어지는 영상은 지금 항상 720p 다 — 그 720p 기본값과 우연이 아니라
//   같은 값으로 맞춘 것이다. 사장님이 480p·1080p 를 골라도 **지금은 실제 청구·생성에
//   반영되지 않는다** — 두 파일이 잠겨서(다른 태스크 병행) 못 고쳤다는 뜻이지, 이 함수의
//   결함이 아니다(태스크 24 보고서의 "후속으로 남긴 것" 참고).
//
