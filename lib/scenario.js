...
칸마다 쓰는 말이 다르다:
- **subject·look·narrator_voice 는 영어로 적는다.** 이 세 값은 그림·영상 모델에 그대로
  실린다 — 번역 단계가 없다. 예시도 영어다: e.g. "a freshly baked loaf of sourdough",
  "golden-brown crust, open crumb", "calm man in his 30s, low and steady tone".
- **topic·angle·beat·speaker 는 한국어로 적는다.** 사장님이 읽고 고치는 값이고, 다음 단계도
  한국어로 읽는다. 화면 밖 목소리는 "내레이션"이라는 그 말이 그대로 판정에 쓰인다.
- **대사(line)는 한국어로 적는다.** 이 글자가 **그대로 자막이 된다** — 우리가 원고를 화면에
  태운다. 영어로 쓰면 사장님 영상에 영어 자막이 박힌다.
