...
export function fakeScenario(project) {
  ...
  const shots = Array.from({ length: n }, (_, i) => ({
    ...
    line: `가짜 대사 ${i + 1}.`,
    // ★ "내레이션"으로 두지 않는다. 화면 밖 목소리는 이제 컷의 narration 표시가 되고 그
    //   프로젝트는 통째로 TTS 경로로 간다(lib/clip-limits.js projectSpeaks) — 가짜 모드로는
    //   말하는 클립의 배선을 한 번도 못 밟게 된다. 가짜 모드는 값이 아니라 배선을 보는 자리다.
    speaker: "40대 남성 사장님",
