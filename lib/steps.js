...
export function clipKey(cut, project) {
  ...
  // ★ 컷의 `narration`(화면 밖 목소리, 2026-08-16)은 여기 **따로 안 붙인다.** 그 값이 클립
  //   프롬프트에 닿는 길은 speechFor 하나뿐이고(narration 이 있으면 projectSpeaks 가 false 라
  //   그 프로젝트의 모든 컷이 안 말하는 가지로 간다), 그 결과는 바로 이 spoken 절의 유무로
  //   이미 각인된다. 프롬프트에 안 실리는 값을 각인에 담지 않는다는 이 함수의 규칙 그대로다 —
  //   담으면 언젠가 프롬프트가 같은데 낡았다고 하는 자리가 생기고, 그 버튼은 유료다.
