...
// promptNoteOf 도 같은 파일에서 가져온다 — 프로젝트 공통 지시를 프롬프트(lib/cuts.js)와
// 각인이 **같은 함수로** 읽어야 판정이 갈리지 않는다.
import { styleKey, promptNoteOf } from "./styles.js";
...
import { spokenOf, usableTone, usableTransition, stageOf, castLooksOf, subjectOf, promptOverride } from "./cuts.js";
...
export function clipKey(cut, project) {
  ...
  // ★ 판독은 프롬프트와 **같은 함수**(promptNoteOf)다. `.trim()` 만 하고 더 다듬지 않는다 —
  //   한쪽에만 정규화가 들어가면 프롬프트가 같은데 각인만 갈려 거짓 낡음이 유료 버튼을 연다
  //   (바로 앞 태스크가 마침표 하나로 겪었다).
...
export function imageContextKey(cut, project) {
  ...
  return [
    ...
    // ★ 판독은 프롬프트와 **같은 함수**(promptNoteOf)다 — `.trim()` 만 한다. 한쪽에만
    //   정규화를 넣으면 프롬프트가 같은데 각인만 갈려 거짓 낡음이 유료 버튼을 연다.
