...
export function currentStepKey(project) {
  ...
  // 컷이 하나라도 있으면 상태 문자열이 뭐든 ⑤이미지다 — 구성·대본을 다시 만들면
  // status가 "synopsis"/"script"로 내려가는데, 그때 돈 주고 만든 컷이 멀쩡한데도 ⑤가 닫혔다.
  if (project.status === "cuts" || (project.cuts || []).length > 0) return "images";
  return "script";
}
