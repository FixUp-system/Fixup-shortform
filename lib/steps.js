...
// 프롬프트 덮어쓰기 판정. spokenOf 와 **같은 이유**로 그 파일에서 가져온다 — 프롬프트가
// 싣는 값과 각인이 재는 값이 같은 함수에서 나와야 한다(두 벌이면 마침표 하나로 갈린다).
// promptNoteOf(프로젝트 공통 지시)도 같은 이유로 같은 파일이다 — 판독이 한 자리여야 한다.
import { spokenOf, usableTone, usableTransition, stageOf, castLooksOf, subjectOf, promptOverride, promptNoteOf, musicOf } from "./cuts.js";
...
export const STEPS = [
  // ★ 사장님에게는 **입력**이라고 부른다(2026-08-18 사장님 지시). 광고 흐름이 이미 그 말을
  //   쓰고 있었고, 같은 제품의 두 흐름이 첫 단계를 다른 이름으로 부르면 배운 것이 안 통한다.
  //   ⚠️ `key`·`seg` 는 그대로다 — 라우팅 가드·각인·저장 문서가 함께 쓰는 값이고, 주소를
  //      바꾸면 사장님이 받아 간 링크가 죽는다. 바꾸는 것은 **읽는 말**뿐이다.
  { key: "material", no: "1", label: "입력", seg: "briefing" },
...
// ①자료·②시나리오를 지나온 프로젝트인가 — currentStepKey 와 isReachable 의 탈출구가
// **같은 판정**을 본다(한쪽만 바뀌면 화면이 여는 문과 가드가 닫는 문이 갈린다).
//
// ★★ 시나리오 확정이 유일한 기준이면 **이 브랜치 앞의 프로젝트가 갇힌다**(2026-08-16 최종
//    리뷰 Important 2). 컷·그림·클립까지 값을 치른 프로젝트에 scenario 가 아예 없어서,
//    가드가 ②시나리오로 되돌려 보내고 그 화면은 들어서자마자 유료 LLM 을 부른다. 게다가
//    거기서 확정하면 컷이 `cuts: []` 로 지워진다 — 유일한 출구가 산 것을 부순다.
//
// 그래서 **시나리오가 아예 없고 컷이 있는** 프로젝트는 지나온 것으로 본다. 시나리오가
// 있는데 확정만 안 된 프로젝트(= 이 브랜치에서 만든 것, 지금 고치는 중)는 그대로 막힌다 —
// 그쪽은 확정이 마지막 무료 관문이라 넘기면 안 된다.
// ★ 컷의 각인(cuts_scenario_of)도 지나온 증거다 — 그것이 붙는 자리는 컷 분할 하나뿐이라
//   "이 프로젝트는 ②를 이미 한 번 통과했다"와 같은 말이다. 이것이 없으면 ②로 돌아와
//   **임시저장**(확정 없이 저장)을 누른 순간 확정이 풀려, 이미 그림·클립을 산 사장님이
//   ④·⑤에서 쫓겨난다. 나가는 문이 다시 닫히면 임시저장은 함정이 된다.
function scenarioSettled(project) {
  if (project?.scenario?.confirmed) return true;
  if (!(project?.cuts || []).length) return false;
  return !project.scenario || Boolean(project.cuts_scenario_of);
}

export function currentStepKey(project) {
  if (!project) return "material";
  // ★ **그 단계에서만 나오는 것으로 그 단계를 닫지 않는다.** 시나리오 확정으로 ①자료를
  //   닫았더니 잠금 고리가 됐다 — 확정은 ②시나리오 화면에서만 하는데, 확정이 없으면
  //   현재 단계가 ①자료라 가드가 ②를 못 열게 막았다(아래 isReachable 이 ⑥완성에 대해
  //   설명하는 것과 **같은 모양의 고리**다).
  //   그래서 ①자료를 닫는 것은 사장님이 실제로 ①에서 하는 일, 즉 **설명을 적는 것**이다.
  //   ⚠️ 이미 확정한 프로젝트는 자료를 다시 보지 않는다 — 나중에 material.text 가 비게
  //      되더라도 앞서간 프로젝트를 ①로 끌어내리면 안 된다(그 판정이 이 순서다).
  if (!scenarioSettled(project)) {
    return project.material?.text?.trim() ? "scenario" : "material";
  }
...
// 컷이 어느 시나리오에서 나왔는가 — 각인이다(버전 번호가 아니다. 아래 "낡음 판정" 절 참고).
//
// 각인에 넣는 것은 **컷과 그 뒤 두 패스가 실제로 읽는 값**이다: 대사·초(컷 자체) 와
// beat·speaker(화면 설계·캐스팅의 입력). 넷 중 하나라도 바뀌면 컷을 다시 나눠야 한다.
// topic·focus·angle 은 넣지 않는다 — 그것들은 컷의 모양을 안 바꾼다.
//
// ⚠️ **JSON 으로 굳힌다.** 넷 중 셋이 사장님이 직접 쓰는 한국어 자유 서술이라 `|` 나 `//` 가
//    그대로 들어온다. 이어 붙이면 `line: "가|나"` 인 장면과 `line:"가", seconds:나` 인 장면의
//    각인이 **똑같아진다** — 고쳤는데 안 낡는다. 그러면 화면이 확인을 안 묻고 POST /cuts 가
//    409 로 조용히 거절되며, 그 409 를 화면은 정상으로 읽는다(옛 시나리오에서 나온 컷 그대로
//    ④·⑤ 로 넘어가 그림·클립을 산다). 같은 부류를 clipKey 의 움직임 축에서 이미 겪었고
//    같은 처방을 썼다(아래 clipKey 의 motionClause 주석).
// ★ 형식이 바뀌므로 옛 각인은 전부 불일치가 된다. 이 브랜치는 아직 배포된 적이 없어
//   `cuts_scenario_of` 를 들고 있는 프로젝트가 세상에 없다 — 지금이 가장 싼 시점이다.
...
//
// ★ 시나리오 프로젝트에는 script.version 이 아예 없다(2026-08-16). 그것을 안 가르면
//   판정이 **언제나 false** 라, 사장님이 시나리오를 고쳐 다시 확정해도 POST /cuts 가
//   409 로 영구 차단하고 컷은 옛 시나리오 그대로 남는다 — 고칠 수 있게 하는 것이 이
//   기능의 전부인데 고쳐도 반영이 안 된다.
//
// ⚠️ 다시 나누면 그 컷에 딸린 그림·클립이 사라진다. 그것이 맞다 — 사장님이 계획을 고쳤으면
//    옛 계획에서 나온 산출물은 무효다(원고를 다시 쓰던 시절과 같은 규칙). 실수로 날리는 것을
//    막는 것은 409 가 아니라 화면의 확인이다.
export function areCutsStale(project) {
  ...
  // 원고 도입 전에 만들어진 컷은 script.version 자체가 없다 — 그때는 판정하지 않는다.
...
export function isReachable(stepKey, project) {
  ...
  //   탈출구가 그것을 넘어서면 안 된다. 그 불변의 기준이 시나리오 확정으로 옮겨졌으므로
  //   여기도 **같은 값**을 봐야 한다 — 한쪽만 옮기면 산출물만 있고 확정이 없는 프로젝트가
  //   뒷 단계를 연다(게이트 둘이 갈리는 자리).
  const phase = PRODUCED_FOR[stepKey];
  if (phase && scenarioSettled(project) && produced(project, phase)) return true;
...
export function clipKey(cut, project) {
  ...
  // ★ 컷의 `narration`(화면 밖 목소리)은 여기 **따로 안 붙인다** — 붙일 필요가 없다.
  //   2026-08-17 부터 그 컷도 말하고(내레이터 목소리), 프롬프트가 갈리는 만큼 spokenOf 가
  //   이미 갈린다(`…|narration` 표시와 narrator_voice 가 그 안에 있다). 규칙은 그대로다:
  //   **프롬프트에 실리는 것만 각인에 담는다.** 두 벌로 재면 프롬프트가 같은데 낡았다고
  //   하는 자리가 생기고, 그 버튼은 유료다.
  const spoken = spokenOf(cut, project);
  const withSpoken = spoken ? `${withSpeed}|${spoken}` : withSpeed;
  // 음악도 클립 프롬프트에 실리므로(clipContextClause) 바꾸면 클립이 낡아야 한다.
  // ★ **있을 때만** 붙인다 — 음악 칸이 없는 옛 프로젝트의 각인을 건드리지 않는다(위 규칙과
  //   같다. 무조건 붙이면 이미 값을 치른 클립이 통째로 낡는다).
  // ★ 판독은 프롬프트와 **같은 함수**다(lib/cuts.js musicOf) — 두 벌로 재면 갈린다.
  const music = musicOf(project);
  const withMusic = music ? `${withSpoken}|${music}` : withSpoken;
  ...
  const withRes = res && res !== fallback ? `${withMusic}|${res}` : withMusic;
  // 무대·인물·톤 — buildClipPrompt 가 clipContextClause 로 싣는 것과 같은 셋이다
  // (lib/cuts.js stageOf·castLooksOf, 그리고 여기서도 usableTone). 넷째였던 **제품은
  // 2026-08-18 에 프롬프트·각인 양쪽에서 함께 빠졌다** — 아래 subjectClause 주석 참고.
  //
  ...
  // ★ 제품(앵커+외형)은 **각인에도 없다**(2026-08-18). 클립 프롬프트에서 그 절을 걷어냈기
  //   때문이다(lib/cuts.js clipContextClause 의 주석에 근거가 있다 — i2v 는 사진을 못 받아
  //   글로 쓴 묘사가 첫 프레임을 이긴다). 이 저장소의 불변은 **프롬프트에 실리는 것만 각인에
  //   담는다** 이므로, 프롬프트에서 뺀 값을 각인에 남기면 "프롬프트는 같은데 낡았다"는
  //   거짓 낡음이 생기고 그 자리에 컷당 8크레딧짜리 [다시 만들기]가 열린다.
  //   반대로 프롬프트만 고치고 각인을 남겨도 같은 자리에서 갈린다 — 양쪽을 함께 걷었다.
  //   (빈 문자열을 이어 붙이지 않고 **절 자체를 없앤다** — 안 실리는 값의 자리를 남겨 두면
  //    다음 사람이 그 자리에 값을 되돌려 놓기 쉽다.)
  // ⚠️ 이 변경으로 저장된 클립이 전부 낡는다(30초 한 편 ~$9). **그대로 둔다** — 지금 저장된
  //    클립에는 틀린 제품이 들어 있어 낡음이 정확한 신호다. 옛 각인과 호환시키려 애쓰면
  //    "고쳤는데 안 낡는" 반대 사고가 된다(설계 문서 "★ 각인 영향" 절, 사용자 승인됨).
  // ★ imageContextKey 는 그대로 제품을 담는다 — 이미지 프롬프트는 제품 묘사를 계속 싣는다.
  ...
  // 사장님이 갈아 끼운 본문(lib/cuts.js buildClipPrompt) — 프롬프트에 실리므로 각인에 담는다.
  // 안 담으면 프롬프트를 고쳐도 클립이 안 낡아, 화면에 적힌 지시와 실제 영상이 조용히 갈린다.
  //
  // ★ 있을 때만. 덮어쓰기를 안 쓴 컷의 각인이 글자 그대로 그대로여야 이미 값을 치른 클립이
  //   통째로 낡지 않는다(Seedance 30초 한 편 ~$9) — style_of·해상도·tone_of·자막 위치에서
  //   네 번 겪은 함정이고, 위 speed·해상도·축이 쓰는 것과 같은 모양이다.
  // ★ **프롬프트에 실제로 실리는 값을 잰다** — buildClipPrompt 와 **같은 판정 함수**
  //   (promptOverride)를 부른다. 공백뿐이거나 문자열이 아닌 값은 덮어쓰기가 아니고,
  //   끝 문장부호도 거기서 정규화된다. 두 벌로 재던 동안 `"it explodes"` 와
  //   `"it explodes."` 가 같은 프롬프트를 내는데 각인만 갈렸다 — 마침표 하나가 거짓
  //   낡음을 만들어 살아 있는 클립(~$9/편)의 재구매를 열었다.
  // ★ **맨 뒤에 붙인다.** 프로젝트 공통 지시(clipnote)가 이 뒤에 이어 붙을 자리다 —
  //   차례가 갈리면 같은 값인데 각인 문자열이 달라져 살아 있는 클립이 낡는다.
  const clipOverride = promptOverride(cut?.clip_prompt);
  const promptClause = clipOverride ? `|prompt:${clipOverride}` : "";
  // 프로젝트 공통 지시(settings.clip_note) — 전 컷의 클립 프롬프트에 실리므로 각인에 담는다.
  // 안 담으면 사장님이 공통 지시를 고쳐도 클립이 안 낡아, 화면에 적힌 지시와 실제 영상이
  // 조용히 갈린다.
  //
  // ★ **prompt 뒤다.** 앞에 끼우면 위 promptClause 의 차례가 갈려 같은 값인데 각인 문자열이
  //   달라진다 — 살아 있는 클립이 통째로 낡는다(Seedance 30초 한 편 ~$9).
  // ★ **있을 때만.** 공통 지시를 안 쓴 프로젝트의 각인이 글자 그대로 그대로여야 한다 —
  //   style_of·해상도·tone_of·자막 위치에서 네 번 겪은 함정이다.
  // ★ 판독은 프롬프트와 **같은 함수**(lib/cuts.js promptNoteOf)다. 그 함수는 `.trim()` 에
  //   더해 끝 부호까지 닫으므로(closeSentence) **각인도 정규화된 값을 본다** — 그것이 맞다:
  //   각인이 재야 하는 것은 사장님이 친 글자가 아니라 **프롬프트에 실제로 실린 값**이다.
  //   여기서 날값을 보게 되돌리면 "shot on 35mm film" 과 "shot on 35mm film." 이 같은
  //   프롬프트를 내는데 각인만 갈려 거짓 낡음이 유료 버튼을 연다(앞 태스크가 덮어쓰기에서
  //   마침표 하나로 겪었다). 정규화는 판독 한 자리에만 있다.
  const clipNote = promptNoteOf(proj, "clip_note");
  const noteClause = clipNote ? `|clipnote:${clipNote}` : "";
  return `${withRes}${stageClause}${castClause}${toneClause}${motionClause}${promptClause}${noteClause}`;
}
...
// ★ 다만 **제품은 이제 여기에만 있다**(2026-08-18) — 클립은 프롬프트에서 제품 절을 걷어냈고
//   각인(clipKey)도 함께 걷었다. 두 각인이 갈린 것이 아니라, 두 프롬프트가 갈린 것을 각인이
//   그대로 따라간 것이다. 기준은 언제나 "그 프롬프트에 실렸는가" 하나다.
...
export function imageContextKey(cut, project) {
  ...
  return [
    ...
    // 사장님이 갈아 끼운 본문(lib/cuts.js buildImagePrompt) — 프롬프트에 실리므로 각인에
    // 담는다. 안 담으면 프롬프트를 고쳐도 그림이 안 낡아 화면과 실물이 갈린다.
    //
    // ★ 있을 때만. 덮어쓰기를 안 쓴 컷의 각인이 글자 그대로 그대로여야 지금 저장된 그림이
    //   통째로 낡지 않는다(컷당 $0.08) — style_of·해상도·tone_of·자막 위치에서 네 번 겪은
    //   함정이다. 공백뿐이거나 문자열이 아닌 값도 덮어쓰기가 아니다 — 프롬프트를 안 가는
    //   값이 각인을 갈면 거짓 낡음이 유료 버튼을 연다.
    // ★ **프롬프트에 실제로 실리는 값**(promptOverride 를 통과한 것)을 담는다. `trim()` 만
    //   하면 끝 문장부호 정규화가 빠져 `"a red shoe"` 와 `"a red shoe."` 가 같은 프롬프트를
    //   내는데 각인만 갈린다 — 마침표 하나로 유료 버튼이 열렸다.
    // ★ **맨 뒤에 둔다.** 프로젝트 공통 지시(imgnote)가 이 뒤에 이어 붙을 자리다 —
    //   차례가 갈리면 같은 값인데 각인 문자열이 달라져 살아 있는 그림이 낡는다.
    promptOverride(cut?.image_prompt) ? `prompt:${promptOverride(cut?.image_prompt)}` : "",
    // 프로젝트 공통 지시(settings.image_note) — 전 컷의 이미지 프롬프트에 실리므로 각인에
    // 담는다. 안 담으면 공통 지시를 고쳐도 그림이 안 낡아 화면과 실물이 갈린다.
    //
    // ★ **prompt 뒤다**(위 항목의 주석이 이 자리를 예고해 두었다) — 차례가 갈리면 같은
    //   값인데 각인 문자열이 달라져 살아 있는 그림이 통째로 낡는다(컷당 $0.08).
    // ★ **있을 때만.** 공통 지시를 안 쓴 프로젝트의 각인은 글자 그대로 그대로다.
    // ★ 판독은 프롬프트와 **같은 함수**(lib/cuts.js promptNoteOf)다 — `.trim()` 에 더해 끝
    //   부호까지 닫힌(closeSentence) 값이 여기 담긴다. 각인이 재는 것은 사장님이 친 글자가
    //   아니라 **프롬프트에 실제로 실린 값**이라 그것이 맞다. 날값으로 되돌리면 프롬프트가
    //   같은데 각인만 갈려 거짓 낡음이 유료 버튼을 연다(위 prompt 항목과 같은 함정이다).
    promptNoteOf(proj, "image_note") ? `imgnote:${promptNoteOf(proj, "image_note")}` : "",
  ]
    .filter(Boolean)
    .join("|");
}
