...
// 목소리가 이미지 뒤에 있는 이유: 낭독은 컷마다 따로 만드는데, 컷 분할은 이미지 단계
// (POST /cuts)에서 일어난다. 목소리가 앞에 오면 읽을 컷이 없다.
// 낭독 길이가 클립 길이와 자막 타이밍을 정하므로, 영상보다 앞이기만 하면 된다.
export const STEPS = [
...
export function currentStepKey(project) {
  ...
  // 뒤 단계부터 확인한다 — 앞선 조건(cuts)에 먼저 걸리면 완성본을 두고 이미지로 돌아간다
