...
export function clipKey(cut, project) {
  ...
  // ★ 공백뿐이거나 문자열이 아닌 값은 덮어쓰기가 **아니다**(promptOverride 와 같은 판정) —
  //   그런 값은 프롬프트를 안 갈므로 각인도 갈리면 안 된다. 갈리면 프롬프트가 똑같은데
  //   낡았다고 말하게 되고, 그 버튼은 유료다.
...
export function imageContextKey(cut, project) {
  ...
  return [
    ...
    //   함정이다. 공백뿐이거나 문자열이 아닌 값도 덮어쓰기가 아니다(promptOverride 와 같은
    //   판정) — 프롬프트를 안 가는 값이 각인을 갈면 거짓 낡음이 유료 버튼을 연다.
