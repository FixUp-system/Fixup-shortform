...
//
// ★ Task 21 — 길이가 모델에 딸린다(2.0 fast 는 15초 하나, 2.5 는 15·30초 둘).
//   그래서 표가 모델 id → 길이 → 크레딧의 2단이다. 키는 lib/ad/models.js 의
//   AD_MODELS[].id·seconds 와 **같아야 한다** — 이 파일은 화면도 import 하므로
//   models.js 를 import 해 대조할 수 없다(아래 규율 참고). tests/pricing.test.js 가
//   AD_MODELS 를 훑어 이 표와 어긋나면 실패하게 대신 대조한다.
//
// 원가(2026-08-13, 계산값 — 2.5 는 실측 아니다. lib/ad/models.js 의 perSecUsd 주석 참고):
//   2.0 fast 720p $0.2419/s 고정 × 15초 = $3.63
//   2.5        9:16 720p 토큰식 $0.4622/s × 15초 = $6.93 · × 30초 = $13.87
// 1크레딧 ≈ 원가 $0.06 로 올림: 2.0/15초=65 · 2.5/15초=120 · 2.5/30초=240.
// 올려 잡는 방향이 안전하다 — 내려 잡으면 팔수록 손해다.
export const AD_VIDEO_PRICE = {
  "seedance-2.0-fast": { 15: 65 },
  "seedance-2.5": { 15: 120, 30: 240 },
};
...
const DEFAULT_AD_MODEL_ID = "seedance-2.0-fast";

// 시나리오 다시 쓰기는 무료지만 무제한은 아니다. LLM 원가가 조금씩 샌다.
// MAX_REGEN_PER_CUT 과 같은 성격의 안전핀이다.
export const MAX_SCENARIO_TRIES = 20;

// modelId 를 **생략하면**(undefined/null — 옛 호출부·옛 문서) 기본 모델(2.0) 값으로
// 본다. 지금까지 만들어진 광고가 전부 2.0 이고(모델 선택 화면이 아직 없다), 화면
// (app/ads/[id]/page.js)도 이 1-인자 호출로 가격표를 읽는다 — 여기서 던지면 그 화면이
// 깨진다. 그래서 "생략"은 안전하게 받는다.
//
// ★★ 그러나 **뭔가를 줬는데 표에 없으면 던진다.** regenPrice 가 모르는 종류에 던지는
// 것과 같은 원칙이다 — 모르는 모델이 조용히 싼 값(2.0)으로 떨어지면 그 차액이 그대로
// 우리 돈이다. "생략"과 "오타"를 가르는 이유: 생략은 옛 문서의 정상 상태이지만, 값이
// 있는데 표에 없다는 것은 라우트의 닫힌 목록 검사(app/api/ads/route.js 의 isAdModel)를
// 어떻게든 피했거나 호출부가 버그로 잘못된 문자열을 넘겼다는 뜻이라 조용히 넘기면 안 된다.
export function adVideoPrice(seconds, modelId) {
  ...
  // 길이가 그 모델의 목록 밖이면 **가장 비싼 값**으로 본다 — 싼 쪽으로 떨어지면 원가보다
  // 적게 청구할 위험이 있다(videoPrice 는 "중간값"으로 떨어뜨리지만, 여기는 모델마다
  // 길이가 최대 둘뿐이라 "비싼 쪽"이 더 안전하다).
  const known = Object.keys(table).map(Number);
  return table[Math.max(...known)];
}
