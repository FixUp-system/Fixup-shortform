...
export const FAILURE_CODES = [
  "no_credits", "budget", "rejected", "rejected_likeness", "rejected_aspect", "busy", "timeout", "network",
  "provider", "empty", "unknown",
];

// ★★★ 2026-09-02 — **fetch 응답 하나를 사장님 말로 옮긴다.**
//
// 왜 필요했나: 화면들이 전부 `data.error || "고정 문구"` 를 손으로 적고 있었다. 그러면
// **본문이 JSON 이 아닐 때 아무 정보도 안 남는다** — Vercel 이 함수를 시간으로 죽이면
// 504 에 HTML 이 오므로 `res.json()` 이 실패하고, 화면은 "시나리오를 만들지 못했어요"
// 라는 **아무것도 안 알려 주는 문구**만 띄웠다. 정작 아래 classifyFailure 에는
// `504 → "만드는 데 너무 오래 걸렸어요"` 갈래가 **이미 있었는데** 그 자리까지 못 갔다.
//
// ★ 서버가 준 말이 있으면 **그 말이 이긴다** — 라우트가 이유를 아는 쪽이라 늘 더 구체적이다
//   (그 문구도 classifyFailure 를 통과시킨다: 안에 `(429)` 같은 상태가 실려 오면 그것도 옮겨진다).
// ★ 본문이 없을 때만 상태 코드로 짓는다. `(NNN)` 꼴로 적는 이유는 아래 판정기가 괄호 안
//   세 자리를 읽기 때문이다 — 같은 규약을 두 벌로 만들지 않는다.
// ★ 순수 함수다(이 파일의 규칙) — Response 객체가 아니라 **status 와 이미 읽은 본문**을
//   받는다. 화면이 `res.json()` 을 어떻게 실패 처리하든 여기는 안 흔들린다.
export function failureFromResponse(status, body) {
  const given = typeof body?.error === "string" ? body.error.trim() : "";
  return classifyFailure(given || `요청 실패 (${status})`);
}
...
export function classifyFailure(raw) {
  ...
  // ★★ 참조 그림의 **가로세로비** 거절 — 상태 코드보다 먼저 본다(2026-08-31 프로덕션 실측).
  //   fal 원문: `loc:["body","reference_image_urls",0]` ·
  //   *"The aspect ratio of the image should be between 0.4 and 2.5."*
  //
  // ★ 아래 4xx 가지가 먼저 물면 *"문장을 조금 바꿔 다시 시도해 주세요"* 가 되는데,
  //   이 경우 **틀린 안내**다 — 문장이 아니라 **그림의 모양**이 걸렸다.
  // ★ **스키마에는 없는 제약**이라 배선할 때 안 보인다(런타임 검사다). 그래서 화면이
  //   정확히 말해 주는 것이 유일한 방어선이다.
  // ★ 단계별 통짜는 이제 이 자리까지 안 온다 — 굽기 전에 컷별로 떨어진다
  //   (lib/reel/oneshot.js 의 planReelBake 넷째 조건). 남는 것은 **사장님이 올린 사진**이다.
  if (/aspect ratio of the image|image_aspect_ratio_error/i.test(text)) {
    return {
      code: "rejected_aspect",
      message: "그림의 가로세로가 너무 길어서 만드는 쪽이 거절했어요 — 너무 길쭉하거나 너무 넓지 않은 사진으로 바꿔 주세요",
      retryable: true,
    };
  }
  ...
  if (/likenesses of real people/i.test(text)
    ...
    return {
      ...
      // ★★ 2026-08-31 — 문구를 **두 경우 다** 가리키게 고쳤다. 그전에는 "그 사진을 빼세요"
      //   뿐이었는데, 단계별 통짜에서 걸리는 것은 **우리가 그린 스토리보드**라 사장님이
      //   할 수 없는 조언이었다(실측: 시나리오가 넣은 인물 장면이 거절을 불렀다).
      message: "사람 얼굴이 또렷하게 담겨 있어서 만드는 쪽이 거절했어요 — 얼굴이 크게 나오는 장면을 빼고 시나리오를 다시 쓰거나, 올리신 사진 중 얼굴이 또렷한 것을 바꿔 주세요",
      retryable: true,
    };
  }
