...
export const STYLE_PRESETS = [
  ...
  },
  {
    ...
    //
    //   낱말을 나눠 쓰면 축이 무너진다. tests/styles.test.js 가 photo·film 의 finish 와
    //   **겹치는 낱말이 하나도 없음**을 코드로 판정한다(눈으로 지킬 수 있는 계약이 아니다).
    //
    ...
    // ⚠️ render3d 의 "shallow depth of field" 를 그대로 쓰지 않고 "shallow-focus" 로 적는다.
    //    같은 뜻이라도 문구가 겹치면 두 칩이 같은 말을 하게 된다.
    finish: "long-lens compression, shallow-focus subject against a melted background, oval anamorphic bokeh with faint horizontal flares, deliberate off-center framing, high-contrast key and deep shadow falloff",
  },
  {
    id: "scifi",
    realistic: false,
...
export function refHintFor(style, kind) {
  ...
  return `${style?.realistic ? set.real : set.drawn} ${REF_FRAMING}`;
}
