...
// 캐스팅 규칙에 반례를 붙여 둔 이유: 이름 없이 "손님"으로만 불리는 사람을 모델이 "스치는
// 사람"으로 읽고 빼 버렸다(손님이 나오는 원고 3편 전부 cast 가 주인 1명뿐). 그 손님이 컷마다
// 다른 얼굴로 그려지면 이 기능이 막으려던 결함이 그대로 남는다.
//
// 그래도 "보이면 전부"로 넓히지는 않는다: 캐스팅에 들어간 인물은 원고당 4명·컷당 레퍼런스
// 2장이라는 유한한 자리를 나눠 쓰므로, 배경 엑스트라가 한 자리를 차지하면 정작 주인·제품
// 레퍼런스가 밀리고 엑스트라에 엉뚱한 얼굴이 고정된다. 게다가 일관성은 두 번 이상 나올 때만
// 값어치가 있다 — 한 컷에만 있는 사람은 달라질 다음 컷이 없다. 그래서 판정선을 "화면에
// 보이는가"가 아니라 "다시 나오거나 이야기의 한 자리를 차지하는가"로 둔다.
...
규칙:
- 여러 컷에 걸쳐 같은 사람으로 다시 나오는 인물, 또는 이야기에서 특정 개인으로 다뤄지는 인물만 넣는다.
  제품을 쓰는 손님·음식을 먹는 손님·사연의 주인공은 이름 없이 "손님"으로만 불려도 한 명의 인물이다.
  반대로 화면을 채우는 배경 손님·지나가는 사람은 누구인지가 중요하지 않으니 넣지 않는다.
  ✗ "카페 안에 손님들이 앉아 있습니다" 의 그 손님들을 cast 에 넣는 것
  ✓ 컷1에서 케이크를 고르고 컷3에서 그것을 먹는 손님은 한 명의 인물로 넣는다
