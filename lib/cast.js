...
//
// who·look·voice 셋은 영어로 받는다. 사람이 읽는 값이 아니라 그림·소리 모델에 그대로
// 실리는 값이라서다(lib/cuts.js): look 은 이미지 프롬프트의 "Characters in this frame …",
// who·voice 는 영상 프롬프트의 "… speaks to the camera" 와 "Voice: …".
// cuts·avatar_id·photo_id 는 번호와 id 라 언어와 무관하다.
// ★ 예시도 함께 영어로 옮겼다 — 언어를 정하는 가장 강한 신호는 지시문이 아니라 예시 값이고,
//   "영어로 써라" 옆에 한국어 예시가 있으면 모델은 예시를 따른다.
const CAST_SYSTEM = `너는 숏폼 영상의 캐스팅을 정한다. 컷별 화면 설명을 읽고 화면에 보이는 사람을 뽑는다.
JSON 하나만 출력한다: {"cast":[{"who":"이 인물이 누구인지 한 마디, 영어로","look":"외형 — 머리·체형·옷차림, 영어로","voice":"목소리 — 음색과 톤, 영어로","avatar_id":"준비된 인물 사진 중 가장 맞는 id(없으면 생략)","cuts":[이 인물이 보이는 컷 번호들]}],"props":[{"photo_id":"올린 사진의 id","cuts":[그 사진에 찍힌 것이 보이는 컷 번호들]}]}
규칙:
- who·look·voice 세 칸은 영어로 쓴다 — 그림·소리 모델에 그대로 실리는 값이다. 답의 나머지(컷 번호·id)는 언어와 무관하다.
- [이 영상이 따라가는 사람]이 주어져 있으면 그 사람을 cast 에 넣는다. 화면 설명이 그 사람을 다르게 불러도 같은 사람으로 묶는다.
...
- who 는 나이대·성별·인종이 드러나게 적는다. 인종은 그림과 목소리의 일관성을 위한 것이다.
  ✗ "the customer" / "that person"
  ✓ "Korean man in his 50s, shop owner" / "East Asian man in his 20s, basketball player"
- look 은 그 인물의 외형이다 — 머리(길이·색), 체형, 옷차림을 눈에 보이게 적는다.
  이것이 전 컷의 그림 지시에 그대로 실려 같은 사람이 그려진다. 사진이 없을 때 일관성을 만드는
  것은 이 한 줄뿐이다 — 실측에서 이것이 없어 컷마다 다른 사람이 나왔고 한 컷은 성별까지 달랐다.
  화면 설명에 적힌 것을 살리고, 없으면 그 인물에 자연스러운 것을 정한다. 한 번 정하면 바꾸지 않는다.
  ✗ "man in his 20s" / "basketball player"
  ✓ "short black hair, lean muscular build, black sleeveless jersey and red shorts, white socks"
  ✓ "neat salt-and-pepper hair, average build, navy apron over a rolled-up white shirt"
  얼굴 생김새는 적지 않는다 — 지금 기술로는 지켜지지 않고, 적으면 컷마다 다른 얼굴이 나온다.
- voice 는 그 인물의 목소리다 — 음색과 톤을 적는다.
  ...
  ✗ "voice of a man in his 20s" / "nice voice"
...
- avatar_id 는 준비된 목록에서만 고른다. 맞는 것이 없으면 적지 않는다 — 억지로 고르면 엉뚱한 얼굴이 나온다.
  사진은 나이대와 성별로 고른다 — 준비된 사진 목록에는 인종이 적혀 있지 않으므로, 인종이 안 맞아 보인다는 이유로 비우지 않는다.
...
//
// opts.speakers — 시나리오가 컷마다 정한 "누가 말하는가". 없으면 블록째 빼서 지금까지와
// 글자 그대로 같은 지문이 된다(옛 호출부가 안 깨진다).
export function buildCastMessages(cuts, avatars, lead = "", props = [], opts = {}) {
  ...
  const propBlock = things.length
    ...
    : "";
  // 화면 설명만 보고 뽑으면 **말하는 사람이 빠질 수 있다** — 그러면 그 대사가 소리로 안 나온다
  // (클립이 직접 말하는 모델에서는 대사 있는 컷마다 화면에 말할 사람이 있어야 한다,
  //  lib/clip-limits.js 의 projectSpeaks). 시나리오가 이미 답을 정했으므로 그대로 알려 준다.
...
${shots}
${leadBlock}${propBlock}${speakerBlock}
[준비된 인물 사진]
${list}`;
...
//
// **사진이 인물보다 앞에 온다.** resolveCutRefs 는 순서가 아니라 종류(사진인지 인물인지)로
// 먼저 나누므로, 뒤에 붙여도 사물이 잘리지는 않는다. 앞자리가 실제로 결정하는 것은 사진이
// 둘 이상일 때 어느 사진이 유일한 사물 자리(resolveCutRefs 의 things[0])를 차지하느냐다 —
// 사장님이 올린 것 중 먼저 들어온 것이 그 자리를 차지하게 한다.
...
//
// 컷 하나에 붙일 수 있는 레퍼런스 장수.
//
// ★★ 2026-08-18 에 2 → 4 로 넓혔다(사장님 실측: **"섞이지 않아"**).
//    옛 상한의 근거는 "얼굴 둘을 함께 넣으면 섞여서 둘 다 아닌 얼굴이 나온다"였는데,
//    바로 그 자리 주석이 스스로 **"아직 실측하지 않았다"**고 적어 두고 있었다(2026-07-29 에
//    확인하기로 하고 그대로 남았다). 실물을 본 사장님이 섞이지 않는다고 했으므로 근거가
//    뒤집혔고, 그 사이 사진 5장을 올려도 컷마다 한두 장만 나가는 일이 실제로 있었다.
//
// ★ 왜 무한이 아닌가: 사진은 **바이트를 인라인**(data URI)으로 실어 보낸다(lib/imagegen.js).
//   장수가 곧 요청 크기이고, 큰 요청은 조용히 실패하는 쪽으로 기운다. 값은 안 든다 —
//   그림 값은 **만드는 장수**로 매겨지지 참조 장수로 매겨지지 않는다.
// ★ 각인에는 안 들어간다(lib/steps.js imageContextKey) — 그래서 이 값을 넓혀도 **이미 산
//   그림은 낡지 않는다.** 넓힌 효과는 다음에 만드는 그림부터다.
// ★★ 2026-08-18 저녁에 4 → 6 으로 다시 넓혔다. 사장님이 키링 사진 **4장**을 올린 컷에
//    인물이 하나 더 붙어 다섯이 됐고, 상한 4 가 **키링4 를 잘라 냈다**(실측: 프로젝트
//    b9f31ac3 컷2 — 화면의 "쓴 사진 3장" 배지가 그 사실을 이미 말하고 있었다).
//    "올린 사진이 잘려나가면 안 된다"는 이 함수의 첫 규칙인데, 값이 낮아 스스로를 어겼다.
//    6 인 이유: 사장님이 제품군 하나를 다 올리는 쓰임(4~5장)에 인물 한 자리를 더한 크기다.
//    무한이 아닌 이유는 아래 그대로 — 장수가 곧 요청 크기다.
const REF_MAX = 6;
...
export function resolveCutRefs(cut, project) {
  ...
  //
  // ★ 다만 **인물 한 자리는 반드시 남긴다.** 앞에서부터 그냥 자르면 사물이 자리를 다
  //   차지해 인물이 통째로 밀리고, 그러면 컷마다 다른 얼굴이 나온다 — 옛 코드가 1+1 로
  //   나눠 쓴 이유가 그것이고, 상한을 넓힌 지금도 그 보장은 그대로여야 한다.
  //   (그래서 `things.slice(0, REF_MAX - 1)` 이다 — 남은 자리는 인물이 채운다.)
  // ★ 인물을 한 명으로 줄이지 않는다(2026-08-18) — 얼굴이 섞인다는 옛 전제가 실측으로
  //   뒤집혔다. 한 컷에 두 사람이 나오면 둘 다 레퍼런스를 받아야 둘 다 일관된다.
  if (things.length && people.length) {
    const t = things.slice(0, REF_MAX - 1);
    return [...t, ...people.slice(0, REF_MAX - t.length)];
  }
