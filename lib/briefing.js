...
- **여기서는 '자료에 없는 사실'만 묻는다.** 이야기 소재(왜 시작했는지·손님 반응·기억에 남는 장면·남과 다른 점)는 묻지 않는다.
  그런 소재는 대본을 실제로 써 보고 길이가 모자랄 때 따로 청한다 — 지금은 필요한지 알 수 없다.
...
// 이야기 소재를 청하는 질문 — 대본을 써 보고 길이가 모자랄 때만 부른다.
// 브리핑 단계에서 미리 묻지 않는 이유: 그때는 모자랄지 알 수 없고, 후보 목록을 주면
// 모델이 자료를 대조하는 대신 목록을 채운다(실제로 "손님 반응"·"차별점"을 자료에
// 답이 있는데도 물었다).
const DEVELOP_SYSTEM = `너는 짧은 영상을 준비하는 사람에게 이야기 소재를 청하는 조수다.
대본을 써 보니 사장님이 원하는 길이를 채우기에 자료가 모자란다. 무엇을 더 들려주면 되는지 묻는다.
반드시 JSON 하나만 출력: {"questions":[{"question":"물을 것"}]}
규칙:
- 최대 3개. 모자란 분량이 적으면 1~2개면 된다.
- 자료·정리된 내용·이미 쓴 대본에 조금이라도 답이 있는 것은 묻지 않는다. 그건 이미 가진 것이다.
- 사실을 확인하는 질문(가격·날짜·수량)이 아니라 **이야기를 청하는 질문**이다.
  후보: 이 일을 시작한 계기, 손님이 한 말이나 달라진 점, 기억에 남는 구체적인 한 장면, 남들과 다르게 하는 것과 그 이유.
  다만 후보를 그대로 옮기지 말고, 이 자료에서 비어 있는 쪽을 골라 그 가게의 말로 묻는다.
- 보기는 붙이지 않는다. 사장님만 아는 이야기라 그럴듯한 보기를 만들면 없는 사실이 굳는다.
- 짧고 구체적으로 묻는다. "어떤 이야기가 있나요" 같은 막연한 질문은 답하기 어렵다.`;

export function buildDevelopMessages(project, shortSeconds) {
  const { material, briefing, script } = project;
  const points = (briefing?.key_points || []).map((k) => `- ${k}`).join("\n") || "(없음)";
  const asked = (briefing?.asked || [])
    .map((a) => `- ${a.question}${a.answer ? ` → ${a.answer}` : ""}`)
    .join("\n") || "(없음)";
  const user = `[자료 원문]
${material?.text || ""}
...
[모자란 분량] 약 ${shortSeconds}초`;
  return { system: DEVELOP_SYSTEM, messages: [{ role: "user", content: user }] };
}
...
export function briefingSignature(briefing) {
  ...
  return JSON.stringify({
    ...
    // 연출 바람이 바뀌면 화면 설계가 달라져야 한다 — 버전이 안 오르면 옛 화면이 남는다
    direction: trim(briefing.direction),
