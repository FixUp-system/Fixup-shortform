  ...
  const res = await fetchImpl(`https://fal.run/${endpoint}`, {
    ...
    body: JSON.stringify({
      ...
      ...(refList
        ...
        // ★★ 2026-08-31 — 필드 이름을 **프로필이 쥔다.** 여기 손으로 적혀 있던 `image_urls`
        //   는 Seedance 의 이름이고, H3 는 `reference_image_urls` 다. 그대로 두면 사진이
        //   통째로 무시된 채 값만 나간다 — 광고 축이 2026-08-21 에 겪고 표로 옮긴 그 일이다.
