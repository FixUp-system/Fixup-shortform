...
export async function submitClip({ imageUrl, refs, seconds, aspect_ratio, prompt, projectId, project, fetchImpl = fetch }) {
  ...
  // ★★★ 2026-09-10 — 접수할 때 **우리 웹훅 주소를 실어 보낸다.** 그러면 fal 이 끝나는
  //   순간 우리를 부른다(app/api/fal/webhook). 크론과 달리 플랜을 안 타고 헛걸음이 없으며,
  //   ★ 접수증이 문서에 적히기 전에 함수가 죽어도 fal 이 projectId 를 들고 찾아온다.
  //   프로덕션 주소를 모르면 안 붙는다(falWebhookUrl 머리말).
  const hook = falWebhookUrl(projectId);
  const res = await fetchImpl(`https://queue.fal.run/${endpoint}${hook ? `?fal_webhook=${encodeURIComponent(hook)}` : ""}`, {
    method: "POST",
