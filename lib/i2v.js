...
import { I2V_STEPS, I2V_MAX_SECONDS, fitDuration } from "./clip-limits";
...
export async function generateClip({ imageUrl, seconds, aspect_ratio, prompt, projectId, fetchImpl = fetch }) {
  ...
  const duration = fitDuration(want);
  ...
  const res = await fetchImpl(`https://fal.run/${endpoint}`, {
    ...
    body: JSON.stringify({ image_url: imageUrl, prompt, duration, aspect_ratio }),
  });
  if (!res.ok) {
