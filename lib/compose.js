...
// 파일 크기를 정하는 두 값 — **굽는 자리 둘이 이것을 함께 쓴다**(합성·자막굽기).
// 한쪽만 고치면 자막이 있는 편과 없는 편의 크기가 갈린다.
//
// ★ 왜 생겼나: 2026-09-07 전송 할당량 초과로 프로덕션이 402 로 막혔다. 인코더에 둘이
//   빠져 있었다. 실측(`reel-final.mp4` 3,047 KB 재인코딩):
//     지금(기본 crf 23) 2,821 KB · **crf 26 = 2,151 KB(−29%)** · crf 28 = 1,752 KB(−42%)
//     preset slow 는 2%p 더 줄이려고 인코딩 시간을 크게 써서 값어치가 없다(2,077 KB).
//
// ★ crf 26 을 고른 이유: 눈에 안 띄는 선에서 가장 크게 줄어드는 값이다. 더 줄이려면
//   28 로 올리면 되지만 **화질은 사장님이 정할 값**이라 env 로 뺐다(숫자가 아니면 26).
// ★ faststart 는 크기를 안 줄인다 — `moov` 를 파일 **앞**으로 옮긴다. 없으면 재생기가
//   재생 전에 파일 끝을 먼저 받아야 해서 왕복이 하나 는다(이 저장소 산출물이 그랬다).
const CRF = /^\d{1,2}$/.test(process.env.SHOTFORM_VIDEO_CRF || "")
  ? process.env.SHOTFORM_VIDEO_CRF
  : "26";
const SIZE_ARGS = ["-crf", CRF, "-movflags", "+faststart"];
...
export function burnArgs({ raw, assPath, out, lang }) {
  return [
    ...
    // ★ 자막이 있으면 **이쪽이 최종본**이다 — 크기 설정을 여기 빼먹으면 절감이 절반만 걸린다.
    ...SIZE_ARGS,
    "-c:a", "copy", out,
  ];
}
...
export async function defaultDownload(url, dest) {
...
// ★★ **통짜(r2v) 한 편인가.** 한 판을 통째로 굽고 그 한 편을 cut[0] 에만 담는 갈래다
//   (lib/reel/pipeline.js 의 runReelOneShot 이 video.whole 을 찍는다).
//   컷별 갈래는 컷마다 클립이 있으므로 이 판정이 늘 거짓이고, 아래 둘도 예전 그대로다.
function isWholeClip(usable) {
  return usable.length === 1 && usable[0]?.video?.whole === true;
}

// ★★ **완성본 길이.** 통짜는 붙일 것이 한 편뿐이라 그 편의 길이가 곧 전체다.
//
// ⚠️ 2026-08-25 사장님 실측에서 여기가 **29초**로 찍혔다(목표 15초). cutSeconds(cut[0]) 이
//   통짜 한 편의 길이(15)를 돌려주는데 그 값이 "컷 1의 화면 시간"으로 쓰여, 나머지 컷의
//   화면 시간(14)이 그 위에 또 더해졌다 — 15 + 14 = 29. 컷들은 이미 그 한 편 **안에** 있다.
export function composeSeconds(cuts, usable) {
  if (isWholeClip(usable)) return cutSeconds(usable[0]);
  return (cuts || []).reduce((s, c) => s + cutSeconds(c), 0);
}

// ★★ **자막이 될 컷**은 이어 붙일 컷과 다른 축이다(2026-08-25 사장님 실측으로 드러났다).
//
//   · 이어 붙일 것 = **클립이 있는 컷**(usable) — 없는 것을 붙일 수는 없다
//   · 자막이 될 것 = **대사가 있는 컷**
//
// 컷별 갈래에서는 컷마다 클립이 있어 둘이 같았고, 그래서 지금까지 usable 하나로 다 됐다.
// 통짜는 다르다 — 나머지 컷에 클립이 없는 것이 **정상**인데 그 필터가 자막까지 걸러,
// 6컷짜리 영상에 **자막이 컷 1 하나**만 깔렸다(사장님: "첫문장 하나만 나와").
//
// ★★ 넓히는 것만으로는 부족하다 — **시각도 고쳐야 한다.** buildCues 는 cutSeconds 를
//   누적해 시작을 잡는데, cut[0] 의 그 값이 한 편의 길이(15)라 컷 2 자막이 15초부터
//   시작한다(영상 밖이다). 그래서 통짜에서는:
//     · cut[0] 의 video 를 떼어 **계획 초**(화면 시간)로 읽게 하고
//     · 계획 초의 합을 실제 한 편의 길이에 **비례로 눌러 담는다**
//   눌러 담아야 하는 이유: 계획 초 합이 한 편과 안 맞는다. 이번 실측이 17초 대 15초였다
//   (LLM 이 2.5 초를 내면 buildReelCuts 의 Math.round 가 3 으로 올린다).
// ★ whisper 가 잰 시각(spoken_start)이 있으면 buildCues 가 그것을 먼저 쓴다 — 이 비례는
//   **못 쟀을 때의 바닥**이다(이번 실측에는 spoken_start 가 하나도 없었다).
// ★★ 2026-08-27 — 세 번째 인자가 **내레이션 한 벌 단위**다(lib/reel/narration.js 의
//   narrationUnits). 새 길에서는 말이 컷이 아니라 한 벌에 살아 컷의 sentence 가 비므로,
//   그것을 그대로 두면 완성본에 자막이 통째로 없다. 주면 그 목록이 곧 자막 원천이다
//   (모양이 컷과 같아 buildCues 는 손대지 않는다).
// ★ 안 주면 **글자 그대로 예전**이다 — 옛 문서·컷별 갈래가 그 길로 간다.
export function subtitleCutsOf(cuts, usable, narrationUnits) {
  if (Array.isArray(narrationUnits) && narrationUnits.length) return narrationUnits;
  if (!isWholeClip(usable)) return usable;
  const list = cuts || [];
  const planned = list.reduce((s, c) => s + (Number(c?.seconds) || 0), 0);
  const total = cutSeconds(usable[0]);
  const k = planned > 0 && total > 0 ? total / planned : 1;
  // video 를 떼는 것이 핵심이다 — 안 떼면 cut[0] 이 다시 한 편의 길이로 읽힌다.
  return list.map(({ video, ...c }) => ({ ...c, seconds: (Number(c?.seconds) || 0) * k }));
}
...
export async function burnSubtitles({
  ...
  sourceLang,
  // ★ 자막만 다시 구울 때도 같은 원천을 쓴다 — 여기만 빠지면 다시 구운 자막이 사라진다.
  narrationUnits,
  aspect_ratio = "9:16",
  ...
  // ★ 자막은 **이어 붙일 컷**이 아니라 대사가 있는 컷에서 나온다(위 subtitleCutsOf).
  const subtitleCuts = subtitleCutsOf(cuts, usable, narrationUnits);
  // 길이는 composeVideo 와 같은 자로 센다 — 자막이 흐르는 시간이 그 값이다(composeSeconds).
  const seconds = composeSeconds(cuts, usable);
  ...
  try {
    ...
    await writeFileImpl(
      assPath,
      toAss(buildCues(subtitleCuts, { width, height, subtitle, lang, sourceLang }), { width, height, position: subtitlePosition, subtitle, lang }),
      "utf8"
    );
...
export async function composeVideo({
  ...
  lang,
  sourceLang,
  // ★★ 내레이션 한 벌의 자막 단위(lib/reel/narration.js 의 narrationUnits). 새 길에서는
  //   말이 컷이 아니라 한 벌에 살아 컷의 sentence 가 빈다 — 안 넘기면 완성본에 자막이
  //   통째로 없다. 안 주면 옛 경로 그대로다(옛 문서·컷별 갈래).
  narrationUnits,
  ...
  //   전에는 null 이라 ⑥완성의 영상 자리가 비어 있었다 — 배치도 자막도 못 봤다.
  //   옆의 주석이 걱정한 것("재생 안 되는 더미를 주면 합성이 깨졌다고 오해한다")은
  //   **진짜 재생되는 표본**이면 생기지 않는다.
  if (fakeFal()) return { url: "/samples/reel-15s.mp4", seconds, fake: true };
