...
// 통짜(r2v)는 다르다 — 한 판을 통째로 굽고 그 한 편을 cut[0] 에만 담는다(video.whole).
// 나머지 컷에 클립이 없는 것이 **정상**인데 그 필터가 자막까지 걸러, 6컷짜리 영상에
// **자막이 컷 1 하나**만 깔렸다(사장님: "첫문장 하나만 나와").
//
// ★ 통짜일 때만 넓힌다 — 컷별 갈래는 한 글자도 안 바뀐다(회귀 0).
function subtitleCutsOf(cuts, usable) {
  const whole = usable.length === 1 && usable[0]?.video?.whole === true;
  return whole ? (cuts || []) : usable;
}
