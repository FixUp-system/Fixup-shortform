...
// 클립이 소리보다 짧으면 마지막 프레임을 정지로 늘려 길이를 맞춘다.
...
export function buildFfmpegArgs({ local, assPath, out, width, height }) {
  ...
  const filters = [];
  local.forEach((l, i) => {
    const pad = Math.max(0, (Number(l.wantSeconds) || 0) - (Number(l.haveSeconds) || 0));
  ...
  // 역슬래시는 subtitles 필터가 이스케이프로 읽으므로 슬래시로 바꾼다(Windows).
  const ass = assPath.replace(/\\/g, "/");
  const fontsdir = path.join(process.cwd(), "assets").replace(/\\/g, "/");
...
async function composeWithFal({ cuts, seconds, fetchImpl }) {
...
export async function composeVideo({
  ...
  if (process.env.SHOTFORM_COMPOSER === "fal") {
    return composeWithFal({ cuts: usable, seconds, fetchImpl });
  }
