...
export async function composeVideo({
  ...
  try {
    ...
    await writeFileImpl(
      assPath,
      // ★ project 를 넘긴다 — 말하는 클립은 안 잘리므로 컷 하나가 차지하는 시간이
      //   받은 클립 길이다. 안 넘기면 자막이 컷마다 앞선다.
      toAss(buildCues(usable, { width, height }, project), { width, height, position: subtitlePosition }),
      "utf8"
    );
