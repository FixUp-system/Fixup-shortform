...
async function defaultDownloadRender(url, dest) {
  ...
  const bytes = await getStore().getObject("renders", key);
...
export async function burnSubtitles({
  ...
  try {
    ...
    // buildCues 에는 설정을 안 넘긴다 — 줄바꿈·폭 계산은 가로 여백과 글자 크기만 쓴다.
    const assPath = path.join(dir, `${projectId}.ass`);
    await writeFileImpl(
      assPath,
      toAss(buildCues(usable, { width, height }), { width, height, subtitle }),
      "utf8"
    );
