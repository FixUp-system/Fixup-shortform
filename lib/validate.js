...
//
// ★ validateScript 도 여기 있었다(2026-08-16 삭제) — 원고 생성이 사라지면서 부르는 곳이
//   0 이 됐다. 시나리오 응답의 방어는 lib/scenario.js 의 validateScenario 가 맡는다.
...
export function validateShows(obj, cutCount) {
  ...
  for (const s of obj.shots) {
    ...
    //
    // 필드 이름을 여기 손으로 적지 않고 MOTION_AXES 를 도는 이유: 축 목록의 원천은
    // lib/motion.js 하나다. 두 벌이면 갈리고, 갈리면 지문이 답하라 한 축을 검증이 버린다.
    //
    // 빈 문자열·공백만 있는 축은 **키 자체를 만들지 않는다** — 각인(clipKey)이 "무엇에서
    // 나왔는가"를 컷의 필드로 재므로, 빈 키가 생기면 뜻이 같은 컷 둘이 다르게 각인되어
    // 멀쩡한 클립이 낡는다(=유료 [다시 만들기]가 열린다).
    //
    // 목록 밖 축(lighting 등)은 위 speed 와 같은 이유로 통째로 무시한다 — 모델이 답한 것과
    // 쓰이는 것이 달라지면 대비 판정이 거짓이 된다. 조명 변화는 ambient 가 받는다.
    ...
    // ★ environment 와 달리 **컷별 tone 은 받지 않는다**(s.tone 을 안 본다) — 컬러·질감이
    // 컷마다 튀면 한 편으로 안 보인다. 무대는 이야기가 옮겨가면 바뀔 근거가 있지만
    // (코트에서 거리로), 현상 방식이 컷마다 달라질 근거는 없다.
...
// ★ validateDevelopQuestions 는 걷어냈다(2026-08-16) — 그것을 부르던 소재 청하기 라운드가
//    원고와 함께 사라졌다(lib/briefing.js 의 같은 자리 주석 참고).
...
export function validateBriefing(obj, materialText = "") {
  ...
  // 외형 — 물건이면 그 물건의 생김새다(색 배치·부위·소재). 인물의 cast[].look 과 같은 방식이다.
  // 없어도 초점을 버리지 않는다: 없으면 지금까지처럼 앵커 낱말만으로 그려진다.
  const look = str(obj.focus?.look);
  if (focus && look) focus.look = look;
