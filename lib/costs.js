...
const PRICE_TABLE = [
  ...
  // 광고 경로(lib/ad). fast 티어 720p — t2v/i2v/r2v 가 같은 단가다. 해상도별 문서가
  // 없어 단일가(숫자)로 둔다(480p 도 이 값 — 안전한 방향, lib/ad/models.js 의 fast
  // perSecUsd 주석 참고).
...
function perSecFor(entry, resolution) {
  if (typeof entry.perSec === "number") return entry.perSec;
...
export async function assertBudget({ projectId, endpoint, amount, skipProjectAxis = false }) {
