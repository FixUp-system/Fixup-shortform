...
const PRICE_TABLE = [
  ...
  // Seedance 2.5 — t2v/i2v/r2v 가 "/fast" 세그먼트 없이 여기 하나로 다 걸린다.
  // ⚠️ 계산값이지 실측이 아니다(2.5 를 실제로 불러본 적이 없다). 2.0 과 달리 해상도가
  //    원가를 정한다(토큰식) — 9:16 720p 기준으로 계산해 넣었다(lib/ad/models.js 의
  //    perSecUsd 주석에 계산식이 있다). 480p 로 내리면 실제 원가가 더 싸다(안전한 방향).
  //    lib/ad/models.js 의 perSecUsd 와 **같은 값이어야 한다** — 갈리면 예산 가드가 틀린다.
  { prefix: "bytedance/seedance-2.5", perSec: 0.4622 },
];
...
export async function assertBudget({ projectId, endpoint, amount }) {
  ...
  // 엔드포인트로 가른다: fal-ai/* 는 fakeFal(), 나머지(openai/* 와 **모르는 것**)는 fakeLlm().
