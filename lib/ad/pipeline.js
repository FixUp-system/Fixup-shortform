...
export async function runScenarioStep(projectId, ownerId, deps = {}) {
  ...
  // ★★ 사진을 한 번 읽는다(2026-08-18). 광고는 지금까지 사진을 **한 번도 안 봤다** —
  //   지문에 "첨부 사진: N장"만 적고 바이트는 참조(r2v)로만 넘겼다. 그래서 제품에 인쇄된
  //   글자를 모델이 화면에서 스스로 읽어야 했고, 사장님이 본 `KONKUK UNVV` 가 그 결과다.
  //   읽는 것보다 받아쓰는 것이 훨씬 정확하므로, 여기서 한 번 읽어 철자를 지문에 싣는다.
  //   ★ 단계별과 **같은 함수**(describePhoto)를 쓴다 — 판정이 두 벌이 되면 두 흐름의
  //     결과가 갈린다. 저장 자리도 같다(photo.vision).
  //   ★ 아직 안 본 사진만. 다시 쓰기를 눌러도 사진값은 다시 안 든다.
  //   ★ 실패는 흘려보낸다 — 글자를 못 읽었다고 광고를 못 만들 이유가 없다.
  const readPhotos = [];
  let readAny = false;
  for (const ph of project.material?.photos || []) {
    const key = ph.url?.split("/").pop();
    if (ph.vision || !key) { readPhotos.push(ph); continue; }
    const bytes = await readRefBytes({ source: "upload", key });
    if (!bytes) { readPhotos.push(ph); continue; }
    const vision = await (deps.describePhoto || describePhoto)({ photoBytes: bytes, photoKey: key, projectId });
    const judged = vision.person || vision.what || vision.lettering;
    if (judged) readAny = true;
    readPhotos.push(judged ? { ...ph, vision } : ph);
  }
  const seen = readAny
    ? { ...project, material: { ...project.material, photos: readPhotos } }
    : project;

  const scenario = await make({ project: seen, edits });
  await updateProject(projectId, ownerId, (p) => ({
    ...
    ...(readAny ? { material: { ...p.material, photos: readPhotos } } : {}),
  }));
}
