...
import { getAdStatus } from "../projects.js";
...
export async function collectAdRender(projectId, ownerId, deps = {}) {
  ...
  // ★★★ 2026-09-11 — **통짜를 안 읽는다.** 이 함수는 2초마다 불리는데 그전에는 매번
  //   문서 전체(실측 16.8KB)를 끌어왔다. 여기서 쓰는 것은 접수증·상태·설정·시나리오 한 줄뿐이라
  //   좁은 셀렉터로 바꿨다(lib/store/*.js 의 selectAdStatus 머리말에 이유가 있다).
  //   ★ 뒤에서 `collect` 에 넘기는 모양(`project.settings`·`project.id`·`scenario.text`)은
  //     **그대로 맞춘다** — lib/ad/generate.js 의 collectAdVideo 가 그 셋만 쓴다.
  // ★ 이음매 — 좁은 모양은 통짜 문서의 **부분집합**이라, 판이 넣어 주는 통짜 읽기를
  //   그대로 쓸 수 있다. 프로덕션에는 주입이 없어 좁은 셀렉터가 돈다.
  const readHead = deps.getAdStatus || deps.getProject || getAdStatus;
  const head = await readHead(projectId, ownerId).catch(() => null);
  const job = head?.ad_job;
  // 굽고 있지 않으면 fal 에 묻지도 않는다.
  if (!job || head.status !== "rendering") return { changed: false };
  const project = { id: projectId, settings: head.settings };
  const scenario = head.scenario || { text: "" };
  // 이미 "끝났다"고 적어 둔 편은 fal 에 다시 묻지 않는다 — 남은 것은 마무리뿐이다
  // (다시 물으면 원장에 같은 굽기가 또 쌓인다).
  if (job.ready) return { changed: false, ready: true };

  try {
    const got = await collect({ project, scenario, job });
    if (!got.done) {
