...
// ★ 2026-08-13 — 이동이 생겼다. 페이지는 여전히 하나지만 **주소에 보는 단계를 남긴다**
//   (`/ads/[id]?step=scenario`). 지나온 단계를 다시 볼 길이 아예 없었기 때문이다.
//   페이지를 넷으로 쪼개지 않은 이유: 진행이 자동이라 각 단계가 "머무는 화면"이 아니고,
//   쪼개면 자동 진행 때마다 주소를 밀어 넣어야 해서 뒤로가기가 이상해진다.
//
...
// 그 단계를 지금 볼 수 있는가 — **지나왔거나 지금 자리**면 열린다.
//
// 아직 안 온 단계는 잠근다: 없는 것을 보여줄 수 없다(시나리오가 없는데 시나리오 화면을
// 열면 빈 상자만 남는다). 모르는 값은 adStepIndex 의 성질을 그대로 물려받는다 —
// status 를 모르면 첫 단계 기준이고, 모르는 단계 키는 -1 이라 늘 잠긴다.
export function isAdStepReachable(stepKey, status) {
  const want = AD_STEPS.findIndex((s) => s.key === stepKey);
  if (want === -1) return false;
  return want <= adStepIndex(status);
}

// 지금 **보는** 단계 — 주소의 ?step 이 지나온 단계면 그것, 아니면 실제 status.
//
// ★ 화면과 사이드바가 같은 답을 보려고 규칙을 여기 한 벌만 둔다. 예전에는 화면만
// 이 계산을 했고 사이드바는 status 만 봐서, 완성된 광고에서 ②시나리오를 다시 봐도
// 사이드바는 ④완성에 불이 켜진 채였다(2026-08-13 실측).
export function adViewStep(wanted, status) {
  return wanted && isAdStepReachable(wanted, status) ? wanted : status;
}
