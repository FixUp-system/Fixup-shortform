...
// ★★★ 2026-09-02 — **4096 이었고, 그 값이 시나리오를 못 만들게 하고 있었다.**
//
// max_tokens 는 **사고 + 본문의 합계 상한**이다(Fable 은 사고가 항상 켜져 있어 끌 수 없다).
// 사고가 예산을 먹으면 JSON 이 끝나기 전에 잘리고, 아래 재시도 루프가 그것을 두 번 사서
// 두 번 다 잘린 뒤 "LLM 응답 해석 실패" 를 던진다 — 화면에는 "아직 시나리오가 없어요" 만
// 남아 사장님에게는 아무 일도 안 일어난 것처럼 보였다.
//
// ★ 실측(프로덕션 원장, 프로젝트 5ddea7b7): 같은 소재(입력 12,036 tok)로 부른 13회 중
//   **10회가 출력 정확히 4096** 에 붙어 잘렸다. 성공한 3회는 3570·3874·3985 였다 —
//   즉 상한을 겨우 넘나드는 자리라 **되는 것이 운이었다.** 잘린 10회 값 $3.25 도 함께 나갔다.
// ★ 16000 은 Anthropic 이 비스트리밍 요청에 권고하는 기본값이고, 이 저장소의 다른 LLM
//   경로(lib/llm.js 의 MAX_TOKENS)가 이미 쓰던 값이다 — **같은 성격의 값이 두 벌이었고
//   낮은 쪽이 터졌다.**
// ⚠️ 더 올릴 거면 스트리밍이 필요하다(SDK·HTTP 타임아웃). 16000 은 비스트리밍 안전권이다.
// ⚠️ 근본 조절은 상한이 아니라 사고 깊이(output_config.effort)일 수 있다 — 사장님이
//    따로 보시기로 한 숙제다. 지금은 급한 불만 끈다.
...
// **거절이 아닌** stop_reason — 정상 완료(end_turn)와 토큰 상한에 닿은 경우(max_tokens).
// 그 밖(예: 안전 분류기 거절)은 같은 프롬프트를 다시 보내도 같은 이유로 다시 막힐 뿐이라
// 아래에서 사장님에게 보일 오류로 바꾼다.
//
// ⚠️ 이름이 "재시도해도 되는" 이었는데 **max_tokens 는 이제 재시도하지 않는다**(2026-09-02).
//   이 집합이 가르는 것은 "거절인가 아닌가" 하나뿐이다 — 재시도 여부는 아래 파싱 자리가 정한다.
const NOT_REFUSAL_STOP = new Set(["end_turn", "max_tokens"]);
...
export async function callJson({
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    // 사장님에게 보일 오류로 바꾼다.
    if (!NOT_REFUSAL_STOP.has(data?.stop_reason)) {
      throw new Error(
    ...
    try {
      ...
      // ★★★ 2026-09-02 — **잘린 것은 재시도하지 않는다.**
      //
      // 입력도 상한도 그대로라 두 번째도 **같은 자리에서 잘린다.** 고쳐지지 않는 것을
      // 한 번 더 사는 것이고, 값만 두 배가 된다(잘린 호출 1회 최대 ~$0.92 · 2연타 ~$1.8).
      //
      // ★ 파싱을 **먼저** 해 보는 순서를 지킨다 — 상한에 닿았어도 JSON 이 우연히 완결됐을
      //   수 있다. 그때는 위 return 으로 이미 정상으로 돌아갔다. 여기 오는 것은
      //   "상한에 닿았고 실제로 깨진" 경우뿐이다.
      // ★ 그 밖의 파싱 실패(모델이 형식을 틀린 경우)는 **그대로 재시도한다** — 그쪽은
      //   다시 부르면 다른 답이 나올 수 있다. 두 실패를 같이 다루면 안 된다.
      if (data?.stop_reason === "max_tokens") {
        throw new Error(
          "소재가 길어 시나리오가 중간에 잘렸어요 — 소재를 줄여서 다시 시도해 주세요"
        );
      }
    }
  }
