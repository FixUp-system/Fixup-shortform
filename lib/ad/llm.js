...
// ★★★ 2026-08-27 — 광고에서 **덜어 내는 목록이 크게 늘었다.** 구조가 뒤집혔기 때문이다:
//   이제 Fable 이 영상 프롬프트 한 편(`text`)을 처음부터 끝까지 쓰고, 코드는 거기에
//   아무것도 안 붙인다. 카메라·조명·동작·소리·목소리·색·무대·옷차림은 전부 **그 글 안에**
//   있다 — 칸으로 또 받으면 같은 것을 두 번 적게 하는 셈이고, 실제로 그 칸들이 프롬프트
//   꼬리에 절로 덧붙어 text 와 중복되거나 말이 안 되는 영어를 만들었다.
//   광고에 남는 것은 셋뿐이다: `text`(영상 프롬프트) · `angle`(사장님이 읽는 한국어 한 줄) ·
//   `shots`(자막을 태우려고 우리가 쓰는 목록 — beat·line·seconds).
//   `endpoint` 도 뺐다 — 사진 수만 보고 코드가 정한다(lib/ad/scenario.js pickEndpointKind).
// ★ film 쪽 스키마는 여전히 **글자 그대로 예전과 같다.**
const AD_DROPPED_SHOT_FIELDS = [
  "shows", "avatar_id", "transition", "speaker", "say_as", "camera", "lighting", "action", "sound",
];
const AD_DROPPED_TOP_FIELDS = [
  "endpoint", "focus", "voice", "music", "tone", "look", "cast", "wardrobe", "environment",
];
...
export function scenarioSchemaFor(kind) {
  ...
  const top = { ...SCENARIO_SCHEMA.properties };
  for (const f of AD_DROPPED_TOP_FIELDS) delete top[f];
  return {
    ...SCENARIO_SCHEMA,
    properties: {
      ...top,
    ...
    required: SCENARIO_SCHEMA.required.filter((f) => !AD_DROPPED_TOP_FIELDS.includes(f)),
  };
}
