...
function fakeAdResponse() {
  return {
    ...
    shots: [
      { beat: "제품이 등장한다", camera: "슬로우 푸시인", action: "테이블 위 제품이 놓인다", line: "가짜 나레이션입니다" },
      { beat: "마무리", camera: "정면 고정", action: "로고가 떠오른다", line: "" },
    ],
    endpoint: "r2v",
  };
}
...
// {text, shots:[{beat,camera,action,line}], endpoint} 와 같다. 이 모듈은 광고 시나리오
// 전용이라(다른 소비자가 없다) 스키마를 여기 하나로 못박는다 — 콜러가 스키마를 넘기게
// 하지 않는다(계약 callJson({system,messages,stage,projectId}) 을 그대로 유지하려고).
//
