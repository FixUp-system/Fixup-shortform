...
import { AD_FORMATS, AD_MOODS, AD_LANGS, AD_STYLE_LINES, AD_VOICES } from "./options.js";
...
export function buildScenarioMessages({ kind, settings, material, edits, globalEdits }) {
  ...
  // ★ 목소리는 **모르는 값에 안 던진다** — 옛 문서에는 이 칸이 아예 없다(그때는 auto 다).
  //   다른 축(포맷·분위기)은 던지는데 여기만 관대한 이유가 그것이다: 저 셋은 옛 문서에도
  //   있던 칸이라 값이 있으면 목록 안이어야 하지만, 이 칸은 **없는 것이 정상**이다.
  const voiceLine = (AD_VOICES.find((v) => v.id === settings?.voice_style) || {}).line || "";
  ...
  const user = [
    ...
    `화풍: ${styleLine}`,
    // ★★ 목소리를 **사장님이 골랐으면 그것이다**(2026-08-24). 안 골랐으면(auto) 이 줄이
    //   통째로 없고 시나리오가 알아서 정한다 — 옛 문서의 지문이 글자 그대로다.
    //   ★ 지문에 싣는 것만으로 끝내지 않는다: generateScenario 가 **코드로 덮어쓴다.**
    //     "지켜져야 하는 것은 프롬프트가 아니라 코드가 판정한다"가 이 저장소의 규율이고,
    //     실제로 지문이 목소리를 정하라고 시켜도 모델은 늘 같은 사람을 냈다(6건 중 5건).
    ...(voiceLine ? [`나레이션 목소리(사장님이 고름): ${voiceLine} — voice 칸에 이대로 적는다`] : []),
...
export async function generateScenario({ project, edits, globalEdits, deps = {} }) {
  ...
  // ★★★ **사장님이 고른 목소리가 이긴다**(2026-08-24). 지문에 실어 두었지만 그것만으로는
  //   부족하다 — 실측에서 모델은 목소리를 정하라는 지시에도 늘 같은 사람을 냈다(6건 중 5건이
  //   "20대 후반 한국 여성"). 이 저장소의 규율 그대로 **코드가 판정한다.**
  //   ★ auto 면 덮지 않는다 — 그때는 시나리오가 정하는 것이 맞다(옛 문서도 그대로 흐른다).
  const picked = (AD_VOICES.find((v) => v.id === project?.settings?.voice_style) || {}).line || "";
  return picked ? { ...out, voice: picked } : out;
}
