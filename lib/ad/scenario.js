...
// ★★ 이 함수는 이제 **film 전용**이다(2026-08-27). 광고는 아래 AD_SYSTEM 한 벌을 쓴다.
//   `draws` 보간을 지우지 않고 상수로 둔 이유: film 지문이 **글자 그대로** 예전과 같아야
//   한다(tests/film-shows.test.js 가 그것을 잰다). 보간을 펼치면 공백 하나가 갈린다.
function filmSystem() {
  const draws = true;
...
const AD_SYSTEM = `너는 세계 최고의 CF 감독이다. 연출·촬영·카피를 혼자 다 한다.
...
// ★ 광고와 film 이 서로 다른 지문을 받는다. 모르는 kind 는 그림이 있는 쪽(film, 예전 그대로)이다.
export function systemFor(kind) {
  return kind === "ad" ? AD_SYSTEM : filmSystem();
}
...
export async function generateScenario({ project, edits, globalEdits, deps = {} }) {
  ...
  const raw = await call({
    system, messages: [{ role: "user", content }], stage: "광고 시나리오", projectId: project.id,
