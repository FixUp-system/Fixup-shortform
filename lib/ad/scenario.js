...
export function buildScenarioMessages({ kind, settings, material, edits }) {
  ...
  const user = [
    ...
    // ★★★ MiniMax H3 만 참조를 **프롬프트 안에서 지칭**해야 한다(2026-08-21).
    //   fal 스키마 설명 그대로다: "URLs of subject/style reference images, referenced in
    //   the prompt as Image 1, Image 2, and so on." Seedance 는 `image_urls` 에 넣으면
    //   모델이 알아서 쓰므로 이 줄이 필요 없다 — 그래서 모델별 한 토막이다.
    //   ★ 지문을 두 벌로 만들지 않는다: 갈리는 것이 이 몇 줄뿐이라 토막만 끼운다
    //     (lib/ad/scenario.js 의 systemFor 가 경로를 가르는 것과 같은 방식).
    //   ★ 사진이 없으면 이 줄도 없다 — 가리킬 것이 없는데 지칭 규칙만 남으면
    //     모델이 없는 Image 1 을 지어낸다.
    ...(photos.length && String(settings?.model || "").startsWith("minimax")
      ? [
          "★ 이 모델은 참조 사진을 **지시문 안에서 이름으로 가리켜야** 쓴다.",
          `  첨부한 순서대로 Image 1${photos.length > 1 ? " · Image 2" : ""}${photos.length > 2 ? " …" : ""} 다.`,
          '  그 사진이 쓰이는 자리에서 "Image 1" 이라고 **영어로 그대로** 적는다',
          '  (예: "the product from Image 1 sits on the table"). 안 가리키면 사진이 통째로 무시된다.',
        ]
      : []),
    ...(photos.length
      ? [
