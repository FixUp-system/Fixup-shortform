...
const buildSystem = (sceneCountRule) => `너는 광고 CF 를 연출하는 감독이다.
...
export function buildScenarioMessages({ settings, material, edits }, opts = {}) {
  ...
  const system = buildSystem(opts.sceneCountRule || AD_SCENE_COUNT_RULE);
  return { system, messages: [{ role: "user", content: user.join("\n") }] };
}
...
export async function generateScenario({ project, edits, sceneCountRule, note, deps = {} }) {
  ...
  const { system, messages } = buildScenarioMessages({ ...project, edits }, { sceneCountRule, note });
