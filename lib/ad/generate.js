...
export function withSpokenLines(text, shots, voice, scenario) {
  ...
  const music = one(scenario?.music);
  ...
  // ★ 가사 없는 음악이라고 못 박는다 — 노래가 깔리면 낭독과 겹쳐 둘 다 안 들린다
  //   (단계별 SYSTEM 이 같은 이유로 가사를 막는다).
  if (music) extra.push("", `Background music for the whole film, one piece throughout: ${music}. Instrumental only, no lyrics or vocals.`);
