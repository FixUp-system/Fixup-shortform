...
// ★★★ 2026-08-27 — 광고는 `lean` 으로 부른다. 아래 절들을 **하나도 안 붙인다.**
//
// 왜: 이제 Fable 이 영상 프롬프트 한 편을 처음부터 끝까지 쓴다(lib/ad/scenario.js 의
// AD_SYSTEM 7단). 이야기·무대·인물·옷·색·목소리가 전부 그 글 **안에** 있으므로, 여기서
// 또 붙이면 같은 말이 두 번 나가거나 — 실제로 그랬다 — 한국어 `angle` 이 영어 프롬프트
// 안에 섞이고, "the person wears ___" 틀에 두 사람이 들어가 말이 안 되는 영어가 됐다.
// 사장님이 준 성공 프롬프트 넷 중 이런 꼬리를 단 것은 하나도 없다.
//
// ★ 대사 보강(missing)만은 lean 에서도 남긴다. 지시문에 대사가 빠지면 모델이 자기가
//   지어낸 말을 하고 자막과 전혀 다른 영상이 나온다 — 그것이 이 함수가 생긴 이유다
//   (2026-08-19 실측). "지켜져야 하는 것은 프롬프트가 아니라 코드가 판정한다."
// ★ film 은 예전 그대로다 — lean 을 안 넘기면 글자 하나 안 바뀐다.
export function withSpokenLines(text, shots, voice, scenario, { lean = false } = {}) {
  ...
  const head = lean ? [] : [...extra, ...voiceLines, ...sayAsLines];
...
// ★ projectKind — "ad" 면 프롬프트 꼬리에 절을 하나도 안 붙인다(withSpokenLines 의 lean).
//   안 넘기면 예전 그대로다 — film 과 옛 테스트가 글자 그대로 안 바뀐다.
export function buildFalInput({ settings, scenario, refs = [], kind, seconds, projectKind }) {
  ...
  const input = {
    ...
    prompt: withSpokenLines(scenario?.text, scenario?.shots, scenario?.voice, scenario, { lean: projectKind === "ad" }),
    duration: seconds,
...
export async function submitAdVideo({ project, scenario, refs = [], fetchImpl = fetch }) {
  ...
  const input = buildFalInput({ settings, scenario, refs, kind, seconds, projectKind: project?.kind });
