...
import { falWebhookUrl } from "../fal-webhook.js";
...
export async function submitAdVideo({ project, scenario, refs = [], fetchImpl = fetch }) {
  ...
  const authHeaders = falHeaders();
  // ★★★ 2026-09-10 — 접수할 때 **우리 웹훅 주소를 실어 보낸다**(reel 과 같은 규율).
  //   fal 이 끝나는 순간 우리를 부르므로 크론 없이도 걷힌다 — 이 계정은 Hobby 라 1분 크론이
  //   배포를 거부한다. ★ 프로덕션 주소를 모르면 안 붙는다(lib/fal-webhook.js).
  //   ⚠️ 광고는 두 단(수거 → 마무리)이라 웹훅이 수거까지 끝내고, 무거운 마무리는 그 뒤
  //     회차가 이어받는다. 겹침은 finishAdRender 의 임대 잠금이 막는다.
  const hook = falWebhookUrl(project?.id);
  const submitRes = await fetchImpl(`https://queue.fal.run/${endpoint}${hook ? `?fal_webhook=${encodeURIComponent(hook)}` : ""}`, {
    method: "POST",
