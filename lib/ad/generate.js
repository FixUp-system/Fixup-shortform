...
export function withSpokenLines(text, shots, voice) {
  ...
  if (!missing.length) return voiceLines.length ? [base, ...voiceLines].join("\n") : base;
...
export async function submitAdVideo({ project, scenario, refs = [], fetchImpl = fetch }) {
  ...
  const input = {
    ...
    prompt: withSpokenLines(scenario?.text, scenario?.shots, scenario?.voice),
    duration: seconds,
