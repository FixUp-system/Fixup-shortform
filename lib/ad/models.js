...
export const AD_MODELS = [
  ...
  },
  {
    ...
    // ★★★ 2026-09-01 — **프로가 되었다**(사장님 지시). 08-31 에 숨겼던 것을 되돌린다.
    //   근거는 그날 하루의 실측이다: 2.5 는 **참조 이미지에 사람 얼굴이 있으면 아홉 번 전부
    //   거절**했고(형식·크기·장수·스타일·그리드 다섯 축), 2.0 은 **같은 판을 첫 시도에
    //   통과**시켰다(얼굴 넷짜리 몬스터 판 · 얼굴 넷짜리 브이로그 판).
    //   값도 싸고(15초 720p $4.55 대 $6.93) 빠르다(217초 대 396초).
    //   ⚠️ 08-28 결함 둘(캔 로고 폰트 · 바벨이 몸을 통과)은 **지워지지 않았다** — 다만
    //     "인물이 아예 안 되는 것"보다 가벼운 문제라고 판단한 것이다.
    label: "프로",
    name: "프로",
  ...
  //   같지만 **화질이 480P·768P 뿐이라 2K 가 없다** — 사장님이 고른 두 칸을 못 채운다.
  {
    ...
    // ★★★ 2026-09-01 — **고르는 자리에서 내렸다**(사장님 지시). 등급 이름을 반납한다 —
    //   이 이름이 보이는 곳은 이제 **이미 2.5 로 만든 옛 문서**뿐이라 모델 이름이 맞다.
    //   ★ `hidden` 은 **안 건다.** 그것은 "아무도 못 쓴다"라 옛 문서의 재굽기까지 막는다
    //     (08-31 에 2.0 을 숨겨 정확히 그 문제를 겪었다). 목록에서 빼는 것만으로 충분하다.
    // ★★★ **은퇴 축**(2026-09-01 신설). `hidden` 은 "아무도 못 쓴다"라 옛 문서의 재굽기까지
    //   막고, 등급은 "누가 쓸 수 있나"다. 그 둘로는 **"새로는 못 고르되 이미 만든 것은
    //   된다"**를 표현할 자리가 없었다 — 08-31 에 2.0 을 숨겼다가 그 문제를 겪었다.
    retired: true,
    label: "2.5",
    name: "Seedance 2.5",
...
// **은퇴한 모델인가** — 새로는 못 고르되 이미 만든 문서는 다시 구워진다(2026-09-01).
//
// ★ 축이 셋이 됐다: `hidden`(아무도 못 씀) · `retired`(새로만 못 씀) · 등급(누가 씀).
//   둘로는 "2.5 를 목록에서 내리되 옛 문서는 살린다"를 표현할 자리가 없었다.
export function isRetiredModel(id) {
  return AD_MODELS.find((m) => m.id === id)?.retired === true;
}
