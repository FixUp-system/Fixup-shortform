...
// ★★ **이 배열의 순서가 곧 칩 순서다**(2026-08-31 사장님 지시: "기본이 프로보다 앞에").
//   화면은 따로 정렬하지 않는다 — components/AdOptionTray.jsx 가 modelsForTier(...) 를
//   그대로 map 한다. 숨긴 모델(2.0)은 걸러지므로 순서 밖이다.
export const AD_MODELS = [
  {
    id: "seedance-2.0",
    // ★★★ **숨긴다**(2026-08-31 사장님 지시). 2026-08-28 실측에서 이 모델 480p 결과에
    //   결함 둘이 나왔다 — 캔 로고의 **폰트가 달라지고**, 운동 장면에서 **바벨이 몸을
    //   통과한다**. 더 고르지 않는다.
    // ★ `hidden` 은 이 저장소에서 **"아무도 못 쓴다"** 다(lib/tiers.js 머리말) — 등급·역할
    //   보다 강해서 관리자에게도 걸러진다. 그래서 **이미 2.0 으로 만든 광고는 다시 굽기가
    //   막힌다**(app/api/ads/[id]/render/route.js 의 tierAllowsModel). 빠져나갈 길은 있다 —
    //   그 문서의 모델을 기본(H3)으로 바꾸면 다시 구워진다.
    // ★★ **표에서 지우지는 않는다.** 지우면 이미 2.0 으로 만든 문서가 adVideoPrice 에서
    //   던져 화면째 죽는다(seedance-2.0-fast 가 정확히 그랬다). 보기·값 표시는 그대로다.
    hidden: true,
    // label 은 **칩에 쓰는 짧은 이름**이다(모델 묶음 안에 있어 "2.0" 만으로 읽힌다).
    // name 은 그 묶음 밖에서 쓰는 전체 이름 — 보관함 상세처럼 맥락이 없는 자리가 쓴다.
    // ★ 숨긴 모델이라 등급 이름(기본·프로)을 안 받는다 — 고르는 자리에 안 나오고,
    //   이 이름이 보이는 곳은 **이미 그것으로 만든 옛 문서**뿐이라 모델 이름이 맞다.
    label: "2.0",
    name: "Seedance 2.0",
    ...
    // ★★★ **참조 사진 규약**(2026-08-21 · 2026-08-28 재확인) — fal 스키마 원문 그대로다:
    //   "Refer to them in the prompt as **@Image1, @Image2**, etc. … Max 30 MB per image.
    //    **Up to 9 images.** Total files across all modalities must not exceed 12."
    //   즉 사진을 넘기는 것만으로는 부족하고 **지시문이 그 이름을 불러야** 쓴다.
    //   이 표가 없던 동안 Seedance 에는 그 지칭이 한 번도 안 실렸다 — 사진 4장을 올려도
    //   지시문은 그것을 한 번도 안 가리켰다(H3 만 우연히 넣어 두었다).
    refs: { field: "image_urls", max: 9, label: "@Image%n" },
    // ★★★ 2026-09-04 — 공식 토큰식에서 다시 뽑았다(lib/costs.js 의 같은 줄 주석 참고).
    //   $0.014/1k 토큰 · 토큰 = (h × w × 초 × 24)/1024. 그전 값은 약 0.3% 높았다.
    perSecUsd: { "480p": 0.13451, "720p": 0.3024, "1080p": 0.6804 },
  },
  ...
  //
  // ★★ **화질은 768P·2K 둘뿐이다**(2026-08-31 사장님 지시: "768이랑 2k만 제공"). 그전에는
  //   2K·4K 만 열려 있었다(480P·768P 는 안 열었다) — 즉 **확대본만 팔고 네이티브는 안 팔던**
  //   상태였다. 이제 네이티브 하나(768P)와 확대본 하나(2K)를 나란히 둔다.
  //   ⚠️ fal 스키마가 명시한다 — "480P and 768P are native generation modes;
  //     **2K and 4K upscale a 768P base result**". 즉 2K 는 768P 로 만든 뒤 확대한
  //     것이라 값은 오르지만 디테일의 출처는 768P 다. 알고 여는 것이다.
  //   ★ 4K·480P 는 **표에서 지우지 않는다**(perSecUsd·정가표) — 4K 로 이미 만든 문서가
  //     값 조회에서 죽지 않게. 고르는 목록(resolutions)에만 없다.
  //
  // ★ **h3-max 는 안 쓴다**(2026-08-31 확인). t2v·i2v·r2v 셋 다 있고 참조 사진도 9장으로
  //   같지만 **화질이 480P·768P 뿐이라 2K 가 없다** — 사장님이 고른 두 칸을 못 채운다.
  {
    id: "minimax-h3",
    // ★ 이름이 **등급**이다(2026-08-31) — 2.5 항목의 그 지시와 같은 줄기다.
    label: "기본",
    name: "기본",
    hint: "768P·2K · 15초 · 값이 가장 싸다",
    endpoints: {
    ...
    // 참조 사진 규약 — 필드 이름도 지칭 표기도 Seedance 와 다르다:
    //   "referenced in the prompt as **Image 1, Image 2**" · 필드는 reference_image_urls.
    //   상한은 스키마의 maxItems 가 **9** 다(2026-08-28 재확인). 설명은 "이미지·영상·오디오를
    //   합쳐 12개"라고 더 넓게 말하지만, 이미지 배열 자체의 상한은 9 다 — 좁은 쪽을 쓴다.
    refs: { field: "reference_image_urls", max: 9, label: "Image %n" },
    perSecUsd: { "480P": 0.05, "768P": 0.06, "2K": 0.13, "4K": 0.16 },
  },
  {
    id: "seedance-2.5",
    // ★★★ 이름이 **등급**이다(2026-08-31 사장님 지시) — 2026-08-13 결정을 뒤집는다.
    //   그때는 "기본/저가"를 걷고 fal 이름으로 맞췄는데(고른 것과 fal 이 받는 것이
    //   머릿속에서 이어지게), 그 사이 모델이 셋이 되고 계보가 다른 두 회사 것이 섞여
    //   ("2.0"·"2.5"·"H3") 이름이 **무엇이 달라지는지**를 더는 말해 주지 못했다.
    //   ★ 갈리는 걱정은 id 로 막는다 — 값·원장·fal 호출은 전부 id 로만 간다.
    label: "프로",
    name: "프로",
    ...
    seconds: [15, 30],
    // ★★★ 2026-09-03 — **실측으로 정정했다.** 15~30 이라고 적혀 있었는데 fal 스키마는
    //   `duration: enum ["auto","4","5",…,"30"] · type=string · "Supports 4 to 30 seconds"`
    //   다(OpenAPI 를 직접 받아 확인). 즉 **최소 15초는 모델의 한계가 아니라 우리가
    //   좁혀 둔 값이었다** — 이 항목에 "미검증"이라고 적혀 있던 그 자리가 실제로 틀렸다.
    //   ★ `seconds`(제품으로 노출하는 닫힌 목록)는 **안 건드린다** — 길이를 열려면 그
    //     길이의 정가가 AD_VIDEO_PRICE 에 있어야 하고(없으면 adVideoPrice 가 던진다),
    //     무엇을 팔지는 값을 정하는 결정이다. 여기서 고치는 것은 **사실 기록**뿐이다.
    //   ⚠️ `duration` 이 **문자열**이라는 것도 함께 알아 두어야 한다 — 우리 코드는 숫자를
    //     보내고 있고 2.0 에서는 통했지만, 스키마를 따르는 쪽이 안전하다.
    minSeconds: 4,
    maxSeconds: 30,
    ...
    // 참조 사진 규약 — 2.0 과 같은 계열이라 지칭도 같다(@Image1).
    // ★★★ **상한이 30 이다**(2026-08-28 fal 스키마 재확인). 2.0(9장)과 다르다.
    //   스키마의 `maxItems` 는 여전히 비어 있지만 **설명 원문이 숫자를 말한다**:
    //     "Max 30 MB per image. **Up to 30 images.** Total files across all modalities
    //      must not exceed 50."
    //   그전에는 maxItems 가 없다는 이유로 2.0 과 같은 9 로 좁혀 두었는데, 그 판단이
    //   **설명을 안 읽고 내린 것**이었다 — 모르는 상한은 좁게 보는 것이 맞지만, 이 값은
    //   모르는 것이 아니라 적혀 있었다.
    refs: { field: "image_urls", max: 30, label: "@Image%n" },
    perSecUsd: { "480p": 0.2056, "720p": 0.4622, "1080p": 1.04 },
  },
];

// **새로 만들 때**의 기본 모델. 2026-08-31 에 2.0 → 기본(H3)으로 옮겼다 — 2.0 을 숨겼으니
// 여기를 그대로 두면 새 광고가 **아무도 못 쓰는 모델**로 접수돼 그 자리에서 403 이 난다.
export const DEFAULT_AD_MODEL = "minimax-h3";

// ★★ 모델을 **안 든 옛 문서**가 떨어질 자리. **기본값과 다른 축이다.**
//
// 없는 것 = 모델 칸이 생기기 전에 만들어진 문서 = 그때 유일한 모델이던 2.0 으로 만들어졌다.
// 여기를 DEFAULT_AD_MODEL 로 두면 그 문서들이 **조용히 H3 로 읽혀** 값이 딴 표에서 조회되고
// (H3 표에는 720p 가 아예 없어 adVideoPrice 가 던진다) 화면째 죽는다.
// ★ lib/clip-limits.js 의 DEFAULT_I2V_MODEL ↔ LEGACY_I2V_MODEL 과 **같은 규율**이다.
//   해상도도 이미 같은 모양으로 갈라 두었다(DEFAULT_AD_RESOLUTION ↔ adDefaultResolution).
export const LEGACY_AD_MODEL = "seedance-2.0";
...
// 모르는 id 는 **옛 문서의 모델**이다(LEGACY_AD_MODEL) — 새로 만들 때의 기본이 아니다.
// 값을 안 든 문서는 그 모델로 만들어졌으므로, 기본이 옮겨 가도 옛 문서의 해석은 안 바뀐다.
// (문서에는 만들 때 명시 저장하므로 정상 흐름에서는 여기 안 온다.)
export function adModel(id) {
  return AD_MODELS.find((m) => m.id === id) || AD_MODELS.find((m) => m.id === LEGACY_AD_MODEL);
}
...
// ★ 2026-09-16 — 보관함 카드·상세의 **모델 배지**용. adModel() 과 달리 **추측하지 않는다**
//   — 값이 없거나 모르면 null 이다. adModel() 의 LEGACY_AD_MODEL 폴백은 "옛 문서가 fal 을
//   부를 때 무엇으로 볼까"를 위한 것이라 값이 걸린 자리에는 맞지만, 화면에 "이건 2.0 으로
//   만들었어요"라고 **추측해서 말하면** 안 된다(reel 쪽과 같은 규율, lib/clip-limits.js 의
//   i2vModelLabelOf 참고).
export function adModelLabelOf(id) {
  return AD_MODELS.find((m) => m.id === id)?.label || null;
}
...
// ── 참조 사진(2026-08-21) ──────────────────────────────────────────────
// 모델마다 **필드 이름·지칭 표기·상한**이 다르다. 세 값을 표에 두고 코드가 읽는다 —
// 그전에는 generate.js 가 `id 가 minimax 로 시작하나`로 갈랐고, 지칭은 지문에 손으로
// 적혀 있었다. 모델이 늘 때마다 그 두 곳을 같이 고쳐야 했고, 실제로 Seedance 쪽 지칭이
// 통째로 빠져 있었다.

// 이 모델이 참조 사진을 몇 장까지 받나.
export function adRefMax(modelId) {
  return adModel(modelId).refs?.max ?? 0;
}

// fal 이 참조 사진을 받는 **파라미터 이름**.
export function adRefField(modelId) {
  return adModel(modelId).refs?.field || "image_urls";
}

// 지시문이 n 번째 사진을 부르는 **이름**. Seedance 는 "@Image1", H3 는 "Image 1" 이다.
// ★ 표기를 여기 한 곳에서 만든다 — 지문이 손으로 적으면 모델이 늘 때 그 자리가 낡는다.
export function adRefLabel(modelId, n) {
  return String(adModel(modelId).refs?.label || "@Image%n").replace("%n", String(n));
}
