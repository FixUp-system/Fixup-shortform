...
export const AD_MODELS = [
  ...
  },
  {
    ...
    // ★ 화면에서 숨긴다(2026-08-13 사용자 결정 — 원가가 크다). **지우지 않는 이유**:
    //   지우면 이미 2.5 로 만든 문서가 adVideoPrice 에서 던져 화면째 죽는다(오늘
    //   seedance-2.0-fast 가 정확히 그랬다). 고르는 길만 닫고 값·엔드포인트·검증은
    //   살려 둔다 — 옛 문서는 계속 열리고 계속 만들 수 있어야 한다.
    //   다시 열려면 이 줄만 지우면 된다.
    hidden: true,
