...
//
// ★ Task 24 — 기본을 fast → standard 로 바로잡는다. 처음 모델을 정할 때 "원가가 싸다"는
...
export const AD_MODELS = [
  ...
  },
  {
    ...
    // 720p fast — fal 대시보드 실측(이전 태스크). 해상도별 문서가 따로 없어 480p 도
    // 이 단일가를 그대로 쓴다(실제 480p 원가가 더 싸더라도 비싼 쪽으로 잡는 것이
    // 안전하다 — 적게 잡으면 예산 가드가 뚫린다). 원가표(lib/costs.js)와 **같은 값이어야
    // 한다** — 갈리면 예산 가드가 틀린다.
    perSecUsd: 0.2419,
  },
  {
    id: "seedance-2.5",
    label: "2.5",
    hint: "720p까지 · 15·30초 · 네이티브 오디오 · 멀티샷",
