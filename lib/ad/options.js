...
export const AD_STYLE_LINES = {
  // ★ 2026-09-02 사장님 지시 — 실사에는 "초현실적/자연스러운 연출" 키워드를 싣는다.
  //   natural 은 film 줄이 이미 쓰고 있어(soft natural light) 겹침 판정에 걸린다 —
  //   같은 뜻의 lifelike 로 적는다(tests/ad-options.test.js 가 실사 계열 겹침을 잰다).
  // ★★★ 2026-09-03 — **이 줄은 원클릭·film 이 쓰는 값이다.** 단계별은 여기서
  //   `hyper-realistic detail` 을 빼고 쓴다(아래 reelStyleLine) — 사장님 지시로 흐름이
  //   갈렸다: *"단계별 영상에서는 일단 제거하고 원클릭은 그대로 두자."*
  photo: "live-action cinematic footage, hyper-realistic detail, lifelike movement, realistic lighting, shallow depth of field",
...
//
// **왜 흐름마다 다른가.** 단계별은 스토리보드 **판 한 장**을 참조로 넘겨 굽는다(r2v).
// 그 판에 사람 얼굴이 사진처럼 그려지면 fal 이 초상 정책으로 거절한다 — 실측(09-02,
// 프로젝트 00b1885a)에서 6칸 중 3칸에 모델 얼굴이 크게 그려졌고 굽기가 422 로 죽었다.
// "실제와 구별 안 되게 그려라"는 요구는 그 방향을 **더 밀어붙이는** 낱말이라 뺀다.
// 원클릭은 그 자리에서 겪는 문제가 아니므로 사장님이 그대로 두라고 했다.
//
// ⚠️ **이 낱말이 거절의 원인이라는 확증은 없다** — 09-03 실측에서 이것을 완전히 빼고
//   같은 판으로 다시 구웠는데 **똑같이 422** 였다(값 $0, billable 0). 즉 뿌리는 판에
//   그려진 얼굴이지 이 낱말이 아니다. 그래도 빼 두는 것은 사장님 지시이고, 단계별에서
//   얼굴을 더 사실적으로 밀 이유가 없기 때문이다.
//
// ★ 표를 복사하지 않는다 — 위 표 하나가 원천이고 여기는 **차이만** 든다.
//   복사하면 화풍이 늘 때 한쪽이 낡는다(이 저장소가 반복해 겪은 자리다).
export const REEL_STYLE_OMIT = {
  photo: ["hyper-realistic detail"],
};

// 단계별이 쓸 화풍 줄. 원클릭·film 은 AD_STYLE_LINES 를 그대로 쓴다.
//
// ★ 앞의 쉼표까지 함께 걷는다 — 낱말만 빼면 ", , " 가 남아 지문이 지저분해진다.
// ★ 모르는 화풍은 **그대로 돌려준다**(빼는 목록이 없으면 원문) — 모르면 안 건드린다.
export function reelStyleLine(styleId) {
  const line = AD_STYLE_LINES[styleId];
  if (typeof line !== "string") return line;
  const omit = REEL_STYLE_OMIT[styleId];
  if (!omit?.length) return line;
  return omit.reduce((s, word) => {
    const esc = word.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    return s.replace(new RegExp(`\\s*,\\s*${esc}|${esc}\\s*,\\s*`, "gi"), "");
  }, line);
}
