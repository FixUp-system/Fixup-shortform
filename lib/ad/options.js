...
export const AD_STYLE_LINES = {
  ...
  // ★★★ 2026-09-03 — 이 줄을 단계별에서만 뺐다가 **같은 날 되돌렸다**(사장님 지시).
  //   뺄 이유였던 초상 거절이 격자 방법으로 풀렸다 — 아래 "흐름별 차이" 절 참고.
  //   지금은 원클릭·film·단계별이 **전부 이 값 하나**를 쓴다.
...
// ── 흐름별 차이 — 지금은 **없다** ────────────────────────────────────────────
//
// ★★★ 2026-09-03 — 하루 안에 두 번 뒤집힌 자리다. 남겨 두는 이유는 다음 세션이 같은
//   길을 다시 걷지 않게 하기 위해서다.
//
//   ① 아침: 단계별 굽기가 초상 정책으로 계속 거절돼, 사장님이 `hyper-realistic detail`
//      을 의심하고 뺐다. 곧 "원클릭은 그대로 두자"고 해서 흐름을 갈랐다(REEL_STYLE_OMIT).
//   ② 낮: **실측으로 반증됐다** — 그 낱말을 완전히 빼고 같은 판으로 다시 구웠는데
//      똑같이 422 였다($0, billable 0). 거절 위치가 `body.image_urls` 라 애초에 글이
//      아니라 **그림**을 가리키고 있었다.
//   ③ 그 뒤: **판에 흰 격자를 덧그리는 방법이 통했다**(사장님이 알려 준 방법 —
//      6×6 · 굵기 12 · 불투명 100% + 프롬프트에 격자 억제 힌트). 4초 480p 와
//      15초 720p 두 번 다 통과했고 출력물에 격자 흔적도 없었다.
//   ④ 그래서 낱말을 뺄 이유가 사라졌다 — 사장님 지시로 **원래대로 되돌렸다.**
//      단계별도 원클릭과 **같은 표**를 쓴다.
//
// ★ 다시 흐름을 가르게 되면 그때 이 자리에 차이만 든다(표는 복사하지 않는다).
//   그전에 §거절의 뿌리를 다시 읽어라 — 글이 아니라 판이다.
