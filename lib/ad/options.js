...
export const AD_STYLE_LINES = {
  // ★★ 2026-09-03 사장님 지시 — **`hyper-realistic detail` 을 도로 뺐다.** 전날(09-02)
  //   같은 사장님 지시로 넣었던 낱말이고, 뺀 이유는 **굽기가 계속 거절되기 때문**이다
  //   (2.0 에서도 났다). "실제와 구별 안 되게 그려라"는 요구가 초상·실인물 정책 쪽으로
  //   읽히는 것이 사장님 가설이고, 지금 코드에서 그 뜻에 가장 가까운 낱말이 이것이었다.
  //   ⚠️ **확증은 아니다** — 거절 뒤 문서에 남은 오류가 없어 이 낱말이 원인이라고 못 박을
  //   근거가 없다. 그래도 뺀 것은 전날 넣기 전에는 거절이 없었고, 뺐을 때 잃는 것이
  //   작아서다(아래 lifelike 줄이 같은 축을 더 약하게 이미 말한다).
  //   ★ 다시 거절이 나면 다음 후보는 **`lifelike movement`** 다 — 같은 축의 남은 낱말이다.
  //   ★ natural 은 film 줄이 이미 쓰고 있어(soft natural light) 겹침 판정에 걸린다 —
  //     그래서 그 자리는 계속 lifelike 로 적는다(tests/ad-options.test.js).
  photo: "live-action cinematic footage, lifelike movement, realistic lighting, shallow depth of field",
