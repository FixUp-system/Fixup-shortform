...
// 나레이션 목소리 — **사장님이 고르는 축**(2026-08-24).
//
// ★★ 왜 생겼나: 지문이 목소리를 정하게 두었더니 **6건 중 5건이 "20대 후반 한국 여성"**
//   이었다. 지문의 ✓ 예시("a warm, unhurried woman in her late twenties…")를 모델이
//   글자 그대로 베낀 것이다 — 이 저장소가 옷차림에서 이미 한 번 겪은 사고와 같다(4cf7af0).
//
// ★★ 그리고 실측으로 **모델은 목소리를 바꿀 수 있다**는 것이 확인됐다
//   (scripts/measure/probe-voice.mjs, 2026-08-24): 같은 장면·같은 대사에 정반대 목소리를
//   시키니 F0 가 **78.8Hz vs 307.7Hz — 3.9배** 갈렸다. 즉 못 하는 게 아니라 안 시킨 것이었다.
//
// ★ 목소리는 **취향**이라 AI 가 정할 이유가 없다. 사장님이 고른다.
//   기본은 `auto` — 안 고르면 예전처럼 시나리오가 정한다(옛 문서도 그대로 흐른다).
//   톤(밝게/차분히)은 여기 안 담는다 — 그것은 이미 **분위기(AD_MOODS)** 축이다.
//   여기서 또 받으면 두 축이 같은 것을 말하고, 어긋날 때 어느 쪽이 이기는지 아무도 모른다.
//
// ⚠️ `line` 은 fal 프롬프트에 **그대로 실린다**(lib/ad/generate.js 의 `Voice:` 절).
//   영어로 적는 이유가 그것이다.
export const AD_VOICES = [
  { id: "auto", label: "자동", line: "" },
  { id: "f20", label: "20대 여성", line: "a woman in her twenties, bright and clear, natural conversational pace, close-mic" },
  { id: "f30", label: "30대 여성", line: "a woman in her thirties, warm and composed, unhurried, close-mic" },
  { id: "f50", label: "50대 여성", line: "a woman in her fifties, seasoned and gentle, slow and steady, close-mic" },
  { id: "m20", label: "20대 남성", line: "a man in his twenties, bright and energetic, quick and clear, close-mic" },
  { id: "m30", label: "30대 남성", line: "a man in his thirties, calm and steady, low and clear, close-mic" },
  { id: "m50", label: "50대 남성", line: "a man in his fifties, deep and seasoned, slow and weighty, close-mic" },
];
...
export function normalizeAdOptions(input) {
  ...
  return {
    ...
    // ★ 다른 축과 **같은 방식으로** 실패시킨다 — 모르는 값은 조용히 기본값으로 안 떨어뜨린다.
    voice_style: pick(AD_VOICES, "voice_style", DEFAULT_AD_OPTIONS.voice_style),
