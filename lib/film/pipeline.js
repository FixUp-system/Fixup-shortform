...
import { imagePlanFor, attachClauseFor, filmMode, isFilmMode } from "./mode.js";
...
export async function startFilmRender(projectId, ownerId, mode, deps = {}) {
  ...
  // ★★ 어느 청구 회차로 이 편을 샀는가. 라우트가 청구 직후에 읽어 넘긴다
  //   (lib/charges.js 의 activeAdAttempt). 이것을 안 적어 두면 수거 실패 때
  //   refundAd 가 "살아 있는 **마지막** 회차"를 되돌리는데, 이 경로는 한 프로젝트에서
  //   두 편을 굽는다 — order 가 실패했는데 refs 의 값이 돌아가는 사고가 난다.
...
//   그 라우트의 정규식이 이 `-<방식>` 갈래를 함께 받도록 넓혀 두었고, 방식 목록은
//   lib/film/mode.js 의 FILM_MODES 한 벌에서 읽는다(손으로 두 벌 적으면 갈린다).
...
export async function collectFilmRender(projectId, ownerId, mode, deps = {}) {
  ...
  const store = deps.storeVideo || storeFilmVideoDefault;
  const now = deps.now || Date.now;
  ...
  try {
    ...
    await updateProject(projectId, ownerId, (p) =>
      putFilm(p, mode, {
        ...
        status: "done",
        video: { url, seconds: got.seconds, rawUrl, subtitled },
        job: null, error: null,
      }));
