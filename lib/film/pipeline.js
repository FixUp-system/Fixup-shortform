...
//
// ★ 새 장치를 만들지 않는다. 시나리오는 lib/film/scenario.js, 이미지는 lib/imagegen.js,
...
import { adRenderTimeoutMs, adGiveUpMs } from "../ad/timing.js";
// 일시 오류와 확정 실패를 가른다 — 접수증을 지킬지 지울지가 여기서 갈린다(광고·reel 과 같은 함수).
...
// ★★★ 2026-08-27 — lib/ad/generate.js 의 withSpokenLines 안에 있던 **일곱 절**을 여기로
// 옮겼다. 광고가 "Fable 이 영상 프롬프트 한 편을 쓰고 코드는 그대로 넘긴다"로 바뀌면서
// 그 값들이 전부 프롬프트 안으로 들어갔고, 광고 쪽에서는 꼬리에 또 붙이면 안 되게 됐다.
// film 은 여전히 칸으로 받아 쓰므로 그 조립을 이쪽이 맡는다.
//
// ★ 옮기면서 고치지 않았다 — 절의 내용과 순서가 글자 그대로 예전과 같다.
// ★ 덤으로 **두 번 붙던 것이 한 번이 됐다**: 그전에는 buildFilmPrompt 가 withSpokenLines 를
//   부르고, 그 결과를 받은 submitAdVideo 가 **또** 불러서 같은 절이 두 벌 나갔다.
//   이제 절은 여기서 한 번만 붙고, withSpokenLines 는 대사 보강만 한다(두 번 불려도 무해).
export function filmClauses(scenario) {
  // ★ 옛 계약에서는 이 둘이 withSpokenLines 의 **인자**였다. 옮겨 오면서 scenario 하나에서
  //   꺼낸다 — 같은 문서에서 온 값이라 뜻은 그대로다.
...
export function buildFilmPrompt(scenario, mode, images) {
  ...
  // ★ 절을 먼저 붙이고, 그 다음에 대사가 빠졌는지 본다 — 순서가 바뀌면 절 안의 글자를
  //   대사로 오인할 수 있다.
  return withSpokenLines([withAttach, filmClauses(scenario)].filter(Boolean).join("\n"), scenario?.shots);
}
...
export async function collectFilmRender(projectId, ownerId, mode, deps = {}) {
  ...
  // 이미 "끝났다"고 적어 둔 방식은 fal 에 다시 묻지 않는다 — 남은 것은 마무리뿐이다
  ...
  try {
    ...
    if (!got.done) {
      ...
      // ★★★ 2026-09-10 — 상한이 **두 단**이 됐다(광고와 같은 규율 · lib/ad/timing.js).
      //   그전에는 대기 상한을 넘기면 곧바로 던져 failFilmRender 로 갔다 — 다 만들어진
      //   영상을 버리고 환불하는 자리다. 옛 상한(15초=780초)은 실측 분포 안쪽이었다
      //   (프로덕션 광고 실물 895초). 이제 대기 상한은 **알리기만** 하고, 포기는 훨씬 뒤다.
      ...
      if (waited >= adRenderTimeoutMs(job.seconds)) {
        // ★ status 는 그대로 rendering 이다 — 아직 굽는 중이고, 화면은 이 문구를 읽기만 한다.
        await updateProject(projectId, ownerId, (p) => {
          const j = filmOf(p, mode).job;
          return j ? putFilm(p, mode, { error: "영상이 예상보다 오래 걸리고 있어요 — 끝나는 대로 보관함에 담깁니다" }) : p;
        }).catch(() => {});
    ...
    //   광고(lib/ad/pipeline.js)가 이 자리에서 내려받기·저장·자막까지 하다가 프로덕션이
    //   죽었다 — 60초짜리 상태 라우트가 2초마다 들어오는데 무거운 일이 겹쳐 인스턴스가
    //   메모리 초과로 죽었고, 마무리를 아무도 못 끝내니 문서가 영영 rendering 이었다.
    //   ★ 필름은 상태 라우트가 **방식마다** 이 함수를 부르므로 그때 두 배로 무거웠다.
    await updateProject(projectId, ownerId, (p) => {
      const j = filmOf(p, mode).job;
      return j ? putFilm(p, mode, { job: { ...j, ready: { url: got.url, seconds: got.seconds } } }) : p;
    });
    return { changed: true, ready: true };
  } catch (e) {
    // ★★★ 2026-09-10 — **일시 오류는 접수증을 지킨다**(광고·reel 과 같은 규율).
    //   네트워크가 한 번 흔들리는 것과 fal 이 "안 만든다"고 답하는 것은 다른 일이다.
    //   fal 에 요청 목록 API 가 없어(09-10 실측) 한 번 지운 접수증은 되찾을 수 없다.
    //   ★ 필름은 방식 둘이 나란히 굽으므로 흔들림 한 번이 **두 편**을 버릴 수 있다.
...
// 광고와 같은 값·같은 이유다(lib/ad/pipeline.js 의 FINISH_LEASE_MS 주석 참고):
// 잠금을 쥔 인스턴스가 죽는 것이 바로 이 사고였으므로 수명이 없으면 그 편이 영영 안 끝난다.
const FINISH_LEASE_MS = 6 * 60_000;

// ★★ 마무리 — 무거운 일(내려받기 · 저장 · 자막 굽기)은 **잠금을 쥔 하나만** 한다.
//
// ★ 잠금은 **방식별**이다(job 안에 산다). 프로젝트 하나로 두 편을 굽는 것이 이 경로의
//   정상 흐름이라, 잠금을 프로젝트 단위로 두면 한 편이 마무리되는 동안 다른 편이 막힌다.
// 부르는 자리는 전용 라우트(app/api/film/[id]/finish/route.js, 상한 300초)다.
export async function finishFilmRender(projectId, ownerId, mode, deps = {}) {
  if (!isFilmMode(mode)) return { nothing: true };

  const store = deps.storeFilmVideo || storeFilmVideoDefault;
  const now = deps.now || Date.now;
  // 이겼는지는 **되돌아온 문서**로 판정한다 — patchFn 은 CAS 에 지면 다시 불리므로
  // 그 안에서 세운 깃발은 못 믿는다(lib/projects.js).
  const token = deps.token || randomUUID();

  const project = await getProject(projectId, ownerId).catch(() => null);
  if (!project) return { nothing: true };
  const before = filmOf(project, mode);
  if (!before.job?.ready || before.status !== "rendering") return { nothing: true };

  const doc = await updateProject(projectId, ownerId, (p) => {
    const job = filmOf(p, mode).job;
    ...
    return putFilm(p, mode, { job: { ...job, finish: { token, at: now() } } });
  });
  const job = filmOf(doc, mode).job;
  if (job?.finish?.token !== token) return { skipped: true };

  try {
    const rawUrl = await store(job.ready.url, projectId, mode);
    const { url, subtitled } = await burnFilmSubtitles(projectId, mode, doc, rawUrl, deps);
    await updateProject(projectId, ownerId, (p) =>
      putFilm(p, mode, {
        ...
        video: { url, seconds: job.ready.seconds, rawUrl, subtitled, ts: now() },
        job: null, error: null,
      }));
    return { finished: true };
  } catch (e) {
    await failFilmRender(projectId, ownerId, mode, job, e);
    return { error: e?.message || "영상을 만들지 못했어요" };
  }
}
