// film 시나리오 — "한 번에 굽는 영상"(/film) 전용.
//
// ★★★ 2026-08-27 에 lib/ad/scenario.js 에서 **통째로 갈라 나왔다.**
//
// 그전에는 광고와 film 이 한 파일에서 지문·스키마·검증을 나눠 썼다. 두 흐름이 같은
// 모양이던 동안에는 그것이 맞았는데, 광고가 "Fable 이 영상 프롬프트 한 편을 쓰고 코드는
// 그대로 넘긴다"로 뒤집히면서 요구하는 것이 완전히 달라졌다:
//
//   광고 : text · angle · shots(beat·line·seconds)         ← 칸 셋
//   film : + shows · avatar_id · transition · speaker · say_as · camera · lighting ·
//          action · sound · focus · voice · music · tone · look · cast · wardrobe ·
//          environment · endpoint                          ← 그림 계획이 읽는 값들
//
// 한 파일에 두면 광고 지문을 손볼 때마다 film 조각을 피해 다녀야 하고, 실제로 그러다가
// film 전용 줄이 광고 지문에 섞여 나간 적이 있다(2026-08-21 · 08-27 두 번). 그래서 나눈다.
//
// ★ 여기 코드는 **글자 그대로 옮겨 온 것**이다 — 옮기면서 고치지 않았다. film 결과가
//   이 분리 때문에 달라지면 안 된다(tests/film-*.test.js 가 그것을 잰다).
// ★ 광고와 진짜로 공유하는 것만 lib/ad/ 에서 가져온다: Fable 호출(llm.js) · 옵션 표
//   (options.js) · 참조 사진 호칭(models.js) · fal 큐 배선(generate.js).
...
// film 시나리오가 요구하는 JSON 모양 — 이 파일의 systemFor() 가 묻는
...
// ★ 광고는 2026-08-27 에 이 분기를 걷었다(사진 수만 본다). film 은 그대로 둔다 —
//   그쪽은 우리가 만든 그림을 넘기는 흐름이라 판단의 근거가 다르다.
...
★ **목소리도 함께 정한다**(voice). 나레이션을 누가 어떤 톤으로 읽는가를 **영어 한 줄**로
적는다. 영상 모델이 읽는 글이라 한국어를 쓰지 않는다.
  · 적을 것 **다섯**: 성별 · 나이대 · 음색 · 속도 · 거리감(가까이/멀리).
  · ★★ **소재에 맞는 사람을 고른다.** 습관처럼 20대 여성을 고르지 마라 —
    누가 읽어야 이 이야기가 믿기는지로 정한다(장인의 이야기라면 그 나이대의 목소리다).
...
export function buildScenarioMessages({ settings, material, edits, globalEdits }) {
  ...
  return { system: systemFor(), messages: [{ role: "user", content: user.join("\n") }] };
}
...
export async function generateScenario({ project, edits, globalEdits, deps = {} }) {
  ...
  const raw = await call({ system, messages, stage: "광고 시나리오", projectId: project.id, schema: SCENARIO_SCHEMA });
