...
export function filmGates(film, busy = false) {
  ...
  // ★ 옛 응답(drawing 없음)의 폴백 — **triesLeft 를 안 본다.** 예전에는 `!canDraw && !triesGone`
  //   이었는데, 그러면 문서에 triesLeft 만 실리고 drawing 이 빠지는 응답이 하나 생기는 순간
  //   위 함정이 그대로 되살아난다(마지막 회차가 도는 중에 굽기가 열린다).
  //   그렇다고 `!canDraw` 하나로 두면 반대쪽 막다른 길이 생긴다: 횟수를 다 쓰고 **멈춘**
  //   프로젝트가 canDraw:false 라 영영 못 굽는다. 그래서 **status 를 함께 본다** —
  //   "그리는 중"은 status 가 말하고, 만료는 canDraw 가 걷어낸다. 둘 다 지나야 참이다.
  const drawingNow = typeof f.drawing === "boolean" ? f.drawing : (!canDraw && status === "drawing");
