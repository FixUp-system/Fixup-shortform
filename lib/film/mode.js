...
export function imagePlanFor(mode, scenario, opts = {}) {
  ...
  // 얼굴이 붙는 축에만 옷차림이 따라간다 — 둘은 한 몸이다(얼굴만 같고 옷이 바뀌면 소용없다).
  const wearIf = (id) => (id ? WEAR : "");
  ...
  if (focus === "person") {
    ...
    axes = [
      ...
      { key: "subject", avatarId: faceOf(firstSource), prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${STAGE}${TONE}${LOOK}${CTX}${wearIf(faceOf(firstSource))}` },
    ];
  } else if (focus === "product") {
    axes = [
      { key: "subject", avatarId: faceOf(firstSource), prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${STAGE}${TONE}${LOOK}${CTX}${wearIf(faceOf(firstSource))}` },
      { key: "subject-in-use", avatarId: faceOf(lastSource), prompt: `The same object being used or held, close-up, in: ${last}. ${NO_TEXT}${STAGE}${TONE}${LOOK}${CTX}${wearIf(faceOf(lastSource))}` },
      { key: "place", prompt: `The setting of: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${TONE}${LOOK}${CTX}` },
    ];
  } else {
