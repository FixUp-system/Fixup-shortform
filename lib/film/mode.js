...
export function imagePlanFor(mode, scenario, opts = {}) {
  ...
  if (mode === "order") {
    ...
    return shots.map((s, i) => ({
      ...
      prompt: `${s.shows || s.beat || ""}. ${NO_TEXT}${STAGE}${CTX}${s.avatar_id ? WEAR : ""}${
  ...
  if (focus === "person") {
    return [
      { key: "person", avatarId: firstAvatar, prompt: `A head-and-shoulders portrait of the main person of this video. ${PERSON_ONLY}. ${NO_TEXT}${STAGE}${CTX}${WEAR}` },
      { key: "person-full", avatarId: firstAvatar, prompt: `A full-body shot of the same person, standing, showing their outfit. ${PERSON_ONLY}. ${NO_TEXT}${STAGE}${CTX}${WEAR}` },
      { key: "place", prompt: `The setting of: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${CTX}` },
    ];
  }

  if (focus === "place") {
    return [
      { key: "place", prompt: `A wide establishing shot of the place in: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${CTX}` },
      { key: "place-detail", prompt: `A close detail of the same place in: ${first}. ${PLACE_ONLY}. ${NO_TEXT}${CTX}` },
      { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${CTX}` },
    ];
  }
  ...
  if (focus === "product") {
    return [
      { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${CTX}` },
      { key: "subject-in-use", prompt: `The same object being used or held, close-up, in: ${last}. ${NO_TEXT}${CTX}` },
      { key: "place", prompt: `The setting of: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${CTX}` },
    ];
  }
  ...
  return [
    { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${CTX}` },
    { key: "person", prompt: `A portrait of the person in: ${last}. ${NO_TEXT}${CTX}` },
    { key: "place", prompt: `The place, empty of people, in: ${all}. ${NO_TEXT}${CTX}` },
  ];
}
