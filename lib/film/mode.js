...
export function imagePlanFor(mode, scenario, opts = {}) {
  ...
  const PERSON_PROMPT = `A head-and-shoulders portrait of the main person of this video. ${PERSON_ONLY}. ${NO_TEXT}${STAGE}${CTX}${WEAR}`;
  ...
  if (focus === "person") {
    axes = [
      ...
      { key: "place", prompt: `The setting of: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${CTX}` },
    ];
  } else if (focus === "place") {
    axes = [
      { key: "place", prompt: `A wide establishing shot of the place in: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${CTX}` },
      { key: "place-detail", prompt: `A close detail of the same place in: ${first}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${CTX}` },
      { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${STAGE}${CTX}` },
    ];
  } else if (focus === "product") {
    axes = [
      { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${STAGE}${CTX}` },
      { key: "subject-in-use", prompt: `The same object being used or held, close-up, in: ${last}. ${NO_TEXT}${STAGE}${CTX}` },
      { key: "place", prompt: `The setting of: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${CTX}` },
    ];
  } else {
    ...
    axes = [
      { key: "subject", prompt: `A clean product shot of the main object in: ${first}. ${NO_TEXT}${STAGE}${CTX}` },
      { key: "person", avatarId: firstAvatar, prompt: PERSON_PROMPT },
      { key: "place", prompt: `The place, empty of people, in: ${placeSource}. ${PLACE_ONLY}. ${NO_TEXT}${STAGE}${CTX}` },
    ];
  }
