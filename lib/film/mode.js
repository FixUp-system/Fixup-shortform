...
export function imagePlanFor(mode, scenario, opts = {}) {
  ...
  if (mode === "order") {
    ...
    return shots.map((s, i) => ({
      key: `shot-${i + 1}`,
      prompt: `${s.shows || s.beat || ""}. ${NO_TEXT}${CTX}`.trim(),
  ...
  if (focus === "person") {
    return [
      { key: "person", avatarId: firstAvatar, prompt: `A head-and-shoulders portrait of the main person of this video. ${PERSON_ONLY}. ${NO_TEXT}${CTX}` },
      { key: "person-full", avatarId: firstAvatar, prompt: `A full-body shot of the same person, standing, showing their outfit. ${PERSON_ONLY}. ${NO_TEXT}${CTX}` },
