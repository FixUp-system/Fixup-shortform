...
export async function generateScript(project, id, { instruction, llm = callJson } = {}) {
  ...
  for (let round = 1; round <= 3; round++) {
    ...
    let rewritten = null;
    try {
      rewritten = validateScript(await llm({ system: rewrite.system, messages: rewrite.messages, stage: "대본 되돌리기", projectId: id }));
    } catch (e) {
      // 예산 오류는 초안을 안고 조용히 끝내면 안 된다 — 위와 같은 이유다.
      if (e instanceof BudgetExceeded) throw e;
      console.error(`${tag} ${round}회차 호출 실패:`, e);
      break;
    }
    if (!rewritten) {
      // 유료 호출이라 무한 재시도는 안 된다 — 라운드당 딱 한 번만 다시 시도한다.
      console.log(`${tag} ${round}회차 스키마 거절 → 재시도`);
      try {
        rewritten = validateScript(await llm({ system: rewrite.system, messages: rewrite.messages, stage: "대본 되돌리기", projectId: id }));
      } catch (e) {
        if (e instanceof BudgetExceeded) throw e;
        console.error(`${tag} ${round}회차 재시도 호출 실패:`, e);
        rewritten = null;
      }
      if (!rewritten) {
        // 이 라운드만 버린다 — 라운드 하나가 스키마를 못 지켰다고 남은 라운드까지 포기하지 않는다.
        console.log(`${tag} ${round}회차 스키마 거절 2회 → 이 라운드 버림`);
        continue;
      }
    }
