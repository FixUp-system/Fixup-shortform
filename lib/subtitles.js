// 자막 — 컷 경계가 곧 자막 경계다.
//
// ★ 값이 둘이다(2026-08-14). `cut.seconds` 는 **이 컷이 화면에 있는 시간**이고
//   `cut.spoken_seconds` 는 **말하는 시간**이다. 그 차이가 여백이다.
//   컷 사이 누적(자막이 언제 뜨는가)은 화면 시간으로 하고, 자막이 머무는 시간은
//   말하는 시간이다 — 여백에는 자막이 없다.
//   (예전에는 cut.seconds 가 ③목소리에서 실측된 낭독 길이 하나뿐이었다. 그 문서는
//    spoken_seconds 가 없어 둘이 같은 값이 되므로 지금도 예전과 똑같이 흐른다.)
//
// 합성이 화면 시간을 그대로 쓰므로(소리 파일이 있으면 클립을 그 길이로 tpad·trim 한다)
// 자막이 뜨는 자리도 화면 시간으로 누적하면 맞는다. 예전에는 둘이 갈라져 있어
...
// 언어가 폰트를 정한다. subtitle-langs.js 는 import 가 없는 순수 데이터라 이 파일의
// "화면이 읽어도 fs 가 안 섞인다"는 성질이 유지된다(lib/steps.js 머리말 참고).
...
//
// 소리 파일이 있으면 합성이 클립을 **주문한 화면 시간**에 맞춰 자르므로(lib/compose.js 의
// `fit` 이 wantSeconds = cut.seconds 를 본다) 그 값이 기준이다. i2v 눈금 올림으로 클립이
// 그보다 긴 것이 보통이고, 예전에는 그 차이가 그대로 무음이었다(30초 요청에 완성본 32.8초,
// 정적 4.8초).
//
...
export function cutSeconds(cut) {
  ...
  // ★ 이 값은 낭독이 아니라 **화면 시간**이다(2026-08-14). 말하는 시간은 spoken_seconds 다.
  //   이름을 spoken 으로 두면 이 파일을 읽는 다음 사람이 여백을 못 본다.
  const screen = Number(cut?.seconds) || 0;
  return cut?.audio?.url ? (screen || clip) : (clip || screen);
}
...
// 이 컷의 화면에 박힐 **글자**. 언어가 정한다.
//
// ★ 낡은 번역은 쓰지 않는다 — 원고를 고쳤는데 옛 글자를 구우면 화면과 말이 어긋난다.
//   그때는 원문(한국어)으로 떨어진다. 두부보다 낫고 틀린 글자보다 낫다. 화면은 같은
//   판정(lib/translate.js 의 isSubtitleStale)으로 "다시 번역" 을 띄운다.
//
// ⚠️ 판정 기준을 여기서 새로 만들지 않는다: `of === cut.sentence` 는 isSubtitleStale 과
//    **같은 자**다. 한쪽만 고치면 화면은 "최신" 이라 하는데 완성본은 원문으로 구워진다.
// ★ sourceLang — 이 영상이 **말한** 언어(= 대사 원문의 언어). 2026-08-18 저녁에 인자가 늘었다.
//   예전에는 "ko" 가 글자 그대로 박혀 있었고, 그것은 원문이 언제나 한국어이던 시절의 사실이다.
//   일본어로 말하는 영상에서 한국어 자막을 고르면 **번역본 대신 원문(일본어)이 구워졌다** —
//   화면에는 한국어가 보이는데 영상에는 일본어가 박혀 "적용이 안 된다"로 보였다.
//   ⚠️ 같은 날 아침 isSubtitleStale 에서만 이 고정을 걷어내 **짝이 어긋난 상태**를 만들었다.
//      위 주석이 그 결과를 미리 적어 두고 있었다("화면은 최신이라 하는데 완성본은 원문으로").
export function subtitleTextFor(cut, lang, sourceLang = "ko") {
  const sentence = (cut?.sentence || "").trim();
  if (!lang || lang === sourceLang) return sentence;
  const t = cut?.subtitles?.[lang];
  return t && t.of === cut?.sentence ? (t.text || "").trim() || sentence : sentence;
}

// opts.lang 을 주면 자막 글자가 그 언어를 따른다. 안 주면 오늘 그대로(한국어 원문)다.
export function buildCues(cuts, opts) {
  // ★ 치수(width·height)가 없으면 나누지 않는다 — 폭 한계가 화면에서 파생되므로 잴 수 없다.
  //   opts 자체가 없을 때뿐 아니라 **치수가 빠진 opts**(예: { lang } 만)도 같다:
  //   그때 lineWidthUnits 는 NaN 이고, NaN 과의 비교는 전부 거짓이라 모든 문장이 글자 단위로
  //   쪼개진다. 조용히 자막이 부서지느니 안 나누는 쪽(옛 동작)이 맞다.
  const raw = opts ? lineWidthUnits(opts) : Infinity;
  const lineUnits = Number.isFinite(raw) ? raw : Infinity;
  const maxUnits = lineUnits * MAX_SUBTITLE_LINES;
  const lang = opts?.lang;
  // 원문 언어 — 안 주면 한국어다(옛 호출자는 글자 그대로 예전처럼 돈다)
  const sourceLang = opts?.sourceLang || "ko";
  ...
  for (const c of cuts || []) {
    ...
    //
    // held 는 cutSeconds(c) 를 그대로 쓴다 — buildFfmpegArgs 의 `fit` 판정과 같은 자를
    // 써야 화면에 실제로 있는 시간과 누적이 어긋나지 않는다(파일 상단 주석 참고).
    ...
    // ★★ **언제 말하나**(2026-08-19). spoken_seconds 는 얼마나 말하는지만 말하고, 시작은
    //   여전히 누적(t)이 정한다. 단계별 광고는 낭독을 우리가 만들고 클립을 그 길이에 맞추므로
    //   시작이 곧 컷 경계라 이 값이 필요 없었다 — 그래서 지금까지 없어도 됐다.
    //   통짜로 굽는 영상(film·광고 r2v)은 모델이 자기 리듬으로 말해 시작이 어긋난다:
    //   실측(fal-ai/whisper)에서 계획 초로는 3·4번 자막이 1.8초·2.7초 늦게 떴다.
    // ★ 0 은 값이 아니라 "안 왔다"로 본다 — 실제 0초에 시작하는 말은 누적 결과와 같으므로
    //   구분할 실익이 없고, `|| 0` 관용구(위 spokenRaw)와 같은 자를 쓰는 편이 읽기 쉽다.
    // ★ span 은 건드리지 않는다. 시작만 갈아끼우는 것이지 두 값이 하나가 아니다.
    const startedAt = Number(c?.spoken_start) || 0;
    // ★ Scribe 실측이 실제 발화보다 +0.09~+0.29초 늦다(2026-09-16). 자막은 말보다 아주 조금
    //   먼저 떠야 읽힌다 — 편집 통상값 0.1~0.2초. 못 잰 문장(비례, startedAt===0)은 기준이
    //   다른 값이라 당기지 않는다.
    const base = startedAt > 0 ? Math.max(0, startedAt - SUBTITLE_LEAD_SECONDS) : t;
    const text = subtitleTextFor(c, lang, sourceLang);
    if (text) {
      ...
      pieces.forEach((piece, i) => {
        ...
        const start = base + acc;
        acc += share;
        // 마지막 조각의 끝은 **말 끝**(base + span)에 못 박는다 — 컷의 끝(held)이 아니다.
        // 여백에는 자막이 없어야 하고, 비례 오차도 다음 컷으로 넘어가지 않게 한다.
        const end = i === pieces.length - 1 ? base + span : base + acc;
        if (shown) cues.push({ start: round2(start), end: round2(end), text: breakTwoLines(shown, lineUnits) });
      });
    }
    // 문장이 없어도 시간은 흐른다 — 건너뛰면 뒤 자막이 전부 밀린다. 화면 시간(held)으로
    // 누적해야 다음 컷이 여백까지 지난 뒤 제자리에서 시작한다.
    t += held;
  }
  return cues;
}
...
// 원본 문자열에서 **코드포인트 경계**의 UTF-16 오프셋 목록을 만든다(첫 글자 제외).
// `s[i]` 나 `[...s].map((_, i) => i)` 를 쓰면 서로게이트 쌍(이모지·확장 한자)을
// 반으로 자른다 — 반드시 `ch.length` 를 누적해야 slice 에 그대로 쓸 수 있다.
...
//
// ★ 중국어·일본어처럼 공백 경계가 없으면(matchAll 이 토큰 1개만 준다) 글자 단위로
//   떨어진다 — 옛 코드는 여기서 그대로 반환해 긴 조각이 한 줄로 화면을 넘겼다.
function packWords(text, from, to, maxUnits) {
  ...
  const wordToks = [...seg.matchAll(/\S+/g)];
  const toks = wordToks.length > 1
    ? wordToks.map((t) => ({ text: t[0], at: t.index }))
    : charOffsets(seg).length
      ? [0, ...charOffsets(seg)].map((at, i, arr) => ({ text: seg.slice(at, arr[i + 1]), at }))
      : [];
  ...
  let cur = 0;
  for (const t of toks) {
    const w = textUnits(t.text);
    // 첫 낱말은 무조건 담는다 — 한 낱말이 한계를 넘어도 그 자리에서 끊을 수는 없다
    if (cur > 0 && cur + 0.3 + w > maxUnits) {
      out.push([pieceStart, from + t.at]);
      pieceStart = from + t.at;
      cur = w;
    } else {
...
function splitBalanced(text, from, to, maxUnits) {
  ...
  const wordToks = [...seg.matchAll(/\S+/g)];
  // ★ 공백 경계가 없으면(중국어·일본어) 글자 사이를 어절 경계 대신 쓴다.
  //   옛 코드는 여기서 빈 배열이 나와 절 경계만으로 못 채우면 그대로 통과시켰다.
  const words = wordToks.length > 1
    ? wordToks.slice(1).map((t) => from + t.index)
    : charOffsets(seg).map((o) => from + o);
...
export function breakTwoLines(text, lineUnits) {
  ...
  const cuts = toks.length > 1 ? toks.slice(1).map((t) => t.index) : charOffsets(s);
...
//
// lang 을 주면 **폰트 이름이 언어를 따른다.** 안 주거나 "ko" 면 오늘과 글자 그대로 같다.
export function toAss(cues, { width, height, position, subtitle, lang }) {
  ...
  // ★★ **한국어일 때만** 스타일(기본·강조·부드럽게)이 폰트를 정한다. 일본어·중국어는
  //    폰트가 한 벌뿐이라 스타일이 이기면 안 된다 — Black Han Sans 에는 한자·가나가 없어
  //    화면 전체가 두부(□)로 나간다. ffmpeg 는 그때도 오류를 내지 않는다.
  //
  // ⚠️ 이 이름 하나만 바꾸면 안 된다. ffmpeg 가 읽는 **폰트 경로**(lib/compose.js 의
  //    fontsDir)가 같은 subtitleFontFor 에서 나와야 둘이 같은 파일을 가리킨다.
  //    이름만 바꾸면 libass 가 조용히 다른 폰트로 대체한다(tests/assets.test.js 가 묶어 둔다).
  const family = lang && lang !== "ko" ? subtitleFontFor(lang).family : font.family;
  const [px, py] = s.pos;
  ...
  // 테두리 두께·그림자는 **설정이 있을 때만** 글자를 따라간다. 옛 경로(설정 없음)는
  // 3·0 그대로다 — 옛 프로젝트를 다시 구우면 완성본이 달라지는 일이 없어야 한다.
  const rim = styled ? rimFor(s.font, fontSize) : { outline: 3, shadow: 0 };
...
Style: Main,${family},${fontSize},${primary},${outline},&H80000000,1,1,${rim.outline},${rim.shadow},${styleAlign},${marginH},${marginH},${marginV},1
...
// `rim` 은 **글자 크기 대비 외곽선 두께의 비율**이다. 획이 가는 글꼴일수록 크다.
//
// ★ 왜 폰트마다 다른가: 외곽선이 하는 일은 글자를 배경에서 떼어 놓는 것인데, 그 힘은
// 테두리 두께 자체가 아니라 **획 굵기에 대한 테두리의 비율**로 느껴진다. Black Han Sans 는
// 획이 두꺼워 같은 테두리도 넉넉하고, Gowun Dodum 은 획이 가늘어 같은 테두리가 얇아 보인다.
// 2026-08-13 실측(생성된 조던 클립의 흰 밑창·밝은 마루 위)에서 `soft` 만 글자가 묻혔다.
//
// ★ basic 의 0.037 은 **오늘을 그대로 두는 값**이다: 1080×1920 기본 크기에서
// fontSize 81 × 0.037 = 3.0 → 옛 상수 3 과 픽셀 동일하다. 여기서부터 갈라져 나간다.
export const SUBTITLE_FONTS = [
  { id: "basic", label: "기본", family: "Pretendard", cssFamily: "Pretendard Subtitle", rim: 0.037 },
  { id: "impact", label: "강조", family: "Black Han Sans", cssFamily: "Black Han Sans Subtitle", rim: 0.030 },
  { id: "soft", label: "부드럽게", family: "Gowun Dodum", cssFamily: "Gowun Dodum Subtitle", rim: 0.055 },
  // ★ 2026-08-25 사장님 지시로 셋을 더했다 — 앞의 셋은 전부 고딕이어서 결이 좀았다.
  //   전부 Google Fonts 의 **OFL 1.1** 로 앞의 셋과 같은 라이선스다(상업 사용·임베딩 자유).
  // ★ rim(외곽선 두께)은 글꼴마다 다르다 — 획이 가는 글꼴은 두꺼우면 글자가 빡빡해진다.
  //   명조는 획 대비가 커서 중간, 손글씨는 가늘어 두꺽게, 둥근 것은 이미 굵어 얇게.
  { id: "serif", label: "명조", family: "NanumMyeongjo", cssFamily: "NanumMyeongjo Subtitle", rim: 0.045 },
  { id: "hand", label: "손글씨", family: "Nanum Pen", cssFamily: "Nanum Pen Subtitle", rim: 0.058 },
  { id: "round", label: "둥글게", family: "Jua", cssFamily: "Jua Subtitle", rim: 0.032 },
];

// 그림자는 글자 **아래쪽으로** 떨어져 밝은 배경에서 아랫변을 떼어 놓는다. 외곽선만으로는
// 흰 글자 + 흰 밑창처럼 명도가 비슷한 자리에서 윤곽이 뭉갠다.
//
// ★ 외곽선과 달리 폰트를 안 탄다 — 그림자는 글자 모양이 아니라 **배경과의 거리**를 만든다.
const SHADOW_RATIO = 0.02;

// ASS 의 Outline·Shadow 값. 글자 크기에서 비례로 뽑는다.
//
// ★ 소수 한 자리로 자른다 — libass 는 소수를 받지만, 0.1 아래의 차이는 눈에 안 보이고
// 각인(ASS 문자열)만 흔들어 "안 바뀐 것 같은데 다시 굽는다"를 만든다.
export function rimFor(fontId, fontSize) {
  const font = SUBTITLE_FONTS.find((f) => f.id === fontId) || SUBTITLE_FONTS[0];
  return {
    outline: q(fontSize * font.rim, 1),
    shadow: q(fontSize * SHADOW_RATIO, 1),
  };
}
