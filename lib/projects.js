...
// 역할은 요청 경계가 세운 컨텍스트에서 읽는다 — 여기서 신원을 다시 확인하지 않는다.
import { currentRole } from "./actor.js";

// ── 운영자는 남의 프로젝트도 소유자처럼 다룬다 (2026-08-27 사장님 지시) ──────────
//
// "admin 은 사용자가 생성한 영상도 편집 과정을 수정할 수 있게" — 즉 읽기뿐 아니라
// **고치기**까지다. 판정을 여기 한 곳에 두는 이유:
//   · 라우트가 스무 곳이 넘는다. 인자로 흘리면 중간 하나가 빠뜨린 자리만 조용히 안 되고,
//     그 자리는 "왜 이 화면만 남의 것을 못 고치지"로 나타난다(찾기 어려운 종류다).
//   · 신원은 요청 경계(withUser)가 이미 세운다 — 그 컨텍스트에 실린 역할을 여기서 읽는다.
//
// ⚠️ **넓히는 값이라 모를 때는 안 넓힌다.** currentRole() 은 컨텍스트가 없으면 빈 문자열을
//   준다(lib/actor.js) — 스크립트·단위 테스트가 조용히 운영자가 되지 않는다.
// ⚠️ **2026-09-03 — 지우기도 열렸다**(사장님 지시: "관리자는 전체 영상을 삭제할 수 있는
//   권한"). 그전 주석은 "지우기는 그대로 소유자 전용 … 열려면 그때 따로 연다"였고,
//   지금이 그때다. 아래 deleteProject 가 같은 ownerScope 를 지난다.
//   ★ 되돌리려면 그 함수에서 ownerScope 대신 requireOwner 를 쓰면 된다(한 줄).
// ★ 목록(listProjects)도 안 넓힌다 — 그것은 "내 영상"이라는 뜻이라 넓히면 말이 깨진다.
//   운영자가 전체를 보는 자리는 보관함 [전체](listAllProjects)로 이미 있다.
const ANY_OWNER = null;   // 소유자 필터를 건다/안 건다 — store 가 이 값을 그렇게 읽는다

function ownerScope(ownerId) {
  requireOwner(ownerId);                       // 안 넘긴 것은 여전히 사고다(아래 참고)
  return currentRole() === "admin" ? ANY_OWNER : ownerId;
}
...
// ★ "reel" 을 더한다(2026-08-21) — 없으면 isStepDoc 이 참이 되어, reel 프로젝트가
//   옛 단계별 유료 라우트(/api/projects/[id]/clips 등)를 그대로 통과한다. 그 경로는
//   clip_prompt 를 모른 채 컷을 i2v 로 구우면서 크레딧까지 받는다 — 순수 가산이라
//   ad·film 에는 영향이 없다(이 배열을 읽는 자리는 바로 아래 한 곳뿐이다).
const KINDS = ["ad", "film", "reel"];

// 단계별(옛) 경로가 다룰 문서인가 — **종류가 없는 문서만** 참이다.
//
// 왜 "ad 가 아니다"가 아니라 "종류가 없다"인가: 예전에는 21곳이 손으로 `kind === "ad"` 를
// 적고 있었다. 그래서 새 종류(kind:"film")가 생기자 그 문서들이 단계별 유료 라우트로
// **그대로 통과**했다 — 거기서 만들어지는 컷·이미지·클립은 다른 장부 키(video:)로
// 청구되므로 장부가 갈린다. 판정을 뒤집으면 앞으로 늘어날 종류도 아무도 기억하지 않아도
// 막힌다(각인이 버전 번호를 버린 이유와 같다 — 사람이 기억해야 하는 자리를 만들지 않는다).
//
// ⚠️ `== null` 인 것이 중요하다. getProject 는 doc 통짜라 옛 문서에 kind 가 **아예 없고**,
// 폴링용 좁은 셀렉터(selectProjectProgress 등)는 `kind: d.kind ?? null` 로 실어 온다.
// `=== undefined` 로 적으면 status 라우트만 조용히 다르게 판정한다.
//
// 없는 문서(null)는 여기서 거짓이다 — 호출부가 기존 문구 그대로 404 로 옮긴다.
export function isStepDoc(doc) {
  return Boolean(doc) && doc.kind == null;
}
...
export async function getProject(id, ownerId) {
  const owner = ownerScope(ownerId);
  const row = await getStore().selectProject(id, owner);
  return row ? row.doc : null;
}

// 지운다. **운영자는 남의 것도 지운다**(2026-09-03 사장님 지시).
//
// ★★ 왜 이 문이 생겼나 — 그전에는 라우트가 `getStore().deleteProject(id, user.id)` 로
//   **스토어를 직접** 불렀다. 그래서 이 저장소의 판정 한 곳(ownerScope)을 지나지 않았고,
//   읽기·고치기는 운영자에게 열려 있는데 지우기만 소유자 전용으로 남아 있었다.
//   위 ownerScope 주석이 그것을 "열려면 그때 따로 연다"고 적어 두었고, 지금이 그때다.
// ★ 판정을 여기 두는 이유는 위 주석 그대로다: 인자로 흘리면 중간 하나가 빠뜨린 자리만
//   조용히 안 되고, 그 자리는 "왜 이 화면만 안 지워지지"로 나타난다.
// ⚠️ **모르면 안 넓힌다** — currentRole() 은 컨텍스트가 없으면 빈 문자열이라(lib/actor.js)
//   스크립트·단위 테스트가 조용히 운영자가 되지 않는다. 넓히는 값이라 그 성질이 중요하다.
// ★ 돌려주는 값은 "지웠는가" 하나다 — 없는 것과 남의 것을 가르지 않는다(라우트가 둘 다
//   404 로 답해 존재 여부를 안 흘린다).
export async function deleteProject(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().deleteProject(id, owner);
}
...
export async function getProjectForViewing(id, viewerId) {
  ...
  //   와디즈로 손님을 받기 시작하면 남의 영상·자료·프롬프트가 서로에게 열린다. 이제 남의
  //   문서는 **운영자만** 본다(문제 영상 관리). 손님(viewerId null)·일반 사용자는 없는 것과
  //   같게 null — 라우트가 404 로 답해 존재 여부도 안 흘린다.
  //   ★ 판정을 이 한 곳에 둔다: 상세 읽기 넷(projects·ads·reel·film)과 영상 파일이 전부 이
  //     문을 지나므로, 여기서 닫으면 한꺼번에 닫힌다.
  if (!mine && currentRole() !== "admin") return null;
  // ★★★ 2026-09-01 — **`mine` 과 `editable` 을 가른다**(사장님 지시: "admin 은 모든
  //   사용자의 게시물을 단계별로 확인할 수 있게").
  //   ⚠️ **2026-09-03 정정** — 그전 주석은 "한 값으로 둘을 겸하면 지우기까지 함께 열린다
  //     — deleteProject 는 설계상 소유자 전용"이라고 적혀 있었는데, **지우기도 열렸다**
  //     (사장님 지시). 그래도 두 값을 계속 가르는 이유는 뜻이 다르기 때문이다:
  //     `mine` 은 "내가 만들었는가"라는 **사실**이고 화면이 그것으로 라벨·문구를 정한다.
  //   · mine     — 내가 만들었는가(사실). 화면 표시가 본다.
  //   · editable — 고칠 수 있는가. **제작 화면 문**이 본다. ownerScope 와 같은 판정이라
  //                "화면은 열어 줬는데 서버가 404" 가 생길 수 없다.
  //   ⚠️ 넓히는 값이라 **모를 때는 안 넓힌다** — currentRole() 은 컨텍스트가 없으면 빈
  //     문자열이다(lib/actor.js). 스크립트·단위 테스트가 조용히 운영자가 되지 않는다.
  return { doc: row.doc, mine, editable: mine || currentRole() === "admin" };
}
...
export async function listAllProjects(viewerId, page = {}) {
  const rows = await getStore().listAllProjects(page);
...
export async function getProjectProgress(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectProjectProgress(id, owner);
}

export async function getProjectRender(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectProjectRender(id, owner);
}

export async function getProjectCuts(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectProjectCuts(id, owner);
}

// 폴링 전용 — 통짜를 안 읽는다(2026-09-11). 왜 생겼는지는 store 쪽 머리말에 있다.
export async function getAdStatus(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectAdStatus(id, owner);
}

export async function getReelStatus(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectReelStatus(id, owner);
}

// 수거 전용 — cuts 를 안 읽는다(문서의 절반이다).
export async function getReelJob(id, ownerId) {
  const owner = ownerScope(ownerId);
  return getStore().selectReelJob(id, owner);
}

// 목록 — doc 통짜를 안 실어 보낸다(store 가 이미 요약해서 준다).
export async function listProjects(ownerId, page = {}) {
  requireOwner(ownerId);
  // mine 을 여기서 세운다 — [전체] 목록과 **같은 모양**이라야 카드가 탭마다 다르게
  // 판정하지 않는다. 이 목록은 정의상 전부 내 것이라 늘 true 다.
  return (await getStore().listProjects(ownerId, page)).map((row) => ({ ...row, mine: true }));
}

// 보관함 한 쪽의 편 수(「더 보기」 · 2026-09-15). 카드 격자가 2·3·4열이라 그 공배수로 둔다 —
// 쪽 끝 줄이 반쯤 빈 채로 「더 보기」가 붙으면 목록이 끝난 것처럼 보인다.
export const PROJECT_PAGE = 24;

// 주소의 before 를 커서로 읽는다. 양의 유한수만 받는다 — before=0 이나 글자를 그대로 쓰면
// 「그보다 앞선 것」이 하나도 없어 목록이 통째로 빈다(첫 쪽으로 읽는 편이 안전하다).
export function projectCursor(raw) {
  const n = Number(raw);
...
export async function updateProject(id, ownerId, patchFn) {
  // ★ 운영자면 소유자 필터를 걷는다 — 판정은 ownerScope 하나다(위 머리말).
  return enqueueWrite(id, () => updateProjectNow(id, ownerScope(ownerId), patchFn));
}
