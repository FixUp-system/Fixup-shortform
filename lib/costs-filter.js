...
// 시각 하나가 [from, to] 날짜 칸 안인가 — 사용자 관리(/admin)의 가입일 좁히기가 쓴다(2026-09-14).
// ★ 규칙은 filterRecords 와 **같은 한 벌**이다(지역 시각 자정 · 끝 날짜 포함 · 시각을 모르면
//   날짜를 골랐을 때만 뺀다). 숫자(ms)와 ISO 문자열을 둘 다 받는다.
// 날짜 칸 → { start, end } ms(비면 null). 마이페이지 크레딧 내역이 이 값을 서버로 보낸다(2026-09-14).
// ★ 경계를 **브라우저에서** 만드는 이유: 서버(Vercel)는 UTC 라 "YYYY-MM-DD" 를 서버에서 자르면
//   한국 오전 기록이 하루 밀린다. 사람의 시계는 브라우저에 있다.
...
export function inDayRange(when, from = "", to = "") {
  ...
  const ts = typeof when === "number" ? when : when ? new Date(when).getTime() : NaN;
...
export function filterRecords(records, { from = "", to = "", person = "", flow = "" } = {}) {
  ...
  return (records || []).filter((r) => {
    ...
    // 시각을 모르는 행은 날짜로 거르지 않는다 — 날짜를 안 골랐을 때와 같이 남긴다(inDayRange 가 판정).
    return inDayRange(Number(r?.ts), from, to);
  });
}
